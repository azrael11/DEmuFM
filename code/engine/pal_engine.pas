...
var
  ...
  ws: array [0 .. (MAX_NETS - 1), 0 .. (MAX_RES_PER_NET - 1)] of single;
  // calulated, scaled weights */
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (j = n) then
        begin // only one resistance in the network connected to Vcc */
          if (r[i, j] <> 0.0) then
            R1 := R1 + (1.0 / r[i, j]);
        end
        else

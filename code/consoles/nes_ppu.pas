...
begin
  ...
  for color_emphasis := 0 to 7 do
  begin
    // loop through the 4 intensities
    for color_intensity := 0 to 3 do
    begin
      // loop through the 16 colors
      for color_num := 0 to 15 do
      begin
        case color_num of
          0:
            begin
...
begin
  ...
  if self.disable_chr then
    ret := self.address and $FF
  else
    ret := self.buffer_read;
  self.buffer_read := self.read_mem(self.address);
  if ((self.linea >= 240) or ((self.control2 and $18) = 0)) then
  begin
    if (self.control1 and $4) <> 0 then
      self.address := (self.address + 32) and $7FFF
    else
      self.address := (self.address + 1) and $7FFF;
  end
  else
  begin
    // X fina
    if (self.address and $1F) = $1F then
...
begin
  case (self.address and $3FFF) of
    $0 .. $1FFF:
      if (not(self.disable_chr) and self.write_chr) then
        self.chr[nes_mapper_0.chr_map[(self.address shr 12) and 1], self.address and $FFF] := valor;
    $2000 .. $3EFF:
      self.name_table[MIRROR_TYPES[self.mirror, (self.address shr 10) and 3], self.address and $3FF] := valor;
    $3F00 .. $3FFF:
      case (self.address and $1F) of // Palete
        0, $10:
          begin
            self.pal_ram[0] := valor;
            self.pal_ram[$04] := valor;
            self.pal_ram[$8] := valor;
            self.pal_ram[$0C] := valor;
            self.pal_ram[$10] := valor;
            self.pal_ram[$14] := valor;
            self.pal_ram[$18] := valor;
            self.pal_ram[$1C] := valor;
          end;
      else
        self.pal_ram[self.address and $1F] := valor;
      end;
  end;
  if ((self.linea >= 240) or ((self.control2 and $18) = 0)) then
  begin
...
begin
  case (address and $3FFF) of
    $0 .. $1FFF:
      if not(self.disable_chr) then
        read_mem := self.chr[nes_mapper_0.chr_map[(address shr 12) and 1], address and $FFF]
      else
        read_mem := address and $FF;
    $2000 .. $3EFF:
      read_mem := self.name_table[MIRROR_TYPES[self.mirror, (address shr 10) and 3], address and $3FF];
    $3F00 .. $3FFF:
      read_mem := self.pal_ram[address and $1F]; // Palete
  end;
end;

function nesppu_chip.set_emphasis(pal_tmp: word): word;
begin
  if ((self.control2 and $80) <> 0) then
  begin // azul
    pal_tmp := (pal_tmp and $CE7F);
  end;
  if ((self.control2 and $40) <> 0) then
  begin // verde
...
begin
  ...
  begin
    ...
    begin
      ...
      if (tmp = $03C0) then
        self.address := self.address xor $0800
      else
        self.address := self.address or (tmp and $03E0);
    end;
...
procedure nesppu_chip.draw_linea(nes_linea: word);
  ...
  begin
    ...
    begin
      ...
      if (((self.sprite_ram[(f * 4) + 2] and $20) <> pri) or (pos_y > 239)) then
        continue;
      pos_x := self.sprite_ram[(f * 4) + 3];
      pos_linea := nes_linea - pos_y;
      if (pos_linea < self.sprite_size) then
      begin
        nsprites := nsprites + 1;
        if nsprites = 9 then
          exit;
        ...
        if @nes_mapper_0.calls.ppu_read <> nil then
          nes_mapper_0.calls.ppu_read((temp * $1000) + (num_char * 16) + def_y);
        tempb2 := self.read_mem((temp * $1000) + (num_char * 16) + 8 + def_y);
        if @nes_mapper_0.calls.ppu_read <> nil then
          nes_mapper_0.calls.ppu_read((temp * $1000) + (num_char * 16) + 8 + def_y);
        if flipx then
        begin
          for x := 0 to 7 do
          begin
            ...
            begin
              // Sprite 0 Hit
              if (((pos_x + x) <> 255) and (f = 0) and ((self.dot_line_trans[pos_x + x] and $3F) <> 0)) then
              begin
                ppu_nes_0.status := ppu_nes_0.status or $40;
              end;
              if (((self.control2 and $4) = 0) and ((pos_x + x) < 8)) then
              begin
                ptemp^ := paleta[MAX_COLORS];
              end
              else
              begin
                if ((self.dot_line_trans[pos_x + x] and $3F) >= f) then
                begin
        ...
        begin
          ...
          begin
            ...
            begin
              // Sprite 0 Hit
              if (((pos_x + (7 - x)) <> 255) and (f = 0) and ((self.dot_line_trans[pos_x + (7 - x)] and $3F) <> 0)) then
              begin
                ppu_nes_0.status := ppu_nes_0.status or $40;
                // self.sprite0_hit:=true;
                // self.sprite0_hit_pos:=nsprites*8*PPU_PIXEL_TIMING;;
              end;
              if (((self.control2 and $4) = 0) and ((pos_x + (7 - x)) < 8)) then
              begin
                ptemp^ := paleta[MAX_COLORS];
              end
              else
              begin
                if ((self.dot_line_trans[pos_x + (7 - x)] and $3F) >= f) then
  ...
  begin
    ...
    if ((self.address and $40) = 0) then
    begin
      if ((self.address and $2) = 0) then
        AttribVal := (self.read_mem(AttribTable) and $3) shl 2
      else
        AttribVal := self.read_mem(AttribTable) and $C;
    end
    else
    begin
      if ((self.address and $2) = 0) then
        AttribVal := (self.read_mem(AttribTable) and $30) shr 2
      else
        AttribVal := (self.read_mem(AttribTable) and $C0) shr 4;
    end;
    for tiles := 32 downto 0 do
    begin
      PatternAdr := (self.pos_bg * $1000) + (self.read_mem($2000 + (self.address and $FFF)) * $10) + TileYOffset;
      if @nes_mapper_0.calls.ppu_read <> nil then
        nes_mapper_0.calls.ppu_read(PatternAdr);
      // Draw tile line
      for x := 7 downto 0 do
      begin
      ...
      if (self.address and $3) = 0 then
        AttribTable := AttribTable + 1;
      if (self.address and $1) = 0 Then
      begin
        if ((self.address and $40) = 0) then
        begin
          if ((self.address and $2) = 0) then
  ...
  begin
    ...
    begin
      pos_y := self.sprite_ram[f * 4];
      if ((pos_y = 255) or (pos_y > 239)) then
        continue;
      pos_linea := nes_linea - pos_y;
      if (pos_linea < self.sprite_size) then
      begin
...
begin
  ...
  if (self.control2 and $18) <> 0 then
    sprite_line_overflow(nes_linea + 1);
  if (self.control2 and $10) <> 0 then
    putsprites(nes_linea, $20);
  if (self.control2 and $8) <> 0 then
  begin
    putbackground;
    update_region(self.tile_x_offset, 0, 256, 1, 1, 0, nes_linea, 256, 1, 2);
  end;
  if (self.control2 and $10) <> 0 then
    putsprites(nes_linea, $0);
  // Si los sprites o el fondo estan desactivados, no hay sprite 0 hit
  if (self.control2 and $18) <> $18 then
    ppu_nes_0.status := ppu_nes_0.status and $BF; // self.sprite0_hit:=false;
end;

procedure nesppu_chip.dma_spr(direccion: byte);
begin
  if self.sprite_ram_pos <> 0 then
  begin
    copymemory(@self.sprite_ram[self.sprite_ram_pos], @memory[$100 * direccion], $100 - self.sprite_ram_pos);
    copymemory(@self.sprite_ram[0], @memory[$100 * direccion], self.sprite_ram_pos);
  end
  else
  begin
    copymemory(@self.sprite_ram[0], @memory[$100 * direccion], $100);
  end;

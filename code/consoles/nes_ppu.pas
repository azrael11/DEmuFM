...
begin
  case (address and $3FFF) of
    $0 .. $1FFF:
      if not(ppu_nes.disable_chr) then
        ppu_read_mem := ppu_nes.chr[mapper_nes.chr_map[(address shr 12) and 1], address and $FFF]
      else
...
begin
  ...
  begin
    ...
    if (((ppu_nes.sprite_ram[(f * 4) + 2] and $20) <> pri) or (pos_y > 239)) then
      continue;
    pos_x := ppu_nes.sprite_ram[(f * 4) + 3];
    pos_linea := nes_linea - (pos_y + 1);
    if (pos_linea < ppu_nes.sprite_size) then
    begin
      nsprites := nsprites + 1;
      if nsprites = 9 then
        exit;
      ...
      if @mapper_nes.ppu_read <> nil then
        mapper_nes.ppu_read((temp * $1000) + (num_char * 16) + def_y);
      tempb2 := ppu_read_mem((temp * $1000) + (num_char * 16) + 8 + def_y);
      if @mapper_nes.ppu_read <> nil then
        mapper_nes.ppu_read((temp * $1000) + (num_char * 16) + 8 + def_y);
      if flipx then
      begin
        for x := 0 to 7 do
        begin
          ...
          begin
            ...
            if (((ppu_nes.control2 and $4) = 0) and ((pos_x + x) < 8)) then
            begin
              ptemp^ := paleta[MAX_COLORS];
            end
            else
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if (((ppu_nes.control2 and $4) = 0) and ((pos_x + (7 - x)) < 8)) then
            begin
              ptemp^ := paleta[MAX_COLORS];
            end
            else
...
begin
  if ((ppu_nes.control2 and $80) <> 0) then
  begin // Azul
    pal_tmp := (pal_tmp and $CE7F);
  end;
  if ((ppu_nes.control2 and $40) <> 0) then
  begin // Verde
    pal_tmp := (pal_tmp and $CFF9);
  end;
  if ((ppu_nes.control2 and $20) <> 0) then
  begin // rojo
...
begin
  ...
  if ((ppu_nes.address and $40) = 0) then
  begin
    if ((ppu_nes.address and $2) = 0) then
      AttribVal := (ppu_read_mem(AttribTable) and $3) shl 2
    else
      AttribVal := ppu_read_mem(AttribTable) and $C;
  end
  else
  begin
    if ((ppu_nes.address and $2) = 0) then
      AttribVal := (ppu_read_mem(AttribTable) and $30) shr 2
    else
      AttribVal := (ppu_read_mem(AttribTable) and $C0) shr 4;
  end;
  ...
  begin
    ...
    if (ppu_nes.address and $1F) = $1F then
    begin
    ...
    if (ppu_nes.address and $3) = 0 then
      AttribTable := AttribTable + 1;
    if (ppu_nes.address and $1) = 0 Then
    begin
      if ((ppu_nes.address and $40) = 0) then
      begin
        if ((ppu_nes.address and $2) = 0) then
  ...
  if (ppu_nes.control2 and $1E) <> $1E then
  begin
    ptemp := punbuf;
...
begin
  ...
  begin
    ...
    if (ppu_nes.address and $8000) <> 0 then
    begin
      ...
      if (tmp = $03C0) then
        ppu_nes.address := ppu_nes.address xor $0800
      else
        ppu_nes.address := ppu_nes.address or (tmp and $03E0);
    end;
...
begin
  ...
  begin
    pos_y := ppu_nes.sprite_ram[f * 4];
    if ((pos_y = 255) or (pos_y = 240)) then
      continue;
...
begin
  ...
  if (ppu_nes.control2 and $18) <> $18 then
...
begin
  ...
  if ppu_nes.disable_chr then
    ret := ppu_nes.address and $FF
  else
    ret := ppu_nes.buffer_read;
  ppu_nes.buffer_read := ppu_read_mem(ppu_nes.address);
  if ((ppu_nes.linea >= 240) or ((ppu_nes.control2 and $18) = 0)) then
  begin
    if (ppu_nes.control1 and $4) <> 0 then
      ppu_nes.address := (ppu_nes.address + 32) and $7FFF
    else
      ppu_nes.address := (ppu_nes.address + 1) and $7FFF;
  end
  else
  begin
    // X fina
  ...
  case (ppu_nes.address and $3FFF) of
    ...
    $3F00 .. $3FFF:
      case (ppu_nes.address and $1F) of // Palete
        0, $10:
          begin
            ppu_nes.pal_ram[0] := valor;
            ppu_nes.pal_ram[$04] := valor;
            ppu_nes.pal_ram[$8] := valor;
            ppu_nes.pal_ram[$0C] := valor;
            ppu_nes.pal_ram[$10] := valor;
            ppu_nes.pal_ram[$14] := valor;
            ppu_nes.pal_ram[$18] := valor;
            ppu_nes.pal_ram[$1C] := valor;
          end;
      else
        ppu_nes.pal_ram[ppu_nes.address and $1F] := valor;
      end;
  end;
  if ((ppu_nes.linea >= 240) or ((ppu_nes.control2 and $18) = 0)) then
  begin
...
begin
  ...
  begin
    copymemory(@ppu_nes.sprite_ram[0], @memory[$100 * direccion], $100);
  end;

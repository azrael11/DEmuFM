...
const
  LINES_NTSC = 262;
  LINES_PAL = 313;
  hpos_conv: array [0 .. 227] of byte = ($00, $01, $02, $02, $03, $04, $05, $05, $06, $07, $08, $08,
    $09, $0A, $0B, $0B, $0C, $0D, $0E, $0E, $0F, $10, $11, $11, $12, $13, $14, $14, $15, $16, $17,
    $17, $18, $19, $1A, $1A, $1B, $1C, $1D, $1D, $1E, $1F, $20, $20, $21, $22, $23, $23, $24, $25,
    $26, $26, $27, $28, $29, $29, $2A, $2B, $2C, $2C, $2D, $2E, $2F, $2F, $30, $31, $32, $32, $33,
    $34, $35, $35, $36, $37, $38, $38, $39, $3A, $3B, $3B, $3C, $3D, $3E, $3E, $3F, $40, $41, $41,
    $42, $43, $44, $44, $45, $46, $47, $47, $48, $49, $4A, $4A, $4B, $4C, $4D, $4D, $4E, $4F, $50,
    $50, $51, $52, $53, $53, $54, $55, $56, $56, $57, $58, $59, $59, $5A, $5B, $5C, $5C, $5D, $5E,
    $5F, $5F, $60, $61, $62, $62, $63, $64, $65, $65, $66, $67, $68, $68, $69, $6A, $6B, $6B, $6C,
    $6D, $6E, $6E, $6F, $70, $71, $71, $72, $73, $74, $74, $75, $76, $77, $77, $78, $79, $7A, $7A,
    $7B, $7C, $7D, $7D, $7E, $7F, $80, $80, $81, $82, $83, $83, $84, $85, $86, $86, $87, $88, $89,
    $89, $8A, $8B, $8C, $8C, $8D, $8E, $8F, $8F, $90, $91, $92, $92, $93, $E9, $EA, $EA, $EB, $EC,
    $ED, $ED, $EE, $EF, $F0, $F0, $F1, $F2, $F3, $F3, $F4, $F5, $F6, $F6, $F7, $F8, $F9, $F9, $FA,
    $FB, $FC, $FC, $FD, $FE, $FF, $FF);
...
type
  ...
  vdp_chip = class
    constructor create(pant: byte; irq_call: irq_type; cpu_num: byte; read_mem: read_mem_type = nil;
      write_mem: write_mem_type = nil; trans: boolean = false);
    destructor free;
  public
...
constructor vdp_chip.create(pant: byte; irq_call: irq_type; cpu_num: byte;
  read_mem: read_mem_type = nil; write_mem: write_mem_type = nil; trans: boolean = false);
begin
  chips_total := chips_total + 1;
  self.trans := trans;
  self.set_gg(false);
...
const
  tms992X_palete: array [0 .. 15, 0 .. 2] of byte = ((0, 0, 0), (0, 0, 0), (0, 160, 0), (0, 238, 0),
    (0, 0, 98), (0, 0, 238), (78, 0, 0), (0, 238, 238), (160, 0, 0), (238, 0, 0), (78, 78, 0),
    (238, 238, 0), (0, 78, 0), (238, 0, 238), (78, 78, 98), (238, 238, 238));
var
  f: word;
  colores: tpaleta;
begin
...
begin
  // 8x8 o 8x16
  if (self.tms.regs[1] and 2) <> 0 then
    sprite_height := 16
  else
    sprite_height := 8;
  // Zoom
  if (self.tms.regs[1] and 1) <> 0 then
    self.sprite_zoom := 2
  else
    self.sprite_zoom := 1;
  self.sprite_count := 0;
  ...
  begin
    ...
    if ((self.Y_PIXELS = 192) and (sprite_y = $D0)) then
      exit;
    if ((self.sprite_zoom > 1) and (self.sprite_count < 8)) then
    begin
    ...
    if (sprite_y > $E0) then
      sprite_line := $FF + sprite_line;
    if ((sprite_line >= 0) and (sprite_line < sprite_height)) then
    begin
      if (self.sprite_count < max_sprites) then
      begin
        ...
        if (self.tms.regs[0] and 8) <> 0 then
          sprite_x := sprite_x - 8;
        if (self.tms.regs[6] and 4) <> 0 then
          sprite_tile_selected := sprite_tile_selected or $100;
        if (sprite_height = 16) then
          sprite_tile_selected := sprite_tile_selected and $1FE;
        if (sprite_line > 7) then
...
begin
  ...
  sprite_col_x := 255;
  if self.sprite_count = 0 then
    exit;
  fillchar(collision_buffer[0], 255, 0);
  // Draw sprite layer
  for sprite_buffer_index := (self.sprite_count - 1) downto 0 do
  begin
    ...
    begin
      ...
      pen_selected := (pen_bit_3 shl 3) or (pen_bit_2 shl 2) or (pen_bit_1 shl 1) or
        (pen_bit_0) or $10;
      if (pen_selected = $10) then
        continue; // Transparent palette so skip draw
      if (self.sprite_zoom > 1) then
        pixel_plot_x := sprite_x + (pixel_x shl 1)
      else
        pixel_plot_x := sprite_x + pixel_x;
      for f := 1 to self.sprite_zoom do
      begin
        pixel_plot_x := pixel_plot_x + (f - 1);
        if (pixel_plot_x < 0) or (pixel_plot_x > 255) then
          continue;
        // Sprite con mas prioridad
        if ((priority_selected[pixel_plot_x] and PRIORITY_BIT) = 0) then
        begin
          ptemp := punbuf;
        ...
        begin // Sprite con menos prioridad que el fondo
          ...
          if (priority_selected[pixel_plot_x] = PRIORITY_BIT) then
          begin
            ptemp := punbuf;
            inc(ptemp, pixel_plot_x + PIXELS_LEFT_BORDER_VISIBLES);
            ptemp^ := paleta[self.current_pal[pen_selected]];
        ...
        if (collision_buffer[pixel_plot_x] <> 1) then
        begin
        ...
        begin
          ...
          if (sprite_col_x < pixel_plot_x) then
            sprite_col_x := sprite_col_x
          else
            sprite_col_x := pixel_plot_x;
        end;
      end;
    end;
    if sprite_col_occurred then
    begin
...
var
  tempw: word;
begin
  if ((vdp.Y_PIXELS = 192) and not(vdp.gg_set)) then
    tempw := ((row shr 3) shl 6) and (((vdp.tms.regs[2] and 1) shl 10) or $3BFF)
  else
    tempw := ((row shr 3) shl 6);
...
begin
  ...
  if (((self.tms.regs[0] and $40) <> 0) and (linea < 16)) then
    x_scroll := 0
  else
    x_scroll := self.reg8tmp;
  x_scroll_start_column := 32 - (x_scroll shr 3); // x starting column tile
  scroll_x_fine := x_scroll and 7;
  if (self.Y_PIXELS <> 192) then
  begin
  ...
  for tile_column := 0 to 31 do
  begin
    // Rightmost 8 columns for SMS (or 2 columns for GG) not affected by
    // vertical scrolling when bit 7 of reg[0x00] is set
    if (((self.tms.regs[0] and $80) <> 0) and (tile_column > 23)) then
      y_scroll := 0
    else
      y_scroll := self.reg9tmp;
    tile_line := ((tile_column + x_scroll_start_column) and $1F) shl 1;
    addr_tmp := name_table_address + get_name_table_row(self, (linea + y_scroll) mod scroll_mod) +
      tile_line;
    ...
    begin
      ...
      if palette_selected then
        pen_selected := pen_selected or $10;
      if not(flip_x) then
        pixel_plot_x := pixel_x
      else
        pixel_plot_x := 7 - pixel_x;
      pixel_plot_x := (tile_column shl 3) + pixel_plot_x + scroll_x_fine;
      if (pixel_plot_x < 256) then
      begin
        ptemp := punbuf;
        inc(ptemp, pixel_plot_x + PIXELS_LEFT_BORDER_VISIBLES);
        if self.trans then
        begin
          if (pen_selected and $F) = $0 then
            ptemp^ := SET_TRANS_COLOR
          else
            ptemp^ := paleta[self.current_pal[pen_selected]];
        end
        else
  ...
  fillword(punbuf, PIXELS_LEFT_BORDER_VISIBLES,
    paleta[self.current_pal[$10 + self.tms.regs[7] and $F]]);
  ptemp := punbuf;
  inc(ptemp, PIXELS_LEFT_BORDER_VISIBLES + 256);
  fillword(ptemp, PIXELS_RIGHT_BORDER_VISIBLES,
    paleta[self.current_pal[$10 + self.tms.regs[7] and $F]]);
end;
...
begin
  ...
  if linea <= self.Y_PIXELS then
  begin
    if self.line_counter = 0 then
    begin
      self.line_counter := self.tms.regs[$A];
      self.hint := true;
      self.tms.int := true;
      if (self.tms.regs[0] and $10) <> 0 then
        if @self.SMS_IRQ_Handler <> nil then
  ...
  begin
    ...
    begin // Visible
      ...
      begin
        self.select_sprites(linea);
        single_line(0, linea + self.LINEAS_TOP_BORDE,
          paleta[self.current_pal[$10 + self.tms.regs[7] and $F]], PIXELS_VISIBLES_TOTAL,
          self.tms.pant);
      end;
    end
    else if linea < self.LINEA_BORDE_DOWN then
    begin // Resto borde inferior
      single_line(0, linea + self.LINEAS_TOP_BORDE,
        paleta[self.current_pal[$10 + self.tms.regs[7] and $F]], PIXELS_VISIBLES_TOTAL,
        self.tms.pant);
    end
    else if linea >= (self.LINEA_BORDE_DOWN + 19) then
    begin // Borde superior
      single_line(0, linea - (self.LINEA_BORDE_DOWN + 19),
        paleta[self.current_pal[$10 + self.tms.regs[7] and $F]], PIXELS_VISIBLES_TOTAL,
        self.tms.pant);
    end;
  end
  else
    self.tms.refresh(linea);
end;
...
begin
  ...
  begin
    self.tms.vdp_mode := true;
    if ((self.tms.regs[0] and 2) <> 0) then
    begin
      if (((self.tms.regs[1] and $10) <> 0) and ((self.tms.regs[1] and $8) = 0)) then
        new_video := 1;
      if (((self.tms.regs[1] and $10) = 0) and ((self.tms.regs[1] and $8) <> 0)) then
        new_video := 2;
    end;
    ...
    self.tms.vdp_mode := false;
  if (self.video_mode <> new_video) then
  begin
    if self.is_pal then
      self.video_pal(new_video)
    else
      self.video_ntsc(new_video);
...
begin
  ...
  begin
    ...
    register_r := self.tms.register_r;
end;
...
var
  reg: byte;
begin
  if not(self.tms.segundo_byte) then
  begin
  ...
  begin
    ...
    begin
      ...
      case self.addr_mode of
        0:
          begin // VRAM reading mode
            self.tms.buffer := self.tms.read_m(self.tms.addr);
            self.tms.addr := (self.tms.addr + 1) and $3FFF;
          end;
        1, 3:
          ; // VRAM writing mode o CRAM writing mode
        2:
          begin // VDP register write
            reg := valor and $F;
            self.tms.regs[reg] := self.tms.addr and $FF;
            self.video_change;
            self.reg8tmp := self.tms.regs[8];
            if ((reg = 0) and self.hint) or ((reg = 1) and ((self.tms.status_reg and $80) <> 0))
            then
            begin
              if ((reg = 0) and ((self.tms.regs[0] and $10) = 0)) or
                ((reg = 1) and ((self.tms.regs[1] and $20) = 0)) then
              begin
                if self.tms.int then
                begin
                  self.tms.int := false;
...
begin
  ...
  if (self.cram[address] <> valor) then
  begin
    ...
    begin
      ...
      self.current_pal[address] := (valor and $3F) + $10;
  end;
end;

procedure vdp_chip.vram_w(valor: byte);
begin
  if self.addr_mode = 3 then
  begin

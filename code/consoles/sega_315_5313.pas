...
begin
  ...
  if (self.imode and 1) <> 0 then
    odd_frame := self.imode_odd_frame xor 1;
  ...
  ret := (1 shl 13) or // ALWAYS 1
    ...
    (sprite_overflow shl 6) or (byte(sprite_collision) shl 5) or (odd_frame shl 4) or
    (byte(vblank) shl 3) or (hblank_flag shl 2) or (dma_active shl 1) or (byte(self.vdp_pal) shl 0);
  // PAL MODE FLAG checked by striker for region prot..
...
begin
  self.vdp_code := ((self.command_part1 and $C000) shr 14) or
    ((self.command_part2 and $00F0) shr 2);
  self.vdp_address := ((self.command_part1 and $3FFF) shr 0) or
    ((self.command_part2 and $0003) shl 14);
end;
...
begin
  ...
  if (regnum = $00) then
  begin
    if self.irq4_pending then
    begin
      if ((self.regs[$00] and $10) <> 0) then
        lv4irqline_callback(true)
      else
        lv4irqline_callback(false);
    end;
  ...
  if (regnum = $01) then
  begin
    if self.irq6_pending then
    begin
      if ((self.regs[$01] and $20) <> 0) then
        lv6irqline_callback(true)
      else
        lv6irqline_callback(false);
    end;
    // ???
    // m_irq6_pending = 0;
    // m_irq4_pending = 0;
...
var
  ...
  long: word;
begin
  if (((self.regs[$17] and $C0) = 0) or (((self.regs[$17] and $C0) shr 6) = 1)) then
  begin
    source := ((self.regs[$15] and $FF) or ((self.regs[$16] and $FF) shl 8) or
      ((self.regs[$17] and $7F) shl 16)) shl 1;
    ...
    case (self.vdp_code and $0F) of
      ...
      $01:
        ; // if ((self.regs[$01] and $10)<>0) then insta_68k_to_vram_dma(source,long);
      // The 68k is frozen during this transfer, it should be safe to throw a few cycles away and do 'instant' DMA because the 68k can't detect it being in progress (can the z80?)
      $03:
        ; // if ((self.regs[$01] and $10)<>0) then insta_68k_to_cram_dma(source,long);
      // The 68k is frozen during this transfer, it should be safe to throw a few cycles away and do 'instant' DMA because the 68k can't detect it being in progress (can the z80?)
      $05:
        ; // if ((self.regs[$01] and $10)<>0) then insta_68k_to_vsram_dma(source,long);
    end;
  end
  else if (((self.regs[$17] and $C0) shr 6) = $2) then
  begin
    if (((self.vdp_code and $0F) = $01) or ((self.vdp_code and $0F) = $03) or
      ((self.vdp_code and $0F) = $05)) then
    begin // only effects when code is write
      if ((self.regs[$01] and $10) <> 0) then
      begin
  ...
  else if (((self.regs[$17] and $C0) shr 6) = $3) then
  begin
    if (((self.vdp_code and $10) <> 0) or ((self.vdp_code and $0F) = $01)) then
    begin // 0x21 can be affects?
      source := ((self.regs[$15] and $FF) or ((self.regs[$16] and $FF) shl 8));
      // source (byte offset)
...
begin
  ...
  if (self.vram_fill_pending) then
  begin
    if ((self.vdp_address and 1) <> 0) then
      self.vram[(self.vdp_address shr 1) and $7FFF] := valor and $FF
    else
      self.vram[(self.vdp_address shr 1) and $7FFF] := (valor and $FF) shl 8;
    for count := 0 to (self.vram_fill_length - 1) do
    begin // <= for james pond 3
      if (self.vdp_address and 1) <> 0 then
        self.vram[(self.vdp_address shr 1) and $7FFF] := valor and $FF00
      else
        self.vram[(self.vdp_address shr 1) and $7FFF] := (valor and $FF00) shr 8;
  ...
  begin
    case (self.vdp_code and $F) of
      1:
        ; // vdp_vram_write(valor);
      3:
        ; // vdp_cram_write(valor);
      5:
        ; // vdp_vsram_write(valor);
    end;
  end;
...
begin
  ...
  begin
    ...
    if ((self.vdp_code and $20) <> 0) then
      self.handle_dma_bits;
  end
  else
  begin
    if ((valor and $C000) = $8000) then
    begin
...
begin
  case direccion of
    0, 2:
      self.data_port_write(valor);
    4, 6:
      self.control_port_write(valor);
    $10, $12, $14, $16:
      if m68000_0.write_8bits_hi_dir then
        self.sega_psg.write(valor and $FF);
  else
    halt(direccion);
  end;
end;

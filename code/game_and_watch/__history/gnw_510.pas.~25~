...
begin
  ...
  begin
    ...
    begin
      for h := 0 to 3 do
      begin
        if (lcd_enabled[row, seg, h] and (lcd_video_data[gnw_video_array[row, seg, h].id] <> nil))
        then
        begin
          punt := lcd_video_data[gnw_video_array[row, seg, h].id];
          for y := 0 to (gnw_video_array[row, seg, h].size_y - 1) do
          begin
          ...
          actualiza_trozo(0, 0, gnw_video_array[row, seg, h].size_x,
            gnw_video_array[row, seg, h].size_y, 3, gnw_video_array[row, seg, h].pos_x + sync_x,
            gnw_video_array[row, seg, h].pos_y + sync_y, gnw_video_array[row, seg, h].size_x,
            gnw_video_array[row, seg, h].size_y, 2);
        end;
      end;
    end;
  end;
...
begin
  ...
  begin
    ...
    if (state <> lcd_output_cache[index]) then
    begin
...
begin
  ...
  begin
    if ((input_mux shr f) and 1) <> 0 then
    begin
      case f of
        0:
          ret := marcade.in0;
...
begin
  ...
  if read_inputs(input_lines) <> 0 then
    sm510_0.set_input_line(SM510_INPUT_LINE_K, ASSERT_LINE)
  else
    sm510_0.set_input_line(SM510_INPUT_LINE_K, CLEAR_LINE);
end;
...
procedure gnw_sound;
begin
  tsample[sample_num, sound_status.sound_position] := speaker * $7FFF;
end;
...
begin
  ...
  sm510_0.change_io_calls(nil, input_r, input_w, speaker_level, lcd_segment_w, lcd_segment_w,
    lcd_segment_w, lcd_segment_w);
  // Sound
  ...
  case main_vars.machine_type of
    2000:
      begin // Donkey Kong Jr.
        ...
        sync_x := 4;
        sync_y := 12;
        if not(roms_load(sm510_0.get_rom_addr, gnw_dj101_rom)) then
          exit;
        ...
        begin
          ...
          begin
            for h := 0 to 3 do
            begin
              if (gnw_dkongjr_video[row, seg, h].size_y <> 0) then
              begin
                getmem(lcd_video_data[gnw_dkongjr_video[row, seg, h].id],
                  gnw_dkongjr_video[row, seg, h].size_x * gnw_dkongjr_video[row, seg,
                  h].size_y * 2);
                copymemory(lcd_video_data[gnw_dkongjr_video[row, seg, h].id],
                  dkjr_video[gnw_dkongjr_video[row, seg, h].id],
                  gnw_dkongjr_video[row, seg, h].size_x * gnw_dkongjr_video[row, seg,
                  h].size_y * 2);
              end;
            end;
    ...
    2001:
      begin // Donkey Kong II
        ...
        sync_x := 10;
        sync_y := 7;
        if not(roms_load(sm510_0.get_rom_addr, gnw_jr55_rom)) then
          exit;
        ...
        begin
          ...
          begin
            for h := 0 to 3 do
            begin
              if (gnw_dkong2_video[row, seg, h].size_y <> 0) then
              begin
                getmem(lcd_video_data[gnw_dkong2_video[row, seg, h].id],
                  gnw_dkong2_video[row, seg, h].size_x * gnw_dkong2_video[row, seg, h].size_y * 2);
                copymemory(lcd_video_data[gnw_dkong2_video[row, seg, h].id],
                  dkong2_video[gnw_dkong2_video[row, seg, h].id],
                  gnw_dkong2_video[row, seg, h].size_x * gnw_dkong2_video[row, seg, h].size_y * 2);
              end;
            end;
          end;
    ...
    2002:
      begin // Mario Bros
        ...
        sync_x := 0;
        sync_y := 0;
        if not(roms_load(sm510_0.get_rom_addr, gnw_mw56_rom)) then
          exit;
...
procedure cargar_gnw_510;
begin
  machine_calls.start := iniciar_gnw_510;
  machine_calls.general_loop := gnw_510_principal;
  machine_calls.reset := reset_gnw_510;
  machine_calls.close := close_gnw_510;
  // Es un lcd... no tiene lineas. Se actualiza cada 33ms
  machine_calls.fps_max := 30.3030303030;
end;

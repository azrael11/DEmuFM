...
procedure update_video_gb;
  ...
  begin
    ...
    begin
      ...
      if (gb_0.lcd_control and 4) = 0 then
      begin // 8x8
        if flipy then
  ...
  begin
    ...
    begin
      ...
      if tile_mid then
        n2 := shortint(gb_0.vram_bank[0, (bg_addr + f + ((linea_pant div 8) * 32)) and $1FFF])
      else
        n2 := byte(gb_0.vram_bank[0, (bg_addr + f + ((linea_pant div 8) * 32)) and $1FFF]);
      ...
      begin
        ...
        case prio of
          ...
          1:
            if (pval <> 0) then
            begin
              if ((gb_0.bg_prio[f * 8 + (7 - x)] and 2) = 0) then
                ptemp^ := paleta[pval]
              else
                ptemp^ := paleta[MAX_COLORS];
            end
    ...
    // Scroll X
    if gb_0.scroll_x <> 0 then
    begin
      update_region(0, 0, gb_0.scroll_x, 1, 1, (256 - gb_0.scroll_x) + 7, gb_0.linea_actual, gb_0.scroll_x, 1, 2);
      update_region(gb_0.scroll_x, 0, 256 - gb_0.scroll_x, 1, 1, 7, gb_0.linea_actual, 256 - gb_0.scroll_x, 1, 2);
    end
    else
      update_region(0, 0, 256, 1, 1, 7, gb_0.linea_actual, 256, 1, 2);
  end;
  ...
  var
    ...
    ptemp: pword;
  begin
    if ((gb_0.linea_actual < gb_0.window_y) or (gb_0.window_x > 166) or (gb_0.window_x = 0)) then
      exit;
    ...
    begin
      if tile_mid then
        n2 := shortint(gb_0.vram_bank[0, (bg_addr + f + ((gb_0.window_y_draw div 8) * 32)) and $1FFF])
      else
        n2 := gb_0.vram_bank[0, (bg_addr + f + ((gb_0.window_y_draw div 8) * 32)) and $1FFF];
      tile_val1 := gb_0.vram_bank[0, (n2 * 16 + tile_addr + y) and $1FFF];
      tile_val2 := gb_0.vram_bank[0, (n2 * 16 + tile_addr + 1 + y) and $1FFF];
      ptemp := punbuf;
      for x := 7 downto 0 do
      begin
        ...
        case prio of
          ...
          1:
            if (pval = 0) then
            begin
              if gb_0.bg_prio[(f * 8 + (7 - x) - (256 - gb_0.scroll_x) + gb_0.window_x - 7) and $FF] = 0 then
                ptemp^ := paleta[0]
              else
                ptemp^ := paleta[MAX_COLORS];
            end
    ...
    update_region(0, 0, 256, 1, 1, gb_0.window_x, gb_0.linea_actual, 256, 1, 2);
  end;

var
  f: byte;
begin
  for f := gb_0.scroll_y_pos to 113 do
    gb_0.scroll_y[f] := gb_0.scroll_y_last;
  gb_0.scroll_y_pos := 0;
  single_line(7, gb_0.linea_actual, paleta[0], 160, 2);
  if gb_0.lcd_ena then
  begin
    fillchar(gb_0.bg_prio[0], $100, 0);
    if (gb_0.lcd_control and 1) <> 0 then
    begin
      update_bg(0);
      if (gb_0.lcd_control and $20) <> 0 then
        update_window(0);
    end;
    if (((gb_0.lcd_control and 2) <> 0) and not(gb_0.oam_dma)) then
    begin
      get_active_sprites;
      draw_sprites($80);
    end;
    ...
    if (((gb_0.lcd_control and 2) <> 0) and not(gb_0.oam_dma)) then
      draw_sprites(0);
    if (not((gb_0.linea_actual < gb_0.window_y) or (gb_0.window_x > 166) or (gb_0.window_x = 0)) and ((gb_0.lcd_control and $20) <> 0)) then
      gb_0.window_y_draw := gb_0.window_y_draw + 1;
  end;
end;
...
procedure update_video_gbc;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if ((gb_0.bg_prio[(pos_x + x) and $FF]) and 4) = 0 then
            begin
      ...
      begin
        ...
        begin
          ...
          begin
            if ((gb_0.bg_prio[(pos_x + (7 - x)) and $FF]) and 4) = 0 then
            begin
              ptemp^ := paleta[(gb_0.spc_pal[(pval + pal) and $1F]) and $7FFF];
              gb_0.bg_prio[(pos_x + (7 - x)) and $FF] := gb_0.bg_prio[(pos_x + (7 - x)) and $FF] or 1;
            end
  ...
  begin
    ...
    begin
      linea_pant := gb_0.linea_actual + gb_0.scroll_y[round(f * 3.5625)];
      if tile_mid then
        n2 := shortint(gb_0.vram_bank[0, bg_addr + (f + ((linea_pant div 8) * 32) and $3FF)])
      else
        n2 := byte(gb_0.vram_bank[0, bg_addr + (f + ((linea_pant div 8) * 32) and $3FF)]);
          ...
          begin
            case prio of
              ...
              1:
                if (pval <> 0) then
                begin
                  if ((gb_0.bg_prio[(f * 8 + x + (256 - gb_0.scroll_x) + 7) and $FF] and 2) = 0) then
                    ptemp^ := paleta[(gb_0.bgc_pal[pval + tile_pal]) and $7FFF]
                  else
                    ptemp^ := paleta[MAX_COLORS];
                end
          ...
          begin
            case prio of
              ...
              1:
                if (pval <> 0) then
                begin
                  if ((gb_0.bg_prio[(f * 8 + (7 - x) + (256 - gb_0.scroll_x) + 7) and $FF] and 2) = 0) then
                    ptemp^ := paleta[(gb_0.bgc_pal[pval + tile_pal]) and $7FFF]
                  else
                    ptemp^ := paleta[MAX_COLORS];
                end
  ...
  begin
    ...
    begin
      if tile_mid then
        ...
        n2 := byte(gb_0.vram_bank[0, (bg_addr + f + ((gb_0.window_y_draw div 8) * 32)) and $1FFF]);
      atrib := gb_0.vram_bank[1, (bg_addr + f + ((gb_0.window_y_draw div 8) * 32)) and $1FFF];
      if (atrib and $40) <> 0 then
        y := (7 - (gb_0.window_y_draw and $7)) * 2
      else
        y := (gb_0.window_y_draw and $7) * 2;
      tile_bank := (atrib shr 3) and 1;
      tile_pal := (atrib and 7) shl 2;
        ...
        begin
          ...
          case prio of
            ...
            1:
              if (pval = 0) then
              begin
                if gb_0.bg_prio[(f * 8 + x - (256 - gb_0.scroll_x) + gb_0.window_x - 7) and $FF] = 0 then
                  ptemp^ := paleta[gb_0.bgc_pal[pval + tile_pal] and $7FFF]
                else
                  ptemp^ := paleta[MAX_COLORS];
              end
...
begin
  ...
  begin
    ...
    if p_contrls.map_arcade.right[0] then
      gb_0.joy_val := (gb_0.joy_val and $FE)
    else
      gb_0.joy_val := (gb_0.joy_val or $1);
    if p_contrls.map_arcade.left[0] then
      gb_0.joy_val := (gb_0.joy_val and $FD)
    else
      gb_0.joy_val := (gb_0.joy_val or $2);
    if p_contrls.map_arcade.up[0] then
      gb_0.joy_val := (gb_0.joy_val and $FB)
    else
      gb_0.joy_val := (gb_0.joy_val or $4);
    if p_contrls.map_arcade.down[0] then
      gb_0.joy_val := (gb_0.joy_val and $F7)
    else
      gb_0.joy_val := (gb_0.joy_val or $8);
    if p_contrls.map_arcade.but0[0] then
      gb_0.joy_val := (gb_0.joy_val and $EF)
    else
      gb_0.joy_val := (gb_0.joy_val or $10);
    if p_contrls.map_arcade.but1[0] then
      gb_0.joy_val := (gb_0.joy_val and $DF)
    else
      gb_0.joy_val := (gb_0.joy_val or $20);
    if p_contrls.map_arcade.coin[0] then
      gb_0.joy_val := (gb_0.joy_val and $BF)
    else
      gb_0.joy_val := (gb_0.joy_val or $40);
    if p_contrls.map_arcade.start[0] then
      gb_0.joy_val := (gb_0.joy_val and $7F)
    else
      gb_0.joy_val := (gb_0.joy_val or $80);
    if tmp_in0 <> gb_0.joy_val then
  ...
  case direccion of
    $00:
      begin
        ...
        if (gb_0.joystick and $10) = 0 then
          gb_0.joystick := (gb_0.joystick or $F) and (gb_0.joy_val or $F0);
        if (gb_0.joystick and $20) = 0 then
          gb_0.joystick := (gb_0.joystick or $F) and ((gb_0.joy_val shr 4) or $F0);
    ...
    $04:
      leer_io := gb_0.mtimer;
    $05:
      leer_io := gb_0.prog_timer;
    $06:
      leer_io := gb_0.tmodulo;
    $07:
      leer_io := $F8 or gb_0.tcontrol;
    $0F:
      begin
        tempb := $E0;
    ...
    $10 .. $26:
      ;
    // leer_io := gb_snd_0.sound_r(direccion - $10); // Sound
    $30 .. $3F:
      ;
    // leer_io := gb_snd_0.wave_r(direccion - $30); // Sound Wav
    $40:
      leer_io := gb_0.lcd_control;
    $41:
      leer_io := $80 or gb_0.stat;
    $42:
      leer_io := gb_0.scroll_y_last;
    $43:
      leer_io := gb_0.scroll_x;
    $44:
      leer_io := gb_0.linea_actual;
    $45:
      leer_io := gb_0.ly_compare;
    $47:
      leer_io := gb_0.bg_pal;
    $48:
      leer_io := gb_0.sprt0_pal;
    $49:
      leer_io := gb_0.sprt1_pal;
    $4A:
      leer_io := gb_0.window_y;
    $4B:
      leer_io := gb_0.window_x;
    // $80..$fe:leer_io:=io_ram[direccion];  //high memory
    $FF:
      leer_io := gb_0.irq_ena;
  else
    leer_io := gb_0.io_ram[direccion];
  end;
end;
...
begin
  ...
  case direccion of
    $00:
      gb_0.joystick := $CF or (valor and $30);
    $01:
      ; // Serial
    $02:
    ...
    $10 .. $26:
      ;
    // gb_snd_0.sound_w(direccion - $10, valor); // Sound
    $30 .. $3F:
      ;
    // gb_snd_0.wave_w(direccion and $F, valor); // Sound Wav
    $40:
      begin
        gb_0.lcd_control := valor;
        gb_0.lcd_ena := (valor and $80) <> 0;
        if not(gb_0.lcd_ena) then
          gb_0.stat := gb_0.stat and $FC;
      end;
    $41:
      gb_0.stat := (gb_0.stat and $7) or (valor and $F8);
    $42:
      begin
        addrs := lr35902_0.contador div 4;
        for f := gb_0.scroll_y_pos to (addrs - 1) do
          gb_0.scroll_y[f] := valor;
    ...
    $46:
      begin // DMA trans OAM
        ...
        begin
          case addrs of
            $0000 .. $7FFF:
              gb_0.sprt_ram[f] := memory[addrs];
            $8000 .. $9FFF:
              gb_0.sprt_ram[f] := $FF; // Lee datos incorrectos
            $A000 .. $BFFF:
              if @gb_mapper_0.calls.ext_ram_getbyte <> nil then
                gb_0.sprt_ram[f] := gb_mapper_0.calls.ext_ram_getbyte(addrs and $1FFF)
              else
                gb_0.sprt_ram[f] := $FF;
            $C000 .. $CFFF:
              gb_0.sprt_ram[f] := gb_0.wram_bank[0, addrs and $FFF];
            $D000 .. $DFFF:
              gb_0.sprt_ram[f] := gb_0.wram_bank[1, addrs and $FFF];
            $E000 .. $FFFF:
              if @gb_mapper_0.calls.ext_ram_getbyte <> nil then
                gb_0.sprt_ram[f] := gb_mapper_0.calls.ext_ram_getbyte(addrs and $1FFF) // a000
              else
                gb_0.sprt_ram[f] := $FF;
          end;
          addrs := addrs + 1;
    ...
    $4B:
      gb_0.window_x := valor;
    $50:
      gb_0.enable_bios := false; // disable ROM
...
begin
  case direccion of
    ...
    $04:
      leer_io_gbc := gb_0.mtimer;
    $05:
      leer_io_gbc := gb_0.prog_timer;
    $06:
      leer_io_gbc := gb_0.tmodulo;
    $07:
      leer_io_gbc := $F8 or gb_0.tcontrol;
    $0F:
      begin
        tempb := $E0;
        if lr35902_0.vblank_req then
    ...
    $10 .. $26:
      ;
    // leer_io_gbc := gb_snd_0.sound_r(direccion - $10); // Sound
    // $27..$2f:leer_io_gbc:=io_ram[direccion];
    $30 .. $3F:
      ;
    // leer_io_gbc := gb_snd_0.wave_r(direccion and $F); // Sound Wav
    $40:
      leer_io_gbc := gb_0.lcd_control;
    $41:
      leer_io_gbc := $80 or gb_0.stat;
    $42:
      leer_io_gbc := gb_0.scroll_y_last;
    $43:
      leer_io_gbc := gb_0.scroll_x;
    $44:
      leer_io_gbc := gb_0.linea_actual;
    $45:
      leer_io_gbc := gb_0.ly_compare;
    $47:
      leer_io_gbc := gb_0.bg_pal;
    $48:
      leer_io_gbc := gb_0.sprt0_pal;
    $49:
      leer_io_gbc := gb_0.sprt1_pal;
    $4A:
      leer_io_gbc := gb_0.window_y;
    $4B:
      leer_io_gbc := gb_0.window_x;
    $4D:
      leer_io_gbc := (lr35902_0.speed shl 7) + $7E + byte(lr35902_0.change_speed);
    $4F:
      leer_io_gbc := $FE or gb_0.vram_nbank;
    $51 .. $54:
      leer_io_gbc := $FF;
    $55:
      leer_io_gbc := gb_0.hdma_size;
    $56:
      leer_io_gbc := 1;
    $68:
      leer_io_gbc := gb_0.bgcolor_index;
    $69:
      if (gb_0.bgcolor_index and 1) <> 0 then
        leer_io_gbc := gb_0.bgc_pal[gb_0.bgcolor_index shr 1] shr 8
      else
        leer_io_gbc := gb_0.bgc_pal[gb_0.bgcolor_index shr 1] and $FF;
    $6A:
      leer_io_gbc := gb_0.spcolor_index;
    $6B:
      if (gb_0.spcolor_index and 1) <> 0 then
        leer_io_gbc := gb_0.spc_pal[gb_0.spcolor_index shr 1] shr 8
      else
        leer_io_gbc := gb_0.spc_pal[gb_0.spcolor_index shr 1] and $FF;
    $70:
      leer_io_gbc := $F8 or gb_0.wram_nbank;
    $80 .. $FE:
      leer_io_gbc := gb_0.io_ram[direccion]; // high memory
    $FF:
      leer_io_gbc := gb_0.irq_ena;
  else
    begin
...
begin
  ...
  begin
    ...
    case gb_0.dma_src of
      $0000 .. $7FFF:
        temp := memory[gb_0.dma_src];
      // $8000 .. $9FFF:
      // temp := $FF;
      $A000 .. $BFFF:
        if @gb_mapper_0.calls.ext_ram_getbyte <> nil then
          temp := gb_mapper_0.calls.ext_ram_getbyte(gb_0.dma_src and $1FFF);
      $C000 .. $CFFF:
        temp := gb_0.wram_bank[0, gb_0.dma_src and $FFF];
      $D000 .. $DFFF:
        temp := gb_0.wram_bank[gb_0.wram_nbank, gb_0.dma_src and $FFF];
      $E000 .. $FFFF:
        if @gb_mapper_0.calls.ext_ram_getbyte <> nil then
          temp := gb_mapper_0.calls.ext_ram_getbyte(gb_0.dma_src and $1FFF);
    end;
...
begin
  ...
  case direccion of
        ...
        begin
          case addrs of
            ...
            $C000 .. $CFFF:
              gb_0.sprt_ram[f] := gb_0.wram_bank[0, addrs and $FFF];
            $D000 .. $DFFF:
              gb_0.sprt_ram[f] := gb_0.wram_bank[gb_0.wram_nbank, addrs and $FFF];
            $E000 .. $FFFF:
              if @gb_mapper_0.calls.ext_ram_getbyte <> nil then
                gb_0.sprt_ram[f] := gb_mapper_0.calls.ext_ram_getbyte(addrs and $1FFF) // a000
              else
    ...
    $4F:
      gb_0.vram_nbank := valor and 1; // VRAM Bank
    $50:
      gb_0.enable_bios := false; // disable ROM
    $51:
      gb_0.dma_src := (gb_0.dma_src and $FF) or (valor shl 8);
    $52:
      gb_0.dma_src := (gb_0.dma_src and $FF00) or (valor and $F0);
    $53:
      gb_0.dma_dst := (gb_0.dma_dst and $FF) or ((valor and $1F) shl 8);
    $54:
      gb_0.dma_dst := (gb_0.dma_dst and $FF00) or (valor and $F0);
    $55:
      if (gb_0.hdma_ena and ((valor and $80) <> 0)) then
    ...
    $69:
      begin
        if (gb_0.stat and 3) <> 3 then
        begin
          if (gb_0.bgcolor_index and 1) <> 0 then
            gb_0.bgc_pal[gb_0.bgcolor_index shr 1] := (gb_0.bgc_pal[gb_0.bgcolor_index shr 1] and $FF) or (valor shl 8)
          else
            gb_0.bgc_pal[gb_0.bgcolor_index shr 1] := (gb_0.bgc_pal[gb_0.bgcolor_index shr 1] and $FF00) or valor;
        end;
        if gb_0.bgcolor_inc then
          gb_0.bgcolor_index := (gb_0.bgcolor_index + 1) and $3F;
      end;
    $6A:
      begin
    ...
    $6B:
      begin
        ...
        begin
          if (gb_0.spcolor_index and 1) <> 0 then
            gb_0.spc_pal[gb_0.spcolor_index shr 1] := (gb_0.spc_pal[gb_0.spcolor_index shr 1] and $FF) or (valor shl 8)
          else
            gb_0.spc_pal[gb_0.spcolor_index shr 1] := (gb_0.spc_pal[gb_0.spcolor_index shr 1] and $FF00) or valor;
        end;
        if gb_0.spcolor_inc then
          gb_0.spcolor_index := (gb_0.spcolor_index + 1) and $3F;
      end;
    $70:
      begin
        gb_0.wram_nbank := valor and 7;
        if gb_0.wram_nbank = 0 then
          gb_0.wram_nbank := 1;
      end;
    $7E, $7F:
      ;
...
begin
  ...
  begin
    while gb_0.linea_actual <> 154 do
    begin
      lr35902_0.run(frame_m);
      frame_m := frame_m + lr35902_0.tframes - lr35902_0.contador;
      if gb_0.linea_actual < 144 then
        gb_calls.video_render; // Modos 2-3-0
...
begin
  case direccion of
    $0 .. $FF, $200 .. $8FF:
      if gb_0.enable_bios then
        gb_getbyte := gb_0.bios_rom[direccion]
      else
        gb_getbyte := memory[direccion];
    $100 .. $1FF, $900 .. $7FFF:
      gb_getbyte := memory[direccion];
    $8000 .. $9FFF:
      gb_getbyte := gb_0.vram_bank[gb_0.vram_nbank, direccion and $1FFF];
    $A000 .. $BFFF:
      if @gb_mapper_0.calls.ext_ram_getbyte <> nil then
        gb_getbyte := gb_mapper_0.calls.ext_ram_getbyte(direccion)
      else
        gb_getbyte := $FF;
    $C000 .. $CFFF, $E000 .. $EFFF:
      gb_getbyte := gb_0.wram_bank[0, direccion and $FFF];
    $D000 .. $DFFF, $F000 .. $FDFF:
      gb_getbyte := gb_0.wram_bank[gb_0.wram_nbank, direccion and $FFF];
    $FE00 .. $FE9F:
      gb_getbyte := gb_0.sprt_ram[direccion and $FF];
    $FEA0 .. $FEFF:
      if not(gb_0.is_gbc) then
        gb_getbyte := 0
      else
      begin
    ...
    $FF00 .. $FFFF:
      gb_getbyte := gb_calls.read_io(direccion and $FF);
  end;
end;
...
begin
  case direccion of
    $0000 .. $7FFF:
      if @gb_mapper_0.calls.rom_putbyte <> nil then
        gb_mapper_0.calls.rom_putbyte(direccion, valor);
    $8000 .. $9FFF:
      gb_0.vram_bank[gb_0.vram_nbank, direccion and $1FFF] := valor;
    $A000 .. $BFFF:
      if @gb_mapper_0.calls.ext_ram_putbyte <> nil then
        gb_mapper_0.calls.ext_ram_putbyte(direccion, valor);
    $C000 .. $CFFF, $E000 .. $EFFF:
      gb_0.wram_bank[0, direccion and $FFF] := valor;
    $D000 .. $DFFF, $F000 .. $FDFF:
      gb_0.wram_bank[gb_0.wram_nbank, direccion and $FFF] := valor;
    $FE00 .. $FE9F:
      gb_0.sprt_ram[direccion and $FF] := valor;
    $FEA0 .. $FEFF:
      if gb_0.is_gbc then
      begin
        case (direccion and $FF) of
    ...
    $FF00 .. $FFFF:
      gb_calls.write_io(direccion and $FF, valor);
  end;
end;
...
begin
  ...
  if gb_0.oam_dma then
  begin
    gb_0.oam_dma_pos := gb_0.oam_dma_pos + estados_t;
    if gb_0.oam_dma_pos >= 160 then
      gb_0.oam_dma := false;
  end;
  if not(gb_0.lcd_ena) then
    exit;
  ...
  case lr35902_0.contador of
    0:
      begin
        if gb_0.linea_actual = gb_0.ly_compare then
        begin // LY compare
          lcd_compare := (gb_0.stat and $40) <> 0;
          gb_0.stat := gb_0.stat or $4;
        end
        else
          gb_0.stat := gb_0.stat and $FB;
        if gb_0.linea_actual < 144 then
        begin // Modo 2
    ...
    248 .. 600:
      if ((gb_0.linea_actual < 144) and ((gb_0.sprites_time + 248) = lr35902_0.contador) and ((gb_0.stat and 3) <> 0)) then
      begin // Modo 0
...
begin
  lcd_compare := false;
  lcd_mode := false;
  if gb_0.wram_bank[0, $1A4] = gb_0.wram_bank[1, $1A4] then
  begin
  ...
  begin
    lr35902_0.tframes := ((GB_CLOCK shl lr35902_0.speed) / 154) / machine_calls.fps_max;
  ...
  case contador of
    ...
    248 .. 600:
      begin
        if (gb_0.linea_actual < 144) then
        begin
          if ((contador >= 300) and gb_0.hdma_ena and not(gb_0.haz_dma)) then
          begin // DMA H-Blank
            dma_trans($10);
            gb_0.hdma_size := gb_0.hdma_size - 1;
            if gb_0.hdma_size = $FF then
              gb_0.hdma_ena := false;
            lr35902_0.contador := lr35902_0.contador + 8;
            gb_0.haz_dma := true;
          end;
          if (((gb_0.sprites_time + 248) = contador) and ((gb_0.stat and 3) <> 0)) then
          begin // Modo 0
            lcd_mode := ((gb_0.stat and $8) <> 0) and ((gb_0.stat and $20) = 0);
            gb_0.stat := gb_0.stat and $FC;
          end;
...
procedure cerrar_gb;
begin
  if (gb_0.hay_nvram and (nv_ram_name <> '')) then
    write_file(nv_ram_name, @gb_mapper_0.ram_bank[0, 0], $2000);
  gb_mapper_0.free;
end;
...
begin
  ...
  begin // GameBoy Color
    ...
    if not(unlicensed) then
      gb_0.rom_exist := roms_load(@gb_0.bios_rom[0], gbc_rom, true, 'gbcolor.zip');
  end
  else
  begin
    ...
    if not(unlicensed) then
      gb_0.rom_exist := roms_load(@gb_0.bios_rom[0], gb_rom, false);
  end;
  gb_set_pal;
...
procedure abrir_gb;
const
  main_logo: array [0 .. $2F] of byte = ($CE, $ED, $66, $66, $CC, $0D, $00, $0B, $03, $73, $00, $83, $00, $0C, $00, $0D, $00, $08, $11, $1F, $88, $89, $00, $0E, $DC, $CC, $6E, $E6, $DD, $DD, $D9, $99, $BB, $BB, $67, $63, $6E, $0E, $EC, $CC, $DD, $DC, $99, $9F, $BB, $B9,
    $33, $3E);
type
...
var
  ...
  gb_head: tgb_head;
begin
  if not(openrom(romfile, SGB)) then
    exit;
  getmem(datos, $1000000); // 8Gb??
  if not(extract_data(romfile, datos, longitud, nombre_file, SGB)) then
  begin
    freemem(datos);
    exit;
  end;
  ...
  begin // Cartucho
    ...
    if gb_head.title = 'WISDOM TREE' then
      gb_head.cart_type := $C0
    else if ((gb_head.title = '') and (gb_head.cart_type = 0) and (gb_head.rom_size = 0) and (longitud > 32768)) then
      gb_head.cart_type := $C0;
    ...
    if rom_size_t = 0 then
      rom_size_t := 1;
    for f := 0 to (rom_size_t - 1) do
    begin
      copymemory(@gb_mapper_0.rom_bank[f, 0], ptemp, $4000);
      inc(ptemp, $4000);
    end;
    if gb_head.cart_type = $C0 then
    begin
      rom_size_t := longitud div 32768;
      gb_head.rom_size := 0;
      while rom_size_t <> 1 do
      begin
    ...
    begin
      ...
      rom_size_t := (32 shl gb_head.rom_size) div 16;
...
begin
  ...
  start_video(160, 144);
  // Main CPU
  lr35902_0 := cpu_lr.create(GB_CLOCK, 154);

...
const
  cs_icon: array [0 .. 3] of string = ('Working just fine', 'Working with minor errors',
    'Working with major errors', 'Not working at all');
...
type
  ...
  public
    ...
    procedure grid_view_change(Sender: TObject; const OldViewportPos, NewViewportPos: TPointF;
    ...
    procedure show_game(show: boolean);
    ...
    procedure edit_dt_info_DragOver(Sender: TObject; const Data: TDragObject; const Point: TPointF;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if dm.tArcadeTGDBImagesimg_type.AsString = 'clearlogo' then
          imgLogo.Bitmap.LoadFromFile(dm.tArcadeConfigtgdb_images.AsString +
            dm.tArcadeTGDBImagesfilename.AsString);
    ...
    dm.tArcadeTGDBImages.Filtered := False;
    dm.tArcadeTGDBImages.Filter := 'rom = ' + game.QuotedString + ' AND img_type = ' +
      QuotedStr('screenshot');
    dm.tArcadeTGDBImages.Filtered := True;
  ...
  main.frm_main.txt_stb_main_total.Text := 'Total play time : ' + multi_platform.int_to_time
    (dm.tArcadetotal_time.AsInteger);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      edtInfoDeveloper.Text := dm.tArcadeTGDBdevelopers.AsString;
...
procedure TFRONTEND.edit_dt_info_DragOver(Sender: TObject; const Data: TDragObject; const Point: TPointF;
  var Operation: TDragOperation);
begin
...
begin
  ...
  begin
    ...
    // if edit then
    // main.frm_main.img_grid_info.Opacity := 0.5
    // else
    // main.frm_main.img_grid_info.Opacity := 1;
...
begin
  ...
  if ((filters[0] = fil_None) and (filters[1] = fil_None) and (filters[2] = fil_None) and
    (filters[3] = fil_None)) or ((filters[0] = fil_Working) and (filters[1] = fil_Working_With_Minor) and
    (filters[2] = fil_Working_With_Major) and (filters[3] = fil_Not_Working)) then
  begin
    filters[0] := fil_All;
    filters[1] := fil_None;
    filters[2] := fil_None;
...
procedure TFRONTEND.grid_view_change(Sender: TObject; const OldViewportPos, NewViewportPos: TPointF;
  const ContentSizeChanged: boolean);
var
  ...
  begin
    for vi := 0 to High(grid_rect) - 1 do
      ...
      begin
        ...
        frm_main.vsb_grid.ViewportPosition := PointF(frm_main.vsb_grid.ViewportPosition.X,
          grid_rect[vi].Position.Y);
        break;
      end;
...
procedure TFRONTEND.show_game(show: boolean);
begin
  frm_main.ani_game.Start;
end;

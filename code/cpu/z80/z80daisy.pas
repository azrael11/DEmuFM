...
procedure z80daisy_init(tipo0: byte = Z80_DAISY_NONE; tipo1: byte = Z80_DAISY_NONE;
  tipo2: byte = Z80_DAISY_NONE; numero0: byte = 0; numero1: byte = 0; numero2: byte = 0);
...
var
  f, state: byte;
begin
  for f := 0 to z80daisy_num_dev do
  begin
    if @z80daisy_llamadas[f].state <> nil then
    begin
      state := z80daisy_llamadas[f].state(z80daisy_llamadas[f].device_num);
      if (state and Z80_DAISY_INT) <> 0 then
      begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if (state and Z80_DAISY_IEO) <> 0 then
      begin

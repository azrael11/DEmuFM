unit mb88xx;

interface

uses
  WinApi.Windows,
  main_engine,
  FMX.Dialogs,
  System.SysUtils,
  timer_engine,
  cpu_misc;

type
  type_mb88xx_inport_r = function(port: byte): byte;
  type_mb88xx_outport_r = procedure(port, valor: byte);

  reg_mb88xx = record
    pc: byte; // Program Counter: 6 bits
    pa: byte; // Page Address: 4 bits
    sp: array [0 .. 4 - 1] of word; // Stack is 4*10 bit addresses deep, but we also use 3 top bits per address to store flags during irq
    si: byte; // Stack index: 2 bits
    a: byte; // Accumulator: 4 bits
    x: byte; // Index X: 4 bits
    y: byte; // Index Y: 4 bits
    st: boolean; // State flag: 1 bit
    zf: boolean; // Zero flag: 1 bit
    cf: boolean; // Carry flag: 1 bit
    vf: boolean; // Timer overflow flag: 1 bit
    sf: boolean; // Serial Full/Empty flag: 1 bit
    nf: boolean; // Interrupt flag: 1 bit
  end;

  preg_mb88xx = ^reg_mb88xx;

  cpu_mb88xx = class(cpu_class)
    constructor create(clock: dword; frames_div: word; serial_cb: exec_type_simple = nil);
    destructor free;
  public
    port_k, port_p_r: cpu_inport_call;
    port_o, port_p_w: cpu_outport_call;
    port_r_r: type_mb88xx_inport_r;
    port_r_w: type_mb88xx_outport_r;
    procedure reset;
    procedure run(maximo: single);
    procedure set_irq_line(state: byte);
    procedure change_io_calls(port_k: cpu_inport_call; port_o: cpu_outport_call; port_p_r: cpu_inport_call; port_p_w: cpu_outport_call; port_r_r: type_mb88xx_inport_r; port_r_w: type_mb88xx_outport_r);
    function get_rom_addr: pbyte;
    procedure clock_w(state: byte);
  private
    r: preg_mb88xx;
    // Peripheral Control
    pio: byte; // Peripheral enable bits: 8 bits
    // Timer registers
    th: byte; // Timer High: 4 bits
    tl: byte; // Timer Low: 4 bits
    tp: byte; // Timer Prescale: 6 bits?
    ctr: byte; // current external counter value
    // Serial registers
    sb: byte; // Serial buffer: 4 bits
    timer_serial: byte;
    sbcount: word; // number of bits received
    // PLA configuration
    in_irq: boolean;
    // IRQ handling
    pending_interrupt: byte;
    // cpu_irq_callback irqcallback;
    rom: array [0 .. $7FF] of byte;
    ram: array [0 .. $7F] of byte;
    // Functions
    procedure update_pio_enable(newpio: byte);
    procedure increment_timer;
    function update_pio(cycles: byte): byte;
  end;

procedure mb88_serial_advance(cpu: cpu_mb88xx);

var
  mb88xx_0: cpu_mb88xx;

implementation

const
  INT_CAUSE_SERIAL = 01;
  INT_CAUSE_TIMER = 02;
  INT_CAUSE_EXTERNAL = 04;
  SERIAL_PRESCALE = 16;
  TIMER_PRESCALE = 32;
  SERIAL_DISABLE_THRESH = 1000;
  MB88_PORTK = 0; // input only, 4 bits
  MB88_PORTO = 1; // output only, PLA function output
  MB88_PORTP = 2; // 4 bits
  MB88_PORTR0 = 3; // R0-R3, 4 bits
  MB88_PORTR1 = 4; // R4-R7, 4 bits
  MB88_PORTR2 = 5; // R8-R11, 4 bits
  MB88_PORTR3 = 6; // R12-R15, 4 bits
  MB88_PORTSI = 7; // SI, 1 bit

procedure mb88_serial_advance(cpu: cpu_mb88xx);
begin
  cpu.sbcount := cpu.sbcount + 1;
  // if we get too many interrupts with no servicing, disable the timer
  // until somebody does something
  if (cpu.sbcount >= SERIAL_DISABLE_THRESH) then
    timers.enabled(cpu.timer_serial, false);
  // only read if not full; this is needed by the Namco 52xx to ensure that
  // the program can write to S and recover the value even if serial is enabled
  if not(cpu.r.sf) then
  begin
    cpu.sb := (cpu.sb shr 1) or 1; // (m_read_si() ? 8 : 0);
    if (cpu.sbcount >= 4) then
    begin
      cpu.r.sf := true;
      cpu.pending_interrupt := cpu.pending_interrupt or INT_CAUSE_SERIAL;
    end;
  end;
end;

constructor cpu_mb88xx.create(clock: dword; frames_div: word; serial_cb: exec_type_simple = nil);
begin
  getmem(self.r, sizeof(reg_mb88xx));
  fillchar(self.r^, sizeof(reg_mb88xx), 0);
  self.numero_cpu := cpu_main_init(clock div 6);
  self.clock := clock div 6;
  self.tframes := (clock / 6 / frames_div) / machine_calls.fps_max;
  if @serial_cb <> nil then
    self.timer_serial := timers.init(self.numero_cpu, self.clock / SERIAL_PRESCALE, serial_cb, nil, false);
  self.port_k := nil;
  self.port_p_r := nil;
  self.port_o := nil;
  self.port_p_w := nil;
  self.port_r_r := nil;
  self.port_r_w := nil;
end;

destructor cpu_mb88xx.free;
begin
  freemem(self.r);
end;

function cpu_mb88xx.get_rom_addr: pbyte;
begin
  get_rom_addr := @self.rom[0];
end;

procedure cpu_mb88xx.change_io_calls(port_k: cpu_inport_call; port_o: cpu_outport_call; port_p_r: cpu_inport_call; port_p_w: cpu_outport_call; port_r_r: type_mb88xx_inport_r; port_r_w: type_mb88xx_outport_r);
begin
  self.port_k := port_k;
  self.port_o := port_o;
  self.port_p_r := port_p_r;
  self.port_p_w := port_p_w;
  self.port_r_r := port_r_r;
  self.port_r_w := port_r_w;
end;

procedure cpu_mb88xx.set_irq_line(state: byte);
begin
  // on falling edge trigger interrupt
  if (((self.pio and 4) <> 0) and not(r.nf) and (state <> CLEAR_LINE)) then
  begin
    self.pending_interrupt := self.pending_interrupt or INT_CAUSE_EXTERNAL;
  end;
  if state <> CLEAR_LINE then
    r.nf := true
  else
    r.nf := false;
end;

procedure cpu_mb88xx.clock_w(state: byte);
begin
  if (state <> self.ctr) then
  begin
    self.ctr := state;
    // on a falling clock, increment the timer, but only if enabled
    if ((self.ctr = 0) and ((self.pio and $40) <> 0)) then
      self.increment_timer;
  end;
end;

procedure cpu_mb88xx.reset;
begin
  r.pc := 0;
  r.pa := 0;
  r.sp[0] := 0;
  r.sp[1] := 0;
  r.sp[2] := 0;
  r.sp[3] := 0;
  r.si := 0;
  r.a := 0;
  r.x := 0;
  r.y := 0;
  r.st := true;
  r.zf := false;
  r.cf := false;
  r.vf := false;
  r.sf := false;
  r.nf := false;
  self.in_irq := false;
  self.pio := 0;
  self.ctr := 0;
  self.th := 0;
  self.tl := 0;
  self.tp := 0;
  self.sb := 0;
  self.sbcount := 0;
  self.pending_interrupt := 0;
  self.change_reset(CLEAR_LINE);
end;

function get_pc(r: preg_mb88xx): word;
begin
  get_pc := (r.pa shl 6) + r.pc;
end;

procedure cpu_mb88xx.update_pio_enable(newpio: byte);
begin
  // if the serial state has changed, configure the timer
  if ((self.pio xor newpio) and $30) <> 0 then
  begin
    if ((newpio and $30) = 0) then
      timers.enabled(self.timer_serial, false)
    else if ((newpio and $30) = $20) then
      timers.enabled(self.timer_serial, true)
    else; // fatalerror("mb88xx: update_pio_enable set serial enable to unsupported value %02X\n", newpio & 0x30);
  end;
  self.pio := newpio;
end;

procedure cpu_mb88xx.increment_timer;
begin
  self.tl := (self.tl + 1) and $F;
  if (self.tl = 0) then
  begin
    self.th := (self.th + 1) and $F;
    if (self.th = 0) then
    begin
      r.vf := true;
      self.pending_interrupt := self.pending_interrupt or INT_CAUSE_TIMER;
    end;
  end;
end;

function cpu_mb88xx.update_pio(cycles: byte): byte;
var
  estados: byte;
begin
  estados := 0;
  // internal clock enable
  if (self.pio and $80) <> 0 then
  begin
    self.tp := self.tp + cycles;
    while (self.tp >= TIMER_PRESCALE) do
    begin
      self.tp := self.tp - TIMER_PRESCALE;
      self.increment_timer;
    end;
  end;
  // process pending interrupts
  if (not(self.in_irq) and ((self.pending_interrupt and self.pio) <> 0)) then
  begin
    self.in_irq := true;
    r.sp[r.si] := get_pc(r);
    r.sp[r.si] := r.sp[r.si] or (byte(r.cf) shl 15);
    r.sp[r.si] := r.sp[r.si] or (byte(r.zf) shl 14);
    r.sp[r.si] := r.sp[r.si] or (byte(r.st) shl 13);
    r.si := (r.si + 1) and 3;
    // the datasheet doesn't mention interrupt vectors but
    // the Arabian MCU program expects the following
    if (self.pending_interrupt and self.pio and INT_CAUSE_EXTERNAL) <> 0 then
    begin
      // if we have a live external source, call the irqcallback
      // (*cpustate->irqcallback)(cpustate->device, 0);
      r.pc := 2;
    end
    else if (self.pending_interrupt and self.pio and INT_CAUSE_TIMER) <> 0 then
    begin
      r.pc := 4;
    end
    else if (self.pending_interrupt and self.pio and INT_CAUSE_SERIAL) <> 0 then
    begin
      r.pc := 6;
    end;
    r.pa := 0;
    r.st := true;
    self.pending_interrupt := 0;
    estados := 3;
  end;
  update_pio := estados;
end;

procedure cpu_mb88xx.run(maximo: single);
  procedure inc_pc(r: preg_mb88xx);
  begin
    r.pc := r.pc + 1;
    if r.pc = $40 then
    begin
      r.pc := 0;
      r.pa := r.pa + 1;
    end;
  end;

  function get_ea(r: preg_mb88xx): word;
  begin
    get_ea := (r.x shl 4) + r.y;
  end;

  function pla(r: preg_mb88xx; inA, inB: byte): byte;
  var
    index: byte;
  begin
    index := ((inB and 1) shl 4) or (inA and $F);
    // if @r.PLA<>nil then return cpustate->PLA[index];
    pla := index;
  end;

var
  instruccion, timming, arg, tempb: byte;
begin
  if self.pedir_reset <> CLEAR_LINE then
  begin
    tempb := self.pedir_reset;
    self.reset;
    if tempb = ASSERT_LINE then
    begin
      self.pedir_reset := ASSERT_LINE;
      self.contador := trunc(maximo);
      exit;
    end;
  end;
  self.contador := 0;
  while self.contador < maximo do
  begin
    timming := 1;
    { if ((r.pa shl 6)+r.pc)=$97 then begin
      r.pa:=0;
      r.pa:=2;
      end;
      if self.ram[9]=$f then begin
      self.ram[9]:=0;
      self.ram[9]:=$f;
      end; }
    instruccion := self.rom[get_pc(r)];
    inc_pc(r);
    // Comprobar irq
    case instruccion of
      0:
        r.st := true; // nop
      1:
        begin // outO
          if @self.port_o <> nil then
            self.port_o(pla(r, r.a, byte(r.cf)));
          r.st := true;
        end;
      2:
        begin // outP
          if @self.port_p_w <> nil then
            self.port_p_w(r.a);
          r.st := true;
        end;
      3:
        begin // outR
          arg := r.y;
          if @self.port_r_w <> nil then
            self.port_r_w(arg and 3, r.a);
          r.st := true;
        end;
      4:
        begin // tay
          r.y := r.a;
          r.st := true;
        end;
      5:
        begin // tath
          self.th := r.a;
          r.st := true;
        end;
      6:
        begin // tatl
          self.tl := r.a;
          r.st := true;
        end;
      7:
        begin // tas
          self.sb := r.a;
          r.st := true;
        end;
      8:
        begin // icy
          r.y := r.y + 1;
          r.st := (r.y and $10) = 0;
          r.y := r.y and $F;
          r.zf := (r.y = 0);
        end;
      9:
        begin // icm
          arg := self.ram[get_ea(r)] and $F;
          arg := arg + 1;
          r.st := (arg and $10) = 0;
          arg := arg and $F;
          r.zf := (arg = 0);
          self.ram[get_ea(r)] := arg;
        end;
      $A:
        begin // stic
          self.ram[get_ea(r)] := r.a and $F;
          r.y := r.y + 1;
          r.st := (r.y and $10) = 0;
          r.y := r.y and $F;
          r.zf := (r.y = 0);
        end;
      $B:
        begin // x
          arg := self.ram[get_ea(r)];
          self.ram[get_ea(r)] := r.a;
          r.a := arg;
          r.zf := (r.a = 0);
          r.st := true;
        end;
      $C:
        begin // rol
          r.a := r.a shl 1;
          r.a := r.a or byte(r.cf);
          r.st := (r.a and $10) = 0;
          r.cf := not(r.st);
          r.a := r.a and $F;
          r.zf := (r.a = 0);
        end;
      $D:
        begin // l
          r.a := self.ram[get_ea(r)];
          r.zf := (r.a = 0);
          r.st := true;
        end;
      $E:
        begin // adc
          arg := self.ram[get_ea(r)];
          arg := arg + r.a + byte(r.cf);
          r.st := (arg and $10) = 0;
          r.cf := not(r.st);
          r.a := arg and $F;
          r.zf := (r.a = 0);
        end;
      $F:
        begin // and ZCS:x.x
          r.a := r.a and self.ram[get_ea(r)];
          r.zf := (r.a = 0);
          r.st := not(r.zf);
        end;
      $10:
        begin // daa ZCS:.xx
          if (r.cf or (r.a > 9)) then
            r.a := r.a + 6;
          r.st := (r.a and $10) = 0;
          r.cf := not(r.st);
          r.a := r.a and $F;
        end;
      $11:
        begin // das ZCS:.xx
          if (r.cf or (r.a > 9)) then
            r.a := r.a + 10;
          r.st := (r.a and $10) = 0;
          r.cf := not(r.st);
          r.a := r.a and $F;
        end;
      $12:
        begin // inK
          if @self.port_k <> nil then
            r.a := self.port_k and $F;
          r.zf := (r.a = 0);
          r.st := true;
        end;
      $13:
        begin // inR
          if @self.port_r_r <> nil then
            r.a := self.port_r_r(r.y and 3) and $F;
          r.zf := (r.a = 0);
          r.st := true;
        end;
      $14:
        begin // tya
          r.a := r.y;
          r.zf := (r.a = 0);
          r.st := true;
        end;
      $15:
        begin // ttha
          r.a := self.th;
          r.zf := (r.a = 0);
          r.st := true;
        end;
      $16:
        begin // ttla
          r.a := self.tl;
          r.zf := (r.a = 0);
          r.st := true;
        end;
      $17:
        begin // tsa
          r.a := self.sb;
          r.zf := (r.a = 0);
          r.st := true;
        end;
      $18:
        begin // dcy
          r.y := r.y - 1;
          r.st := (r.y and $10) = 0;
          r.y := r.y and $F;
        end;
      $19:
        begin // dcm
          arg := self.ram[get_ea(r)];
          arg := arg - 1;
          r.st := (arg and $10) = 0;
          arg := arg and $F;
          r.zf := (arg = 0);
          self.ram[get_ea(r)] := arg;
        end;
      $1A:
        begin // stdc ZCS:x.x
          self.ram[get_ea(r)] := r.a and $F;
          r.y := r.y - 1;
          r.st := (r.y and $10) = 0;
          r.y := r.y and $F;
          r.zf := (r.y = 0);
        end;
      $1B:
        begin // xx
          arg := r.x;
          r.x := r.a;
          r.a := arg;
          r.zf := (r.a = 0);
          r.st := true;
        end;
      $1C:
        begin // ror
          r.a := r.a or (byte(r.cf) shl 4);
          r.st := (r.a and 1) = 0;
          r.cf := not(r.st);
          r.a := (r.a shr 1) and $F;
          r.zf := (r.a = 0);
        end;
      $1D:
        begin // st
          self.ram[get_ea(r)] := r.a and $F;
          r.st := true;
        end;
      $1E:
        begin // sbc
          arg := self.ram[get_ea(r)];
          arg := arg - r.a - byte(r.cf);
          r.st := (arg and $10) = 0;
          r.cf := not(r.st);
          r.a := arg and $F;
          r.zf := (r.a = 0);
        end;
      $1F:
        begin // or
          r.a := r.a or self.ram[get_ea(r)];
          r.zf := (r.a = 0);
          r.st := not(r.zf);
        end;
      $20:
        begin // setR ZCS:...
          if @self.port_r_r <> nil then
            arg := self.port_r_r(r.y div 4) and $F;
          if @self.port_r_w <> nil then
            self.port_r_w(r.y div 4, arg or (1 shl (r.y mod 4)));
          r.st := true;
        end;
      $21:
        begin // setc
          r.cf := true;
          r.st := true;
        end;
      $22:
        begin // rstR ZCS:...
          if @self.port_r_r <> nil then
            arg := self.port_r_r(r.y div 4);
          if @self.port_r_w <> nil then
            self.port_r_w(r.y div 4, arg and not(1 shl (r.y mod 4)));
          r.st := true;
        end;
      $23:
        begin // rstc
          r.cf := false;
          r.st := true;
        end;
      $24:
        begin // tstr
          if @self.port_r_r <> nil then
            arg := self.port_r_r(r.y div 4) and $F;
          r.st := (arg and (1 shl (r.y mod 4))) = 0;
        end;
      $25:
        r.st := not(r.nf); // tsti
      $28:
        r.st := not(r.cf); // tstc
      $2C:
        begin // rts
          r.si := (r.si - 1) and 3;
          r.pc := r.sp[r.si] and $3F;
          r.pa := (r.sp[r.si] shr 6) and $1F;
          r.st := true;
        end;
      $2D:
        begin // neg ZCS: ..x
          r.a := not(r.a) + 1;
          r.a := r.a and $F;
          r.st := (r.a <> 0);
        end;
      $2E:
        begin // c
          arg := self.ram[get_ea(r)];
          arg := arg - r.a;
          r.cf := (arg and $10) <> 0;
          arg := arg and $F;
          r.st := (arg <> 0);
          r.zf := not(r.st);
        end;
      $2F:
        begin // eor ZCS:x.x
          r.a := r.a xor self.ram[get_ea(r)];
          r.st := (r.a <> 0);
          r.zf := not(r.st);
        end;
      $30 .. $33:
        begin // sbit
          arg := self.ram[get_ea(r)];
          self.ram[get_ea(r)] := arg or (1 shl (instruccion and 3));
          r.st := true;
        end;
      $34 .. $37:
        begin // rbit
          arg := self.ram[get_ea(r)];
          self.ram[get_ea(r)] := arg and not(1 shl (instruccion and 3));
          r.st := true;
        end;
      $38 .. $3B:
        begin // tbit
          arg := self.ram[get_ea(r)];
          if (arg and (1 shl (instruccion and 3))) <> 0 then
            r.st := false
          else
            r.st := true;
        end;
      $3C:
        begin // rti
          self.in_irq := false;
          r.si := (r.si - 1) and 3;
          r.pc := r.sp[r.si] and $3F;
          r.pa := (r.sp[r.si] shr 6) and $1F;
          r.st := ((r.sp[r.si] shr 13) and 1) <> 0;
          r.zf := ((r.sp[r.si] shr 14) and 1) <> 0;
          r.cf := ((r.sp[r.si] shr 15) and 1) <> 0;
        end;
      $3D:
        begin // jpa imm
          r.pa := self.rom[get_pc(r)] and $1F;
          r.pc := r.a * 4;
          timming := 2;
          r.st := true;
        end;
      $3E:
        begin // en imm
          self.update_pio_enable(self.pio or self.rom[get_pc(r)]);
          inc_pc(r);
          timming := 2;
          r.st := true;
        end;
      $3F:
        begin // dis
          self.update_pio_enable(self.pio and not(self.rom[get_pc(r)]));
          inc_pc(r);
          timming := 2;
          r.st := true;
        end;
      $40 .. $43:
        begin // setD
          if @self.port_r_r <> nil then
            arg := self.port_r_r(0) and $F;
          arg := arg or (1 shl (instruccion and 3));
          if @self.port_r_w <> nil then
            self.port_r_w(0, arg);
          r.st := true;
        end;
      $44 .. $47:
        begin // rstD
          if @self.port_r_r <> nil then
            arg := self.port_r_r(0) and $F;
          arg := arg and not(1 shl (instruccion and 3));
          if @self.port_r_w <> nil then
            self.port_r_w(0, arg);
          r.st := true;
        end;
      $48 .. $4B:
        begin // tstD ZCS:..x
          if @self.port_r_r <> nil then
            arg := self.port_r_r(2);
          if (arg and (1 shl (instruccion and 3))) <> 0 then
            r.st := false
          else
            r.st := true;
        end;
      $4C .. $4F:
        if (r.a and (1 shl (instruccion and 3))) <> 0 then
          r.st := false
        else
          r.st := true; // tba
      $50 .. $53:
        begin // xd
          arg := self.ram[instruccion and 3];
          self.ram[instruccion and 3] := r.a and $F;
          r.a := arg;
          r.zf := (r.a = 0);
          r.st := true;
        end;
      $54 .. $57:
        begin // xyd
          arg := self.ram[(instruccion and 3) + 4];
          self.ram[(instruccion and 3) + 4] := r.y and $F;
          r.y := arg;
          r.zf := (r.y = 0);
          r.st := true;
        end;
      $58 .. $5F:
        begin // lxi
          r.x := instruccion and 7;
          r.zf := (r.x = 0);
          r.st := true;
        end;
      $60 .. $67:
        begin // call imm
          arg := self.rom[get_pc(r)];
          inc_pc(r);
          timming := 2;
          if r.st then
          begin
            r.sp[r.si] := get_pc(r);
            r.si := (r.si + 1) and 3;
            r.pc := arg and $3F;
            r.pa := ((instruccion and 7) shl 2) or (arg shr 6);
          end;
          r.st := true;
        end;
      $68 .. $6F:
        begin // jpl imm
          arg := self.rom[get_pc(r)];
          inc_pc(r);
          timming := 2;
          if r.st then
          begin
            r.pc := arg and $3F;
            r.pa := ((instruccion and 7) shl 2) or (arg shr 6);
          end;
          r.st := true;
        end;
      $70 .. $7F:
        begin // ai
          arg := instruccion and $F;
          arg := arg + r.a;
          r.st := (arg and $10) = 0;
          r.cf := not(r.st);
          r.a := arg and $F;
          r.zf := (r.a = 0);
        end;
      $80 .. $8F:
        begin // lyi
          r.y := instruccion and $F;
          r.zf := (r.y = 0);
          r.st := true;
        end;
      $90 .. $9F:
        begin // li
          r.a := instruccion and $F;
          r.zf := (r.a = 0);
          r.st := true;
        end;
      $A0 .. $AF:
        begin // cyi
          arg := (instruccion and $F) - r.y;
          r.cf := (arg and $10) <> 0;
          arg := arg and $F;
          r.st := (arg <> 0);
          r.zf := not(r.st);
        end;
      $B0 .. $BF:
        begin // ci
          arg := (instruccion and $F) - r.a;
          r.cf := (arg and $10) <> 0;
          arg := arg and $F;
          r.st := (arg <> 0);
          r.zf := not(r.st);
        end;
      $C0 .. $FF:
        begin // jmp
          if r.st then
            r.pc := instruccion and $3F;
          r.st := true;
        end;
    else
      begin
        // MessageDlg('Instruccion MB88XX ' + inttohex(instruccion, 2) + ' desconocida. PC=' + inttohex(get_pc(r) - 1, 10), mtInformation, [mbOk], 0);
      end;
    end;
    tempb := timming + self.update_pio(timming);
    self.contador := self.contador + tempb;
    timers.update(tempb, self.numero_cpu);
  end;
end;

end.

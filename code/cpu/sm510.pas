...
type
  ...
  public
    ...
    procedure reset;
    procedure change_io_calls(read_b, read_k: tread_X; write_s, write_r: twrite_X;
  ...
  private
    ...
    l, x, y, melody_rd, melody_step_count, melody_duty_count, melody_duty_index, melody_address,
      stack_levels, tipo: byte;
...
begin
  ...
  begin
    ...
    if @sm510_0.write_sega <> nil then
      sm510_0.write_sega(h or SM510_PORT_SEGA, sm510_0.get_lcd_row(h, @sm510_0.ram[$60]));
    if @sm510_0.write_segb <> nil then
      sm510_0.write_segb(h or SM510_PORT_SEGB, sm510_0.get_lcd_row(h, @sm510_0.ram[$70]));
    if @sm510_0.write_segc <> nil then
      sm510_0.write_segc(h or SM510_PORT_SEGC, sm510_0.get_lcd_row(h, nil));
    // bs output from L/X and Y regs
    if @sm510_0.write_segbs <> nil then
    begin
      bs := ((sm510_0.l shr h) and 1) or (((sm510_0.x * 2) shr h) and 2);
      if (sm510_0.bc or not(sm510_0.bp)) then
        sm510_0.write_segbs(h or SM510_PORT_SEGBS, 0)
      else
        sm510_0.write_segbs(h or SM510_PORT_SEGBS, bs);
    end;
  end;
end;
...
begin
  ...
  self.tframes := clock / 2 / machine_calls.fps_max / frames;
...
begin
  ...
  self.r.r := 0;
  self.r.r_out := 0;
  if @self.write_r <> nil then
    self.write_r(0);
end;

procedure cpu_sm510.change_io_calls(read_b, read_k: tread_X; write_s, write_r: twrite_X;
  write_sega, write_segb, write_segc, write_segbs: twrite_lcd);
begin
  self.read_b := read_b;
  self.read_k := read_k;
...
begin
  if (line <> SM510_INPUT_LINE_K) then
    exit;
...
begin
  ...
  if ((ram = nil) or self.bc or not(self.bp)) then
  begin
    get_lcd_row := 0;
    exit;
  end;
...
begin
  ...
  if ((self.r.pc shr 1 xor self.r.pc) and 1) <> 0 then
    feed := 0
  else
    feed := $20;
...
var
  i: byte;
begin
  for i := self.stack_levels downto 1 do
...
begin
  ...
  for i := 0 to (self.stack_levels - 1) do
    self.stack[i] := self.stack[i + 1];
end;
...
begin
  ...
  if (self.k_active or self.one_s) then
  begin
...
begin
  case self.tipo of
    SM_510:
      if @self.write_s <> nil then
        self.write_s(self.r.w);
  end;
end;
...
begin
  ...
  begin
    if (self.halt_ and not(self.wake_me_up)) then
    begin
      // got nothing to do
    ...
    begin
      case (self.op and $F0) of
        $20:
          if ((self.op and not($F)) <> (self.prev_op and not($F))) then
            self.r.acc := self.op and $F; // LAX x: load ACC with immediate value, skip any next LAX
        $30:
          begin // ADX x: add immediate value to ACC, skip next on carry except if x = 10
            self.r.acc := self.r.acc + (self.op and $F);
            self.skip := (((self.op and $F) <> 10) and ((self.r.acc and $10) <> 0));
            self.r.acc := self.r.acc and $F;
          end;
        $40:
          begin // LB x: load BM/BL with 4-bit immediate value (partial)
            self.r.bm := (self.r.bm and 4) or (self.op and 3);
            if (self.op and $C) <> 0 then
              self.r.bl := (self.op shr 2 and 3) or $C
            else
              self.r.bl := (self.op shr 2 and 3);
          end;
        $80, $90, $A0, $B0:
          self.r.pc := (self.r.pc and not($3F)) or (self.op and $3F);
        // T xy: jump(transfer) within current page
        $C0, $D0, $E0, $F0:
          begin // TM x: indirect subroutine call, pointers(IDX) are in page 0
            estados_demas := 2;
            self.push_stack;
        ...
        case (self.op and $FC) of
          $04:
            self.ram_w(self.ram_r and not(bitmask(self.op))); // RM x: reset RAM bit
          $0C:
            self.ram_w(self.ram_r or bitmask(self.op)); // SM x: set RAM bit
          $10:
            begin // EXC x: exchange ACC with RAM, xor BM with x
              tempb := self.r.acc;
              self.r.acc := self.ram_r;
              self.ram_w(tempb);
              self.r.bm := self.r.bm xor (self.op and 3);
          ...
          $14:
            begin // EXCI x: EXC x, INCB
              self.op_exc;
              self.op_incb;
            end;
          $18:
            begin // LDA x: load ACC with RAM, xor BM with x
              self.r.acc := self.ram_r;
              self.r.bm := self.r.bm xor (self.op and 3);
            end;
          $1C:
            begin // EXCD x: EXC x, DECB
              self.op_exc;
              self.op_decb;
            end;
          $54:
            self.skip := (self.ram_r and bitmask(self.op)) <> 0;
          // TMI x: skip next if RAM bit is set
          $70, $74, $78:
            begin // TL xyz: long jump
              estados_demas := 2;
              param := self.rom[r.pc];
              self.increment_pc;
          ...
          case self.op of
            $00:
              ; // SKIP: no operation
            $01:
              self.bp := (self.r.acc and 1) <> 0;
            // ATBP: output ACC to BP(internal LCD backplate signal)
            $02:
              ; // SBM: set BM high bit for next opcode - se hace al final de los opcodes
            $03:
              self.r.pc := (self.r.old_pc and not($F)) or self.r.acc;
            // ATPL: load Pl(PC low bits) with ACC
            $08:
              self.r.acc := (self.r.acc + self.ram_r) and $F; // ADD: add RAM to ACC
            $09:
              begin // ADD11: add RAM and carry to ACC and carry, skip next on carry
            ...
            $0A:
              self.r.acc := self.r.acc xor $F; // COMA: complement ACC
            $0B:
              begin // EXBLA: exchange BL with ACC
                tempb := self.r.acc;
                self.r.acc := self.r.bl;
                self.r.bl := tempb;
              end;
            $51:
              if @self.read_b <> nil then
                self.skip := (self.read_b <> 0) // TB: skip next if B(beta) pin is set
              else
                self.skip := true;
            $52:
              self.skip := not(self.r.c); // TC: skip next if no carry
            $53:
              self.skip := (self.r.acc = self.ram_r); // TAM: skip next if ACC equals RAM
            $58:
              begin // TIS: skip next if 1S(gamma flag) is clear, reset it after
                self.skip := not(self.one_s);
                self.one_s := false;
              end;
            $59:
              self.l := self.r.acc; // ATL: output ACC to L
            $5A:
              self.skip := (self.r.acc = 0); // TA0: skip next if ACC is clear
            $5B:
              self.skip := (self.r.acc = self.r.bl); // TABL: skip next if ACC equals BL
            $5D:
              self.halt_ := true; // CEND: stop clock (halt the cpu and go into low-power mode)
            $5E:
              if @self.read_ba <> nil then
                self.skip := (self.read_ba <> 0) // TAL: skip next if BA pin is set
              else
                self.skip := true;
            $5F:
              begin // LBL xy: load BM/BL with 8-bit immediate value
                estados_demas := 2;
                param := self.rom[r.pc];
                self.increment_pc;
            ...
            $60:
              self.y := self.r.acc; // ATFC: output ACC to Y
            $61:
              self.r.r := self.r.acc; // ATR: output ACC to R
            $62:
              begin // WR: shift 0 into W
                self.r.w := ((self.r.w shl 1) and $FF) or 0;
                self.update_w_latch;
              end;
            $63:
              begin // WS: shift 1 into W
                self.r.w := ((self.r.w shl 1) and $FF) or 1;
                self.update_w_latch;
              end;
            $64:
              self.op_incb; // INCB: increment BL, skip next on overflow
            $65:
              self.div_ := 0; // IDIV: reset divider
            $66:
              self.r.c := false; // RC: reset carry
            $67:
              self.r.c := true; // SC: set carry
            $68:
              self.skip := (self.div_ and $4000) <> 0; // TF1: skip next if divider F1(d14) is set
            $69:
              self.skip := (self.div_ and $800) <> 0; // TF4: skip next if divider F4(d11) is set
            $6A:
              if @self.read_k <> nil then
                self.r.acc := self.read_k and $F // KTA: input K to ACC
              else
                self.r.acc := $F;
            $6B:
              begin // ROT: rotate ACC right through carry
                tempb := self.r.acc and 1;
                self.r.acc := (self.r.acc shr 1) or (byte(self.r.c) shl 3);
                self.r.c := (tempb <> 0);
              end;
            $6C:
              self.op_decb; // DECB: decrement BL, skip next on overflow
            $6D:
              halt(0); // op_bdc
            $6E:
              self.pop_stack; // RTN0: return from subroutine
            $6F:
              begin // RTN1: return from subroutine, skip next
                self.pop_stack;
                self.skip := true;
              end;
          else
...
//                inttohex(self.op, 2) + ' desconocida. PC=' + inttohex(self.r.old_pc - 1, 10),

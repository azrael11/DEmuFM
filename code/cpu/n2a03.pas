...
const
  ...
  vbl_length: array [0 .. 31] of byte = (10, 254, 20, 2, 40, 4, 80, 6, 160, 8, 60, 10, 14, 12, 26, 14, 12, 16, 24, 18, 48, 20, 96, 22, 192, 24, 72, 26, 16, 28, 32, 30);
  // frequency limit of square channels
  freq_limit: array [0 .. 7] of word = ($3FF, $555, $666, $71C, $787, $7C1, $7E0, $7F0);
  // table of noise frequencies
  noise_freq: array [0 .. 15] of word = (4, 8, 16, 32, 64, 96, 128, 160, 202, 254, 380, 508, 762, 1016, 2034, 4068);
  // dpcm transfer freqs
  dpcm_clocks: array [0 .. 15] of word = (428, 380, 340, 320, 286, 254, 226, 214, 190, 160, 142, 128, 106, 85, 72, 54);
...
var
  ...
  val, tnd_out: single;
begin
  val := clock / machine_calls.fps_max / 4;
  for t := 0 to (SYNCS_MAX1 - 1) do
  begin
    vbl_times[t] := trunc(vbl_length[t] * val / 2);
    sync_times1[t] := trunc(val * (t + 1));
  end;
  for t := 0 to (SYNCS_MAX2 - 1) do
    sync_times2[t] := trunc(val * t) shr 2;
  square_lut[0] := 0;
  for t := 1 to 31 do
    square_lut[t] := 95.88 / ((8128 / t) + 100);
  for t := 0 to 15 do
    for n := 0 to 15 do
      for d := 0 to 127 do
      begin
...
begin
  case index of
    0:
      if n2a03_0.frame_irq then
        n2a03_0.m6502.change_irq(HOLD_LINE);
    1:
      if n2a03_1.frame_irq then
        n2a03_1.m6502.change_irq(HOLD_LINE);
  end;
end;
...
begin
  ...
  self.samps_per_sync := round(clock / 4 / machine_calls.fps_max);
  chips_total := chips_total + 1;
  self.frame_irq_timer := timers.init(self.m6502.numero_cpu, clock / machine_calls.fps_max { 29830 } , nil, n2a03_irq_call, false, chips_total);
  case chips_total of
    0:
      begin
        create_syncs(clock);
    ...
    1:
      timers.init(self.m6502.numero_cpu, clock / FREQ_BASE_AUDIO, nil, n2a03_update_sound, true, chips_total);
  end;
...
begin
  ...
  case address of
    // squares
    APU_WRA0, APU_WRB0:
      self.apu.squ[chan].regs[0] := value;
    APU_WRA1, APU_WRB1:
      self.apu.squ[chan].regs[1] := value;
    APU_WRA2, APU_WRB2:
      begin
        self.apu.squ[chan].regs[2] := value;
        if (self.apu.squ[chan].enabled) then
          self.apu.squ[chan].freq := ((((self.apu.squ[chan].regs[3] and 7) shl 8) + value) + 1) shl 16;
      end;
    APU_WRA3, APU_WRB3:
      begin
    ...
    APU_WRC0:
      begin
        ...
        if self.apu.tri.enabled then
        begin
          if not(self.apu.tri.counter_started) then
            self.apu.tri.linear_length := sync_times2[value and $7F];
        end;
      end;
    $09:
      self.apu.tri.regs[1] := value; // unused
    APU_WRC2:
      self.apu.tri.regs[2] := value;
    APU_WRC3:
      begin
    ...
    // noise
    APU_WRD0:
      self.apu.noi.regs[0] := value;
    $0D:
      self.apu.noi.regs[1] := value; // unused
    APU_WRD2:
      self.apu.noi.regs[2] := value;
    APU_WRD3:
      begin
        self.apu.noi.regs[3] := value;
        if self.apu.noi.enabled then
        begin
    ...
    APU_SMASK:
      begin
        ...
        begin
          ...
          if not(self.apu.dpcm.enabled) then
            self.apu_dpcm_reset;
        end
        else
          self.apu.dpcm.enabled := false;
...
begin
  ...
  begin
    readval := 0;
    if self.apu.squ[0].vbl_length <> 0 then
      readval := readval or 1;
    if self.apu.squ[1].vbl_length <> 0 then
      readval := readval or 2;
    if self.apu.tri.vbl_length <> 0 then
      readval := readval or 4;
    if self.apu.noi.vbl_length <> 0 then
      readval := readval or 8;
    if self.apu.dpcm.length <> 0 then
      readval := readval or $10;
    if self.frame_irq then
      readval := readval or $40;
    if self.apu.dpcm.irq then
      readval := readval or $80;
    self.frame_irq := false;
  end
  else
    readval := self.apu.regs[address];
  read := readval;
end;
...
begin
  ...
  if not(self.apu.squ[chan].enabled) then
  begin
  ...
  while (self.apu.squ[chan].env_phase < 0) do
  begin
    self.apu.squ[chan].env_phase := self.apu.squ[chan].env_phase + env_delay;
    if (self.apu.squ[chan].regs[0] and $20) <> 0 then
      self.apu.squ[chan].env_vol := (self.apu.squ[chan].env_vol + 1) and $F
    else if (self.apu.squ[chan].env_vol < 15) then
      self.apu.squ[chan].env_vol := self.apu.squ[chan].env_vol + 1;
  end;
  // vbl length counter
  if ((self.apu.squ[chan].vbl_length > 0) and ((self.apu.squ[chan].regs[0] and $20) = 0)) then
    self.apu.squ[chan].vbl_length := self.apu.squ[chan].vbl_length - 1;
  if (self.apu.squ[chan].vbl_length = 0) then
  begin
    self.apu.squ[chan].output_ := 0;
    exit;
  end;
  // freqsweeps
  if (((self.apu.squ[chan].regs[1] and $80) <> 0) and ((self.apu.squ[chan].regs[1] and 7) <> 0)) then
  begin
    ...
    while (self.apu.squ[chan].sweep_phase < 0) do
    begin
      self.apu.squ[chan].sweep_phase := self.apu.squ[chan].sweep_phase + sweep_delay;
      if (self.apu.squ[chan].regs[1] and 8) <> 0 then
        self.apu.squ[chan].freq := self.apu.squ[chan].freq - (self.apu.squ[chan].freq shr (self.apu.squ[chan].regs[1] and 7))
      else
        self.apu.squ[chan].freq := self.apu.squ[chan].freq + (self.apu.squ[chan].freq shr (self.apu.squ[chan].regs[1] and 7));
    end;
  end;
  if (((self.apu.squ[chan].regs[1] and 8) = 0) and ((self.apu.squ[chan].freq shr 16) > (freq_limit[self.apu.squ[chan].regs[1] and 7])) or ((self.apu.squ[chan].freq shr 16) < 4)) then
  begin
    self.apu.squ[chan].output_ := 0;
    exit;
  end;
  self.apu.squ[chan].phaseacc := self.apu.squ[chan].phaseacc - 4;
  while (self.apu.squ[chan].phaseacc < 0) do
  begin
  ...
  if (self.apu.squ[chan].regs[0] and $10) <> 0 then
    temp3 := self.apu.squ[chan].regs[0] and $F
  else
    temp3 := $F - self.apu.squ[chan].env_vol;
...
begin
  ...
  if not(self.apu.tri.enabled) then
    exit;
  not_held := (self.apu.tri.regs[0] and $80) = 0;
  if (not(self.apu.tri.counter_started) and not_held) then
  begin
    if (self.apu.tri.write_latency) <> 0 then
      self.apu.tri.write_latency := self.apu.tri.write_latency - 1;
    if (self.apu.tri.write_latency = 0) then
      self.apu.tri.counter_started := true;
  end;
  if (self.apu.tri.counter_started) then
  begin
    if self.apu.tri.linear_reload then
      self.apu.tri.linear_length := sync_times2[self.apu.tri.regs[0] and $7F]
    else if (self.apu.tri.linear_length > 0) then
      self.apu.tri.linear_length := self.apu.tri.linear_length - 1;
    if not_held then
      self.apu.tri.linear_reload := false;
    if ((self.apu.tri.vbl_length <> 0) and not_held) then
      self.apu.tri.vbl_length := self.apu.tri.vbl_length - 1;
  end;
  if not((self.apu.tri.linear_length <> 0) and (self.apu.tri.vbl_length <> 0)) then
    exit;
  ...
  while (self.apu.tri.phaseacc < 0) do
  begin
    ...
    if (self.apu.tri.adder and $10) = 0 then
      self.apu.tri.output_ := self.apu.tri.output_ xor $F;
  end;
end;
...
begin
  ...
  if not(self.apu.noi.enabled) then
  begin
  ...
  while (self.apu.noi.env_phase < 0) do
  begin
    self.apu.noi.env_phase := self.apu.noi.env_phase + env_delay;
    if (self.apu.noi.regs[0] and $20) <> 0 then
      self.apu.noi.env_vol := (self.apu.noi.env_vol + 1) and 15
    else if (self.apu.noi.env_vol < 15) then
      self.apu.noi.env_vol := self.apu.noi.env_vol + 1;
  end;
  // length counter
  if (self.apu.noi.regs[0] and $20) = 0 then
  begin
    if (self.apu.noi.vbl_length > 0) then
      self.apu.noi.vbl_length := self.apu.noi.vbl_length - 1;
  end;
  if self.apu.noi.vbl_length = 0 then
  begin
    self.apu.noi.output_ := 0;
  ...
  while (self.apu.noi.phaseacc < 0) do
  begin
  ...
  if (self.apu.noi.lfsr and 1) <> 0 then
  begin // silence channel
    self.apu.noi.output_ := 0;
  end
  else
  begin // fixed volume
    if (self.apu.noi.regs[0] and $10) <> 0 then
      self.apu.noi.output_ := self.apu.noi.regs[0] and $0F
    else
      self.apu.noi.output_ := $0F - self.apu.noi.env_vol;
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        self.apu.dpcm.enabled := false;
        // self.apu.dpcm.vol:=0;
        if (self.apu.dpcm.regs[0] and $40) <> 0 then
        begin // Loop
          self.apu_dpcm_reset;
        end
        else
        begin // Final - IRQ Generator
          if self.apu.dpcm.irq then
            n2a03_0.m6502.change_irq(HOLD_LINE);
          self.apu.dpcm.vol := 0;
          break;
      ...
      begin
        ...
        if self.apu.dpcm.address = $10000 then
          self.apu.dpcm.address := $8000;
        self.apu.dpcm.length := self.apu.dpcm.length - 1;
        self.apu.dpcm.bits_left := 8;
      end;
      if (((self.apu.dpcm.cur_byte and (1 shl bit_pos)) <> 0) and (self.apu.dpcm.vol < 126)) then
        self.apu.dpcm.vol := self.apu.dpcm.vol + 2
      else if (self.apu.dpcm.vol > 1) then
        self.apu.dpcm.vol := self.apu.dpcm.vol - 2;
    end;
  end;
...
begin // Resample from 447443hz to 44100hz
  if self.buffer_pos > 11 then
  begin
    res := 0;
    for f := 1 to 11 do
      res := res + self.buffer[f];
    res := (res / 11) * $7FFF;
    for f := 12 to self.buffer_pos do
      self.buffer[f - 11] := self.buffer[f];
    self.buffer_pos := self.buffer_pos - 11;
    self.old_res := res;
  end
  else
    res := self.old_res;
  tsample[self.tsample_num, sound_status.sound_position] := trunc(res);
  if @self.additional_sound <> nil then
    self.additional_sound;
end;

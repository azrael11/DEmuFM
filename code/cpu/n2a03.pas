...
const
  ...
  vbl_length: array [0 .. 31] of byte = (5, 127, 10, 1, 19, 2, 40, 3, 80, 4, 30, 5, 7, 6, 13, 7, 6,
  ...
  noise_freq: array [0 .. 15] of word = (4, 8, 16, 32, 64, 96, 128, 160, 202, 254, 380, 508, 762,
    1016, 2034, 2046);
  // dpcm transfer freqs */
  dpcm_clocks: array [0 .. 15] of word = (428, 380, 340, 320, 286, 254, 226, 214, 190, 160, 142,
    128, 106, 85, 72, 54);
...
begin
  for i := 0 to $1F do
    self.vbl_times[i] := vbl_length[i] * rate;
end;
...
begin
  val := sps;
  for i := 0 to (SYNCS_MAX1 - 1) do
  begin
    self.sync_times1[i] := val;
    val := val + sps;
  end;
  val := 0;
  for i := 0 to (SYNCS_MAX2 - 1) do
  begin
...
begin
  ...
  rate := clock div 4;
  self.samps_per_sync := round(rate / machine_calls.fps_max);
  // Use initializer calls */
  self.create_vbltimes(self.samps_per_sync);
  self.create_syncs(self.samps_per_sync);
  chips_total := chips_total + 1;
  self.frame_irq_timer := timers.init(self.m6502.numero_cpu, 29830, nil, n2a03_irq_call, false,
    chips_total);
  if chips_total = 0 then
    self.m6502.init_sound(n2a03_update_sound_0)
  else
    timers.init(self.m6502.numero_cpu, clock / FREQ_BASE_AUDIO, nil, n2a03_update_sound, true,
      chips_total);
    ...
    APU_WRA2, APU_WRB2:
      begin
        ...
        if (self.apu.squ[chan].enabled) then
          self.apu.squ[chan].freq := ((((self.apu.squ[chan].regs[3] and 7) shl 8) + value) +
            1) shl 16;
      end;
    APU_WRA3, APU_WRB3:
      begin
        ...
        begin
          ...
          self.apu.squ[chan].freq := ((((value and 7) shl 8) + self.apu.squ[chan].regs[2]) +
            1) shl 16;
        end;
      end;
    // triangle */
      ...
      begin
        ...
        begin
          if not(self.apu.tri.counter_started) then
            self.apu.tri.linear_length := self.sync_times2[value and $7F];
        end;
      end;
    $09:
...
begin
  ...
  begin
    ...
    if (self.apu.squ[chan].regs[0] and $20) <> 0 then
      self.apu.squ[chan].env_vol := (self.apu.squ[chan].env_vol + 1) and 15
  ...
  // freqsweeps */
  if (((self.apu.squ[chan].regs[1] and $80) <> 0) and ((self.apu.squ[chan].regs[1] and 7) <> 0))
  then
  begin
    ...
    begin
      ...
      if (self.apu.squ[chan].regs[1] and 8) <> 0 then
        self.apu.squ[chan].freq := self.apu.squ[chan].freq - sshr(self.apu.squ[chan].freq,
          (self.apu.squ[chan].regs[1] and 7))
      else
        self.apu.squ[chan].freq := self.apu.squ[chan].freq + sshr(self.apu.squ[chan].freq,
          (self.apu.squ[chan].regs[1] and 7));
    end;
  end;
  if ((((self.apu.squ[chan].regs[1] and 8) = 0) and (sshr(self.apu.squ[chan].freq,
    16) > freq_limit[self.apu.squ[chan].regs[1] and 7])) or (sshr(self.apu.squ[chan].freq, 16) < 4))
  then
  begin
    apu_square := 0;
    exit;
  ...
  if (self.apu.squ[chan].regs[0] and $10) <> 0 then
    output_ := self.apu.squ[chan].regs[0] and $0F
  else
    output_ := $0F - self.apu.squ[chan].env_vol;
  if (self.apu.squ[chan].adder < (duty_lut[self.apu.squ[chan].regs[0] shr 6])) then
    output_ := -output_;
...
begin
  ...
  if not(self.apu.tri.enabled) then
    exit;
  if (not(self.apu.tri.counter_started) and ((self.apu.tri.regs[0] and $80) = 0)) then
  begin
  ...
  begin
    if (self.apu.tri.linear_length > 0) then
      self.apu.tri.linear_length := self.apu.tri.linear_length - 1;
    if ((self.apu.tri.vbl_length <> 0) and ((self.apu.tri.regs[0] and $80) = 0)) then
  ...
  begin
    ...
    if (self.apu.tri.adder and 8) <> 0 then
      output_ := $10 - output_;
...
begin
  ...
  begin
    ...
    if (self.apu.noi.regs[2] and $80) <> 0 then
      self.apu.noi.seed := (self.apu.noi.seed shr 1) or
        ((BIT_n(self.apu.noi.seed, 0) xor BIT_n(self.apu.noi.seed, 6)) shl 14)
    else
      self.apu.noi.seed := (self.apu.noi.seed shr 1) or
        ((BIT_n(self.apu.noi.seed, 0) xor BIT_n(self.apu.noi.seed, 1)) shl 14);
  end;
  if (self.apu.noi.regs[0] and $10) <> 0 then
    outvol := self.apu.noi.regs[0] and $F
  else
    outvol := $0F - self.apu.noi.env_vol;
  output_ := self.apu.noi.seed and $FF;
  if (output_ > outvol) then
    output_ := outvol;
  if (self.apu.noi.seed and $80) <> 0 then
    apu_noise := -output_
  else
    apu_noise := output_;
end;
...
begin
  ...
  begin
    ...
    if (self.apu.dpcm.bits_left = 0) then
    begin
      self.apu.dpcm.bits_left := 8;
      self.apu.dpcm.cur_byte := self.apu.dpcm.getbyte(self.apu.dpcm.address);
      if self.apu.dpcm.address = $FFFF then
    ...
    if (self.apu.dpcm.cur_byte and (1 shl self.apu.dpcm.bits_left)) <> 0 then
    begin
      if (self.apu.dpcm.output < 126) then
        self.apu.dpcm.output := self.apu.dpcm.output + 2;
    end
    else
    begin
      if (self.apu.dpcm.output > 1) then
        self.apu.dpcm.output := self.apu.dpcm.output - 2;
    end;
    if (self.apu.dpcm.length = 0) then
    begin
...
begin // Resample from 447443hz to 44100hz
  ...
  begin
    ...
    res := self.old_res;
  tsample[self.num_sample, sound_status.sound_position] := res;
  self.old_res := res;

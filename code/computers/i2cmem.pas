...
type
  ...
  private
    ...
    page_written_size, page_offset, addresshigh, write_page_size, slave_address, devsel, scl, sdaw,
      shift, bits, state, sdar, e0, e1, e2, read_page_size: byte;
...
var
  ...
  mask: word;
begin
  if self.devsel_address_low then
  begin
  ...
  if (self.data_size <= $800) then
    mask := DEVSEL_ADDRESS and not(self.address_mask shr 7)
  else
    mask := DEVSEL_ADDRESS and $FFFF;
  if ((self.devsel and mask) = (device and mask)) then
  begin
    select_device := true;
    exit;
  end;
...
var
  offset: integer;
begin
  if (self.scl <> valor) then
  begin
    self.scl := valor;
    case self.state of
      STATE_DEVSEL, STATE_ADDRESSHIGH, STATE_ADDRESSLOW, STATE_DATAIN:
        begin
          if (self.bits < 8) then
          begin
            if (self.scl <> 0) then
                ...
                case self.state of
                  STATE_DEVSEL:
                    begin
                      ...
                      if ((self.devsel = 0) and not(self.devsel_address_low)) then
                      begin
                      ...
                      else if not(self.select_device) then
                      begin
                        self.state := STATE_IDLE;
                      end
                      else if ((self.devsel and DEVSEL_RW) = 0) then
                      begin
                        if (self.devsel_address_low) then
                        begin
                  ...
                  STATE_ADDRESSLOW:
                    begin
                      if (self.data_size <= $800) then
                        self.byteaddr := self.shift or ((self.devsel and DEVSEL_ADDRESS) shl 7) and
                          self.address_mask
                      else
                        self.byteaddr := self.shift or (self.addresshigh shl 8);
                      self.page_offset := 0;
                      self.page_written_size := 0;
                      self.state := STATE_DATAIN;
                    end;
                  STATE_DATAIN:
                    begin
                      ...
                      else if (self.write_page_size > 0) then
                      begin
                        self.page[self.page_offset] := self.shift;
                        self.page_offset := self.page_offset + 1;
                        if (self.page_offset = self.write_page_size) then
                          self.page_offset := 0;
                        self.page_written_size := self.page_written_size + 1;
                        if (self.page_written_size > self.write_page_size) then
                          self.page_written_size := self.write_page_size;
                      end
                      else
                      begin
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                self.byteaddr := (self.byteaddr and not(self.read_page_size - 1)) or
                  ((self.byteaddr + 1) and (self.read_page_size - 1));
              end;
...
begin
  ...
  begin
    ...
    begin
      if (self.sdaw <> 0) then
      begin
        if (self.page_written_size > 0) then
        begin

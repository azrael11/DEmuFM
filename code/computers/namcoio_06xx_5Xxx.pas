...
type
  ...
  // Namco 06XX
procedure namco_06xx_init(num: byte; chip0, chip1, chip2, chip3: byte;
  nmi_function: exec_type_simple);
...
// 53XX
function namcoio_53xx_init(port_k: cpu_inport_call; port_r_r: type_mb88xx_inport_r;
  zip_name: string): boolean;
...
begin
  if (valor and $10) <> 0 then
    namco_50xx[0].port_o := (namco_50xx[0].port_o and $F) or ((valor and $F) shl 4)
  else
    namco_50xx[0].port_o := (namco_50xx[0].port_o and $F0) or (valor and $F);
end;

procedure namco_50xx_o_w_1(valor: byte);
begin
  if (valor and $10) <> 0 then
    namco_50xx[1].port_o := (namco_50xx[1].port_o and $F) or ((valor and $F) shl 4)
  else
    namco_50xx[1].port_o := (namco_50xx[1].port_o and $F0) or (valor and $F);
end;
...
begin
  case port of
    0:
      namco_50xx_r_r_0 := namco_50xx[0].latched_cmd and $F;
    2:
      namco_50xx_r_r_0 := namco_50xx[0].latched_rw and 1;
  end;
end;
...
begin
  case port of
    0:
      namco_50xx_r_r_1 := namco_50xx[1].latched_cmd and $F;
    2:
      namco_50xx_r_r_1 := namco_50xx[1].latched_rw and 1;
  end;
end;
...
begin
  ...
  case num of
    0:
      begin
        namco_50xx[0].mb88.change_io_calls(namco_50xx_k_r_0, namco_50xx_o_w_0, nil, nil,
          namco_50xx_r_r_0, nil);
        namco_50xx[0].timer := timers.init(namco_50xx[0].mb88.numero_cpu, 32.256,
          namcoio_50xx_irq_clear_0, nil, false);
      end;
    1:
      begin
        namco_50xx[1].mb88.change_io_calls(namco_50xx_k_r_1, namco_50xx_o_w_1, nil, nil,
          namco_50xx_r_r_1, nil);
        namco_50xx[1].timer := timers.init(namco_50xx[1].mb88.numero_cpu, 32.256,
          namcoio_50xx_irq_clear_1, nil, false);
      end;
  end;
  // rom
...
begin
  ...
  namco_50xx[num].frames := namco_50xx[num].frames + namco_50xx[num].mb88.tframes - namco_50xx[num]
    .mb88.contador;
end;
...
begin
  data := data and $7;
  if (namco_51xx.coincred_mode <> 0) then
  begin
    case (namco_51xx.coincred_mode) of
      4:
        namco_51xx.coins_per_cred[0] := data;
      3:
        namco_51xx.creds_per_coin[0] := data;
      2:
        namco_51xx.coins_per_cred[1] := data;
      1:
        namco_51xx.creds_per_coin[1] := data;
    end;
  ...
  begin
    case data of
      ...
      2:
        begin // go in "credits" mode and enable start buttons
          namco_51xx.mode := 1;
          namco_51xx.in_count := 0;
        end;
      3:
        namco_51xx.remap_joy := false; // disable joystick remapping
      4:
        namco_51xx.remap_joy := true; // enable joystick remapping
      5:
        begin // go in "switch" mode
          namco_51xx.mode := 0;
          namco_51xx.in_count := 0;
        end;
    end;
...
var
  ...
  joy: integer;
const
  joy_map: array [0 .. 15] of byte = ($F, $E, $D, $5, $C, $9, $7, $6, $B, $3, $A, $4, $1,
    $2, $0, $8);
begin
  if (namco_51xx.mode = 0) then
  begin // switch mode
    case namco_51xx.in_count of
      0:
        res := namco_51xx.read_port[0]^;
      1:
        res := namco_51xx.read_port[1]^;
      2:
        res := 0; // nothing?
    end;
  ...
  begin // credits mode
    case namco_51xx.in_count of
      0:
        begin // number of credits in BCD format
          in_ := not(namco_51xx.read_port[0]^);
          toggle := in_ xor namco_51xx.lastcoins;
          namco_51xx.lastcoins := in_;
          if (namco_51xx.coins_per_cred[0] > 0) then
          begin
            if (namco_51xx.credits >= 99) then
            begin
              if @namco_51xx.write_port[1] <> nil then
                namco_51xx.write_port[1](1); // coin lockout
            end
            else
            begin
              if @namco_51xx.write_port[1] <> nil then
                namco_51xx.write_port[1](0); // coin lockout
              // check if the user inserted a coin */
              if (toggle and in_ and $10) <> 0 then
              begin
                namco_51xx.coins[0] := namco_51xx.coins[0] + 1;
                if @namco_51xx.write_port[0] <> nil then
                  namco_51xx.write_port[0]($04); // coin counter
                if @namco_51xx.write_port[0] <> nil then
                  namco_51xx.write_port[0]($0C);
                if (namco_51xx.coins[0] >= namco_51xx.coins_per_cred[0]) then
                begin
              ...
              if (toggle and in_ and $20) <> 0 then
              begin
                namco_51xx.coins[1] := namco_51xx.coins[1] + 1;
                if @namco_51xx.write_port[0] <> nil then
                  namco_51xx.write_port[0]($08); // coin counter
                if @namco_51xx.write_port[0] <> nil then
                  namco_51xx.write_port[0]($0C);
                if (namco_51xx.coins[1] >= namco_51xx.coins_per_cred[1]) then
                begin
          ...
          if (namco_51xx.mode = 1) then
          begin
            on_ := (main_vars.frames_sec and $10) shr 4;
            if (namco_51xx.credits >= 2) then
              if @namco_51xx.write_port[0] <> nil then
                namco_51xx.write_port[0]($0C or 3 * on_) // lamps
              else if (namco_51xx.credits >= 1) then
                if @namco_51xx.write_port[0] <> nil then
                  namco_51xx.write_port[0]($0C or 2 * on_) // lamps
                else if @namco_51xx.write_port[0] <> nil then
                  namco_51xx.write_port[0]($0C); // lamps off
            // check for 1 player start button */
            if (toggle and in_ and $04) <> 0 then
            begin
              if (namco_51xx.credits >= 1) then
              begin
            ...
            if (toggle and in_ and $08) <> 0 then
            begin
              if (namco_51xx.credits >= 2) then
              begin
          ...
          if (not(namco_51xx.read_port[0]^ shr 4) and $8) <> 0 then
          begin // check test mode switch */
...
begin
  ...
  namco_53xx.timer := timers.init(namco_53xx.mb88.numero_cpu, 32.256, namcoio_53xx_irq_clear,
    nil, false);
  // rom
...
begin
  res := (valor and $0F);
  if (valor and $10) <> 0 then
    namco_53xx.port_o := (namco_53xx.port_o and $0F) or (res shl 4)
  else
    namco_53xx.port_o := (namco_53xx.port_o and $F0) or res;
end;
...
begin
  ...
  begin
    if (((valor and $F) <> 0) and (namco_54xx.old_sam = 0)) then
    begin
      if (valor and $10) <> 0 then
        start_sample(1)
      else
...
begin
  if port = 0 then
    namco_54xx_r_r := namco_54xx.latched_cmd and $F
  else
    namco_54xx_r_r := $FF;
end;
...
begin
  if port = 1 then
  begin
    if ((valor = 11) and (namco_54xx.old_sam2 = 0)) then
      start_sample(2);
...
begin
  ...
  namco_54xx.mb88.change_io_calls(namco_54xx_k_r, namco_54xx_o_w, nil, nil, namco_54xx_r_r,
    namco_54xx_r_w);
  namco_54xx.frame := namco_54xx.mb88.tframes;
  // namco 53XX clock 1536000*0.000021=32.256
  namco_54xx.timer := timers.init(namco_54xx.mb88.numero_cpu, 32.256, namcoio_54xx_irq_clear,
    nil, false);
  // rom
...
begin
  ...
  case chip0 of
    IO50XX_0:
      namco_50xx_chip(num, 0, 0); // 50XX
    IO50XX_1:
      namco_50xx_chip(num, 0, 1); // 50XX
    IO51XX:
      namco_51xx_chip(num, 0); // 51XX
    IO53XX:
      namco_53xx_chip(num, 0); // 53XX
    IO54XX:
      namco_54xx_chip(num, 0); // 54XX
  end;
  case chip1 of
    NONE:
      none_chip(num, 1);
    IO50XX_0:
      namco_50xx_chip(num, 1, 0); // 50XX
    IO50XX_1:
      namco_50xx_chip(num, 1, 1); // 50XX
    IO51XX:
      namco_51xx_chip(num, 1); // 51XX
    IO53XX:
      namco_53xx_chip(num, 1); // 53XX
    IO54XX:
      namco_54xx_chip(num, 1); // 54XX
  end;
  case chip2 of
    NONE:
      none_chip(num, 2);
    IO50XX_0:
      namco_50xx_chip(num, 2, 0); // 50XX
    IO50XX_1:
      namco_50xx_chip(num, 2, 1); // 50XX
    IO51XX:
      namco_51xx_chip(num, 2); // 51XX
    IO53XX:
      namco_53xx_chip(num, 2); // 53XX
    IO54XX:
      namco_54xx_chip(num, 2); // 54XX
  end;
  case chip3 of
    NONE:
      none_chip(num, 3);
    IO50XX_0:
      namco_50xx_chip(num, 3, 0); // 50XX
    IO50XX_1:
      namco_50xx_chip(num, 3, 1); // 50XX
    IO51XX:
      namco_51xx_chip(num, 3); // 51XX
    IO53XX:
      namco_53xx_chip(num, 3); // 53XX
    IO54XX:
      namco_54xx_chip(num, 3); // 54XX
  end;
end;
...
begin
  res := $FF;
  if (namco_06xx[num].control and $10) = 0 then
  begin
    res := 0;
  end
  else
  begin
    for f := 0 to 3 do
    begin
      if (((namco_06xx[num].control and (1 shl f)) <> 0) and (@namco_06xx[num].fread[f] <> nil))
      then
...
begin
  ...
  begin
    for f := 0 to 3 do
    begin
      if (((namco_06xx[num].control and (1 shl f)) <> 0) and (@namco_06xx[num].fwrite[f] <> nil))
      then
  ...
  begin
    for f := 0 to 3 do
      if ((data and (1 shl f)) <> 0) then
        if @namco_06xx[num].fread_req[f] <> nil then
          namco_06xx[num].fread_req[f];
  end;
end;

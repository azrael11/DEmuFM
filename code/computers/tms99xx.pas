...
type
  ...
  tms99xx_chip = class
    constructor create(pant: byte; irq_call: irq_type; read_mem: read_mem_type = nil;
      write_mem: write_mem_type = nil);
    destructor free;
  public
...
begin
  temp := data;
  copymemory(temp, @self.regs[0], $10);
  inc(temp, $10);
  copymemory(temp, @self.mem[0], $4000);
  inc(temp, $4000);
...
begin
  temp := data;
  copymemory(@self.regs[0], temp, $10);
  inc(temp, $10);
  copymemory(@self.mem[0], temp, $4000);
  inc(temp, $4000);
  self.modo_video := temp^;
  inc(temp);
  self.status_reg := temp^;
  inc(temp);
  self.buffer := temp^;
  inc(temp);
  copymemory(@self.addr, temp, 2);
  inc(temp, 2);
  self.vdp_mode := temp^ <> 0;
  inc(temp);
  self.int := temp^ <> 0;
  inc(temp);
  self.segundo_byte := temp^ <> 0;
  inc(temp);
  self.pant := temp^;
  inc(temp);
  copymemory(@self.color, temp, 2);
  inc(temp, 2);
  copymemory(@self.pattern, temp, 2);
  inc(temp, 2);
  copymemory(@self.nametbl, temp, 2);
  inc(temp, 2);
  copymemory(@self.spriteattribute, temp, 2);
  inc(temp, 2);
  copymemory(@self.spritepattern, temp, 2);
  inc(temp, 2);
  copymemory(@self.colormask, temp, 2);
  inc(temp, 2);
  copymemory(@self.patternmask, temp, 2);
  inc(temp, 2);
  self.fgcolor := temp^;
  inc(temp);
  self.bgcolor := temp^;
  inc(temp);
  self.FifthSprite := temp^;
  inc(temp);
  self.espera_read := temp^ <> 0;
  inc(temp);
...
constructor tms99xx_chip.create(pant: byte; irq_call: irq_type; read_mem: read_mem_type = nil;
  write_mem: write_mem_type = nil);
const
  tms992X_palete: array [0 .. 15, 0 .. 2] of byte = ((0, 0, 0), (0, 0, 0), (33, 200, 66),
    (94, 220, 120), (84, 85, 237), (125, 118, 252), (212, 82, 77), (66, 235, 245), (252, 85, 84),
    (255, 121, 120), (212, 193, 84), (230, 206, 128), (33, 176, 59), (201, 91, 186),
...
begin
  ...
  begin
    ...
    if ((spr_y <= linea) and (linea < (spr_y + sprite_height))) then
    begin
      ...
      if (sprite_size = 16) then
        pataddr := self.spritepattern + (sprcode and $FC) * 8
      else
        pataddr := self.spritepattern + sprcode * 8;
      num_sprites := num_sprites + 1;
      // El quinto sprite no lo pinto pero lo evaluo
      if (num_sprites = 5) then
        fifth_encountered := true;
      // En el sexto me salgo
      if (num_sprites = 6) then
        break;
      if (sprite_mag <> 0) then
        pataddr := pataddr + (((linea - spr_y) and $1F) shr 1)
      else
        pataddr := pataddr + ((linea - spr_y) and $F);
      pattern := self.read_m(pataddr);
      if (sprcol and $80) <> 0 then
        spr_x := spr_x - 32;
      sprcol := sprcol and $0F;
      for s := 0 to ((sprite_size - 1) div 8) do
      begin
        for i := 0 to 7 do
        begin
          if sprite_mag <> 0 then
            colission_index := spr_x + (i * 2) + 32
          else
            colission_index := spr_x + i + 32;
          for z := 0 to sprite_mag do
          begin
            // Check if pixel should be drawn
            if (pattern and $80) <> 0 then
            begin
              if ((colission_index >= 32) and (colission_index < 32 + 256)) then
              begin
                // Check for colission
                if (spr_drawn[colission_index] <> 0) then
                  self.status_reg := self.status_reg or $20;
                spr_drawn[colission_index] := spr_drawn[colission_index] or $01;
                if (sprcol <> 0) then
                begin
                  // Has another sprite already drawn here?
                  if ((spr_drawn[colission_index] and $02) = 0) then
                  begin
                    spr_drawn[colission_index] := spr_drawn[colission_index] or $02;
                    if not(fifth_encountered) then
                    begin
                      punbuf^ := paleta[sprcol];
                      putpixel(colission_index - 32 + PIXELS_LEFT_BORDER_VISIBLES,
                        linea + LINEAS_TOP_BORDE, 1, punbuf, self.pant);
                    end;
                  end;
                end; // del if sprcol
              end; // del colision
  ...
  if (self.status_reg and $40) = 0 then
  begin
    self.status_reg := (self.status_reg and $E0) or self.FifthSprite;
    if (fifth_encountered and ((self.status_reg and $80) = 0)) then
      self.status_reg := self.status_reg or $40;
  end;
end;
...
begin // 256x192 --> Caracteres de 4x4 en dos bloques
  ...
  begin
    charcode := self.read_m(name_base);
    name_base := name_base + 1;
    colorptr := self.pattern + (((charcode + ((linea shr 2) and 7) + ((linea shr 6) shl 8)) and
      self.patternmask) shl 3);
    fc := self.read_m(colorptr) shr 4;
    bg := self.read_m(colorptr) and $F;
    ptemp^ := paleta[fc];
    inc(ptemp); // (x+0)
...
begin
  ...
  begin
    ...
    begin
      ...
      if @self.IRQ_Handler <> nil then
        self.IRQ_Handler(self.int);
    end;
    exit;
  end;
  case linea of
    0 .. 191:
      begin // Pantalla visible (192)
        case self.modo_video of
          0:
            self.draw_mode0(linea);
          1:
            self.draw_mode1(linea);
          2:
            self.draw_mode2(linea);
          3:
            self.draw_mode12(linea);
          4:
            self.draw_mode3(linea);
          6:
            self.draw_mode23(linea);
          5, 7:
            self.draw_modebogus;
        end;
        putpixel(0, linea + LINEAS_TOP_BORDE, PIXELS_VISIBLES_TOTAL, punbuf, self.pant);
        if ((self.regs[1] and $50) = $40) then
          draw_sprites(linea)
        else
          self.FifthSprite := $1F;
    ...
    192:
      begin
        single_line(0, linea + LINEAS_TOP_BORDE, paleta[self.bgcolor], PIXELS_VISIBLES_TOTAL,
          self.pant);
        self.status_reg := self.status_reg or $80;
        self.exec_interrupt;
      end;
    ...
    // Borde superior (27)
    235 .. 261:
      single_line(0, linea - 235, paleta[self.bgcolor], PIXELS_VISIBLES_TOTAL, self.pant);
  end;
end;
  ...
  case addr of
    0:
      begin
        ...
        tms.modo_video := (tms.regs[0] and 2) or ((tms.regs[1] and $10) shr 4) or
          ((tms.regs[1] and 8) shr 1);
      end;
    1:
      begin
        tms.exec_interrupt;
    ...
    2:
      tms.nametbl := (Val * 1024) and TMS9918A_VRAM_SIZE;
    3:
      begin
        if (tms.regs[0] and 2) <> 0 then
        begin
    ...
    5:
      tms.spriteattribute := (Val * 128) and TMS9918A_VRAM_SIZE;
    6:
      tms.spritepattern := (Val * 2048) and TMS9918A_VRAM_SIZE;
    7:
      begin
        tms.fgcolor := Val shr 4;
        if tms.bgcolor <> (Val and $F) then
        begin
          tms.bgcolor := (Val and $F);
          if tms.bgcolor = 0 then
            paleta[0] := 0
          else
            paleta[0] := paleta[tms.bgcolor];
...
begin
  ...
  begin
    ...
    case (valor and $C0) of
      $80:
        change_reg(self, valor and $F, self.addr and $FF);
      $00:
        begin

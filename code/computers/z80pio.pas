...
procedure z80pio_init(num: byte; int_func: tm_out_int_func = nil; infunc_a: tm_in_p_func = nil;
  outfunc_a: tm_out_p_func = nil; rdyfunc_a: tm_out_rdy_func = nil; infunc_b: tm_in_p_func = nil;
  outfunc_b: tm_out_p_func = nil; rdyfunc_b: tm_out_rdy_func = nil);
...
procedure z80pio_init(num: byte; int_func: tm_out_int_func; infunc_a: tm_in_p_func;
  outfunc_a: tm_out_p_func; rdyfunc_a: tm_out_rdy_func; infunc_b: tm_in_p_func;
  outfunc_b: tm_out_p_func; rdyfunc_b: tm_out_rdy_func);
var
  pio: ptipo_z80pio;
begin
...
procedure z80pio_close(num: byte);
begin
  if z80_pio[num] <> nil then
  begin
    if z80_pio[num].m_port[PORT_A] <> nil then
      freemem(z80_pio[num].m_port[PORT_A]);
    if z80_pio[num].m_port[PORT_B] <> nil then
      freemem(z80_pio[num].m_port[PORT_B]);
...
begin
  if (port.m_rdy <> state) then
  begin
    port.m_rdy := state;
    if @port.m_out_rdy_func <> nil then
      port.m_out_rdy_func(state);
  end;
end;
...
var
  ...
  match: boolean;
begin
  if (port.m_mode = MODE_BIT_CONTROL) then
  begin
    ...
    match := false;
    data := data and MASK;
    if (((port.m_icw and $60) = 0) and (data <> MASK)) then
      match := true
    else if (((port.m_icw and $60) = $20) and (data <> 0)) then
      match := true
    else if (((port.m_icw and $60) = $40) and (data = 0)) then
      match := true
    else if (((port.m_icw and $60) = $60) and (data = MASK)) then
      match := true;
    if (not(port.m_match) and match) then
    begin
      // trigger interrupt
      port.m_ip := true;
    end;
...
begin
  state := CLEAR_LINE;
  for f := 0 to 1 do
    if interrupt_signalled(pio.m_port[f]) then
      state := ASSERT_LINE;
  if @pio.m_out_int_func <> nil then
    pio.m_out_int_func(state);
end;
...
begin
  ...
  case mode of
    MODE_OUTPUT:
      begin
        // enable data output
        if @pio_port.m_out_p_func <> nil then
          pio_port.m_out_p_func(pio_port.m_output);
        // assert ready line
    ...
    MODE_BIT_CONTROL:
      begin
        if ((port = PORT_A) or (pio.m_port[PORT_A].m_mode <> MODE_BIDIRECTIONAL)) then
        begin
          // clear ready line
...
begin
  ...
  case pio_port.m_mode of
    MODE_OUTPUT:
      data := pio_port.m_output;
    MODE_INPUT:
      begin
        if not(pio_port.m_stb) then
        begin
          // input port data
          if @pio_port.m_in_p_func <> nil then
            pio_port.m_input := pio_port.m_in_p_func;
        end;
    ...
    MODE_BIT_CONTROL:
      begin
        ...
        if @pio_port.m_in_p_func <> nil then
          pio_port.m_input := pio_port.m_in_p_func;
        data := (pio_port.m_input and pio_port.m_ior) or
          (pio_port.m_output and (pio_port.m_ior xor $FF));
      end;
  end;
  data_read := data;
end;
...
begin
  ...
  if bit_n(offset, 1) <> 0 then
    z80pio_cd_ba_r := z80pio_c_r(num)
  else
    z80pio_cd_ba_r := z80pio_d_r(num, index);
end;
...
begin
  ...
  if bit_n(offset, 0) <> 0 then
    z80pio_ba_cd_r := z80pio_c_r(num)
  else
    z80pio_ba_cd_r := z80pio_d_r(num, index);
end;
    ...
    ANY:
      begin
        ...
        begin
          case (data and $0F) of
            $F:
              set_mode(pio, port, data shr 6); // select operating mode
            $07:
              begin // set interrupt control word
                pio_port.m_icw := data;
                if (pio_port.m_icw and ICW_MASK_FOLLOWS) <> 0 then
                begin
            ...
            $03:
              begin // set interrupt enable flip-flop
  ...
  case (pio_port.m_mode) of
    MODE_OUTPUT:
      begin
        ...
        // output data to port
        if @pio_port.m_out_p_func <> nil then
          pio_port.m_out_p_func(data);
        // assert ready line
        set_rdy(pio_port, true);
      end;
    MODE_INPUT:
      pio_port.m_output := data; // latch output data
    MODE_BIDIRECTIONAL:
      begin
        // clear ready line
        set_rdy(pio_port, false);
    ...
    MODE_BIT_CONTROL:
      begin
        ...
        // output data to port
        if @pio_port.m_out_p_func <> nil then
          pio_port.m_out_p_func(pio_port.m_ior or (pio_port.m_output and (pio_port.m_ior xor $FF)));
      end;
  end;
end;
...
begin
  index := bit_n(offset, 0);
  if bit_n(offset, 1) <> 0 then
    z80pio_c_w(num, index, valor)
  else
    z80pio_d_w(num, index, valor);
end;
...
begin
  pio := z80_pio[num];
  pio_port := pio.m_port[port];
  if (pio.m_port[PORT_A].m_mode = MODE_BIDIRECTIONAL) then
  begin
    if (pio_port.m_rdy) then
    begin // port ready
      if (pio_port.m_stb and not(state)) then
      begin // falling edge
        if (port = PORT_A) then
          if @pio_port.m_out_p_func <> nil then
            pio_port.m_out_p_func(pio_port.m_output)
          else if @pio_port.m_in_p_func <> nil then
            pio.m_port[PORT_A].m_input := pio_port.m_in_p_func;
      end
      else
      begin
        if (not(pio_port.m_stb) and state) then
        begin // rising edge
  ...
  case pio_port.m_mode of
    ...
    MODE_BIDIRECTIONAL:
      if port = PORT_A then
        data := pio_port.m_output;
    MODE_BIT_CONTROL:
      data := pio_port.m_ior or (pio_port.m_output and (pio_port.m_ior xor $FF));
  end;
  read := data;
end;
...
begin
  ...
  begin
    ...
    if pio_port.m_ius then
    begin
      z80pio_irq_state := Z80_DAISY_IEO;
      exit;
    end
    else
    begin // interrupt pending
      if (pio_port.m_ie and pio_port.m_ip) then
        state := Z80_DAISY_INT;
    end;
  end;

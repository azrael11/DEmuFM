...
var
  f: word;
begin
  for f := 0 to ((1 shl self.address_bits) - 1) do
    self.internal_write(f, self.internal_read(f) and valor);
end;
...
begin
  for f := 0 to ((1 shl self.address_bits) - 1) do
    self.internal_write(f, $FFFF);
end;

function eepromser_chip.internal_read(direccion: word): word;
begin
  if (self.data_bits = 16) then
    internal_read := (self.addrspace[direccion * 2] shl 8) + self.addrspace[(direccion * 2) + 1]
  else
    internal_read := self.addrspace[direccion];
end;
...
begin
  ...
  case self.tipo of
    E93C06, E93C46, E93C56, E93C57, E93C66, E93C76, E93C86:
      self.e93cxx_parse_command_and_address;
    ER5911:
      self.er5911_parse_command_and_address;
  end;
  self.bits_accum := 0;
  // each command advances differently
  case self.command of
    ...
    // reset the shift register and wait for enough data to be clocked through
    COMMAND_WRITE, COMMAND_WRITEALL:
      begin
        self.shift_register := 0;
...
begin
  ...
  case self.state of
    ...
    STATE_IN_RESET:
      begin
        if (event = EVENT_CS_RISING_EDGE) then
          self.state := STATE_WAIT_FOR_START_BIT;
      end;
    ...
    STATE_WAIT_FOR_START_BIT:
      begin
        if ((event = EVENT_CLK_RISING_EDGE) and (self.di_state = ASSERT_LINE) and self.is_ready) then
        begin // --> and machine().time() > m_last_cs_rising_edge_time)
          self.command_address_accum := 0;
          self.bits_accum := 0;
        ...
        begin
          if (event = EVENT_CS_FALLING_EDGE) then
            self.state := STATE_IN_RESET;
        end;
      end;
    ...
    STATE_WAIT_FOR_COMMAND:
      begin
        if (event = EVENT_CLK_RISING_EDGE) then
        begin
          ...
          if (self.bits_accum = (2 + self.command_address_bits)) then
      ...
      begin
        ...
        begin
          ...
          if (((bit_index mod self.data_bits) = 0) and (bit_index = 0) or self.streaming_enabled) then
            self.shift_register := self.read_((self.address + bits_accum div self.data_bits) and
              ((1 shl self.address_bits) - 1)) shl (32 - self.data_bits)
          else
            self.shift_register := (self.shift_register shl 1) or 1;
        end
        else
        begin
          if (event = EVENT_CS_FALLING_EDGE) then
    ...
    STATE_WAIT_FOR_DATA:
      begin
        ...
        begin
          ...
          self.bits_accum := self.bits_accum + 1;
          if (self.bits_accum = self.data_bits) then
...
begin
  case self.tipo of
    E93C06, E93C46, E93C56, E93C57, E93C66, E93C76, E93C86, ER5911:
      begin
        ...
        // bit is a 0
        if ((self.state = STATE_READING_DATA) and ((self.shift_register and $80000000) = 0)) then
          res := CLEAR_LINE
        else
          res := ASSERT_LINE;
      end;
  ...
  case self.tipo of
    E93C06, E93C46, E93C56, E93C57, E93C66, E93C76, E93C86, ER5911:
      begin
        ...
        if (epr_state = self.cs_state) then
          exit;
        // set the new state
        ...
        if (self.cs_state = ASSERT_LINE) then
          self.handle_event(EVENT_CS_RISING_EDGE)
        else
          self.handle_event(EVENT_CS_FALLING_EDGE);
      end;
  end;
end;
  ...
  case self.tipo of
    E93C06, E93C46, E93C56, E93C57, E93C66, E93C76, E93C86, ER5911:
      begin
        // ignore if the state is not changing
        epr_state := epr_state and 1;
        if (epr_state = self.clk_state) then
          exit;
        // set the new state
        self.clk_state := epr_state;
        if (self.clk_state = ASSERT_LINE) then
          self.handle_event(EVENT_CLK_RISING_EDGE)
        else
          self.handle_event(EVENT_CLK_FALLING_EDGE);
      end;
  end;
end;
...
begin
  ...
  case (self.command_address_accum shr self.command_address_bits) of
    ...
    0:
      begin
        case (self.address shr (self.command_address_bits - 2)) of
          0:
            self.command := COMMAND_LOCK;
          1:
            self.command := COMMAND_WRITEALL;
          2:
            self.command := COMMAND_ERASEALL;
          3:
            self.command := COMMAND_UNLOCK;
        end;
        self.address := 0;
      end;
    1:
    ...
    2:
      self.command := COMMAND_READ;
    3:
      self.command := COMMAND_ERASE;
  end;
end;
...
begin
  ...
  case (self.command_address_accum shr self.command_address_bits) of
      ...
      begin
        case (self.address shr (self.command_address_bits - 2)) of
          0:
            self.command := COMMAND_LOCK;
          1:
            self.command := COMMAND_INVALID; // not on ER5911
          2:
            self.command := COMMAND_ERASEALL;
          3:
            self.command := COMMAND_UNLOCK;
    ...
    2:
      self.command := COMMAND_READ;
    3:
      self.command := COMMAND_WRITE; // WRITE instead of ERASE on ER5911
  end;
end;

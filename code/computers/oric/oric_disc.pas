...
type
  ...
  private
    ...
    distatus: integer;
    // The new contents for r_status when delayedint expires (or -1 to leave it untouched)
    ...
    currsector: boolean;
    // Pointers to the current sector in the disk image being used by an active read or write operation
    currseclen: word; // The length of the current sector
    ddstatus: integer;
    // The new contents for r_status when delayeddrq expires (or -1 to leave it untouched)
    num: byte;
    crc: word;
...
begin
  ...
  if not(dsk[self.c_drive].abierto) then
    exit;
  // No sectors on this track? Someone needs to format their disk...
  if (dsk[self.c_drive].Tracks[self.c_side, self.c_track].number_sector = 0) then
    exit;
  ...
  begin
    ...
    self.c_sector := (self.c_sector + 1) mod dsk[self.c_drive].Tracks[self.c_side, self.c_track]
      .number_sector;
    // If we passed through the start of the track, set the pulse bit in the status register
    if (self.c_sector = 0) then
    begin
      revs := revs + 1;
      self.r_status := self.r_status or WSFI_PULSE;
    end;
    // Found the required sector?
    if (dsk[self.c_drive].Tracks[self.c_side, self.c_track].sector[self.c_sector].sector = secid)
    then
    begin
      find_sector := true;
      exit;
...
begin
  ...
  if dsk[self.c_drive].abierto then
  begin
    // Yes. If we are trying to seek to a non-existant track, just seek as far as we can
    if (track >= dsk[self.c_drive].DiskHeader.nbof_tracks) then
    begin
      if dsk[self.c_drive].DiskHeader.nbof_tracks > 0 then
        track := dsk[self.c_drive].DiskHeader.nbof_tracks - 1
      else
        track := 0;
      self.distatus := WSFI_HEADL or WSFI_SEEKERR;
    end
    else
      self.distatus := WSFI_HEADL or WSFI_PULSE;
    // Update our status
    self.c_track := track;
    self.c_sector := 0;
    self.r_track := track;
    ...
    self.delayedint := 20;
    if (track = 0) then
      self.distatus := self.distatus or WSFI_TRK0;
    exit;
  end;
  // No disk in drive
...
begin
  ...
  if not(dsk[self.c_drive].abierto) then
    exit;
  // No sectors?!
  if (dsk[self.c_drive].Tracks[self.c_side, self.c_track].number_sector < 1) then
    exit;
        ...
        case (valor and $E0) of
          ...
          $20:
            begin // Step (Type I)
              ...
              if (valor and 8) <> 0 then
                self.r_status := self.r_status or WSFI_HEADL;
              if self.last_step_in then
                self.seek_track(self.c_track + 1)
              else if (self.c_track > 0) then
                self.seek_track(self.c_track - 1)
              else
                self.seek_track(0);
              self.currentop := COP_NUFFINK;
          ...
          $60:
            begin // Step-out (Type I)
              self.r_status := WSF_BUSY;
              if (valor and 8) <> 0 then
                self.r_status := self.r_status or WSFI_HEADL;
              if (self.c_track > 0) then
                self.seek_track(self.c_track - 1);
          ...
          $80:
            begin // Read sector (Type II)
              self.curroffs := 0;
              self.currsector := self.find_sector(self.r_sector);
              if not(self.currsector) then
              begin
              ...
              self.currseclen := 1 shl (dsk[self.c_drive].Tracks[self.c_side, self.c_track].sector
                [self.c_sector].sector_size + 7);
              self.r_status := WSF_BUSY or WSF_NOTREADY;
              self.delayeddrq := 60;
              if (valor and $10) <> 0 then
                self.currentop := COP_READ_SECTORS
              else
                self.currentop := COP_READ_SECTOR;
              self.crc := $E295;
              refreshdisks := true;
            end;
          $A0:
            begin // Write sector (Type II)
              self.curroffs := 0;
              self.currsector := self.find_sector(self.r_sector);
              if not(self.currsector) then
              begin
              ...
              self.delayeddrq := 500;
              if (valor and $10) <> 0 then
                self.currentop := COP_WRITE_SECTORS
              else
                self.currentop := COP_WRITE_SECTOR;
              self.crc := $E295;
              refreshdisks := true;
            end;
          $C0:
            begin // Read address / Force IRQ
              case (valor and $10) of
                $00:
                  begin // Read address (Type III)
                    self.curroffs := 0;
                    if not(self.currsector) then
                      self.currsector := self.first_sector
                    else
                      self.currsector := self.next_sector;
                    if not(self.currsector) then
                    begin
                      self.r_status := WSF_RNF;
                      self.clrdrq(self.num);
          ...
          $E0:
            begin // Read track / Write track
              case (valor and $10) of
                $00:
                  begin // Read track (Type III)
                    self.curroffs := 0;
                    self.r_status := WSF_BUSY or WSF_NOTREADY;
                    self.delayeddrq := 60;
    ...
    3:
      begin
        ...
        case self.currentop of
          COP_WRITE_SECTOR, COP_WRITE_SECTORS:
            begin
              if not(self.currsector) then
              begin
              ...
              inc(ptemp, dsk[self.c_drive].Tracks[self.c_side, self.c_track].sector[self.c_sector]
                .posicion_data);
              if (self.curroffs = 0) then
                dsk[self.c_drive].Tracks[self.c_side, self.c_track].sector[self.c_sector]
                  .status1 := $FB;
              ...
              self.r_status := self.r_status and not(WSF_DRQ);
              self.clrdrq(self.num);
              if (self.curroffs > (self.currseclen - 1)) then
              begin
                dsk[self.c_drive].Tracks[self.c_side, self.c_track].sector[self.c_sector]
                  .data_length := self.crc;
                if (self.currentop = COP_WRITE_SECTORS) then
                begin
...
begin
  case direccion of
    ...
    1:
      ret := self.r_track; // Track register
    2:
      ret := self.r_sector; // Sector register
    3:
      case self.currentop of
        COP_READ_SECTOR, COP_READ_SECTORS:
          begin
            ...
            if (self.curroffs = 0) then
            begin
              if (dsk[self.c_drive].Tracks[self.c_side, self.c_track].sector[self.c_sector]
                .status1 = $F8) then
                self.sectype := WSFR_RECTYP
              else
                self.sectype := 0;
            end;
            ...
            begin
              ...
              if (self.currentop = COP_READ_SECTORS) then
              begin
                // Get the next sector, and carry on!
                self.r_sector := self.r_sector + 1;
                self.curroffs := 0;
                ...
                begin
                  ...
                  self.distatus := self.sectype;
                  // ...and when doing so, set the status to reflect the record type
        ...
        COP_READ_ADDRESS:
          begin
            ...
            case self.curroffs of
              0:
                begin
                  self.r_sector := dsk[self.c_drive].Tracks[self.c_side, self.c_track].sector
                    [self.c_sector].track;
                  self.r_data := self.r_sector;
                end;
              1:
                self.r_data := dsk[self.c_drive].Tracks[self.c_side, self.c_track].sector
                  [self.c_sector].head;
              2:
                self.r_data := dsk[self.c_drive].Tracks[self.c_side, self.c_track].sector
                  [self.c_sector].sector;
              3:
                self.r_data := dsk[self.c_drive].Tracks[self.c_side, self.c_track].sector
                  [self.c_sector].sector_size;
              4:
                self.r_data := dsk[self.c_drive].Tracks[self.c_side, self.c_track].sector
                  [self.c_sector].data_length shr 8;
              5:
                self.r_data := dsk[self.c_drive].Tracks[self.c_side, self.c_track].sector
                  [self.c_sector].data_length and $FF;
            end;
            self.curroffs := self.curroffs + 1;
            self.r_status := self.r_status and not(WSF_DRQ);
            self.clrdrq(self.num);
            ret := self.r_data;
            if (self.curroffs >= 6) then
            begin
              self.delayedint := 20;
              self.distatus := 0;
              self.currentop := COP_NUFFINK;
...
begin
  ...
  md.intrq := 0;
  if (md.port_314 and MDSF_INTENA) <> 0 then
    md.irq_handle(ASSERT_LINE);
end;
...
begin
  case (direccion and $F) of
    0 .. 3:
      self.wd.write(direccion, valor);
    $4:
      begin
        self.port_314 := valor;
        self.wd.c_drive := (valor shr 5) and $3;
        self.wd.c_side := (valor and $10) shr 4;
        if @self.irq_handle <> nil then
          if (((valor and MDSF_INTENA) <> 0) and (self.intrq = 0)) then
            self.irq_handle(ASSERT_LINE)
          else
            self.irq_handle(CLEAR_LINE);
      end;
    $8:
      halt(0);
  end;
...
begin
  case (direccion and $F) of
    0 .. 3:
      read := self.wd.read(direccion);
    4:
      read := self.intrq or $7F;
    8:
      read := self.drq or $7F;
  end;
end;

...
type
  ...
  private
    a_input_overrides_output_mask, ctl_a, ctl_b, ddr_a, ddr_b, in_a, in_b, port_a_z_mask, out_a,
      out_b: byte;
    logged_ca1_not_connected, in_ca1_pushed, logged_ca2_not_connected, in_ca2_pushed, in_ca1,
      in_ca2, irq_a1, irq_a2, irq_b1, irq_b2: boolean;
    out_ca2, out_cb2, out_ca2_needs_pulled, in_a_pushed, in_b_pushed, logged_port_a_not_connected,
      logged_port_b_not_connected: boolean;
    out_cb2_needs_pulled, last_out_cb2_z, out_a_needs_pulled, out_b_needs_pulled: boolean;
    logged_cb1_not_connected, logged_cb2_not_connected, in_cb1_pushed, in_cb2_pushed,
      in_cb1: boolean;
...
function irq1_enabled(c: byte): boolean;
begin
  irq1_enabled := (c and 1) <> 0;
end;

function c1_low_to_high(c: byte): boolean;
begin
  c1_low_to_high := (c and 2) <> 0;
end;

function c1_high_to_low(c: byte): boolean;
begin
  c1_high_to_low := (c and 2) = 0;
end;

function output_selected(c: byte): boolean;
begin
  output_selected := (c and 4) <> 0;
end;

function irq2_enabled(c: byte): boolean;
begin
  irq2_enabled := (c and 8) <> 0;
end;

function strobe_e_reset(c: byte): boolean;
begin
  strobe_e_reset := (c and 8) <> 0;
end;

function strobe_c1_reset(c: byte): boolean;
begin
  strobe_c1_reset := (c and 8) = 0;
end;

function c2_set(c: byte): boolean;
begin
  c2_set := (c and 8) <> 0;
end;

function c2_low_to_high(c: byte): boolean;
begin
  c2_low_to_high := (c and $10) <> 0;
end;

function c2_high_to_low(c: byte): boolean;
begin
  c2_high_to_low := (c and $10) = 0;
end;

function c2_set_mode(c: byte): boolean;
begin
  c2_set_mode := (c and $10) <> 0;
end;

function c2_strobe_mode(c: byte): boolean;
begin
  c2_strobe_mode := (c and $10) = 0;
end;

function c2_output(c: byte): boolean;
begin
  c2_output := (c and $20) <> 0;
end;

function c2_input(c: byte): boolean;
begin
  c2_input := (c and $20) = 0;
end;
...
begin
  ...
  if addr(self.irqa_handler) <> nil then
    self.irqa_handler(false);
  if addr(self.irqb_handler) <> nil then
    self.irqb_handler(false);
  if addr(self.out_a_handler) <> nil then
    self.out_a_handler($FF);
  if addr(self.ca2_handler) <> nil then
    self.ca2_handler(true);
...
begin
  // start with IRQ A
  new_state := (self.irq_a1 and irq1_enabled(self.ctl_a)) or
    (self.irq_a2 and irq2_enabled(self.ctl_a));
  if (new_state <> self.irq_a_state) then
  begin
    self.irq_a_state := new_state;
    if (addr(irqa_handler) <> nil) then
      self.irqa_handler(self.irq_a_state);
  end;
  // then do IRQ B
  new_state := (self.irq_b1 and irq1_enabled(self.ctl_b)) or
    (self.irq_b2 and irq2_enabled(self.ctl_b));
  if (new_state <> self.irq_b_state) then
  begin
    self.irq_b_state := new_state;
    if (addr(irqb_handler) <> nil) then
      self.irqb_handler(self.irq_b_state);
  end;
end;
...
begin
  ...
  if (addr(self.ca2_handler) <> nil) then
    self.ca2_handler(self.out_ca2)
  else
    self.out_ca2_needs_pulled := true;
end;

procedure pia6821_chip.ca1_w(state: boolean);
begin
  // the new state has caused a transition
  if ((self.in_ca1 <> state) and ((state and c1_low_to_high(self.ctl_a)) or
    (not(state) and c1_high_to_low(self.ctl_a)))) then
  begin
    // mark the IRQ
    self.irq_a1 := true;
    // update externals
    ...
    if (c2_output(self.ctl_a) and c2_strobe_mode(self.ctl_a) and strobe_c1_reset(self.ctl_a) and
      not(self.out_ca2)) then
      set_out_ca2(true);
  end;
...
begin
  ...
  if (c2_input(self.ctl_a) and (self.in_ca2 <> state) and ((state and c2_low_to_high(self.ctl_a)) or
    (not(state) and c2_high_to_low(self.ctl_a)))) then
  begin
    // mark the IRQ
    self.irq_a2 := true;
    // update externals
...
begin
  ...
  if (addr(self.in_ca1_handler) <> nil) then
  begin
    ca1_w(self.in_ca1_handler);
  end
  else if (not(logged_ca1_not_connected) and not(in_ca1_pushed)) then
  begin
    self.logged_ca1_not_connected := true;
  end;
  if (addr(in_ca2_handler) <> nil) then
  begin
    ca2_w(in_ca2_handler);
  end
  else if (not(self.logged_ca2_not_connected) and c2_input(ctl_a) and not(in_ca2_pushed)) then
  begin
  ...
  if irq_a1 then
    ret := ret or PIA_IRQ1;
  if (irq_a2 and c2_input(ctl_a)) then
    ret := ret or PIA_IRQ2;
  control_a_r := ret;
end;
...
begin
  // update the input
  if (addr(in_a_handler) <> nil) then
  begin
    port_a_data := self.in_a_handler;
  end
  else
  begin
    if (self.in_a_pushed) then
    begin
    ...
    begin
      ...
      port_a_data := $FF;
      if (not(self.logged_port_a_not_connected) and (self.ddr_a <> $FF)) then
        self.logged_port_a_not_connected := true;
    end;
  end;
  ...
  ret := (not(self.ddr_a) and port_a_data) // input pins
    ...
    or (self.ddr_a and port_a_data and self.a_input_overrides_output_mask);
  // overridden output pins
  get_in_a_value := ret;
end;
...
begin
  ...
  if (c2_output(self.ctl_a) and c2_strobe_mode(self.ctl_a)) then
  begin
    // this will cause a transition low
    if self.out_ca2 then
      set_out_ca2(false);
    // if the CA2 strobe is cleared by the E, reset it right away
    if (strobe_e_reset(self.ctl_a)) then
      set_out_ca2(true);
  end;
  port_a_r := ret;
end;
...
begin
  ...
  begin
    // update the input
    if (addr(in_b_handler) <> nil) then
    begin
      port_b_data := self.in_b_handler;
    end
    else
    begin
      if (self.in_b_pushed) then
      begin
        port_b_data := self.in_b;
      end
      else
      begin
        if (not(self.logged_port_b_not_connected) and (self.ddr_b <> $FF)) then
          self.logged_port_b_not_connected := true;
...
begin
  z := cb2_output_z;
  if ((data <> self.out_cb2) or (z <> self.last_out_cb2_z)) then
  begin
    self.out_cb2 := data;
    self.last_out_cb2_z := z;
    // send to output function
    if (addr(self.cb2_handler) <> nil) then
      self.cb2_handler(self.out_cb2)
    else
      self.out_cb2_needs_pulled := true;
  end;
end;
...
begin
  ...
  if (self.irq_b1 and c2_strobe_mode(self.ctl_b) and strobe_c1_reset(self.ctl_b)) then
    set_out_cb2(true);
  // IRQ flags implicitly cleared by a read
  self.irq_b1 := false;
  self.irq_b2 := false;
...
begin
  // the new state has caused a transition
  if ((self.in_cb1 <> state) and ((state and c1_low_to_high(self.ctl_b)) or
    (not(state) and c1_high_to_low(self.ctl_b)))) then
  begin
    // mark the IRQ
    self.irq_b1 := true;
    // update externals
...
begin
  ...
  if (addr(self.in_cb1_handler) <> nil) then
  begin
    cb1_w(in_cb1_handler);
  end
  else if (not(self.logged_cb1_not_connected) and not(in_cb1_pushed)) then
  begin
    self.logged_cb1_not_connected := true;
  end;
  if (not(logged_cb2_not_connected) and c2_input(self.ctl_b) and not(in_cb2_pushed)) then
  begin
  ...
  if (self.irq_b1) then
    ret := ret or PIA_IRQ1;
  if (self.irq_b2 and c2_input(self.ctl_b)) then
    ret := ret or PIA_IRQ2;
  control_b_r := ret;
end;
...
begin
  ...
  if (addr(self.out_a_handler) <> nil) then
    self.out_a_handler(data)
  else
    self.out_a_needs_pulled := true;
end;

procedure pia6821_chip.ddr_a_w(valor: byte);
begin
  if (self.ddr_a <> valor) then
  begin
...
begin
  ...
  if (c2_output(self.ctl_a)) then
  begin
    if (c2_set_mode(self.ctl_a)) then
    begin // set/reset mode - bit value determines the new output
      tempb2 := c2_set(self.ctl_a);
      if (not(ca2_was_output) or (self.out_ca2 <> tempb2)) then
        set_out_ca2(tempb2);
    end
    else
    begin // strobe mode - output is always high unless strobed
      if (not(ca2_was_output) or not(self.out_ca2)) then
        self.set_out_ca2(true); // strobe mode - output is always high unless strobed
    end;
  end
  else
  begin
    if ca2_was_output then
    begin
      if addr(self.ca2_handler) <> nil then
        self.ca2_handler(true);
    end;
...
begin
  ...
  if (addr(self.out_b_handler) <> nil) then
    self.out_b_handler(data)
  else
    self.out_b_needs_pulled := true;
end;
...
begin
  ...
  if (c2_strobe_mode(self.ctl_b)) then
  begin
    // this will cause a transition low
    self.set_out_cb2(false);
    // if the CB2 strobe is cleared by the E, reset it right away
    if (strobe_e_reset(self.ctl_b)) then
      set_out_cb2(true);
  end;
end;
...
begin
  case (dir and $3) of
    $00:
      if (output_selected(self.ctl_a)) then
        port_a_w(valor)
      else
        ddr_a_w(valor);
    $01:
      control_a_w(valor);
    $02:
      if (output_selected(self.ctl_b)) then
        port_b_w(valor)
      else
        ddr_b_w(valor);
    $03:

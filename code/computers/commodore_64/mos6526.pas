...
type
  ...
  public
    ...
    procedure change_calls(pa_read, pb_read: cpu_inport_call;
      pa_write, pb_write, irq_call1, irq_call2: cpu_outport_call);
...
procedure mos6526_chip.change_calls(pa_read, pb_read: cpu_inport_call;
  pa_write, pb_write, irq_call1, irq_call2: cpu_outport_call);
begin
...
begin
  ...
  if (self.reg1.int_mask and bit) <> 0 then
  begin
...
begin
  ...
  if (self.reg2.int_mask and bit) <> 0 then
  begin
  ...
  case direccion of
    $00:
      begin
        ...
        // Columnas
        if ((tst and $01) = 0) then
          ret := ret and c64_keyboard_i[0];
        if ((tst and $02) = 0) then
          ret := ret and c64_keyboard_i[1];
        if ((tst and $04) = 0) then
          ret := ret and c64_keyboard_i[2];
        if ((tst and $08) = 0) then
          ret := ret and c64_keyboard_i[3];
        if ((tst and $10) = 0) then
          ret := ret and c64_keyboard_i[4];
        if ((tst and $20) = 0) then
          ret := ret and c64_keyboard_i[5];
        if ((tst and $40) = 0) then
          ret := ret and c64_keyboard_i[6];
        if ((tst and $80) = 0) then
          ret := ret and c64_keyboard_i[7];
    ...
    $03:
      ret := self.reg1.ddrb;
    $04:
      ret := self.reg1.ta and $FF;
    $05:
      ret := self.reg1.ta shr 8;
    $06:
      ret := self.reg1.tb and $FF;
    $07:
      ret := self.reg1.tb shr 8;
    $08:
      begin
        self.reg1.tod_halt := false;
    ...
    $09:
      ret := self.reg1.tod_sec;
    $0A:
      ret := self.reg1.tod_min;
    $0B:
      begin
...
begin
  case direccion of
    ...
    $04:
      self.reg1.latcha := (self.reg1.latcha and $FF00) or valor;
    $05:
      begin
        self.reg1.latcha := (self.reg1.latcha and $FF) or (valor shl 8);
        if ((self.reg1.cra and 1) = 0) then
          self.reg1.ta := self.reg1.latcha; // Reload timer if stopped
      end;
    $06:
      self.reg1.latchb := (self.reg1.latchb and $FF00) or valor;
    $07:
      begin
        self.reg1.latchb := (self.reg1.latchb and $FF) or (valor shl 8);
        if ((self.reg1.crb and 1) = 0) then
          self.reg1.tb := self.reg1.latchb; // Reload timer if stopped
      end;
    $08:
      if (self.reg1.crb and $80) <> 0 then
        self.reg1.alm_10ths := valor and $0F
      else
        self.reg1.tod_10ths := valor and $F;
    $09:
      if (self.reg1.crb and $80) <> 0 then
        self.reg1.alm_sec := valor and $7F
      else
        self.reg1.tod_sec := valor and $7F;
    $0A:
      if (self.reg1.crb and $80) <> 0 then
        self.reg1.alm_min := valor and $7F
      else
        self.reg1.tod_min := valor and $7F;
    $0B:
      if (self.reg1.crb and $80) <> 0 then
        self.reg1.alm_hr := valor and $9F
      else
        self.reg1.tod_hr := valor and $9F;
    $0C:
      begin
        self.reg1.sdr := valor;
    ...
    $0D:
      begin
        if (valor and $80) <> 0 then
          self.reg1.int_mask := self.reg1.int_mask or (valor and $1F)
        else
          self.reg1.int_mask := self.reg1.int_mask and not(valor);
        if (self.reg1.icr and self.reg1.int_mask and $1F) <> 0 then
        begin // Trigger IRQ if pending
          self.reg1.icr := self.reg1.icr or $80;
          self.irq_call1(ASSERT_LINE);
        end;
      end;
    $0E:
      begin
        ...
        if (valor and $10) <> 0 then
          self.reg1.ta := self.reg1.latcha; // Force load
    ...
    $0F:
      begin
        ...
        if (valor and $10) <> 0 then
          self.reg1.tb := self.reg1.latchb; // Force load
...
begin
  ...
  case direccion of
    $00:
      ret := (self.reg2.pra or not(self.reg2.ddra)) and $3F;
    // or IECLines & the_cpu_1541->IECLines;
    $01:
      ret := self.reg2.prb or not(self.reg2.ddrb);
    $02:
      ret := self.reg2.ddra;
    $03:
      ret := self.reg2.ddrb;
    $04:
      ret := self.reg2.ta and $FF;
    $05:
      ret := self.reg2.ta shr 8;
    $06:
      ret := self.reg2.tb and $FF;
    $07:
      ret := self.reg2.tb shr 8;
    $08:
      begin
        self.reg2.tod_halt := false;
    ...
    $09:
      ret := self.reg2.tod_sec;
    $0A:
      ret := self.reg2.tod_min;
    $0B:
      begin
...
begin
  case direccion of
    ...
    $03:
      self.reg2.ddrb := valor;
    $04:
      self.reg2.latcha := (self.reg2.latcha and $FF00) or valor;
    $05:
      begin
        self.reg2.latcha := (self.reg2.latcha and $FF) or (valor shl 8);
        if ((self.reg2.cra and 1) = 0) then
          self.reg2.ta := self.reg2.latcha; // Reload timer if stopped
      end;
    $06:
      self.reg2.latchb := (self.reg2.latchb and $FF00) or valor;
    $07:
      begin
        self.reg2.latchb := (self.reg2.latchb and $FF) or (valor shl 8);
        if ((self.reg2.crb and 1) = 0) then
          self.reg2.tb := self.reg2.latchb; // Reload timer if stopped
      end;
    $08:
      if (self.reg2.crb and $80) <> 0 then
        self.reg2.alm_10ths := valor and $0F
      else
        self.reg2.tod_10ths := valor and $0F;
    $09:
      if (self.reg2.crb and $80) <> 0 then
        self.reg2.alm_sec := valor and $7F
      else
        self.reg2.tod_sec := valor and $7F;
    $0A:
      if (self.reg2.crb and $80) <> 0 then
        self.reg2.alm_min := valor and $7F
      else
        self.reg2.tod_min := valor and $7F;
    $0B:
      if (self.reg2.crb and $80) <> 0 then
        self.reg2.alm_hr := valor and $9F
      else
        self.reg2.tod_hr := valor and $9F;
    $0C:
      begin
        self.reg2.sdr := valor;
    ...
    $0D:
      begin
        if (valor and $80) <> 0 then
          self.reg2.int_mask := self.reg2.int_mask or (valor and $1F)
        else
          self.reg2.int_mask := self.reg2.int_mask and not(valor);
        if (self.reg2.icr and self.reg2.int_mask and $1F) <> 0 then
        begin // Trigger NMI if pending
          self.reg2.icr := self.reg2.icr or $80;
          self.irq_call2(ASSERT_LINE);
        end;
      end;
    $0E:
      begin
        ...
        if (valor and $10) <> 0 then
          self.reg2.ta := self.reg2.latcha; // Force load
    ...
    $0F:
      begin
        ...
        if (valor and $10) <> 0 then
          self.reg2.tb := self.reg2.latchb; // Force load
...
begin
  ...
  if (self.reg1.tod_divider <> 0) then
    self.reg1.tod_divider := self.reg1.tod_divider - 1
  else
  begin
    // Reload divider according to 50/60 Hz flag
    if (self.reg1.cra and $80) <> 0 then
      self.reg1.tod_divider := 4
    else
      self.reg1.tod_divider := 5;
    // 1/10 seconds
    self.reg1.tod_10ths := self.reg1.tod_10ths + 1;
    if (self.reg1.tod_10ths > 9) then
    begin
      self.reg1.tod_10ths := 0;
      // Seconds
      ...
      begin
        ...
        begin
          ...
          if ((self.reg1.tod_hr and $1F) > $11) then
            self.reg1.tod_hr := self.reg1.tod_hr and $80 xor $80;
        end
        else
          self.reg1.tod_min := (hi shl 4) or lo;
      end
      else
        self.reg1.tod_sec := (hi shl 4) or lo;
    end;
    // Alarm time reached? Trigger interrupt if enabled
    if ((self.reg1.tod_10ths = self.reg1.alm_10ths) and (self.reg1.tod_sec = self.reg1.alm_sec) and
      (self.reg1.tod_min = self.reg1.alm_min) and (self.reg1.tod_hr = self.reg1.alm_hr)) then
...
begin
  // Decrement frequency divider
  if (self.reg2.tod_divider <> 0) then
    self.reg2.tod_divider := self.reg2.tod_divider - 1
  else
  begin
    // Reload divider according to 50/60 Hz flag
    if (self.reg2.cra and $80) <> 0 then
      self.reg2.tod_divider := 4
    else
      self.reg2.tod_divider := 5;
    // 1/10 seconds
    self.reg2.tod_10ths := self.reg2.tod_10ths + 1;
    if (self.reg2.tod_10ths > 9) then
    begin
      self.reg2.tod_10ths := 0;
      // Seconds
      ...
      begin
        ...
        begin
          ...
          if ((self.reg2.tod_hr and $1F) > $11) then
            self.reg2.tod_hr := self.reg2.tod_hr and $80 xor $80;
        end
        else
          self.reg2.tod_min := (hi shl 4) or lo;
      end
      else
        self.reg2.tod_sec := (hi shl 4) or lo;
    end;
    // Alarm time reached? Trigger interrupt if enabled
    if ((self.reg2.tod_10ths = self.reg2.alm_10ths) and (self.reg2.tod_sec = self.reg2.alm_sec) and
      (self.reg2.tod_min = self.reg2.alm_min) and (self.reg2.tod_hr = self.reg2.alm_hr)) then

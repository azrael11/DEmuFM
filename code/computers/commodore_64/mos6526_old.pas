...
type
  ...
  public
    ...
    procedure change_calls(pa_read, pb_read: cpu_inport_call;
      pa_write, pb_write, irq_call: cpu_outport_call);
  ...
  private
    ...
    tod_count, bits, ta_out, tb_out, ir0, ir1, load_a0, load_a1, load_a2, load_b0, load_b1, load_b2,
      pc: integer;
    count_a0, count_a1, count_a2, count_a3, oneshot_a0, count_b0, count_b1, count_b2, count_b3,
      oneshot_b0, cnt: integer;
...
begin
  if (self.count_a3 <> 0) then
    self.ta := self.ta - 1;
  self.ta_out := byte((self.count_a2 <> 0) and (self.ta = 0));
  if (self.ta_out <> 0) then
  begin
    self.ta_pb6 := not(self.ta_pb6);
    if (((self.cra and 8) <> 0) or (self.oneshot_a0 <> 0)) then
    begin
...
begin
  if (self.count_b3 <> 0) then
    self.tb := self.tb - 1;
  self.tb_out := byte((self.count_b2 <> 0) and (self.tb = 0));
  if (self.tb_out <> 0) then
  begin
    self.tb_pb7 := not(self.tb_pb7);
    if (((self.crb and 8) <> 0) or (self.oneshot_b0 <> 0)) then
    begin
...
begin
  if (not(self.irq) and (self.ir1 <> 0)) then
  begin
    if addr(self.irq_call) <> nil then
      self.irq_call(ASSERT_LINE);
    self.irq := true;
  end;
  if (self.ta_out <> 0) then
    self.icr := self.icr or ICR_TA;
  if ((self.tb_out <> 0) and not(self.icr_read)) then
    self.icr := self.icr or ICR_TB;
  self.icr_read := false;
end;
...
begin
  ...
  if ((self.cra and $20) = CRA_INMODE_PHI2) then
    self.count_a2 := 1;
  ...
  case ((self.crb and $60) shr 5) of
    CRB_INMODE_PHI2:
      self.count_b2 := 1;
    CRB_INMODE_TA:
      self.count_b2 := self.ta_out;
    CRB_INMODE_CNT_TA:
      self.count_b2 := byte((self.ta_out <> 0) and (self.cnt <> 0));
  end;
  ...
  if (self.ir0 <> 0) then
    self.ir1 := 1;
  if (self.icr and self.imr) <> 0 then
    self.ir0 := 1
  else
    self.ir0 := 0;
end;
...
begin
  ...
  begin
    self.pa := pa;
    if addr(self.pa_write) <> nil then
      self.pa_write(pa);
  end;
end;
...
begin
  shift := 8 * offset;
  if (self.crb and $80) <> 0 then
    self.alarm := (self.alarm and not($FF shl shift)) or (data shl shift)
  else
    self.tod := (self.tod and not($FF shl shift)) or (data shl shift);
end;
...
begin
  if (((self.cra and 1) = 0) and ((data and 1) <> 0)) then
    self.ta_pb6 := 1;
  // switching to serial output mode causes sp to go high?
  if (((self.cra and $40) = 0) and ((data and $40) <> 0)) then
  begin
    self.bits := 0;
...
begin
  ...
  if (self.cra and $80) <> 0 then
    tempb := 5
  else
    tempb := 6;
  if (self.tod_count = tempb) then
  begin
    self.tod_count := 0;
    subsecond := bcd_increment(subsecond);
    if (subsecond >= $10) then
    begin
      subsecond := 0;
...
begin
  if (((self.crb and 1) = 0) and ((data and $1) <> 0)) then
    self.tb_pb7 := 1;
  self.crb := data;
  self.update_pb;
end;
...
begin
  ...
  case (direccion and $F) of
    PRA_:
      begin
        if addr(self.pa_read) <> nil then
          tempb := self.pa_read
        else
          tempb := $FF;
        if (self.ddra <> $FF) then
          res := (tempb and not(self.ddra)) or (self.pra and self.ddra)
        else
          res := tempb and self.pra;
        self.pa_in := res;
      end;
    PRB_:
      begin
        if addr(self.pb_read) <> nil then
          tempb := self.pb_read
        else
          tempb := $FF;
        if (self.ddrb <> $FF) then
          res := (tempb and not(self.ddrb)) or (self.prb and self.ddrb)
        else
          res := tempb and self.prb;
        self.pb_in := res;
        if (self.cra and $2) <> 0 then
        begin
          if (self.cra and $4) <> 0 then
            pb6 := self.ta_pb6
          else
            pb6 := self.ta_out;
			res:=res and not($40);
        ...
        if (self.crb and $2) <> 0 then
        begin
          if (self.crb and 4) <> 0 then
            pb7 := self.tb_pb7
          else
            pb7 := self.tb_out;
			res:=res and not($80);
    ...
    begin
      // MessageDlg('read mos6526 desconocido ' + inttohex(direccion, 4), mtInformation, [mbOk], 0);
    end;
  end;
  read := res;
end;
  ...
  case (direccion and $F) of
    ...
    TA_LO_:
      begin
        ...
        if (self.load_a2 <> 0) then
          self.ta := (self.ta and $FF00) or valor;
      end;
    TA_HI_:
      begin
        self.ta_latch := (valor shl 8) or (self.ta_latch and $FF);
        if ((self.cra and 1) = 0) then
          self.load_a0 := 1;
        if ((self.cra and $8) <> 0) then
        begin
          self.ta := self.ta_latch;
          self.set_cra(self.cra or 1);
        end;
        if (self.load_a2 <> 0) then
          self.ta := (valor shl 8) or (self.ta and $FF);
      end;
    TB_LO_:
      begin
        self.tb_latch := (self.tb_latch and $FF00) or valor;
        if (self.load_b2 <> 0) then
          self.tb := (self.tb and $FF00) or valor;
      end;

    TB_HI_:
      begin
        self.tb_latch := (valor shl 8) or (self.tb_latch and $FF);
        if ((self.crb and 1) = 0) then
          self.load_b0 := 1;
        if ((self.crb and $8) <> 0) then
        begin
          self.tb := self.tb_latch;
          self.set_crb(self.crb or 1);
        end;
        if (self.load_b2 <> 0) then
          self.tb := (valor shl 8) or (self.tb and $FF);
      end;
    TOD_10THS:
      begin
    ...
    IMR_:
      begin // Mascara de las IRQs
        // Si el bit 7 es 0 --> Lo que esté a 1 lo desabilito
        // Si el bit 7 es 1 --> Lo que esté a 1 lo habilito
        if (valor and $80) <> 0 then
          self.imr := self.imr or (valor and $1F)
        else
          self.imr := self.imr and not(valor and $1F);
        // Alguna IRQ?
        if (not(self.irq) and ((self.icr and self.imr) <> 0)) then
          self.ir0 := 1;
      end;
    CRA_:
      self.set_cra(valor);
    ...
    begin
      // MessageDlg('write mos6526 desconocido ' + inttohex(direccion, 4), mtInformation, [mbOk], 0);
    end;
  end;
end;

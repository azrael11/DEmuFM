...
var
  addr: dword;
begin
  if (((k053244_regs[5] and $10) <> 0) and (direccion >= $0C) and (direccion < $10)) then
  begin
    addr := (rombank shl 19) or ((k053244_regs[11] and $7) shl 18) or (k053244_regs[8] shl 10) or
      (k053244_regs[9] shl 2) or ((direccion and 3) xor 1);
    addr := addr and (sprite_size - 1);
    k053244_read := sprite_rom[addr];
  end
  else if (direccion = $06) then
  begin
...
begin
  ...
  begin
    ...
    if (pri_code and $8000) <> 0 then
    begin
      pri_code := pri_code and $007F;
      if (((offs * 8) <> 0) and (pri_code = z_rejection)) then
        continue;
      if (sorted_list[pri_code] = -1) then
        sorted_list[pri_code] := offs * 8;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    pri := 0;
    if (@k053245_cb <> nil) then
      k053245_cb(code, color, pri);
    if pri <> prioridad then
      continue;
    size := (buffer_ram[offs] and $0F00) shr 8;
    w := 1 shl (size and $03);
    h := 1 shl ((size shr 2) and $03);
    zoom_y := buffer_ram[offs + 4];
    if (zoom_y and $FF) <> 0 then
      zy := $40 / (zoom_y and $FF)
    else
      zy := 0;
    if (zoom_y > $2000) then
      continue;
    if ((buffer_ram[offs] and $4000) = 0) then
    begin
      zoom_x := buffer_ram[offs + 5];
      if (zoom_x and $FF) <> 0 then
        zx := $40 / (zoom_x and $FF)
      else
        zx := 0;
      if (zoom_x > $2000) then
    ...
    if mirrorx then
      flipx := false; // documented and confirmed

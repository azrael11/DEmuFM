...
type
  ...
  private
    ...
    procedure k053247_draw_yxloop_gx(height, width, ox, oy, code, color, shadow, xa, ya: integer;
      zoomx, zoomy: single; nozoom, mirrorx, mirrory, flipx, flipy: boolean);
  end;
...
var
  addr: dword;
begin
  if self.objcha_line = ASSERT_LINE then
  begin
    addr := (kx46_regs[6] shl 17) or (kx46_regs[7] shl 9) or (kx46_regs[4] shl 1) or
      ((direccion and 1) xor 1);
...
begin
  if (direccion and 1) <> 0 then
    k053247_r := self.ram[direccion shr 1] and $FF
  else
    k053247_r := self.ram[direccion shr 1] shr 8;
end;

procedure k053246_chip.k053247_w(direccion: word; valor: byte);
begin
  if (direccion and 1) <> 0 then
    self.ram[direccion shr 1] := (self.ram[direccion shr 1] and $FF00) or valor
  else
    self.ram[direccion shr 1] := (self.ram[direccion shr 1] and $FF) or (valor shl 8);
end;
...
procedure k053246_chip.k053247_draw_yxloop_gx(height, width, ox, oy, code, color, shadow, xa,
  ya: integer; zoomx, zoomy: single; nozoom, mirrorx, mirrory, flipx, flipy: boolean);
const
...
begin
  ...
  begin
    ...
    begin
      ...
      if mirrorx then
      begin
        if (not(flipx) xor ((x shl 1) < width)) then
        begin
          // mirror left/right */
      ...
      if mirrory then
      begin
        if (not(flipy) xor ((y shl 1) >= height)) then
        begin
          // mirror top/bottom */
      ...
      begin
        ...
        begin
          ...
          update_gfx_sprite(sx and $3FF, sy and $3FF, 4, 1);
        end;
      end;
      if (mirrory and (height = 1)) then
      begin // Simpsons shadows
        if not(nozoom) then
        begin
...
procedure k053246_chip.k053247_draw_single_sprite_gxcore(code: dword; offs: word;
  color, shadow: word);
var
  xa, ya, ox, oy: integer;
  scalex, scaley: byte;
  wrapsize, xwraplim, ywraplim, screenwidth, objset1, temp, temp4, flipscreenx,
    flipscreeny: integer;
...
begin
  ...
  xa := 0;
  ya := 0;
  if (code and $01) <> 0 then
    xa := xa + 1;
  if (code and $02) <> 0 then
    ya := ya + 1;
  if (code and $04) <> 0 then
    xa := xa + 2;
  if (code and $08) <> 0 then
    ya := ya + 2;
  if (code and $10) <> 0 then
    xa := xa + 4;
  if (code and $20) <> 0 then
    ya := ya + 4;
  ...
  if mirrorx then
    flipx := false; // only applies to x mirror, proven
  mirrory := (temp and $8000) <> 0;
  objset1 := self.kx46_regs[5];
  // for Escape Kids (GX975)
  if (objset1 and 8) <> 0 then
  begin // Check only "Bit #3 is '1'?"
  ...
  if (k053247_opset and $40) <> 0 then
  begin
    wrapsize := 512;
    xwraplim := 512 - 64;
    ywraplim := 512 - 128;
  ...
  self.k053247_draw_yxloop_gx(height, width, ox + 53 - self.dx, oy - 6 - self.dy, code, color,
    shadow, xa, ya, zoomx, zoomy, nozoom, mirrorx, mirrory, flipx, flipy);
end;
...
begin
  count := 0;
  if (self.z_rejection = -1) then
  begin
    for f := 0 to $FF do
      if (self.ram[f * 8] and $8000) <> 0 then
      begin
  ...
  begin
    for f := 0 to $FF do
      if (((self.ram[f * 8] and $8000) <> 0) and ((self.ram[f * 8] and $FF) <> self.z_rejection))
      then
      begin
        self.sorted_list[count] := f * 8;
        count := count + 1;
...
begin
  if self.sprite_count = 0 then
    exit;
  if ((self.kx47_regs[$C div 2] and $10) = 0) then
  begin
    // sort objects in decending order(smaller z closer) when OPSET PRI is clear
    for y := 0 to (self.sprite_count - 1) do
    begin
      f := sorted_list[y];
      zcode := self.ram[f] and $FF;
      for x := y + 1 to self.sprite_count do
      begin
        temp := sorted_list[x];
  ...
  for f := 0 to (self.sprite_count - 1) do
  begin
    w := sorted_list[f];

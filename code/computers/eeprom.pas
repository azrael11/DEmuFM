...
type
  ...
  private
    ...
    enable_multi_read: boolean;
    // set to 1 to enable multiple values to be read from one read command
...
begin
  ...
  if read_file_size(self.nombre, f) then
    read_file(self.nombre, @self.addrspace, f);
end;
...
begin
  if (serial_count >= (SERIAL_BUFFER_LENGTH - 1)) then
  begin
    // MessageDlg('Buffer eeprom superado', mtInformation, [mbOk], 0);
    exit;
  end;
  if latch <> 0 then
    serial_buffer[serial_count] := '1'
  else
    serial_buffer[serial_count] := '0';
  serial_count := serial_count + 1;
  serial_buffer[serial_count] := chr(0); // nul terminate so we can treat it as a string
  if (serial_count > address_bits) then
  begin
    for i := 0 to length(self.cmd_read) do
      command[i] := self.cmd_read[i + 1];
    // READ Command
    if command_match(@serial_buffer, @command, serial_count - address_bits) then
    begin
      address := 0;
      for i := serial_count - address_bits to (serial_count - 1) do
      begin
        address := address shl 1;
        if (serial_buffer[i] = '1') then
          address := address or 1;
      end;
      if (data_bits = 16) then
        data_buffer := (addrspace[address] shr 8) or ((addrspace[address] and $FF) shl 8)
      else
        data_buffer := addrspace[address] and $FF;
    ...
    // ERASE Command
    for i := 0 to length(self.cmd_erase) do
      command[i] := self.cmd_erase[i + 1];
    if command_match(@serial_buffer, @command, serial_count - address_bits) then
    begin
      address := 0;
      for i := serial_count - address_bits to (serial_count - 1) do
      ...
      begin
        if (data_bits = 16) then
          addrspace[address] := $FFFF
        else
          addrspace[address] := $FF;
      end
      else
      begin
    ...
    // WRITE Command
    for i := 0 to length(self.cmd_write) do
      command[i] := self.cmd_write[i + 1];
    if command_match(@serial_buffer, @command, serial_count - (address_bits + data_bits)) then
    begin
      address := 0;
      for i := serial_count - data_bits - address_bits to (serial_count - data_bits - 1) do
      begin
        address := address shl 1;
        if (serial_buffer[i] = '1') then
          address := address or 1;
      ...
      data := 0;
      for i := serial_count - data_bits to (serial_count - 1) do
      begin
        data := data shl 1;
        if (serial_buffer[i] = '1') then
          data := data or 1;
      ...
      begin
        if (data_bits = 16) then
          addrspace[address] := (data shr 8) or ((data and $FF) shl 8)
        else
          addrspace[address] := data;
      end
      else
      begin
  ...
  begin
    if self.cmd_lock <> '' then
    begin
      for i := 0 to length(self.cmd_lock) do
        command[i] := self.cmd_lock[i + 1];
      if (command_match(@serial_buffer, @command, serial_count)) then
      begin
        locked := true;
        serial_count := 0;
      end;
    ...
    begin
      // UNLOCK Command
      if self.cmd_unlock <> '' then
      begin
        for i := 0 to length(self.cmd_unlock) do
          command[i] := self.cmd_unlock[i + 1];
        if (command_match(@serial_buffer, @command, serial_count)) then
        begin
          locked := false;
          serial_count := 0;
...
begin
  reset_line := state;
  if (reset_line <> CLEAR_LINE) then
  begin
    if (serial_count <> 0) then
    begin
      // MessageDlg('Comando CS serial count<>0', mtInformation,[mbOk], 0);
    end;
    serial_count := 0;
    sending := false;
...
begin
  if ((state = PULSE_LINE) or ((clock_line = CLEAR_LINE) and (state <> CLEAR_LINE))) then
  begin
    if (reset_line = CLEAR_LINE) then
    begin
      if sending then

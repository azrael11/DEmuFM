...
begin
  ...
  case chips_total of
    0:
      self.timer_io := timers.init(num_cpu, 76.8, run_io_0, nil, false);
    1:
      self.timer_io := timers.init(num_cpu, 76.8, run_io_1, nil, false);
  end;
end;
...
begin
  ...
  if (val and toggled and $1) <> 0 then
  begin
    self.coins[0] := self.coins[0] + 1;
    if (self.coins[0] >= (self.coins_per_cred[0] and 7)) then
    begin
    ...
    begin
      if (self.coins_per_cred[0] and 8) <> 0 then
        credit_add := 1;
    end;
  end;
  if (val and toggled and $02) <> 0 then
  begin
    self.coins[1] := self.coins[1] + 1;
    if (self.coins[1] >= (self.coins_per_cred[1] and 7)) then
    begin
    ...
    begin
      if (self.coins_per_cred[1] and 8) <> 0 then
        credit_add := 1;
    end;
  end;
  if (val and toggled and $08) <> 0 then
    credit_add := 1;
  ...
  if ((self.ram[9] and $F) = 0) then
  begin
    // the other argument is IORAM_READ(10) = 1, meaning unknown
    if (val and toggled and $04) <> 0 then
    begin
...
begin
  case self.tipo of
    NAMCO_56XX:
      case (self.ram[8] and $F) of // Namco 56XX
        0:
          ; // nop?
        1:
          begin // read switch inputs
            ...
            if @self.out_f[0] <> nil then
              self.out_f[0](self.ram[9] and $F); // output to pins 13-16 (motos, pacnpal, gaplus)
            if @self.out_f[1] <> nil then
              self.out_f[1](self.ram[$A] and $F); // output to pins 17-20 (gaplus)
          end;
        2:
          begin // initialize coinage settings
        ...
        4:
          begin // druaga, digdug chip #1: read dip switches and inputs
            // superpac chip #0: process coin and start inputs, read switch inputs
            self.handle_coins(0);
          end;
        7:
          begin // bootup check (liblrabl only)
        ...
        8:
          begin // bootup check
            ...
            n := 0;
            for i := 9 to 15 do
              n := n + (self.ram[i] and $F);
            self.ram[0] := n shr 4;
            self.ram[1] := n and $F;
          end;
        9:
          begin // read dip switches and inputs
            if @self.out_f[0] <> nil then
              self.out_f[0](0); // set pin 13 = 0
    ...
    NAMCO_58XX:
      case (self.ram[8] and $F) of // Namco 58XX
        0:
          ; // nop
        1:
          begin // read switch inputs
        ...
        3:
          self.handle_coins(2); // coin handle
        4:
          begin // read dip switches and inputs
            if @self.out_f[0] <> nil then
              self.out_f[0](0); // set pin 13 = 0
        ...
        5:
          begin // bootup check
            ...
            begin
              ...
              rng := seed;
              if (rng and 1) <> 0 then
                n := n xor (not(self.ram[$B]) and $F);
              rng := NEXT(rng);
              seed := rng; // save state for next loop
              if (rng and 1) <> 0 then
                n := n xor (not(self.ram[$A]) and $F);
              rng := NEXT(rng);
              if (rng and 1) <> 0 then
                n := n xor (not(self.ram[9]) and $F);
              rng := NEXT(rng);
              if (rng and 1) <> 0 then
                n := n xor (not(self.ram[$F]) and $F);
              rng := NEXT(rng);
              if (rng and 1) <> 0 then
                n := n xor (not(self.ram[$E]) and $F);
              rng := NEXT(rng);
              if (rng and 1) <> 0 then
                n := n xor (not(self.ram[$D]) and $F);
              rng := NEXT(rng);
              if (rng and 1) <> 0 then
                n := n xor (not(self.ram[$C]) and $F);
              self.ram[i] := not(n) and $F;
            end;
            self.ram[0] := 0;
            // kludge for gaplus
            if (self.ram[9] and $F) = $F then
              self.ram[0] := $F;
          end;
      end;
    NAMCO_59XX:
      case (self.ram[8] and $F) of // Namco 59XX
        0:
          ; // nop?
        3:
          begin // pacnpal chip #1: read dip switches and inputs

...
type
  ...
  tz80ctc = class
    constructor create(num_cpu: byte; clock, clock_cpu: uint64; notimer: byte; zc0_call: byte;
      zc1_call: byte = 4; zc2_call: byte = 4);
    destructor free;
  public
    procedure reset;
...
// CTC
constructor tz80ctc.create(num_cpu: byte; clock, clock_cpu: uint64; notimer: byte; zc0_call: byte;
  zc1_call: byte = 4; zc2_call: byte = 4);
  function select_zc(zc_call: byte): zc_call;
  begin
    case zc_call of
      0:
        select_zc := ctc0_trg00_w;
      1:
        select_zc := ctc0_trg01_w;
      2:
        select_zc := ctc0_trg02_w;
      3:
        select_zc := ctc0_trg03_w;
      4:
        select_zc := nil;
    end;
  end;
...
begin
  ...
  // start each channel
  self.ctc_channel[0] := tctc_channel.create(num_cpu, 0, (notimer and NOTIMER_0) <> 0,
    select_zc(zc0_call));
  self.ctc_channel[1] := tctc_channel.create(num_cpu, 1, (notimer and NOTIMER_1) <> 0,
    select_zc(zc1_call));
  self.ctc_channel[2] := tctc_channel.create(num_cpu, 2, (notimer and NOTIMER_2) <> 0,
    select_zc(zc2_call));
...
begin
  ...
  if (((self.ctc_channel[chan].mode and mode) = MODE_COUNTER) or
    ((self.ctc_channel[chan].mode and WAITING_FOR_TRIG) <> 0)) then
  begin
  ...
  begin
    ...
    if ((self.ctc_channel[chan].mode and PRESCALER) = PRESCALER_16) then
      period := self.period16
    else
      period := self.period256;
...
begin
  ...
  if ((self.ctc_channel[chan].mode and CONSTANT) = CONSTANT_LOAD) then
  begin
    // set the time constant (0 -> 0x100)
    if valor <> 0 then
      self.ctc_channel[chan].tconst := valor
    else
      self.ctc_channel[chan].tconst := $100;
    ...
    if ((self.ctc_channel[chan].mode and mode) = MODE_TIMER) then
    begin
      // if we're triggering on the time constant, reset the down counter now
      if ((self.ctc_channel[chan].mode and TRIGGER_) = TRIGGER_AUTO) then
      begin
        if not(self.ctc_channel[chan].notimer) then
        begin
...
begin
  ...
  if ((self.ctc_channel[chan].mode and RESET_) = RESET_ACTIVE) then
  begin
    period := 0;
    exit;
  end;
  // if counter mode, no real period
  if ((self.ctc_channel[chan].mode and mode) = MODE_COUNTER) then
  begin
    period := 0;
    exit;
  end;
...
var
  state: byte;
begin
  if (self.irq_state and Z80_DAISY_INT) <> 0 then
    state := ASSERT_LINE
  else
    state := CLEAR_LINE;
  self.intr(state);
...
begin
  ...
  begin
    ...
    if (self.ctc_channel[f].int_state and Z80_DAISY_IEO) <> 0 then
    begin
...
begin
  ...
  begin
    ...
    if (self.ctc_channel[f].int_state and Z80_DAISY_INT) <> 0 then
    begin
...
begin
  ...
  if (valor <> self.ctc_channel[chan].extclk) then
  begin
    self.ctc_channel[chan].extclk := valor;
    // see if this is the active edge of the trigger
    if ((((self.ctc_channel[chan].mode and EDGE) = EDGE_RISING) and valor) or
      (((self.ctc_channel[chan].mode and EDGE) = EDGE_FALLING) and not(valor))) then
    begin
      // if we're waiting for a trigger, start the timer
      if (((self.ctc_channel[chan].mode and WAITING_FOR_TRIG) <> 0) and
        ((self.ctc_channel[chan].mode and mode) = MODE_TIMER)) then
      begin
        if not(self.ctc_channel[chan].notimer) then
        begin
          curperiod := self.period(chan);
      ...
      begin
        ...
        if (self.ctc_channel[chan].down = 0) then
          self.timer_callback(chan);
      end;
    end;
  end;
end;
...
begin
  ...
  if ((self.ctc_channel[chan].mode and INTERRUPT) = INTERRUPT_ON) then
  begin
    self.ctc_channel[chan].int_state := self.ctc_channel[chan].int_state or Z80_DAISY_INT;
    self.interrupt_check;
  end;
  // generate the clock pulse
  if @self.ctc_channel[chan].zc <> nil then
  begin
...
begin
  ...
  case chips_total of
    0:
      self.timer := timers.init(num_cpu, 0, nil, ctc0_channnel, false, index);
  end;
end;

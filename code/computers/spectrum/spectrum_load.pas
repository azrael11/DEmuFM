...
begin
  ...
  begin
    ...
    if search_file_from_zip(nombre, '*.scr', nombre_file, t1, t2, false) then
    begin
      hay_scr := true;
      if datos <> nil then
      begin
    ...
    if spec_rom.hay_rom then
    begin
      spec_rom.rom_size := t1;
      // load_spec.label3.Caption := 'ROM';
      if spec_rom.datos_rom <> nil then
      begin
    ...
    repeat
      ...
      if ((extension = 'TAP') or (extension = 'TZX') or (extension = 'PZX') or (extension = 'CSW')
        or (extension = 'WAV') or (extension = 'DSP') or (extension = 'SZX') or (extension = 'ZX')
        or (extension = 'Z80') or (extension = 'SP') or (extension = 'SNA')) then
      begin
  ...
  begin // fichero normal
    if not(read_file_size(nombre, file_size)) then
      exit;
    ...
    begin
      ...
      if not(read_file(nombre, spec_rom.datos_rom, file_size)) then
        exit;
    ...
    begin
      ...
      getmem(datos, file_size);
      if not(read_file(nombre, datos, file_size)) then
        exit;
    end;
  end;
  if datos = nil then
  ...
  temp2 := nil;
  nombre := extractfilename(nombre);
  if extension = 'SZX' then
  begin
    getmem(szx_header, sizeof(tszx_header));
    copymemory(szx_header, temp, 8);
    inc(temp, 8);
    g := 8;
    if szx_header.minor_version < 10 then
    begin
      // load_spec.label3.Caption := 'SZX Spectrum Snapshot v' + inttostr(szx_header.major_version) +
      // '.0' + inttostr(szx_header.minor_version)
    end
    else
    begin
      // load_spec.label3.Caption := 'SZX Spectrum Snapshot v' + inttostr(szx_header.major_version) +
      // '.' + inttostr(szx_header.minor_version);
    end;
    // case szx_header.machine_type of
    // 0:
    // load_spec.label4.Caption := 'Spectrum 16K';
    // 1:
    // load_spec.label4.Caption := 'Spectrum 48K';
    // 2:
    // load_spec.label4.Caption := 'Spectrum 128K';
    // 3:
    // load_spec.label4.Caption := 'Spectrum +2';
    // 4:
    // load_spec.label4.Caption := 'Spectrum +2A';
    // 5:
    // load_spec.label4.Caption := 'Spectrum +3';
    // end;
    freemem(szx_header);
    getmem(szx_block, sizeof(tszx_block));
    while (not(hay_imagen) and (g < file_size)) do
    begin
  ...
  begin
    ...
    inc(temp, 10);
    g := 10;
    // if tzx_header.minor < 10 then
    // load_spec.label3.Caption := 'TZX Spectrum Tape v' + inttostr(tzx_header.major) + '.0' +
    // inttostr(tzx_header.minor)
    // else
    // load_spec.label3.Caption := 'TZX Spectrum Tape v' + inttostr(tzx_header.major) + '.' +
    // inttostr(tzx_header.minor);
    freemem(tzx_header);
    salida := false;
    while (not(hay_imagen) and (g < file_size)) do
    begin
      ...
      inc(g, long_bloque);
      if ((long_bloque > 6911) and salida) then
      begin
        hay_imagen := true;
        if ((f <> $14) or (f <> $19)) then
          inc(temp);
      end
      else
        inc(temp, long_bloque);
    end;
  end;
  if extension = 'PZX' then
  begin
    ...
    inc(temp, pzx_header.size);
    g := pzx_header.size + 8;
    // if tape_version.minor < 10 then
    // load_spec.label3.Caption := 'PZX Spectrum Tape v' + inttostr(tape_version.major) + '.0' +
    // inttostr(tape_version.minor)
    // else
    // load_spec.label3.Caption := 'PZX Spectrum Tape v' + inttostr(tape_version.major) + '.' +
    // inttostr(tape_version.minor);
    ...
    begin
      ...
      begin
        ...
        if pzx_data.bit_count > 6911 then
        begin
          hay_imagen := true;
          temp := temp3;
          inc(temp);
  ...
  if ((extension = 'Z80') or (extension = 'DSP')) then
  begin
    hay_imagen := true;
    getmem(z80_regs, sizeof(tz80_regs));
    copymemory(z80_regs, temp, 30);
    inc(temp, 30);
    ...
    begin
      ...
      case z80_ext.hw_mode of
        0, 1:
          begin
            ...
            // load_spec.label4.Caption := 'Spectrum 16K' // Modo 16k
            // else
            // load_spec.label4.Caption := 'Spectrum 48K'; // Modo 48k
            z80_ext.reg_7ffd := 0;
          end;
        2:
          begin
        ...
        4, 5, 6:
          begin
            // if (z80_ext.modify_hw and $80) <> 0 then
            // load_spec.label4.Caption := 'Spectrum +2' // Modo +2
            // else
            // load_spec.label4.Caption := 'Spectrum 128K'; // Modo 128K
          end;
        7, 8:
          begin
            // if (z80_ext.modify_hw and $80) <> 0 then
            // load_spec.label4.Caption := 'Spectrum +2A' // Modo +2A
            // else
            // load_spec.label4.Caption := 'Spectrum +3'; // Modo +3
          end;
        12:
          // load_spec.label4.Caption := 'Spectrum +2A'
          ; // Modo +2A
      ...
      begin
        ...
        begin // Comprimida
          ...
          inc(temp, z80_ram.longitud + 3);
          inc(g, z80_ram.longitud + 3);
          getmem(temp2, $5000);
          t1 := z80_ram.longitud;
        ...
        if z80_ram.numero = (((z80_ext.reg_7ffd and 8) shr 2) + 8) then
        begin
          freemem(z80_ext);
          freemem(z80_ram);
          break;
    ...
    // if extension = 'Z80' then
    // load_spec.label3.Caption := 'Z80 Spectrum Snapshot v' + cadena
    // else
    // load_spec.label3.Caption := 'DSP Spectrum Snapshot';
    temp := temp2;
    freemem(z80_regs);
  end;
  ...
  begin
    ...
    // load_spec.label3.Caption := 'Compressed Square Wave v' + inttostr(csw_header.major) + '.0' +
    // inttostr(csw_header.minor)
    // else
    // load_spec.label3.Caption := 'Compressed Square Wave v' + inttostr(csw_header.major) + '.' +
    // inttostr(csw_header.minor);
    // load_spec.label4.Caption := ' ';
    hay_imagen := false;
    freemem(csw_header);
...
begin
  ...
  case rom_crc of
    $7BCD642C:
      begin // Knight Lore, solo puede ser Spectrum 48k
    ...
    begin
      ...
      if ((main_vars.machine_type <> 0) and (main_vars.machine_type <> 5)) then
        spectrum_change_model(0);
      copymemory(@memory[0], spec_rom.datos_rom, $4000);
    end;
  end;
end;
...
var
  ...
  cadena: string;
begin
  if ((datos = nil) and not(spec_rom.hay_rom)) then
    exit;
  ...
  if ((extension = 'Z80') or (extension = 'DSP')) then
    resultado := abrir_z80(datos, file_size, extension = 'DSP');
  if extension = 'SNA' then
    resultado := abrir_sna(datos, file_size);
  if extension = 'SP' then
    resultado := abrir_sp(datos, file_size);
  if extension = 'ZX' then
    resultado := abrir_zx(datos, file_size);
  if extension = 'SZX' then
    resultado := abrir_szx(datos, file_size);
  if not(resultado) then
  begin
    // MessageDlg('No es una cinta o un snapshot válido.' + chr(10) + chr(13) +
    // 'Not a valid tape or snapshot', mtInformation, [mbOk], 0);
    cadena := '';
  end
  else
  begin
    // Si todo ha ido bien y no hay ROM, devolver la original!
    if not(rom_cambiada_48) then
      copymemory(@memory[0], @mem_snd[0], $4000);
    if cinta then
    begin

...
{
  - Version 3.5.3
  - Limpieza de la parte de la pausa en la cinta
  - Limpieza de algunas funciones
  - Eliminada la pausa del ultimo bloque TZX
  - Version 3.5.2
...
begin
  case num_pulsos of
    2:
      calcular_pulso_inicial := (datos and $80) shr 7;
    256:
      calcular_pulso_inicial := datos;
  end;
end;
...
begin
  case num_pulsos of
    2:
      calcular_bit_actual_inicial := $80;
    256:
      calcular_bit_actual_inicial := 1;
  end;
end;
...
begin
  ...
  if cinta_tzx.estados < tzx_estados_necesarios then
    exit;
  main_vars.mainmessage := '    ' + leng[main_vars.language].mensajes[1] + ': ' + inttostr(datos_totales_tzx);
  cinta_tzx.estados := cinta_tzx.estados - tzx_estados_necesarios;
  if cinta_tzx.en_pausa then
  begin
  ...
  case cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].tipo_bloque of
    $10, $11, $14:
      begin // cargas normal, turbo y datos puros
        case cinta_tzx.estado_actual of
          0:
            begin // cabecera
              cinta_tzx.value := cinta_tzx.value xor $40;
              tzx_temp := tzx_temp + 1;
              if tzx_temp < cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].ltono_cab then
              begin
              ...
              begin
                ...
                begin
                  if cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].lpausa <> 0 then
                  begin
          ...
          2:
            begin // sync 2
              ...
              if (tzx_datos_p^ and $80) <> 0 then
                tzx_estados_necesarios := cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].luno
              else
                tzx_estados_necesarios := cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].lcero;
          ...
          3:
            begin // datos puros
              ...
              if tzx_pulsos = 0 then
              begin
                if cinta_tzx.bit_actual > tzx_ultimo_bit then
                begin // no estoy en el ultimo bit
                  cinta_tzx.bit_actual := cinta_tzx.bit_actual shr 1; // pillo el siguiente
                  tzx_pulsos := 2;
                  if (tzx_datos_p^ and cinta_tzx.bit_actual) <> 0 then
                    tzx_estados_necesarios := cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].luno
                  else
                    tzx_estados_necesarios := cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].lcero;
                end
                else
                begin // estoy en el ultimo bit
                  ...
                  if tzx_contador_datos < cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].lbloque then
                  begin // se ha acabado?
                    // Es el ultimo bit?
                    if tzx_contador_datos = (cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].lbloque - 1) then
                      tzx_ultimo_bit := 1 shl (8 - cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].lbyte);
                    cinta_tzx.bit_actual := $80;
                    tzx_pulsos := 2;
                    if (tzx_datos_p^ and $80) <> 0 then
                      tzx_estados_necesarios := cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].luno
              ...
              begin // no ha completado la onda
                if (tzx_datos_p^ and cinta_tzx.bit_actual) <> 0 then
                  tzx_estados_necesarios := cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].luno
                else
                  tzx_estados_necesarios := cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].lcero;
    ...
    $13:
      begin // secuencia de pulsos
        cinta_tzx.value := cinta_tzx.value xor $40;
        if tzx_pulsos < cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].lbloque then
        begin
    ...
    $19:
      begin // datos especiales
        ...
        begin
          ...
          if cinta_tzx.bit_actual > 1 then
          begin
            // no
            cinta_tzx.bit_actual := cinta_tzx.bit_actual shr 1;
            if (tzx_datos_p^ and cinta_tzx.bit_actual) <> 0 then
              cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].pulse_num := 1
            else
              cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].pulse_num := 0;
          ...
          begin // estoy en el ultimo bit
            ...
            begin // Se ha terminado... hago la pausa
              if cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].lpausa <> 0 then
              begin
                tzx_estados_necesarios := cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].lpausa;
                cinta_tzx.en_pausa := true;
    ...
    $F3:
      begin // secuencia de pulsos grandes
        cinta_tzx.value := cinta_tzx.value xor $40;
        if tzx_pulsos < cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].lbloque then
        begin
          tzx_pulsos := tzx_pulsos + 1;
    ...
    $FD:
      begin
        ...
        for f := 0 to (cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].lbloque - 1) do
        begin
          c64_putbyte(cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].lcabecera + f, ptemp^);
          inc(ptemp);
        end;
        cinta_tzx.indice_cinta := cinta_tzx.indice_cinta + 1;
        siguiente_bloque_tzx;
...
{$ENDIF}
  ...
  temp_jump: smallint;
begin
  if not(cinta_tzx.grupo) then
  begin
    datos_totales_tzx := 0;
{$IFNDEF fpc}
    // p := tape_window1.StringGrid1.Selection;
    // p.top := cinta_tzx.indice_saltos[cinta_tzx.indice_cinta];
    // if ((tape_window1.StringGrid1.Row > 6) and (tape_window1.StringGrid1.Row < (tape_window1.StringGrid1.RowCount - 6)) and (tzx_contador_loop = 0)) then
    // tape_window1.StringGrid1.TopRow := tape_window1.StringGrid1.Row - 4;
    // p.Bottom := p.top;
  ...
  case cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].tipo_bloque of
    ...
    $14:
      begin // datos puros
        ...
        if (cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].lbloque = 1) then
          tzx_ultimo_bit := 1 shl (8 - cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].lbyte)
        else
          tzx_ultimo_bit := 1; // No son bits, es un byte entero
      end;
    $15:
      begin // direct recording
        tzx_estados_necesarios := cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].luno;
    ...
    $20:
      begin // pausa
        tzx_estados_necesarios := cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].lpausa;
        if tzx_estados_necesarios = 0 then
        begin
//          tape_window1.fStopCinta(nil);
          cinta_tzx.indice_cinta := cinta_tzx.indice_cinta + 1;
          siguiente_bloque_tzx;
    ...
    $27:
      begin // return call sequence
        if cinta_tzx.datos_tzx[cinta_tzx.indice_cinta + 1].tipo_bloque = $22 then
          cinta_tzx.grupo := false;
        indice_llamadas := indice_llamadas + 1;
        // Hay mas llamadas dentro de la sequencia??
        if indice_llamadas < cinta_tzx.datos_tzx[indice_vuelta].lbloque then
        begin
    ...
    $2A:
      if ((main_vars.machine_type = 0) or (main_vars.machine_type = 5)) then
      begin
//        tape_window1.fStopCinta(nil) // stop if 48K
      end
      else
      begin
        cinta_tzx.indice_cinta := cinta_tzx.indice_cinta + 1;
    ...
    $2B:
      begin // Set signal level
        if cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].datos^ <> 0 then
          cinta_tzx.value := $40
        else
          cinta_tzx.value := 0;
        cinta_tzx.indice_cinta := cinta_tzx.indice_cinta + 1;
    ...
    $31:
      begin
        cadena := '';
        ptemp := cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].datos;
        for f := 1 to cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].lbloque do
        begin
...
//        MessageDlg(cadena, mtInformation, [mbOk], 0);
    ...
    $F3:
      begin // secuencias de pulsos muy grandes
        case cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].initial_val of
          0:
            ;
          1:
    ...
    $FE:
      begin // final de la cinta, la paro y me voy al principio
...
begin
  ...
  if checksum = z80_val.a2 then
  begin
    if z80_val.de.w > (cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].lbloque - 2) then
      tam_bloque := cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].lbloque
    else
      tam_bloque := z80_val.de.w;
    for f := 0 to (tam_bloque - 1) do
    begin
      case main_vars.machine_type of
        0, 5:
          spec48_putbyte(z80_val.ix.w + f, ptemp^); // 48k and 16k
        1, 4:
          spec128_putbyte(z80_val.ix.w + f, ptemp^); // 128k and +2
        2, 3:
          spec3_putbyte(z80_val.ix.w + f, ptemp^); // +2A and +3
      end;
...
var
  ...
  f: byte;
begin
//  if not(tape_window1.Showing) or not(cinta_tzx.cargada) then
//    exit;
  cinta_tzx.stop_tap := false;
//  principal1.BitBtn14.Enabled := false;
  temp := 0;
  while cinta_tzx.datos_tzx[temp] <> nil do
  begin
    for f := 0 to $FF do
      if cinta_tzx.datos_tzx[temp].pulsos_sym[f] <> nil then
      begin
...
//  lenslok.activo := false;
//  lenslok.indice := 255;
  while cinta_tzx.datos_tzx[indice].tipo_bloque <> $FE do
  begin
    case cinta_tzx.datos_tzx[indice].crc32 of
      $92DC40D8:
        var_spectrum.sd_1 := true; // Camelot Warriors con SD1
      $45169147, $EAD7B3A9:
        begin // TT RACER
//          lenslok.indice := 6;
//          lenslok.activo := true;
      ...
      $8B28031E, $881BD3E1:
        begin // TOMAHAWK Spectrum & Amstrad
//          lenslok.indice := 5;
//          lenslok.activo := true;
          break;
        end;
...
//    tape_window1.label2.caption := 'SD1 Protection Active';
end;
...
begin
  ...
  begin
    ...
    if tap_header.flag = 0 then
      cinta_tzx.datos_tzx[indice].ltono_cab := 8064
    else
      cinta_tzx.datos_tzx[indice].ltono_cab := 3220;
    ...
    inc(datos, tap_header.size - 1);
    inc(longitud, tap_header.size);
    cinta_tzx.datos_tzx[indice].checksum := datos^;
    inc(datos);
    // Por defecto pongo como nombre 'datos'
    cadena := leng[main_vars.language].cinta[2];
    case tap_header.flag of
      $00:
        if tap_header.size > 18 then
        begin // Pongo el nombre si hay mas de 18!
          cadena := leng[main_vars.language].cinta[0] + ': ' + tap_header.file_name; // cabecera
        end;
      $FF:
        cadena := leng[main_vars.language].cinta[1]; // bytes
    end;
...
begin
  abrir_wav := false;
  if not(convert_wav(data, ptemp_w, long, ltemp)) then
    exit;
  vaciar_cintas;
  ...
  if (ltemp mod 8) <> 0 then
    cinta_tzx.datos_tzx[0].lbloque := cinta_tzx.datos_tzx[0].lbloque + 1;
  ...
  pos := 8;
  vtemp := 0;
  for f := 0 to (ltemp - 1) do
  begin
    temp_w := ptemp_w2^;
    inc(ptemp_w2);
    if (temp_w and $8000) <> 0 then
      vtemp := vtemp or (1 shl (pos - 1));
    pos := pos - 1;
    if pos = 0 then
    begin
      ptemp_b^ := vtemp;
...
begin
  ...
  inc(data, 25);
  inc(longitud, 25);
  if csw_header.magic <> 'Compressed Square Wave' then
  begin
    freemem(csw_header);
    exit;
  end;
  ...
  case csw_header.major of
    ...
    2:
      begin
        ...
        inc(data, csw_t2.header + 27);
        inc(longitud, csw_t2.header + 27);
  ...
  freemem(datos_out);
  // Pongo la info
  // if csw_header.minor < 10 then
  // tape_window1.StringGrid1.Cells[0, 0] := 'Compressed Square Wave v' + inttostr(csw_header.major)
  // + '.0' + inttostr(csw_header.minor)
  // else
  // tape_window1.StringGrid1.Cells[0, 0] := 'Compressed Square Wave v' + inttostr(csw_header.major)
  // + '.' + inttostr(csw_header.minor);
  // tape_window1.StringGrid1.RowCount := 1;
...
begin
  ...
  if cinta_tzx.datos_tzx[0].tipo_bloque <> $20 then
  begin
    // tape_window1.StringGrid1.RowCount := tape_window1.StringGrid1.RowCount + 1;
    for f := cinta_tzx.total_bloques downto 1 do
    begin
...
begin
  ...
  if tzx_header.magic <> 'ZXTape!' then
  begin
    freemem(tzx_header);
    exit;
  end;
  ...
  begin
    ...
    case selector of
      $10:
        begin // carga normal con cabecera
          ...
          inc(data, tzx_type_10.size - 1);
          inc(longitud, tzx_type_10.size);
          inc(long_final, tzx_type_10.size);
          freemem(tzx_type_10);
          cinta_tzx.datos_tzx[temp].checksum := data^;
          inc(data);
          cadena := leng[main_vars.language].cinta[2]; // datos
          if ((main_vars.machine_type = 0) or (main_vars.machine_type = 1) or (main_vars.machine_type = 2) or (main_vars.machine_type = 3) or (main_vars.machine_type = 4) or
            (main_vars.machine_type = 5)) then
          begin
      ...
      $11:
        begin // carga turbo con cabecera
          ...
          cadena := leng[main_vars.language].cinta[3]; // bytes turbo
          inc(long_final, cinta_tzx.datos_tzx[temp].lbloque);
          if ((main_vars.machine_type = 7) or (main_vars.machine_type = 8) or (main_vars.machine_type = 9)) then
          begin
            ptemp := cinta_tzx.datos_tzx[temp].datos;
            getmem(tzx_cpc_header, sizeof(ttzx_cpc_header));
            copymemory(tzx_cpc_header, ptemp, 18);
            if tzx_cpc_header.sync = $2C then
            begin
              cadena := leng[main_vars.language].cinta[0] + ': ' + tzx_cpc_header.name;
      ...
      $12:
        begin // tono puro
          ...
          cadena := leng[main_vars.language].cinta[4]; // Tono Puro
          cadena2 := ' ';
          freemem(tzx_type_12);
        end;
      $13:
        begin // secuencia de pulsos
          ...
          inc(data, tempb * 2);
          inc(longitud, tempb * 2);
          cadena := leng[main_vars.language].cinta[5]; // Secuencia Pulsos
          cadena2 := ' ';
        end;
      $14:
        begin // datos puros
          ...
          inc(data, cinta_tzx.datos_tzx[temp].lbloque);
          inc(longitud, cinta_tzx.datos_tzx[temp].lbloque);
          cadena := leng[main_vars.language].cinta[6]; // Datos Puros
          inc(long_final, cinta_tzx.datos_tzx[temp].lbloque);
        end;
      $15:
        begin // direct recording
          ...
          inc(data, cinta_tzx.datos_tzx[temp].lbloque);
          inc(longitud, cinta_tzx.datos_tzx[temp].lbloque);
          cadena := leng[main_vars.language].cinta[7]; // Grabacion Directa
          inc(long_final, cinta_tzx.datos_tzx[temp].lbloque);
        end;
      $19:
        begin
          ...
          inc(longitud, 18);
          // asp
          if tzx_type_19.asp = 0 then
            asp_tzx := 256
          else
            asp_tzx := tzx_type_19.asp;
          // asd
          if tzx_type_19.asd = 0 then
            asd_tzx := 256
          else
            asd_tzx := tzx_type_19.asd;
          // Creo grupo si no estoy dentro de otro!!!
          if not(cinta_tzx.grupo) then
          begin
            cinta_tzx.datos_tzx[temp].tipo_bloque := $21;
            getmem(cinta_tzx.datos_tzx[temp].datos, 1);
            cadena := leng[main_vars.language].cinta[10] + ': Generalized Data';
            inicio_grupo := temp;
          ...
          if tzx_type_19.totp <> 0 then
          begin
            // cojo los simbolos del alfabeto
            for f := 1 to asp_tzx do
            begin
              simbolos[f - 1].flag := data^;
              inc(data);
              inc(longitud);
              for g := tzx_type_19.npp downto 1 do
              begin
            ...
            for f := 1 to tzx_type_19.totp do
            begin
              pulsos_total := 0;
              // ¿que simbolo es?
              tempb := data^;
          ...
          if not(cinta_tzx.grupo) then
            long_final := 0;
          // DATOS
          if tzx_type_19.totd <> 0 then
          begin
            getmem(cinta_tzx.datos_tzx[temp], sizeof(tipo_datos_tzx));
            zero_tape_data(temp);
            // Simbolos de los datos
            for f := 1 to asd_tzx do
            begin
              ...
              inc(data);
              inc(longitud);
              for g := tzx_type_19.npd downto 1 do
              begin
            ...
            case asd_tzx of
              2:
                if (tzx_type_19.totd mod 8) <> 0 then
                  cinta_tzx.datos_tzx[temp].lbloque := (tzx_type_19.totd div 8) + 1
                else
                  cinta_tzx.datos_tzx[temp].lbloque := (tzx_type_19.totd div 8);
              256:
                cinta_tzx.datos_tzx[temp].lbloque := tzx_type_19.totd;
            else
              begin
//                MessageDlg('Simbolos div extraño!! ' + inttostr(asd_tzx), mtInformation, [mbOk], 0);
              end;
            end;
      ...
      $20:
        begin // pausa
          ...
          if not(cinta_tzx.grupo) then
          begin
            if tzx_gen_word.valor = 0 then
            begin
              cadena := leng[main_vars.language].cinta[8]; // STOP the tape
              cadena2 := ' ';
              cinta_tzx.datos_tzx[temp - 1].lpausa := 0;
            end
            else
            begin
              cadena := leng[main_vars.language].cinta[9]; // Pausa
      ...
      $21:
        begin // inicio del grupo
          ...
          inc(data, tempb);
          inc(longitud, tempb);
          nombre_grupo := leng[main_vars.language].cinta[10] + ': '; // grupo
          ptemp := cinta_tzx.datos_tzx[temp].datos;
          for f := 0 to (tempb - 1) do
          begin
      ...
      $23:
        begin // saltar a posicion
          ...
          cadena := leng[main_vars.language].cinta[11] + ' ' + inttostr(smallint(tzx_gen_word.valor));
          cadena2 := ' ';
          freemem(tzx_gen_word);
        end;
      $24:
        begin // loop
          ...
          getmem(cinta_tzx.datos_tzx[temp].datos, 1);
          punto_loop := temp + 1;
          cadena := leng[main_vars.language].cinta[12]; // Loop
          cadena2 := ' ';
          freemem(tzx_gen_word);
        end;
      $25:
        begin // loop next
          ...
          cadena := leng[main_vars.language].cinta[13]; // fin del loop
          cadena2 := ' ';
        end;
      $26:
        begin // Call sequence
          ...
          inc(data, tzx_gen_word.valor * 2);
          inc(longitud, tzx_gen_word.valor * 2);
      ...
      $28:
        begin // select block
          ...
          inc(data, tzx_gen_word.valor);
          inc(longitud, tzx_gen_word.valor);
          cadena := 'Select Block';
          cadena2 := ' ';
          freemem(tzx_gen_word);
        end;
      $2A:
        begin // stop the tape if 48k
          cinta_tzx.datos_tzx[temp].tipo_bloque := $2A;
          getmem(cinta_tzx.datos_tzx[temp].datos, 1);
          inc(data, 4);
          inc(longitud, 4);
          cadena := leng[main_vars.language].cinta[14];
          cadena2 := ' ';
        end;
      $2B:
        begin // Set signal level
      ...
      $30:
        begin // Text description
          ...
          inc(longitud, tempb);
          ptemp := cinta_tzx.datos_tzx[temp].datos;
          cadena := leng[main_vars.language].cinta[15] + ': ';
          for f := 1 to tempb do
          begin
            cadena := cadena + chr(ptemp^);
            inc(ptemp);
      ...
      $31:
        begin // mensaje
          ...
          inc(data, tempb);
          inc(longitud, tempb);
          cadena := leng[main_vars.language].cinta[16];
          cadena2 := ' ';
        end;
      $32, $33:
        begin // informacion
          ...
          begin
            ...
            inc(data, 2);
            inc(longitud, 2);
            cadena := leng[main_vars.language].cinta[17]; // Archivo
          end
          else
          begin
            ...
            inc(data);
            inc(longitud);
            cadena := leng[main_vars.language].cinta[18]; // Hardware
          end;
          getmem(cinta_tzx.datos_tzx[temp].datos, cinta_tzx.datos_tzx[temp].lbloque);
          copymemory(cinta_tzx.datos_tzx[temp].datos, data, cinta_tzx.datos_tzx[temp].lbloque);
          inc(data, cinta_tzx.datos_tzx[temp].lbloque);
      ...
      $35:
        begin // custom Data Block
          ...
          inc(data, tzx_type_35.size);
          inc(longitud, tzx_type_35.size);
...
//        MessageDlg('Bloque TZX desconocido: ' + inttohex(selector, 2), mtInformation, [mbOk], 0);
    ...
    if cinta_tzx.datos_tzx[temp].datos <> nil then
      ...
      cinta_tzx.datos_tzx[temp].crc32 := 0;
    crc_grupo := (crc_grupo + cinta_tzx.datos_tzx[temp].crc32) and $FFFFFFFF;
//    if cadena2 = '' then
//      tape_window1.StringGrid1.Cells[1, contador] := inttostr(long_final)
//    else
//      tape_window1.StringGrid1.Cells[1, contador] := cadena2;
//    cadena2 := '';
    if (not(cinta_tzx.grupo) and not(fin_grupo)) then
    begin
//      tape_window1.StringGrid1.RowCount := tape_window1.StringGrid1.RowCount + 1;
...
//    tape_window1.StringGrid1.Cells[0, contador] := nombre_grupo;
  ...
  freemem(tzx_header);
  // Quito la pusa del ultimo bloque...
  cinta_tzx.datos_tzx[temp - 1].lpausa := 0;
  // Creo el bloque final
//  tape_window1.StringGrid1.RowCount := tape_window1.StringGrid1.RowCount - 1;
  getmem(cinta_tzx.datos_tzx[temp], sizeof(tipo_datos_tzx));
...
begin
  ...
  inc(data, 8);
  longitud := 8;
  if pzx_header.name <> 'PZXT' then
  begin
    freemem(pzx_header);
    exit;
  end;
  // Aqui dentro hay informacion, pero de momento no la muestro...
  inc(data, pzx_header.size);
  inc(longitud, pzx_header.size);
  vaciar_cintas;
...
//      MessageDlg('Cinta PZX demasiado grande.', mtInformation, [mbOk], 0);
    ...
    ptemp := data;
    inc(data, pzx_header.size);
    inc(longitud, pzx_header.size); // La incremento
    datos_ok := false;
    set_signal := false;
    set_data_signal := 0;
    if pzx_header.name = 'PULS' then
    begin
      tempdw := 0;
      puls_total_long := 0;
      init_ear := 0;
      getmem(pzx_pulse, sizeof(tpzx_pulse));
      while tempdw < pzx_header.size do
      begin
        // Cojo el contador
        ...
        if (pulse_length and $8000) <> 0 then
        begin
...
//              tape_window1.StringGrid1.Cells[0, contador] := leng[main_vars.idioma].cinta[5];
              ...
              puls_total_long := puls_total_long + 1;
          end;
        end
        else
        begin // Si la durecion es 0 cambio el ear
          init_ear := 1;
        end;
      end;
...
//      tape_window1.StringGrid1.Cells[1, contador] := ' ';
//      tape_window1.StringGrid1.Cells[0, contador] := leng[main_vars.idioma].cinta[5];
      cadena3 := leng[main_vars.language].cinta[5]; // Secuencia Pulsos
      cadena2 := ' ';
      datos_ok := true;
      freemem(pzx_pulse);
    end;
    if pzx_header.name = 'PAUS' then
    begin
    ...
    begin
      ...
      if (pzx_data.bit_count mod 8) = 0 then
      begin
    ...
    begin
      ...
      cadena3 := leng[main_vars.language].cinta[15] + ': ';
      cadena2 := ' ';
      for f := 1 to pzx_header.size do
      begin
    ...
    begin
      ...
      case tempw of
        0:
          begin
            cinta_tzx.datos_tzx[contador].tipo_bloque := $20;
            cadena3 := leng[main_vars.language].cinta[8];
          end;
        1:
          begin
            cinta_tzx.datos_tzx[contador].tipo_bloque := $2A;
            cadena3 := leng[main_vars.language].cinta[14];
          end;
      end;
...
//    tape_window1.StringGrid1.RowCount := tape_window1.StringGrid1.RowCount + 1;
//    if cadena2 = '' then
//      tape_window1.StringGrid1.Cells[1, contador] := inttostr(cinta_tzx.datos_tzx[contador].lbloque)
//    else
//      tape_window1.StringGrid1.Cells[1, contador] := cadena2;
//    cadena2 := '';
...
//  tape_window1.StringGrid1.RowCount := tape_window1.StringGrid1.RowCount - 1;
  getmem(cinta_tzx.datos_tzx[contador], sizeof(tipo_datos_tzx));
  // Quito la pusa del ultimo bloque...
  cinta_tzx.datos_tzx[contador - 1].lpausa := 0;
...
begin
  ...
  inc(temp_data, sizeof(ttap_c64));
  longitud := sizeof(ttap_c64);
  if tap_c64.header <> 'C64-TAPE-RAW' then
  begin
    freemem(tap_c64);
    exit;
  end;
      ...
      GETDATA:
        begin
          ...
          begin
            ...
            data_block_size := contador div 8;
...
//            0:
//              tape_window1.StringGrid1.Cells[0, cont_tape] := 'Basic:' + nombre;
//            $40:
//              tape_window1.StringGrid1.Cells[0, cont_tape] := 'Array:' + nombre;
//            $80:
//              tape_window1.StringGrid1.Cells[0, cont_tape] := 'Machine Code:' + nombre;
//          end;
...
// T64
type
  t64_header_def = packed record
    magic: array [0 .. 2] of ansichar;
    rest: array [0 .. 28] of byte;
  end;

  t64_info_def = packed record
    version_l, version_h: byte;
    max_l, max_h: byte;
    used_l, used_h: byte;
    rest: array [0 .. 1] of byte;
    name: array [0 .. 23] of ansichar;
  end;

  t64_file_def = packed record
    c64_filetype: byte;
    filetype: byte;
    addr_l, addr_h: byte;
    end_addr_l, end_addr_h: byte;
    rest: array [0 .. 1] of byte;
    offset_addr: array [0 .. 3] of byte;
    rest2: array [0 .. 3] of byte;
    file_name: array [0 .. 15] of ansichar;
  end;

function abrir_t64(data: Pbyte; long: dword): boolean;
var
  t64_header: ^t64_header_def;
  t64_info: ^t64_info_def;
  t64_file: ^t64_file_def;
  pos_data: Pbyte;
  start_addr, file_size, f, g, cantidad, cont_tape: word;
begin
  abrir_t64 := false;
  if data = nil then
    exit;
  ...
  cont_tape := 0;
  cinta_tzx.grupo := false;
  getmem(t64_header, sizeof(t64_header_def));
  copymemory(t64_header, data, 32);
  if t64_header.magic <> 'C64' then
    exit;
  freemem(t64_header);
  inc(data, 32);
  getmem(t64_info, sizeof(t64_info_def));
  copymemory(t64_info, data, 32);
  inc(data, 32);
  cantidad := t64_info.used_l + (t64_info.used_h shl 8);
  freemem(t64_info);
  // Saco la posicion inicial de los datos
  pos_data := data;
  inc(pos_data, cantidad * 32);
  getmem(t64_file, sizeof(t64_file_def));
  for f := 0 to (cantidad - 1) do
  begin
    copymemory(t64_file, data, 32);
    inc(data, 32);
    if t64_file.c64_filetype <> 0 then
    begin
      start_addr := t64_file.addr_l + (t64_file.addr_h shl 8);
      file_size := t64_file.end_addr_l + (t64_file.end_addr_h shl 8);
      if file_size = $C3C6 then
        halt(0);
      file_size := file_size - start_addr;
      getmem(cinta_tzx.datos_tzx[cont_tape], sizeof(tipo_datos_tzx));
      zero_tape_data(cont_tape);
      cinta_tzx.datos_tzx[cont_tape].lbloque := file_size;
      cinta_tzx.datos_tzx[cont_tape].lcabecera := start_addr;
      cinta_tzx.datos_tzx[cont_tape].tipo_bloque := $FD;
      getmem(cinta_tzx.datos_tzx[cont_tape].datos, file_size);
      copymemory(cinta_tzx.datos_tzx[cont_tape].datos, pos_data, file_size);
      cinta_tzx.indice_saltos[cont_tape] := cont_tape;
      cinta_tzx.indice_select[cont_tape] := cont_tape;
//      tape_window1.StringGrid1.RowCount := tape_window1.StringGrid1.RowCount + 1;
//      tape_window1.StringGrid1.Cells[0, cont_tape] := t64_file.file_name;
//      tape_window1.StringGrid1.Cells[1, cont_tape] := inttostr(file_size);
      cont_tape := cont_tape + 1;
      inc(pos_data, file_size);
    end;
  end;
//  tape_window1.StringGrid1.RowCount := tape_window1.StringGrid1.RowCount - 1;
  // Creo el bloque final

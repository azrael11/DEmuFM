...
begin
  ...
  if cinta_tzx.estados < tzx_estados_necesarios then
    exit;
  main_vars.mainmessage := '    '' + leng.mensajes[1] + '': ' + inttostr(datos_totales_tzx);
  ...
  case cinta_tzx.datos_tzx[cinta_tzx.indice_cinta].tipo_bloque of
    ...
    $31:
      begin
        ...
        MessageDlg(cadena, TMsgDlgType.mtInformation, [TMsgDlgBtn.mbOk], 0);
        cinta_tzx.indice_cinta := cinta_tzx.indice_cinta + 1;
        siguiente_bloque_tzx;
      end;
    $F3:
...
begin
  ...
  begin
    ...
    case tap_header.flag of
      ...
      // Ficheros WAV
      function abrir_wav(data: Pbyte; long: integer; cpu_clock: dword):
        boolean;
      var
      ...
      begin
        ...
        // tape_window1.StringGrid1.Cells[0, 0] := 'Compressed Square Wave v' + inttostr(csw_header.major) + '.0' + inttostr(csw_header.minor)
        // else
        // tape_window1.StringGrid1.Cells[0, 0] := 'Compressed Square Wave v' + inttostr(csw_header.major) + '.' + inttostr(csw_header.minor);
        // tape_window1.StringGrid1.RowCount := 1;
          ...
          case selector of
            $10:
              begin // carga normal con cabecera
                ...
                inc(data);
                // cadena := leng.cinta[2]; // datos
                if ((main_vars.machine_type = 0) or (main_vars.machine_type = 1) or (main_vars.machine_type = 2) or (main_vars.machine_type = 3) or (main_vars.machine_type = 4) or (main_vars.machine_type = 5)) then
                begin
            ...
            $11:
              begin // carga turbo con cabecera
                ...
                // cadena := leng.cinta[3]; // bytes turbo
                ...
                begin
                  ...
                  begin
                    // cadena := leng.cinta[0] + ': ' + tzx_cpc_header.name;
            ...
            $13:
              begin // secuencia de pulsos
                ...
                // cadena := leng.cinta[5]; // Secuencia Pulsos
                cadena2 := ' ';
              end;
            $14:
              begin // datos puros
                ...
                inc(data, cinta_tzx.datos_tzx[temp].lbloque);
                inc(longitud, cinta_tzx.datos_tzx[temp].lbloque);
                // cadena := leng.cinta[6]; // Datos Puros
                inc(long_final, cinta_tzx.datos_tzx[temp].lbloque);
              end;
            $15:
              begin // direct recording
                ...
                inc(data, cinta_tzx.datos_tzx[temp].lbloque);
                inc(longitud, cinta_tzx.datos_tzx[temp].lbloque);
                // cadena := leng.cinta[7]; // Grabacion Directa
                inc(long_final, cinta_tzx.datos_tzx[temp].lbloque);
              end;
            $19:
              begin
                ...
                begin
                  ...
                  getmem(cinta_tzx.datos_tzx[temp].datos, 1);
                  // cadena := leng.cinta[10] + ': Generalized Data';
                  inicio_grupo := temp;
                ...
                begin
                  ...
                  case asd_tzx of
                    ...
                    MessageDlg('Weird div symbols!!' + inttostr(asd_tzx), TMsgDlgType.mtInformation, [TMsgDlgBtn.mbOk], 0);
                  end;
            ...
            $21:
              begin // inicio del grupo
                ...
                inc(data, tempb);
                inc(longitud, tempb);
                // nombre_grupo := leng.cinta[10] + ': '; // grupo
                ptemp := cinta_tzx.datos_tzx[temp].datos;
            ...
            $23:
              begin // saltar a posicion
                ...
                // cadena := leng.cinta[11] + ' ' + inttostr(smallint(tzx_gen_word.valor));
                cadena2 := ' ';
                freemem(tzx_gen_word);
              end;
            $24:
            ...
            $25:
              begin // loop next
                ...
                // cadena := leng.cinta[13]; // fin del loop
                cadena2 := ' ';
              end;
            $26:
              begin // Call sequence
            ...
            begin
              MessageDlg('Unknown TZX block: ' + inttohex(selector, 2), TMsgDlgType.mtInformation, [TMsgDlgBtn.mbOk], 0);
              // exit;
            end;
          end; // del case
          cinta_tzx.indice_saltos[temp] := contador;
      ...
      begin
        ...
        begin
          ...
          begin
            MessageDlg('PZX tape too large', TMsgDlgType.mtInformation, [TMsgDlgBtn.mbOk], 0);
            vaciar_cintas;
            freemem(pzx_header);
            exit;
          end;
          ...
          begin
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    // tape_window1.StringGrid1.Cells[0, contador] := leng.cinta[5];
                    puls_total_long := 0;
                    // Otro bloque...
                    inc(contador);
                    getmem(cinta_tzx.datos_tzx[contador], sizeof(tipo_datos_tzx));
            ...
            // tape_window1.StringGrid1.Cells[1, contador] := ' ';
            // tape_window1.StringGrid1.Cells[0, contador] := leng.cinta[5];
            // cadena3 := leng.cinta[5]; // Secuencia Pulsos
            cadena2 := ' ';
            datos_ok := true;
            freemem(pzx_pulse);
          end;
          ...
          begin
            MessageDlg('Unknown block' + pzx_header.name, TMsgDlgType.mtInformation, [TMsgDlgBtn.mbOk], 0);
            vaciar_cintas;
            exit;
          end;
          // Pongo descripcion

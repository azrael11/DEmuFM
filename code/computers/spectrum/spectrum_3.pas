...
begin
  ...
  if (direccion and $C000) = $4000 then
    spec_z80.contador := spec_z80.contador + var_spectrum.retraso[linea_3 * 227 + spec_z80.contador];
end;
    ...
    case (puerto and $F002) of
      ...
      $3000:
        if disk_present then
          temp := ReadFDCData; // 3ffd
      $2000:
        if disk_present then
          temp := ReadFDCStatus; // 2ffd
    end;
...
begin
  case main_vars.machine_type of
    2:
      begin
        machine_calls.cartridges := spectrum3_loaddisk;
        disk_present := true;
      end;
    3:
      begin

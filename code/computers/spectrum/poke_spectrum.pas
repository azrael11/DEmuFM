...
begin
  posicion := 0;
  while BBDD_pokes[posicion].nombre <> '' do
  begin
    indice := 0;
    ...
    begin
      for f := 0 to 4 do
      begin
        if cinta_tzx.datos_tzx[indice].crc32 = BBDD_pokes[posicion].crc[f] then
        begin
...
//  assignfile(fichero, ExtractFilePath(application.ExeName) + 'database.pok');
  ...
  begin
    ...
    if cadena2 = '//' then;
    if cadena2 = 'GN' then
      BBDD_pokes[posicion].nombre := copy(cadena, 4, length(cadena) - 3);
    if cadena2 = 'GC' then
    begin
    ...
    if cadena2 = 'PT' then
      BBDD_pokes[posicion].poke[poke_num].nombre_tipo := copy(cadena, 4, length(cadena) - 3);
    if cadena2 = 'PQ' then
    begin
      ...
      begin
        ...
        cadena2 := copy(cadena, 1, 2);
        if cadena2 = 'PD' then
          BBDD_pokes[posicion].poke[poke_num].direccion[f - 1] :=
            strtoint(copy(cadena, 4, length(cadena) - 3));
        // Poke valor
        readln(fichero, cadena);
        cadena2 := copy(cadena, 1, 2);
        if cadena2 = 'PV' then
          BBDD_pokes[posicion].poke[poke_num].valor[f - 1] :=
            strtoint(copy(cadena, 4, length(cadena) - 3));
      end;
      inc(poke_num);
    end;

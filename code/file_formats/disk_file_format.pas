...
type
  tdsk_header = packed record
    ...
    // Realmente son mas, pero solo hay que mirar el 'EXTENDED' (no siempre se sigue el estandar)
  ...
  tdsk_track = packed record
    ...
    // El estandar termina con chr(13)+chr(10), pero no lo siguen algunos...
...
begin
  ...
  // Disk Header
  if dsk_header.magic = 'MV - CPC' then
    estandar := true
  else if dsk_header.magic = 'EXTENDED' then
    estandar := false
  else
  begin
    freemem(dsk_track);
  ...
  f := 0;
  for track_num := 0 to (dsk_header.Tracks - 1) do
  begin
    for side_num := 0 to (dsk_header.sides - 1) do
    begin
  ...
  begin
    ...
    posicion := 0;
    // Sectores
    for sector_count := 0 to (dsk_track.number_of_sectors - 1) do
    begin
      copymemory(dsk_sector, ptemp, 8);
      inc(ptemp, 8);
      // Comprobar el track que es...
      if primer_sector then
      begin
        ...
        dsk[DrvNum].Tracks[dsk_track.side, dsk_track.track].number_sector :=
          dsk_track.number_of_sectors;
      ...
      dsk[DrvNum].Tracks[dsk_track.side, dsk_track.track].sector[sector_count].track :=
        dsk_sector.track;
      dsk[DrvNum].Tracks[dsk_track.side, dsk_track.track].sector[sector_count].head :=
        dsk_sector.side;
      dsk[DrvNum].Tracks[dsk_track.side, dsk_track.track].sector[sector_count].sector :=
        dsk_sector.id;
      dsk[DrvNum].Tracks[dsk_track.side, dsk_track.track].sector[sector_count].sector_size :=
        dsk_sector.size;
      dsk[DrvNum].Tracks[dsk_track.side, dsk_track.track].sector[sector_count].status1 :=
        dsk_sector.status1;
      dsk[DrvNum].Tracks[dsk_track.side, dsk_track.track].sector[sector_count].status2 :=
        dsk_sector.status2;
      // Calcular la longitud del sector
      if not(estandar) then
        dsk[DrvNum].Tracks[dsk_track.side, dsk_track.track].sector[sector_count].data_length :=
          dsk_sector.length
      else
        dsk[DrvNum].Tracks[dsk_track.side, dsk_track.track].sector[sector_count].data_length :=
          1 shl (dsk_sector.size + 7);
      dsk[DrvNum].Tracks[dsk_track.side, dsk_track.track].sector[sector_count].posicion_data
        := posicion;
      inc(posicion, dsk[DrvNum].Tracks[dsk_track.side, dsk_track.track].sector[sector_count]
        .data_length);
      // Weak sectors
      tempw := dsk[DrvNum].Tracks[dsk_track.side, dsk_track.track].sector[sector_count]
        .data_length div (1 shl (dsk_sector.size + 7));
      if (tempw > 1) then
      begin
        if tempw > 4 then
          tempw := 4;
    ...
    if dsk[DrvNum].DiskHeader.track_size_table[side_num, track_num] <> 0 then
    begin
      dsk[DrvNum].Tracks[side_num, track_num].track_lenght :=
        256 * (dsk[DrvNum].DiskHeader.track_size_table[side_num, track_num] - 1);
    end
    else
    begin
      if not(estandar) then
        dsk[DrvNum].Tracks[side_num, track_num].track_lenght := track_long
      else
        dsk[DrvNum].Tracks[side_num, track_num].track_lenght := dsk_header.track_size;
    end;
...
var
  f, h, g: byte;
begin
  if not(dsk[DrvNum].abierto) then
    exit;
  dsk[DrvNum].cont_multi := 0;
  dsk[DrvNum].max_multi := 0;
  for g := 0 to 1 do
  begin
    for f := 0 to dsk[DrvNum].DiskHeader.nbof_tracks do
    begin
      ...
      if dsk[DrvNum].Tracks[g, f].data <> nil then
      begin
        freemem(dsk[DrvNum].Tracks[g, f].data);
        dsk[DrvNum].Tracks[g, f].data := nil;
      end;
      for h := 0 to dsk[DrvNum].Tracks[g, f].number_sector do
      begin
...
begin
  case main_vars.machine_type of
    8, 9:
      begin // Comprobar algunas protecciones para poder parchearlas...
        if dsk[DrvNum].Tracks[0, 0].data <> nil then
          tempdw := calc_crc(dsk[DrvNum].Tracks[0, 0].data,
            dsk[DrvNum].Tracks[0, 0].sector[0].data_length)
        else
          exit;
        case tempdw of
          $8C817E25, $4B616C83:
            dsk[DrvNum].Tracks[0, 40].sector[6].sector_size := 2; // Titus the fox
          $57A3276F:
            dsk[DrvNum].Tracks[0, 39].sector[10].sector_size := 0; // Prehistorik
          $F05FE06E:
            dsk[DrvNum].Tracks[0, 39].sector[0].sector_size := 2; // Prehistorik alt
          $31388451:
            begin // Tomahawk
//              lenslok.indice := 5;
//              lenslock1.Show;
          ...
          $23A68C26:
            begin // Graphic Adventure Creator
//              lenslok.indice := 7;
//              lenslock1.Show;
            end;
          $4C23DDA4:
...
//          lenslock1.combobox1.ItemIndex := lenslok.indice;
    ...
    2:
      begin // Comprobar SpeedLock +3
        puntero := dsk[DrvNum].Tracks[0, 0].data;
        cadena2 := 'SPEEDLOCK';
        cadena := '';
        for h := 0 to (dsk[DrvNum].Tracks[0, 0].sector[0].data_length - 1) do
        begin
          cadena := cadena + char(puntero^);
          inc(puntero);
        end;
        sp3_presente := pos(cadena2, cadena) <> 0;
        if ((sp3_presente) and not(hay_multi)) then
        begin
          ...
          // Creo los nuevos
          getmem(dsk[DrvNum].Tracks[0, 0].data, dsk[DrvNum].Tracks[0, 0].track_lenght +
            (dsk[DrvNum].Tracks[0, 0].sector[1].data_length * 2));
          ...
          for f := 2 to (dsk[DrvNum].Tracks[0, 0].number_sector - 1) do
          begin
...
begin
  ...
  inc(longi, $100);
  dsk[DrvNum].protegido := false;
  if mfm_header.firma = 'MFM_DISK' then
  begin
    if mfm_header.sec_geo <> 1 then
    begin
//      MessageDlg('Geometria del disco<>1', mtInformation, [mbOk], 0);
    ...
    begin
      ...
      begin
        ...
        if datos_track[40 + wtemp] = 0 then
          inc(wtemp, 96) // formato Sedoric
        else
          inc(wtemp, 146); // formato IBM
        // Leo los sectores
        while (wtemp < 6400) do
        begin
          ...
          wtemp := wtemp + 44;
          // Algunos checks...
          if mfm_sector.nside > mfm_header.side then
            exit;
          if mfm_sector.ntrack > mfm_header.track then
            exit;
  ...
  else if mfm_header.firma = 'ORICDISK' then
  begin
    // Este es fijo... Todo va segido y se asume que cada sector es de 256bytes
    dsk[DrvNum].DiskHeader.nbof_tracks := mfm_header.track;
    dsk[DrvNum].DiskHeader.nbof_heads := mfm_header.side;
    // caras
    for f := 0 to mfm_header.side - 1 do
    begin
      // pistas
      for g := 0 to mfm_header.track - 1 do
      begin
        ...
        // sectores
        for h := 0 to (mfm_header.sec_geo - 1) do
        begin

...
begin
  // Es el mismo modelo?
  if main_vars.machine_type = model then
  begin
    machine_calls.reset;
    exit;
  end;
  ...
  { principal1.BitBtn10.Enabled := false;
    case model of
    0, 5:
    begin // Spectrum 48k y Spectrum 16k
    if model = 0 then
    main_vars.tipo_maquina := tipo_cambio_maquina(principal1.Spectrum48K1)
    else
    main_vars.tipo_maquina := tipo_cambio_maquina(principal1.Spectrum16K1);
    llamadas_maquina.iniciar := iniciar_48k;
    end;
    1, 4:
    begin // Spectrum 128k y Spectrum +2
    if model = 1 then
    main_vars.tipo_maquina := tipo_cambio_maquina(principal1.Spectrum128K1)
    else
    main_vars.tipo_maquina := tipo_cambio_maquina(principal1.Spectrum21);
    llamadas_maquina.iniciar := iniciar_128k;
    end;
    2, 3:
    begin // Spectrum +3 y Spectrum +2A
    if model = 2 then
    begin
    ...
    main_vars.tipo_maquina := tipo_cambio_maquina(principal1.Spectrum2A1);
...
begin
  ...
  copymemory(sna_regs, datos, 27);
  inc(datos, 27);
  if ((long - 27) > $C000) then
  begin
    getmem(sna_regs_128k, sizeof(tsna_regs_128k));
    getmem(ptemp, $4000);
    spectrum_change_model(1);
    copymemory(@memory_128k[5, 0], datos, $4000);
    inc(datos, $4000);
    copymemory(@memory_128k[2, 0], datos, $4000);
    inc(datos, $4000);
    copymemory(ptemp, datos, $4000);
    inc(datos, $4000);
    copymemory(sna_regs_128k, datos, 4);
    inc(datos, 4);
    spec_z80_reg.pc := sna_regs_128k.pc;
    spec_z80_reg.sp := sna_regs.sp;
    ay8910_0.reset;
    spec128_outbyte($7FFD, sna_regs_128k.reg_7ffd);
    copymemory(@memory_128k[var_spectrum.marco[3], 0], ptemp, 16384);
    for f := 0 to 7 do
      if ((f <> 2) and (f <> 5) and (f <> var_spectrum.marco[3])) then
      begin
        copymemory(@memory_128k[f, 0], datos, 16384);
        inc(datos, 16384);
      end;
    freemem(ptemp);
  ...
  begin
    if ((long - 27) < 49152) then
    begin // Spectrum 16k
      spectrum_change_model(5);
      copymemory(@memory[$4000], datos, $4000);
    end
    else
    begin // Spectrum 48k
      spectrum_change_model(0);
      copymemory(@memory[$4000], datos, $C000);
    end;
    spec_z80_reg.sp := sna_regs.sp;
    copymemory(@spec_z80_reg.pc, @memory[spec_z80_reg.sp], 2);
...
begin
  { grabar_sna := false;
    ...
    if spec_z80_reg.f2.s then
    sna_regs.flags2 := sna_regs.flags2 or $80;
    if spec_z80_reg.f2.z then
    sna_regs.flags2 := sna_regs.flags2 or $40;
    if spec_z80_reg.f2.bit5 then
    sna_regs.flags2 := sna_regs.flags2 or $20;
    if spec_z80_reg.f2.h then
    sna_regs.flags2 := sna_regs.flags2 or $10;
    if spec_z80_reg.f2.bit3 then
    sna_regs.flags2 := sna_regs.flags2 or 8;
    if spec_z80_reg.f2.p_v then
    sna_regs.flags2 := sna_regs.flags2 or 4;
    if spec_z80_reg.f2.n then
    sna_regs.flags2 := sna_regs.flags2 or 2;
    if spec_z80_reg.f2.c then
    sna_regs.flags2 := sna_regs.flags2 or 1;
    ...
    if spec_z80_reg.iff1 then
    sna_regs.iff := sna_regs.iff or 2;
    if spec_z80_reg.iff2 then
    sna_regs.iff := sna_regs.iff or 4;
    sna_regs.r := spec_z80_reg.r;
    if spec_z80_reg.f.s then
    sna_regs.flags := sna_regs.flags or $80;
    if spec_z80_reg.f.z then
    sna_regs.flags := sna_regs.flags or $40;
    if spec_z80_reg.f.bit5 then
    sna_regs.flags := sna_regs.flags or $20;
    if spec_z80_reg.f.h then
    sna_regs.flags := sna_regs.flags or $10;
    if spec_z80_reg.f.bit3 then
    sna_regs.flags := sna_regs.flags or 8;
    if spec_z80_reg.f.p_v then
    sna_regs.flags := sna_regs.flags or 4;
    if spec_z80_reg.f.n then
    sna_regs.flags := sna_regs.flags or 2;
    if spec_z80_reg.f.c then
    sna_regs.flags := sna_regs.flags or 1;
    ...
    copymemory(ptemp, sna_regs, 27);
    inc(ptemp, 27);
    if main_vars.tipo_maquina = 5 then
    begin // Spectrum 16k
    ...
    copymemory(ptemp, sna_regs, 27);
    inc(ptemp, 27);
    copymemory(ptemp, @memoria_128k[5, 0], 16384);
    inc(ptemp, $4000);
    copymemory(ptemp, @memoria_128k[2, 0], 16384);
    inc(ptemp, $4000);
    copymemory(ptemp, @memoria_128k[var_spectrum.marco[3], 0], $4000);
    inc(ptemp, $4000);
    getmem(sna_regs_128k, sizeof(tsna_regs_128k));
    sna_regs_128k.pc := spec_z80_reg.pc;
    sna_regs_128k.reg_7ffd := var_spectrum.old_7ffd;
    sna_regs_128k.trdos := 0;
    copymemory(ptemp, sna_regs_128k, 4);
    inc(ptemp, 4);
    longitud := 27 + $4000 * 3 + 4;
    freemem(sna_regs_128k);
    for temp := 0 to 7 do
    if ((temp <> 2) and (temp <> 5) and (temp <> var_spectrum.marco[3])) then
    begin
    ...
    begin
    MessageDlg('Modelo no soportado formato SNA.' + chr(10) + chr(13) +
    'Model not supported for SNA format.', mtInformation, [mbOk], 0);
    exit;
    end;
    end;
    freemem(sna_regs);
...
begin
  { abrir_zx := false;
    ...
    inc(datos, 132);
    copymemory(@memoria[$4000], datos, 49152);
    inc(datos, 49152);
    // 132 bytes sin uso + 10 bytes de configuracion interna
    inc(datos, 142);
    spec_z80_reg := spec_z80.get_internal_r;
    spec_z80_reg.iff1 := (datos^ = 1);
    inc(datos);
...
begin
  { abrir_sp := false;
    spec_z80_reg := spec_z80.get_internal_r;
    getmem(sp_regs, 38);
    copymemory(sp_regs, datos, 38);
    inc(datos, 38);
    if ((sp_regs.nombre <> 'SP') or ((sp_regs.long + 38) < long)) then
    exit;
    // Puede ser 16K o 48K
    if sp_regs.long < 16385 then
    spectrum_change_model(5)
    else
    spectrum_change_model(0);
    ...
    if (sp_regs.misc and 8) <> 0 then
    spec_z80_reg.im := 0
    else
    spec_z80_reg.im := ((sp_regs.misc and 2) shr 1) + 1;
    freemem(sp_regs);
    abrir_sp := true; }
end;
...
begin
  ...
  copymemory(szx_header, data, 8);
  inc(data, 8);
  longitud := 8;
  if (szx_header.magic <> 'ZXST') then
  begin
    freemem(szx_header);
    exit;
  end;
  case szx_header.tipo_maquina of
    ...
    2:
      spectrum_change_model(1);
    3:
      spectrum_change_model(4);
    4:
      spectrum_change_model(3);
    5:
      spectrum_change_model(2);
  else
    begin
      // MessageDlg('Modelo no de Spectrum soportado.' + chr(10) + chr(13) +
      // 'Spectrum model not supported.', mtInformation, [mbOk], 0);
      freemem(szx_header);
      exit;
    end;
  end;
  getmem(szx_block, sizeof(tszx_block));
  while longitud <> long do
  begin
    copymemory(szx_block, data, 8);
    inc(data, 8);
    inc(longitud, 8);
    if szx_block.name = 'Z80R' then
    begin
      ...
      // opppssss!!!
      if szx_regs.estados_t_irq < 40 then
        var_spectrum.irq_pos := szx_regs.estados_t_irq;
      if szx_regs.estados_t < 71000 then
        spec_z80.contador := szx_regs.estados_t;
      freemem(szx_regs);
    end;
    if szx_block.name = 'SPCR' then
    begin
      getmem(szx_spcr, sizeof(tszx_spcr));
      copymemory(szx_spcr, data, 9);
      case szx_header.tipo_maquina of
        2, 3:
          spec128_outbyte($7FFD, szx_spcr.reg_7ffd);
        4, 5:
          spec3_outbyte($7FFD, szx_spcr.reg_7ffd);
      end;
      case szx_header.tipo_maquina of
        4, 5:
          spec3_outbyte($1FFD, szx_spcr.reg_1ffd);
      end;
      case szx_header.tipo_maquina of
        0, 1:
          spec48_outbyte($FE, szx_spcr.reg_fe);
        2, 3:
          spec128_outbyte(szx_spcr.reg_fe, $FE);
        4, 5:
          spec3_outbyte($FE, szx_spcr.reg_fe);
      end;
      borde.color := szx_spcr.borde;
      freemem(szx_spcr);
    end;
    if szx_block.name = 'RAMP' then
    begin
      getmem(szx_ramp, sizeof(tszx_ramp));
      copymemory(szx_ramp, data, szx_block.longitud);
      if (szx_ramp.flags and 1) <> 0 then
      begin // Pagina RAM comprimida
        getmem(ram_sp, $4000);
        Decompress_zlib(pointer(@szx_ramp.data[0]), szx_block.longitud - 3, pointer(ram_sp),
          temp_long);
        if temp_long <> 16384 then
        begin
          freemem(szx_header);
          freemem(szx_ramp);
          exit;
        ...
        ram_sp := @szx_ramp.data[0]; // Sin comprimir
      // case szx_header.tipo_maquina of
      // 1:
      // case szx_ramp.numero of
      // 0:
      // copymemory(@memoria[$C000], ram_sp, $4000);
      // 2:
      // copymemory(@memoria[$8000], ram_sp, $4000);
      // 5:
      // copymemory(@memoria[$4000], ram_sp, $4000);
      // end;
      // 2, 3:
      // copymemory(@memoria_128k[szx_ramp.numero, 0], ram_sp, $4000);
      // 4, 5:
      // copymemory(@memoria_3[szx_ramp.numero, 0], ram_sp, $4000);
      // 0:
      // copymemory(@memoria[$4000], ram_sp, $4000);
      // end;
...
begin
  ...
  // case main_vars.tipo_maquina of
  // 0:
  // szx_header.tipo_maquina := 1;
  // 1:
  // szx_header.tipo_maquina := 2;
  // 2:
  // szx_header.tipo_maquina := 5;
  // 3:
  // szx_header.tipo_maquina := 4;
  // 4:
  // szx_header.tipo_maquina := 3;
  // 5:
  // szx_header.tipo_maquina := 0;
  // end;
  szx_header.flags := 0;
  copymemory(ptemp, szx_header, 8);
  inc(ptemp, 8);
  longitud := 8;
  freemem(szx_header);
  getmem(szx_block, sizeof(tszx_block));
  ...
  copymemory(ptemp, szx_block, 8);
  inc(ptemp, 8);
  longitud := longitud + 8;
  copymemory(ptemp, szx_crtr, 36);
  inc(ptemp, 36);
  longitud := longitud + 36;
  freemem(szx_crtr);
  // Registros
  szx_block.name := 'Z80R';
  ...
  if spec_z80_reg.f.s then
    szx_regs.flags := szx_regs.flags or $80;
  if spec_z80_reg.f.z then
    szx_regs.flags := szx_regs.flags or $40;
  if spec_z80_reg.f.bit5 then
    szx_regs.flags := szx_regs.flags or $20;
  if spec_z80_reg.f.h then
    szx_regs.flags := szx_regs.flags or $10;
  if spec_z80_reg.f.bit3 then
    szx_regs.flags := szx_regs.flags or 8;
  if spec_z80_reg.f.p_v then
    szx_regs.flags := szx_regs.flags or 4;
  if spec_z80_reg.f.n then
    szx_regs.flags := szx_regs.flags or 2;
  if spec_z80_reg.f.c then
    szx_regs.flags := szx_regs.flags or 1;
  szx_regs.bc := spec_z80_reg.bc.w;
  szx_regs.de := spec_z80_reg.de.w;
  szx_regs.hl := spec_z80_reg.hl.w;
  szx_regs.a2 := spec_z80_reg.a2;
  if spec_z80_reg.f2.s then
    szx_regs.flags2 := szx_regs.flags2 or $80;
  if spec_z80_reg.f2.z then
    szx_regs.flags2 := szx_regs.flags2 or $40;
  if spec_z80_reg.f2.bit5 then
    szx_regs.flags2 := szx_regs.flags2 or $20;
  if spec_z80_reg.f2.h then
    szx_regs.flags2 := szx_regs.flags2 or $10;
  if spec_z80_reg.f2.bit3 then
    szx_regs.flags2 := szx_regs.flags2 or 8;
  if spec_z80_reg.f2.p_v then
    szx_regs.flags2 := szx_regs.flags2 or 4;
  if spec_z80_reg.f2.n then
    szx_regs.flags2 := szx_regs.flags2 or 2;
  if spec_z80_reg.f2.c then
    szx_regs.flags2 := szx_regs.flags2 or 1;
  ...
  inc(ptemp, 8);
  longitud := longitud + 8;
  copymemory(ptemp, szx_regs, 37);
  inc(ptemp, 37);
  longitud := longitud + 37;
  freemem(szx_regs);
  // Valores de la ULA
  szx_block.name := 'SPCR';
  ...
  // for f := 0 to 2 do
  // begin // Spectrum 48k
  // Compress_zlib(pointer(@memoria[$4000 + (f * $4000)]), $4000, pointer(@szx_ramp.data[0]),
  // cantidad);
  ...
  // inc(ptemp, 8);
  // longitud := longitud + 8;
  // copymemory(ptemp, szx_ramp, cantidad + 3);
  // inc(ptemp, cantidad + 3);
  // longitud := longitud + cantidad + 3;
  // end;
  // 1, 2, 3, 4:
  // begin // Spectrum 128,+2,+2A o +3
  // for f := 0 to 7 do
  // begin
  // case main_vars.tipo_maquina of
  // 1, 4:
  // Compress_zlib(pointer(@memoria_128k[f, 0]), $4000, pointer(@szx_ramp.data[0]),
  // cantidad);
  // 2, 3:
  // Compress_zlib(pointer(@memoria_3[f, 0]), $4000, pointer(@szx_ramp.data[0]), cantidad);
  // end;
  // szx_ramp.numero := f;
  // szx_block.longitud := cantidad + 3;
  // copymemory(ptemp, szx_block, 8);
  ...
  // for f := 0 to 15 do
  // szx_ay.regs[f] := ay8910_0.get_reg(f);
  // szx_ay.flags := 0;
  // copymemory(ptemp, szx_block, 8);
  // inc(ptemp, 8);
  // longitud := longitud + 8;
  // copymemory(ptemp, szx_ay, 18);
  // inc(ptemp, 18);
  // longitud := longitud + 18;
  // freemem(szx_ay);
  // end;
  // 5:
  // begin // Spectrum 16k
  ...
  if rom_cambiada_48 then
  begin
    getmem(szx_rom, sizeof(tszx_rom));
    szx_block.name := 'ROM' + char(0);
    szx_rom.flags := 1; // Comprimida!
    Compress_zlib(pointer(@memory[0]), $4000, pointer(@szx_rom.data[0]), cantidad);
    szx_block.longitud := cantidad + 6;
    copymemory(ptemp, szx_block, 8);
    inc(ptemp, 8);
    longitud := longitud + 8;
    copymemory(ptemp, szx_rom, cantidad + 6);
    inc(ptemp, cantidad + 6);
    longitud := longitud + cantidad + 6;
    freemem(szx_rom);
  end;
  freemem(szx_ramp);
  freemem(szx_block);
...
begin
  ...
  copymemory(z80_regs, datos, 30);
  inc(datos, 30);
  longitud := 30;
  puntero := nil;
  if z80_regs.misc = $FF then
    z80_regs.misc := 1;
  if (z80_regs.pc = 0) then
  begin // version 2 o 3
    ...
    inc(datos, z80_ext.long + 2);
    inc(longitud, z80_ext.long + 2);
    // Cambiar tipo de spectrum
    case z80_ext.hw_mode of
      0, 1:
        if (z80_ext.modify_hw and $80) <> 0 then
          spectrum_change_model(5) // Modo 16k
        else
          spectrum_change_model(0); // Modo 48k
      // 2:Samram
      3:
        if z80_ext.long = 23 then
          spectrum_change_model(1) // Modo 128k
        else if (z80_ext.modify_hw and $80) <> 0 then
          spectrum_change_model(5) // Modo 16k
        else
          spectrum_change_model(0); // Modo 48k
      4, 5, 6:
        if (z80_ext.modify_hw and $80) <> 0 then
          spectrum_change_model(4) // Modo +2
        else
          spectrum_change_model(1); // Modo 128K
      7, 8:
        if (z80_ext.modify_hw and $80) <> 0 then
          spectrum_change_model(3) // Modo +2A
        else
          spectrum_change_model(2); // Modo +3
      12:
        spectrum_change_model(3); // Modo +2A
      13:
        spectrum_change_model(4); // Modo +2
    ...
    { case main_vars.tipo_maquina of
      ...
      ay8910_0.reset;
      for f := 0 to $F do
      ay8910_0.set_reg(f, z80_ext.ay_regs[f]);
      ...
      if (z80_ext.long = 55) then
      spec3_outbyte($1FFD, z80_ext.reg_1ffd);
      end;
      end; }
    ...
    begin
      ...
      begin
        ...
        inc(datos, z80_ram.longitud + 3);
        inc(longitud, z80_ram.longitud + 3);
      ...
      begin // Si no esta comprimida, copio los datos, pero con longitud fija de $4000
      ...
      { case main_vars.tipo_maquina of
        0, 5:
        case z80_ram.numero of // Spectrum 48k
        0:
        begin
        ...
        4:
        copymemory(@memoria[$8000], @z80_ram.datos[0], $4000);
        5:
        copymemory(@memoria[$C000], @z80_ram.datos[0], $4000);
        8:
        copymemory(@memoria[$4000], @z80_ram.datos[0], $4000);
        else
        exit;
        end;
        1, 4:
        case z80_ram.numero of // Spectrum 128k o Spectrum +2
        0, 1:
        copymemory(@memoria_128k[z80_ram.numero + 8, 0], @z80_ram.datos[0], $4000);
        3 .. 10:
        copymemory(@memoria_128k[z80_ram.numero - 3, 0], @z80_ram.datos[0], $4000);
        else
        exit;
        end;
        2, 3:
        case z80_ram.numero of // Spectrum +3 y Spectrum +2A
        0, 1:
        copymemory(@memoria_3[z80_ram.numero + 8, 0], @z80_ram.datos[0], $4000);
        3 .. 10:
        copymemory(@memoria_3[z80_ram.numero - 3, 0], @z80_ram.datos[0], $4000);
        else
        exit;
        end;
        end; }
  ...
  begin // version 1.XX solo 48k
    spectrum_change_model(0);
    spec_z80_reg.pc := z80_regs.pc;
    if (z80_regs.misc and $20) <> 0 then
    begin // comprimido
    ...
    // Si no esta comprimida copio directamente los datos...
  end;
...
begin
  ...
  begin
    ...
    inc(total);
    contador := 1;
    if (porigen^ = base) then
    begin
      while ((porigen^ = base) and (contador <> 255) and (total <> 16384)) do
      begin
        inc(porigen);
        inc(contador);
        inc(total);
      end;
      if ((contador > 4) or (base = $ED)) then
      begin
        pdestino^ := $ED;
        inc(pdestino);
        pdestino^ := $ED;
    ...
    begin // son bytes sueltos
      pdestino^ := base;
      inc(pdestino);
      inc(long);
      if base = $ED then
      begin // si el byte es $ed hay que meter el siguiente, para no meter tres ed seguidos!!!
        pdestino^ := porigen^;
        inc(total);
        inc(long);
        inc(pdestino);
    ...
    if total = $3FFF then
    begin // si estoy en el ultimo byte no puedo comprimir
      pdestino^ := porigen^;
      inc(total);
      inc(long);
    end;
...
begin
  ...
  if spec_z80_reg.f.s then
    z80_regs.flags := z80_regs.flags or $80;
  if spec_z80_reg.f.z then
    z80_regs.flags := z80_regs.flags or $40;
  if spec_z80_reg.f.bit5 then
    z80_regs.flags := z80_regs.flags or $20;
  if spec_z80_reg.f.h then
    z80_regs.flags := z80_regs.flags or $10;
  if spec_z80_reg.f.bit3 then
    z80_regs.flags := z80_regs.flags or 8;
  if spec_z80_reg.f.p_v then
    z80_regs.flags := z80_regs.flags or 4;
  if spec_z80_reg.f.n then
    z80_regs.flags := z80_regs.flags or 2;
  if spec_z80_reg.f.c then
    z80_regs.flags := z80_regs.flags or 1;
  ...
  if spec_z80_reg.f2.s then
    z80_regs.flags2 := z80_regs.flags2 or $80;
  if spec_z80_reg.f2.z then
    z80_regs.flags2 := z80_regs.flags2 or $40;
  if spec_z80_reg.f2.bit5 then
    z80_regs.flags2 := z80_regs.flags2 or $20;
  if spec_z80_reg.f2.h then
    z80_regs.flags2 := z80_regs.flags2 or $10;
  if spec_z80_reg.f2.bit3 then
    z80_regs.flags2 := z80_regs.flags2 or 8;
  if spec_z80_reg.f2.p_v then
    z80_regs.flags2 := z80_regs.flags2 or 4;
  if spec_z80_reg.f2.n then
    z80_regs.flags2 := z80_regs.flags2 or 2;
  if spec_z80_reg.f2.c then
    z80_regs.flags2 := z80_regs.flags2 or 1;
  ...
  // for f := 0 to 15 do
  // z80_ext.ay_regs[f] := ay8910_0.get_reg(f);
  // end;
  // 2, 3:
  // begin
  // z80_ext.hw_mode := 7;
  ...
  // end;
  // end;
  // if ((main_vars.tipo_maquina = 3) or (main_vars.tipo_maquina = 4) or (main_vars.tipo_maquina = 5))
  // then
  // z80_ext.modify_hw := $80;
  copymemory(ptemp, z80_regs, 30);
  inc(ptemp, 30);
  copymemory(ptemp, z80_ext, 57);
  inc(ptemp, 57);
  long := 87;
  freemem(z80_regs);
  freemem(z80_ext);
  ...
  // begin
  // if es_dsp then
  // Compress_zlib(pointer(@memoria[$4000 + (f * $4000)]), $4000,
  // pointer(@z80_ram.datos[0]), r)
  // else
  // r := comprimir_z80(@memoria[$4000 + (f * $4000)], @z80_ram.datos[0]);
  // z80_ram.longitud := r;
  // if r = $FFFF then
  // r := $4000;
  // z80_ram.numero := spec48_ram[f];
  // copymemory(ptemp, z80_ram, r + 3);
  // inc(ptemp, r + 3);
  // inc(long, r + 3);
  // end;
  // if rom_cambiada_48 then
  // begin
  // if es_dsp then
  // Compress_zlib(pointer(@memoria[0]), $4000, pointer(@z80_ram.datos[0]), r)
  // else
  // r := comprimir_z80(@memoria[0], @z80_ram.datos[0]);
  // z80_ram.longitud := r;
  // if r = $FFFF then
  // r := $4000;
  // z80_ram.numero := 0;
  ...
  // begin
  // if es_dsp then
  // Compress_zlib(pointer(@memoria_128k[f, 0]), $4000, pointer(@z80_ram.datos[0]), r)
  // else
  // r := comprimir_z80(@memoria_128k[f, 0], @z80_ram.datos[0]);
  // z80_ram.longitud := r;
  // if r = $FFFF then
  // r := $4000;
  // z80_ram.numero := f - 3;
  ...
  // begin
  // if es_dsp then
  // Compress_zlib(pointer(@memoria_3[f, 0]), $4000, pointer(@z80_ram.datos[0]), r)
  // else
  // r := comprimir_z80(@memoria_3[f, 0], @z80_ram.datos[0]);
  // z80_ram.longitud := r;
  // if r = $FFFF then
  // r := $4000;
  // z80_ram.numero := f - 3;
  ...
  // begin
  // if es_dsp then
  // Compress_zlib(pointer(@memoria[$4000]), $4000, pointer(@z80_ram.datos[0]), r)
  // else
  // r := comprimir_z80(@memoria[$4000], @z80_ram.datos[0]);
  // z80_ram.longitud := r;
  // if r = $FFFF then
  // r := $4000;
  // z80_ram.numero := 5;
...
begin
  ...
  cpc_sna.version := 3;
  if main_z80_reg.f.s then
    cpc_sna.flags := cpc_sna.flags or $80;
  if main_z80_reg.f.z then
    cpc_sna.flags := cpc_sna.flags or $40;
  if main_z80_reg.f.bit5 then
    cpc_sna.flags := cpc_sna.flags or $20;
  if main_z80_reg.f.h then
    cpc_sna.flags := cpc_sna.flags or $10;
  if main_z80_reg.f.bit3 then
    cpc_sna.flags := cpc_sna.flags or $8;
  if main_z80_reg.f.p_v then
    cpc_sna.flags := cpc_sna.flags or $4;
  if main_z80_reg.f.n then
    cpc_sna.flags := cpc_sna.flags or $2;
  if main_z80_reg.f.c then
    cpc_sna.flags := cpc_sna.flags or $1;
  ...
  if main_z80_reg.f2.s then
    cpc_sna.flags2 := cpc_sna.flags2 or $80;
  if main_z80_reg.f2.z then
    cpc_sna.flags2 := cpc_sna.flags2 or $40;
  if main_z80_reg.f2.bit5 then
    cpc_sna.flags2 := cpc_sna.flags2 or $20;
  if main_z80_reg.f2.h then
    cpc_sna.flags2 := cpc_sna.flags2 or $10;
  if main_z80_reg.f2.bit3 then
    cpc_sna.flags2 := cpc_sna.flags2 or $8;
  if main_z80_reg.f2.p_v then
    cpc_sna.flags2 := cpc_sna.flags2 or $4;
  if main_z80_reg.f2.n then
    cpc_sna.flags2 := cpc_sna.flags2 or $2;
  if main_z80_reg.f2.c then
    cpc_sna.flags2 := cpc_sna.flags2 or $1;
  ...
  cpc_sna.ga_conf := cpc_ga.video_mode + (byte(not cpc_ga.rom_low) shl 2) +
    (byte(not cpc_ga.rom_high) shl 3);
  // RAM
...
begin
  ...
  for f := 0 to 17 do
    cpc_crt.regs[f] := cpc_sna.crt_regs[f];
  ...
  // PSG control,reg
  for f := 0 to $F do
    ay8910_0.set_reg(f, cpc_sna.ay_regs[f]);
  ay8910_0.control(cpc_sna.ay_control);
  case cpc_sna.mem_size of
    0:
      ; // La informacion de la memoria viene adjunta en Chuncks
    64:
      for f := 0 to 3 do
      begin
    ...
    exit; // Si hay mas memoria, es otro modelo --> No soportado
  end;
  ...
  // end;
  // else
  // exit; // Modelo no soportado
  // end;
  // cpc_load_roms;
  // if cpc_sna.version = 3 then
  // begin
  // cpc_ga.lines_sync := cpc_sna.ga_lines_sync;
  // cpc_ga.lines_count := cpc_sna.ga_lines_count;
  // if cpc_sna.irq <> 0 then
  // z80_0.change_irq(ASSERT_LINE)
  // else
  // z80_0.change_irq(CLEAR_LINE);
  // getmem(cpc_chunk, sizeof(tcpc_chunk));
  // while position <> longitud do
  // begin // Hay chunks??
  // copymemory(cpc_chunk, data, 8);
  // inc(data, 8);
  // position := position + 8;
  // if cpc_chunk.name = 'MEM0' then
  // begin
  ...
  // freemem(mem_temp);
  // end;
  // if ((cpc_chunk.name[0] = 'R') and (cpc_chunk.name[1] = 'O') and
  // (cpc_chunk.name[2] = 'M')) then
  // copymemory(@cpc_rom[strtoint(cpc_chunk.name[3])].data, data, $4000);
  // if cpc_chunk.name = 'LROM' then
  // begin
  // // copymemory(@cpc_low_rom[0],data,$4000);
  // copymemory(@cpc_rom[16].data, data, $4000);
  // cpc_ga.cpc_model := 4;
  // end;
  // if cpc_chunk.name = 'LOCL' then
  // cpc_ga.cpc_model := data^;
  // if cpc_chunk.name = 'CPC+' then
  // exit; // Es un CPC plus de verdad!
...
type
  ...
  tz80_v1 = packed record
    ...
    tframes: single;
    Enabled: boolean;
    estados_demas: word end;
    tz80_v2 = packed record ppc, pc, sp: word;
    bc, de, hl: word;
    bc2, de2, hl2: word;
    ix, iy: word;
    iff1, iff2: boolean;
...
begin
  ...
  inc(data, SIZE_MH);
  longitud := SIZE_MH;
  if coleco_header.magic <> 'CLSN' then
  begin
    freemem(coleco_header);
    exit;
  end;
  if ((coleco_header.version <> 1) and (coleco_header.version <> 2) and
    (coleco_header.version <> $1002) and (coleco_header.version <> $220) and
    (coleco_header.version <> $300) and (coleco_header.version <> $301) and
    (coleco_header.version <> $310)) then
  begin
    freemem(coleco_header);
    exit;
  end;
  ...
  begin
    ...
    if coleco_block.nombre = 'CRAM' then
    begin
      getmem(ptemp, $10000);
      Decompress_zlib(data, coleco_block.longitud, pointer(ptemp), descomprimido);
//      case coleco_header.version of
//        $300, $301, $310:
//          copymemory(@memoria[0], ptemp, descomprimido)
//      else
//        copymemory(@memoria[$2000], ptemp, descomprimido);
//      end;
      freemem(ptemp);
    end;
    if coleco_block.nombre = 'Z80R' then
    begin
      case coleco_header.version of
        $1:
          begin // Version 1.00
        ...
        $300, $301, $310:
          z80_0.load_snapshot(data); // Version 3.X
      end;
    end;
    if coleco_block.nombre = 'TMSR' then
    begin
      ...
      tms_0.pant := 1;
      if tms_0.bgcolor = 0 then
        paleta[0] := 0
      else
        paleta[0] := paleta[tms_0.bgcolor];
    end;
    if coleco_block.nombre = 'MISC' then
    begin
      getmem(ptemp, $100000);
      Decompress_zlib(data, coleco_block.longitud, pointer(ptemp), descomprimido);
    ...
    if coleco_block.nombre = '7649' then
    begin
      case coleco_header.version of
        $301, $310:
        ...
        begin
          ...
          for tempb := 0 to 15 do
            sn_76496_0.VolTable[tempb] := sn76496_v1.VolTable[tempb];
          for tempb := 0 to 7 do
            sn_76496_0.Registers[tempb] := sn76496_v1.Registers[tempb];
          sn_76496_0.LastRegister := sn76496_v1.LastRegister;
          for tempb := 0 to 3 do
            sn_76496_0.Volume[tempb] := sn76496_v1.Volume[tempb];
          for tempb := 0 to 3 do
            sn_76496_0.Count[tempb] := sn76496_v1.Count[tempb];
          for tempb := 0 to 3 do
            sn_76496_0.Period[tempb] := sn76496_v1.Period[tempb];
          for tempb := 0 to 3 do
            sn_76496_0.Output[tempb] := sn76496_v1.Output[tempb];
    ...
    if coleco_block.nombre = 'AY89' then
      ay8910_0.load_snapshot(data);
    if coleco_block.nombre = 'I2CM' then
    begin
    ...
    inc(data, coleco_block.longitud);
    inc(longitud, coleco_block.longitud);
  end;
...
begin
  ...
  if copy(vsf_header.magic, 1, 18) <> 'VICE Snapshot File' then
  begin
    freemem(vsf_header);
    exit;
  end;
  if copy(vsf_header.machinename, 1, 3) <> 'C64' then
  begin
    freemem(vsf_header);
    exit;
  end;
  ...
  while posicion <> long do
  begin
    copymemory(vsf_block_head, data, VSF_HEAD);
    inc(data, VSF_HEAD);
    if copy(vsf_block_head.modulename, 1, 7) = 'MAINCPU' then
    begin
    ...
    else if copy(vsf_block_head.modulename, 1, 6) = 'C64MEM' then
    begin
...
var
  ...
  begin
    ...
    for f := 0 to 3 do
      snapshot_header.magic[f] := ansichar(magic[f + 1]);
...
begin
  ...
  System := $FF;
  if snapshot_header.magic = 'PV1K' then
    System := SPV1000;
  if snapshot_header.magic = 'PV2K' then
    System := SPV2000;
  if snapshot_header.magic = 'SG1K' then
    System := SSG1000;
  if snapshot_header.magic = 'SMS0' then
    System := SSMS;
  if snapshot_header.magic = 'SGG0' then
    System := SGG;
  if snapshot_header.magic = 'SCVI' then
    System := SSUPERCASSETTE;
  if snapshot_header.magic = 'CHP8' then
    System := SCHIP8;
  if ((System = $FF) or (System <> main_vars.system_type)) then
  begin
//    MessageDlg('Snapshot no válido para este sistema.' + chr(10) + chr(13) +
//      'Snapshot not valid for this system.', mtInformation, [mbOk], 0);
    freemem(snapshot_header);
    exit;
  end;
  inc(data, SIZE_MH);
  ...
  begin
    ...
    case main_vars.system_type of
      SCHIP8:
        begin
          if snapshot_block.nombre = 'CRAM' then
//            load_ram(@memoria[0]);
          if snapshot_block.nombre = 'MISC' then
            load_misc(@chip8_0);
        end;
      SSUPERCASSETTE:
        begin
          ...
          if snapshot_block.nombre = 'CRAM' then
//            load_ram(@memoria[0]);
          if snapshot_block.nombre = '7810' then
            upd7810_0.load_snapshot(data);
          if snapshot_block.nombre = '1771' then
            load_upd1771;
        end;
      SSMS:
        begin
          if snapshot_block.nombre = 'Z80R' then
...
//            change_sms_model(sms_0.model, false);
          ...
          if snapshot_block.nombre = 'VDP0' then
            load_smsvdp;
          if snapshot_block.nombre = '7649' then
            sn_76496_0.load_snapshot(data);
        end;
      SGG:
        begin
...
//            load_ram(@memoria[0]);
          ...
          if snapshot_block.nombre = 'MISC' then
            load_misc(@sg1000_0);
          if snapshot_block.nombre = 'TMSR' then
            load_tms99X8;
        end;
      SPV1000:
        begin

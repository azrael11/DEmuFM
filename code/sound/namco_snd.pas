...
begin
  ...
  if not(self.wave_on_ram) then
    for f := 0 to $FF do
      self.update_waveform(f, self.onda[f]);
end;
...
begin
  ...
  begin
    ...
    namco_snd_0.voice[numero_voz].activa := true;
end;
...
begin
  if not(self.enabled) then
    exit;
  sample := 0;
  for numero_voz := 0 to self.num_voces - 1 do
  begin
    if not(self.wave_on_ram) then
    begin
      if self.num_voces = 3 then
        getvoice_3(numero_voz)
      else
        getvoice_8(numero_voz);
    end;
    if voice[numero_voz].activa then
    begin
  ...
  if sample > 32767 then
    tsample[self.tsample_num, sound_status.sound_position] := 32767
  else
    tsample[self.tsample_num, sound_status.sound_position] := sample;
end;
...
var
  ch: byte;
begin
  if (self.regs[direccion] = valor) then
    exit;
  // set the register */
  ...
  case (direccion - ch * 8) of
    $00:
      self.voice[ch].volume := valor and $0F;
    $01:
      self.voice[ch].numero_onda := (valor shr 4) and $F;
    $02, $03:
      begin // the frequency has 20 bits */
        self.voice[ch].frecuencia := (self.regs[ch * 8 + $01] and 15) shl 16;
  ...
  if ((self.voice[ch].frecuencia = 0) and (self.voice[ch].volume = 0)) then
  begin
    self.voice[ch].activa := false;
    self.voice[ch].dentro_onda := 0;
  end
  else
    self.voice[ch].activa := true;
end;
...
begin
  case direccion of
    0 .. $FF:
      begin
        if (self.ram[direccion] <> valor) then
        begin
    ...
    $100 .. $13F:
      self.namcos1_sound_w(direccion and $3F, valor);
    $140 .. $3FF:
      self.ram[direccion] := valor;
  end;
end;
...
begin
  ...
  size := 0;
  for f := 0 to 7 do
    copymemory(temp, @self.voice[f], sizeof(tipo_voice));
  inc(temp, sizeof(tipo_voice));
  size := size + sizeof(tipo_voice);
  copymemory(temp, @self.enabled, sizeof(boolean));
  inc(temp, sizeof(boolean));
  size := size + sizeof(boolean);
  copymemory(temp, @self.regs, $40);
  inc(temp, $40);
  size := size + $40;
  copymemory(temp, @self.ram, $400);
  size := size + $400;
  save_snapshot := size;
end;
...
begin
  temp := data;
  for f := 0 to 7 do
    copymemory(@self.voice[f], temp, sizeof(tipo_voice));
  inc(temp, sizeof(tipo_voice));
  copymemory(@self.enabled, temp, sizeof(boolean));
  inc(temp, sizeof(boolean));
  copymemory(@self.regs, temp, $40);
  inc(temp, $40);
...
begin
  for f := 0 to 1 do
  begin
    tsample[namco_63701[f].tsample, sound_status.sound_position] := namco_63701[f].signal;
    if sound_status.stereo then
      tsample[namco_63701[f].tsample, sound_status.sound_position + 1] := namco_63701[f].signal;
  end;
end;
...
begin
  ...
  begin
    ...
    if (namco_63701[ch].select and $1F) <> 0 then
    begin
      // update the streams */

...
const
  ...
  energytable: array [0 .. $20 - 1] of byte = (0, 2, 4, 6, 10, 12, 14, 18, // 0-7
  ...
  pitchtable: array [0 .. $20 - 1] of byte = (1, // 0     : random mode */
    22, // 1     : start=22    */
...
begin
  ...
  begin
    ...
    u[10] := current_val;
    for i := 9 downto 0 do
      u[i] := u[i + 1] - ((self.current_k[i] * self.x[i]) div 32768);
    for i := 9 downto 1 do
      self.x[i] := self.x[i - 1] + ((self.current_k[i - 1] * u[i - 1]) div 32768);
    self.x[0] := u[0];

    // clipping, buffering */
    if (u[0] > 511) then
      val := 511 shl 6
    else if (u[0] < -511) then
      val := -511 shl 6
    else
      val := u[0] shl 6;

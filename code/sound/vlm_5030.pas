...
const
  ...
  // ROM Tables
  VLM5030_speed_table: array [0 .. 8 - 1] of byte = (IP_SIZE_NORMAL, IP_SIZE_FAST, IP_SIZE_FASTER, IP_SIZE_FASTER, IP_SIZE_NORMAL, IP_SIZE_SLOWER, IP_SIZE_SLOW, IP_SIZE_SLOW);

  // This is the energy lookup table
  // sampled from real chip
  energytable: array [0 .. $20 - 1] of byte = (0, 1, 2, 3, 5, 6, 7, 9, 11, 13, 15, 17, 19, 22, 24, 27, 31, 34, 38, 42, 47, 51, 57, 62, 68, 75, 82, 89, 98, 107, 116, 127);
  // This is the pitch lookup table
  pitchtable: array [0 .. $20 - 1] of byte = (0, 21, 22, 23, 24, 25, 26, 27, 28, 29, 31, 33, 35, 37, 39, 41, 43, 45, 49, 53, 57, 61, 65, 69, 73, 77, 85, 93, 101, 109, 117, 125);

  K1_table: array [0 .. 63] of integer = (390, 403, 414, 425, 434, 443, 450, 457, 463, 469, 474, 478, 482, 485, 488, 491, 494, 496, 498, 499, 501, 502, 503, 504, 505, 506, 507, 507, 508, 508, 509, 509, -390, -376, -360, -344, -325, -305, -284, -261, -237, -211, -183, -155, -125,
    -95, -64, -32, 0, 32, 64, 95, 125, 155, 183, 211, 237, 261, 284, 305, 325, 344, 360, 376);

  K2_table: array [0 .. 31] of integer = (0, 50, 100, 149, 196, 241, 284, 325, 362, 396, 426, 452, 473, 490, 502, 510, 0, -510, -502, -490, -473, -452, -426, -396, -362, -325, -284, -241, -196, -149, -100, -50);
  K3_table: array [0 .. 15] of integer = (0, 64, 128, 192, 256, 320, 384, 448, -512, -448, -384, -320, -256, -192, -128, -64);
  K5_table: array [0 .. 7] of integer = (0, 128, 256, 384, -512, -384, -256, -128);
...
begin
  temp := data;
  copymemory(temp, @self.address_mask, 4);
  inc(temp, 4);
  size := 4;
  copymemory(temp, @self.address, 2);
  inc(temp, 2);
  size := size + 2;
  temp^ := self.pin_BSY;
  inc(temp);
  ...
  inc(temp);
  size := size + 1;
  temp^ := self.latch_data;
  inc(temp);
  size := size + 1;
  copymemory(temp, @self.vcu_addr_h, 2);
  inc(temp, 2);
  size := size + 2;
  temp^ := self.parameter;
  inc(temp);
  ...
  inc(temp);
  size := size + 1;
  copymemory(temp, @self.frame_size, 4);
  inc(temp, 4);
  size := size + 4;
  copymemory(temp, @self.pitch_offset, 4);
  inc(temp, 4);
  size := size + 4;
  temp^ := self.interp_step;
  inc(temp);
  size := size + 1;
  temp^ := self.interp_count;
  inc(temp);
  size := size + 1;
  temp^ := self.sample_count;
  inc(temp);
  size := size + 1;
  temp^ := self.pitch_count;
  inc(temp);
  size := size + 1;
  copymemory(temp, @self.old_energy, 2);
  inc(temp, 2);
  size := size + 2;
  temp^ := self.old_pitch;
  inc(temp);
  size := size + 1;
  copymemory(temp, @self.old_k[0], 4 * 10);
  inc(temp, 4 * 10);
  size := size + (4 * 10);
  copymemory(temp, @self.target_energy, 2);
  inc(temp, 2);
  size := size + 2;
  temp^ := self.target_pitch;
  inc(temp);
  size := size + 1;
  copymemory(temp, @self.target_k[0], 4 * 10);
  inc(temp, 4 * 10);
  size := size + (4 * 10);
  copymemory(temp, @self.new_energy, 2);
  inc(temp, 2);
  size := size + 2;
  temp^ := self.new_pitch;
  inc(temp);
  size := size + 1;
  copymemory(temp, @self.new_k[0], 4 * 10);
  inc(temp, 4 * 10);
  size := size + (4 * 10);
  copymemory(temp, @self.current_energy, 4);
  inc(temp, 4);
  size := size + 4;
  copymemory(temp, @self.current_pitch, 4);
  inc(temp, 4);
  size := size + 4;
  copymemory(temp, @self.current_k[0], 4 * 10);
  inc(temp, 4 * 10);
  size := size + (4 * 10);
  copymemory(temp, @self.x[0], 4 * 10);
  inc(temp, 4 * 10);
  size := size + (4 * 10);
  copymemory(temp, @self.out_, 4);
  size := size + 4;
  save_snapshot := size;
end;
...
begin
  temp := data;
  copymemory(@self.address_mask, temp, 4);
  inc(temp, 4);
  copymemory(@self.address, temp, 2);
  inc(temp, 2);
  self.pin_BSY := temp^;
  inc(temp);
  self.pin_ST := temp^;
  ...
  self.pin_RST := temp^;
  inc(temp);
  self.latch_data := temp^;
  inc(temp);
  copymemory(@self.vcu_addr_h, temp, 2);
  inc(temp, 2);
  self.parameter := temp^;
  inc(temp);
  self.phase := temp^;
  inc(temp);
  copymemory(@self.frame_size, temp, 4);
  inc(temp, 4);
  copymemory(@self.pitch_offset, temp, 4);
  inc(temp, 4);
  self.interp_step := temp^;
  inc(temp);
  self.interp_count := temp^;
  inc(temp);
  self.sample_count := temp^;
  inc(temp);
  self.pitch_count := temp^;
  inc(temp);
  copymemory(@self.old_energy, temp, 2);
  inc(temp, 2);
  self.old_pitch := temp^;
  inc(temp);
  copymemory(@self.old_k[0], temp, 4 * 10);
  inc(temp, 4 * 10);
  copymemory(@self.target_energy, temp, 2);
  inc(temp, 2);
  self.target_pitch := temp^;
  inc(temp);
  copymemory(@self.target_k[0], temp, 4 * 10);
  inc(temp, 4 * 10);
  copymemory(@self.new_energy, temp, 2);
  inc(temp, 2);
  self.new_pitch := temp^;
  inc(temp);
  copymemory(@self.new_k[0], temp, 4 * 10);
  inc(temp, 4 * 10);
  copymemory(@self.current_energy, temp, 4);
  inc(temp, 4);
  copymemory(@self.current_pitch, temp, 4);
  inc(temp, 4);
  copymemory(@self.current_k[0], temp, 4 * 10);
  inc(temp, 4 * 10);
  copymemory(@self.x[0], temp, 4 * 10);
  inc(temp, 4 * 10);
...
begin
  ...
  if self.new_pitch > 0 then
    self.new_pitch := self.new_pitch + +self.pitch_offset;
  // energy
...
begin
  // running
  if ((self.phase = PH_RUN) or (self.phase = PH_STOP)) then
  begin
    // playing speech */
    // check new interpolator or new frame
    if (self.sample_count = 0) then
    begin
      if (self.phase = PH_STOP) then
      begin
        self.phase := PH_END;
        self.sample_count := 1;
      ...
      begin
        ...
        begin
          ...
          for i := 0 to 9 do
            self.target_k[i] := self.current_k[i];
        end
        else
        begin
    ...
    else if (self.old_pitch <= 1) then
    begin
      // generate unvoiced samples here
      if (random(256) and 1) <> 0 then
        current_val := self.current_energy
      else
        current_val := -self.current_energy;
    end
    else
    ...
    u[10] := current_val;
    for i := 9 downto 0 do
      u[i] := u[i + 1] - ((-self.current_k[i] * self.x[i]) div 512);
    for i := 9 downto 1 do
      self.x[i] := self.x[i - 1] + ((-self.current_k[i - 1] * u[i - 1]) div 512);
    self.x[0] := u[0];
    self.out_ := u[0] * 64;
    // clipping, buffering
    if (self.out_ > 32768) then
      self.out_ := 32768
    ...
    if (self.pitch_count >= self.current_pitch) then
      self.pitch_count := 0;
    exit;
  end;
  // stop phase

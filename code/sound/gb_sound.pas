...
begin
  ...
  begin
    ...
    begin
      ...
      if i = 0 then
        period_mode4_table[i, j] := trunc(((1 shl FIXED_POINT) / (524288 / 0.5 / (1 shl (j + 1)))) * FREQ_BASE_AUDIO)
      else
        period_mode4_table[i, j] := trunc(((1 shl FIXED_POINT) / (524288 / i / (1 shl (j + 1)))) * FREQ_BASE_AUDIO);
    end;
  end;
...
begin
  case offset of
    NR10:
      sound_r := $80 or self.regs[offset];
    NR11:
      sound_r := $3F or self.regs[offset];
    NR12:
      sound_r := self.regs[offset];
    NR13:
      sound_r := $FF;
    NR14:
      sound_r := $BF or self.regs[offset];
    NR21:
      sound_r := $3F or self.regs[offset];
    NR22:
      sound_r := self.regs[offset];
    ...
    $05, $0A:
      sound_r := $FF;
    NR52:
      sound_r := $70 or self.regs[offset];
  else
    sound_r := self.regs[offset];
  end;
end;
...
begin
  ...
  if (not(self.control.on_) and (offset <> NR52)) then
    exit;
  ...
  case offset of
    ...
    NR14:
      begin // Frequency hi / Initialize (R/W)
        ...
        begin
          if not(self.channel[1].on_) then
    ...
    NR24:
      begin // Frequency hi / Initialize (R/W)
        ...
        if (valor and $80) <> 0 then
        begin
          if not(self.channel[2].on_) then
    ...
    NR34:
      begin // Frequency hi / Initialize (W)
        ...
        if (valor and $80) <> 0 then
        begin
          if not(self.channel[3].on_) then
          begin
    ...
    NR44:
      begin // Counter/Consecutive / Initialize (R/W)
        ...
        if (valor and $80) <> 0 then
        begin
          if not(self.channel[4].on_) then
...
begin
  ...
  if self.channel[1].on_ then
  begin
    sample := self.channel[1].signal * self.channel[1].env_value;
    self.channel[1].pos := self.channel[1].pos + 1;
    if (self.channel[1].pos = (trunc(self.channel[1].period / wave_duty_table[self.channel[1].duty]) shr 16)) then
    begin
      self.channel[1].signal := -self.channel[1].signal;
    end
    else if (self.channel[1].pos > (self.channel[1].period shr 16)) then
    begin
      self.channel[1].pos := 0;
      self.channel[1].signal := -self.channel[1].signal;
    end;
    if ((self.channel[1].length <> 0) and self.channel[1].mode) then
    begin
      self.channel[1].count := self.channel[1].count + 1;
      if (self.channel[1].count >= self.channel[1].length) then
      begin
        self.channel[1].on_ := false;
        self.regs[NR52] := self.regs[NR52] and $FE;
      end;
    end;
    if (self.channel[1].env_length <> 0) then
    begin
      self.channel[1].env_count := self.channel[1].env_count + 1;
      if (self.channel[1].env_count >= self.channel[1].env_length) then
      begin
        self.channel[1].env_count := 0;
        self.channel[1].env_value := self.channel[1].env_value + self.channel[1].env_direction;
        if (self.channel[1].env_value < 0) then
          self.channel[1].env_value := 0;
        if (self.channel[1].env_value > 15) then
          self.channel[1].env_value := 15;
      end;
    end;
    if (self.channel[1].swp_time <> 0) then
    begin
      self.channel[1].swp_count := self.channel[1].swp_count + 1;
      if (self.channel[1].swp_count >= self.channel[1].swp_time) then
      begin
        self.channel[1].swp_count := 0;
        if (self.channel[1].swp_direction > 0) then
        begin
          self.channel[1].frequency := self.channel[1].frequency - (self.channel[1].frequency div (1 shl self.channel[1].swp_shift));
          if (self.channel[1].frequency <= 0) then
          begin
            self.channel[1].on_ := false;
            self.regs[NR52] := self.regs[NR52] and $FE;
          end;
        ...
        begin
          self.channel[1].frequency := self.channel[1].frequency + (self.channel[1].frequency div (1 shl self.channel[1].swp_shift));
          if (self.channel[1].frequency >= MAX_FREQUENCIES) then
          begin
    ...
    if self.control.mode_left[1] then
      left := left + sample;
    if self.control.mode_right[1] then
      right := right + sample;
  end; // del on1

  // Mode 2 - Wave with Envelope */
  if self.channel[2].on_ then
  begin
    sample := self.channel[2].signal * self.channel[2].env_value;
    self.channel[2].pos := self.channel[2].pos + 1;
    if (self.channel[2].pos = (trunc(self.channel[2].period / wave_duty_table[self.channel[2].duty]) shr 16)) then
    begin
      self.channel[2].signal := -self.channel[2].signal;
    end
    else if (self.channel[2].pos > (self.channel[2].period shr 16)) then
    begin
      self.channel[2].pos := 0;
      self.channel[2].signal := -self.channel[2].signal;
    end;

    if ((self.channel[2].length <> 0) and self.channel[2].mode) then
    begin
      self.channel[2].count := self.channel[2].count + 1;
      if (self.channel[2].count >= self.channel[2].length) then
      begin
        self.channel[2].on_ := false;
    ...
    if (self.channel[2].env_length <> 0) then
    begin
      self.channel[2].env_count := self.channel[2].env_count + 1;
      if (self.channel[2].env_count >= self.channel[2].env_length) then
      begin
        self.channel[2].env_count := 0;
        self.channel[2].env_value := self.channel[2].env_value + self.channel[2].env_direction;
        if (self.channel[2].env_value < 0) then
          self.channel[2].env_value := 0;
        if (self.channel[2].env_value > 15) then
          self.channel[2].env_value := 15;
      end;
    end;
    if self.control.mode_left[2] then
      left := left + sample;
    if self.control.mode_right[2] then
      right := right + sample;
  end;

  // Mode 3 - Wave patterns from WaveRAM */
  if self.channel[3].on_ then
  begin
    ...
    if ((self.channel[3].offset mod 2) = 0) then
      sample := sample shr 4;
    sample := (sample and $F) - 8;
    if (self.channel[3].level <> 0) then
      sample := sample shr (self.channel[3].level - 1)
    else
      sample := 0;
    self.channel[3].pos := self.channel[3].pos + 1;
    if (self.channel[3].pos >= (dword(self.channel[3].period shr 21) + self.channel[3].duty)) then
    begin
      self.channel[3].pos := 0;
      if (self.channel[3].dutycount = (dword(self.channel[3].period shr 16) mod 32)) then
      begin
      ...
      if (self.channel[3].offset > 31) then
      begin
        self.channel[3].offset := 0;
        self.channel[3].duty := 1;
        self.channel[3].dutycount := 0;
    ...
    if ((self.channel[3].length <> 0) and self.channel[3].mode) then
    begin
      self.channel[3].count := self.channel[3].count + 1;
      if (self.channel[3].count >= self.channel[3].length) then
      begin
        self.channel[3].on_ := false;
        self.regs[NR52] := self.regs[NR52] and $FB;
      end;
    end;
    if self.control.mode_left[3] then
      left := left + sample;
    if self.control.mode_right[3] then
      right := right + sample;
  end;

  // Mode 4 - Noise with Envelope */
  if self.channel[4].on_ then
  begin
    ...
    if (self.channel[4].pos = (self.channel[4].period shr 17)) then
    begin
      ...
      if self.channel[4].ply_step then
        mode4_mask := (((self.channel[4].ply_value and $2) div 2) xor (self.channel[4].ply_value and $1)) shl 6
      else
        mode4_mask := (((self.channel[4].ply_value and $2) div 2) xor (self.channel[4].ply_value and $1)) shl 14;

      self.channel[4].ply_value := self.channel[4].ply_value div 2;
      self.channel[4].ply_value := self.channel[4].ply_value or mode4_mask;
      if self.channel[4].ply_step then
        self.channel[4].ply_value := self.channel[4].ply_value and $7F
      else
        self.channel[4].ply_value := self.channel[4].ply_value and $7FFF;
      self.channel[4].signal := shortint(self.channel[4].ply_value);
    end
    else if (self.channel[4].pos > (self.channel[4].period shr 16)) then
    begin
      self.channel[4].pos := 0;
      if self.channel[4].ply_step then
        mode4_mask := (((self.channel[4].ply_value and $2) div 2) xor (self.channel[4].ply_value and $1)) shl 6
    ...
    if ((self.channel[4].length <> 0) and self.channel[4].mode) then
    begin
      self.channel[4].count := self.channel[4].count + 1;
      if (self.channel[4].count >= self.channel[4].length) then
      begin
        self.channel[4].on_ := false;
        self.regs[NR52] := self.regs[NR52] and $F7;
      end;
    end;
    if (self.channel[4].env_length <> 0) then
    begin
      self.channel[4].env_count := self.channel[4].env_count + 1;
      if (self.channel[4].env_count >= self.channel[4].env_length) then
      begin
        self.channel[4].env_count := 0;
        self.channel[4].env_value := self.channel[4].env_value + self.channel[4].env_direction;
        if (self.channel[4].env_value < 0) then
          self.channel[4].env_value := 0;
        if (self.channel[4].env_value > 15) then
          self.channel[4].env_value := 15;
      end;
    end;
    if self.control.mode_left[4] then
      left := left + sample;
    if self.control.mode_right[4] then
      right := right + sample;
  end;
  ...
  right := right shl 6;
  // Update the buffers
  tsample[self.tsample_num, sound_status.sound_position] := left;
  tsample[self.tsample_num, sound_status.sound_position + 1] := right;

...
begin
  case offset of
    NR10:
      gb_sound_r := $80 or gb_snd.snd_regs[offset];
    NR11:
      gb_sound_r := $3F or gb_snd.snd_regs[offset];
    NR12:
      gb_sound_r := gb_snd.snd_regs[offset];
    NR13:
      gb_sound_r := $FF;
    NR14:
      gb_sound_r := $BF or gb_snd.snd_regs[offset];
    NR21:
      gb_sound_r := $3F or gb_snd.snd_regs[offset];
    NR22:
      gb_sound_r := gb_snd.snd_regs[offset];
    ...
    $05, $0A:
      gb_sound_r := $FF;
    NR52:
      gb_sound_r := $70 or gb_snd.snd_regs[offset];
  else
    gb_sound_r := gb_snd.snd_regs[offset];
  end;
end;
  ...
  case offset of
    ...
    NR10:
      begin // Sweep (R/W)
        ...
        gb_snd.snd_1.swp_direction := gb_snd.snd_1.swp_direction or
          (gb_snd.snd_1.swp_direction - 1);
        gb_snd.snd_1.swp_time := gb_snd.swp_time_table[(valor and $70) shr 4];
      end;
    NR11:
      begin // Sound length/Wave pattern duty (R/W) */
    ...
    NR12:
      begin // Envelope (R/W) */
        ...
        gb_snd.snd_1.env_direction := gb_snd.snd_1.env_direction or
          (gb_snd.snd_1.env_direction - 1);
        gb_snd.snd_1.env_length := gb_snd.env_length_table[valor and $7];
      end;
    NR13:
      begin // Frequency lo (R/W) */
    ...
    NR14:
      begin // Frequency hi / Initialize (R/W) */
        gb_snd.snd_1.mode := (valor and $40) shr 6;
        gb_snd.snd_1.frequency := ((gb_snd.snd_regs[NR14] and $7) shl 8) or gb_snd.snd_regs[NR13];
        gb_snd.snd_1.period := gb_snd.period_table[gb_snd.snd_1.frequency];
        if (valor and $80) <> 0 then
        begin
          if (gb_snd.snd_1.on_ = 0) then
            gb_snd.snd_1.pos := 0;
          gb_snd.snd_1.on_ := 1;
    ...
    NR22:
      begin // Envelope (R/W)
        ...
        gb_snd.snd_2.env_direction := gb_snd.snd_2.env_direction or
          (gb_snd.snd_2.env_direction - 1);
        gb_snd.snd_2.env_length := gb_snd.env_length_table[valor and $7];
      end;
    NR23:
      begin // Frequency lo (R/W) */
        gb_snd.snd_2.period := gb_snd.period_table[((gb_snd.snd_regs[NR24] and $7) shl 8) or
          gb_snd.snd_regs[NR23]];
      end;
    NR24:
      begin // Frequency hi / Initialize (R/W) */
    ...
    // MODE 3 */
    NR30:
      begin // Sound On/Off (R/W) */
        gb_snd.snd_3.on_ := (valor and $80) shr 7;
      end;
    NR31:
      begin // Sound Length (R/W) */
        gb_snd.snd_3.length := gb_snd.length_mode3_table[valor];
      end;
    NR32:
      begin // Select Output Level */
        gb_snd.snd_3.level := (valor and $60) shr 5;
      end;
    NR33:
      begin // Frequency lo (W) */
        gb_snd.snd_3.period := gb_snd.period_mode3_table[((gb_snd.snd_regs[NR34] and $7) shl 8) or
          gb_snd.snd_regs[NR33]];
      end;
    NR34:
      begin // Frequency hi / Initialize (W) */
        gb_snd.snd_3.mode := (valor and $40) shr 6;
        gb_snd.snd_3.period := gb_snd.period_mode3_table[((gb_snd.snd_regs[NR34] and $7) shl 8) or
          gb_snd.snd_regs[NR33]];
        if (valor and $80) <> 0 then
        begin
          if (gb_snd.snd_3.on_ = 0) then
          begin
            gb_snd.snd_3.pos := 0;
    ...
    NR52:
      begin // Sound On/Off (R/W) */
        ...
        if (gb_snd.snd_control.on_ = 0) then
        begin
...
begin
  ...
  if ((gb_snd.snd_control.on_ = 0) and (offset <> NR52)) then
    exit;
...
begin
  ...
  for I := 0 to 7 do
  begin
    gb_snd.env_length_table[I] := trunc((I * ((1 shl FIXED_POINT) / 64) * FREQ_BASE_AUDIO))
      shr FIXED_POINT;
    gb_snd.swp_time_table[I] := trunc((((I shl FIXED_POINT) / 128) * FREQ_BASE_AUDIO))
      shr (FIXED_POINT - 1);
  end;
  // Calculate the period tables */
  for I := 0 to (MAX_FREQUENCIES - 1) do
  begin
    gb_snd.period_table[I] := trunc(((1 shl FIXED_POINT) / (131072 / (2048 - I))) *
      FREQ_BASE_AUDIO);
    gb_snd.period_mode3_table[I] := trunc(((1 shl FIXED_POINT) / (65536 / (2048 - I))) *
      FREQ_BASE_AUDIO);
  end;
  ...
  begin
    ...
    begin
      ...
      if I = 0 then
        gb_snd.period_mode4_table[I, J] :=
          trunc(((1 shl FIXED_POINT) / (524288 / 0.5 / (1 shl (J + 1)))) * FREQ_BASE_AUDIO)
      else
        gb_snd.period_mode4_table[I, J] :=
          trunc(((1 shl FIXED_POINT) / (524288 / I / (1 shl (J + 1)))) * FREQ_BASE_AUDIO);
    end;
  end;
  ...
  for I := 0 to 63 do
  begin
    gb_snd.length_table[I] := trunc((64 - I) * ((1 shl FIXED_POINT) / 256) * FREQ_BASE_AUDIO)
      shr FIXED_POINT;
  end;
  // Calculate the length table for mode 3 */
  for I := 0 to 255 do
  begin
    gb_snd.length_mode3_table[I] := trunc((256 - I) * ((1 shl FIXED_POINT) / 256) * FREQ_BASE_AUDIO)
      shr FIXED_POINT;
  end;
  gameboy_sound_reset;
...
begin
  ...
  begin
    ...
    if (gb_snd.snd_1.pos = (trunc(gb_snd.snd_1.period / wave_duty_table[gb_snd.snd_1.duty]) shr 16))
    then
    begin
      gb_snd.snd_1.signal := -gb_snd.snd_1.signal;
    end
    else if (gb_snd.snd_1.pos > (gb_snd.snd_1.period shr 16)) then
    begin
      gb_snd.snd_1.pos := 0;
      gb_snd.snd_1.signal := -gb_snd.snd_1.signal;
    end;
    if ((gb_snd.snd_1.length <> 0) and (gb_snd.snd_1.mode <> 0)) then
    begin
      gb_snd.snd_1.count := gb_snd.snd_1.count + 1;
      if (gb_snd.snd_1.count >= gb_snd.snd_1.length) then
      begin
        gb_snd.snd_1.on_ := 0;
        gb_snd.snd_regs[NR52] := gb_snd.snd_regs[NR52] and $FE;
      end;
    end;
    if (gb_snd.snd_1.env_length <> 0) then
    begin
      gb_snd.snd_1.env_count := gb_snd.snd_1.env_count + 1;
      if (gb_snd.snd_1.env_count >= gb_snd.snd_1.env_length) then
      begin
        gb_snd.snd_1.env_count := 0;
        gb_snd.snd_1.env_value := gb_snd.snd_1.env_value + gb_snd.snd_1.env_direction;
        if (gb_snd.snd_1.env_value < 0) then
          gb_snd.snd_1.env_value := 0;
        if (gb_snd.snd_1.env_value > 15) then
          gb_snd.snd_1.env_value := 15;
      end;
    end;
    if (gb_snd.snd_1.swp_time <> 0) then
    begin
      gb_snd.snd_1.swp_count := gb_snd.snd_1.swp_count + 1;
      if (gb_snd.snd_1.swp_count >= gb_snd.snd_1.swp_time) then
      begin
        gb_snd.snd_1.swp_count := 0;
        if (gb_snd.snd_1.swp_direction > 0) then
        begin
          gb_snd.snd_1.frequency := gb_snd.snd_1.frequency -
            (gb_snd.snd_1.frequency div (1 shl gb_snd.snd_1.swp_shift));
          if (gb_snd.snd_1.frequency <= 0) then
          begin
            gb_snd.snd_1.on_ := 0;
            gb_snd.snd_regs[NR52] := gb_snd.snd_regs[NR52] and $FE;
          end;
        ...
        begin
          gb_snd.snd_1.frequency := gb_snd.snd_1.frequency +
            (gb_snd.snd_1.frequency div (1 shl gb_snd.snd_1.swp_shift));
          if (gb_snd.snd_1.frequency >= MAX_FREQUENCIES) then
          begin
    ...
    if (gb_snd.snd_control.mode1_left <> 0) then
      LEFT := LEFT + sample;
    if (gb_snd.snd_control.mode1_right <> 0) then
      RIGHT := RIGHT + sample;
  end; // del on1
  ...
  begin
    ...
    if (gb_snd.snd_2.pos = (trunc(gb_snd.snd_2.period / wave_duty_table[gb_snd.snd_2.duty]) shr 16))
    then
    begin
      gb_snd.snd_2.signal := -gb_snd.snd_2.signal;
    end
    else if (gb_snd.snd_2.pos > (gb_snd.snd_2.period shr 16)) then
    begin
      gb_snd.snd_2.pos := 0;
      gb_snd.snd_2.signal := -gb_snd.snd_2.signal;
    end;

    if ((gb_snd.snd_2.length <> 0) and (gb_snd.snd_2.mode <> 0)) then
    begin
      gb_snd.snd_2.count := gb_snd.snd_2.count + 1;
      if (gb_snd.snd_2.count >= gb_snd.snd_2.length) then
      begin
        gb_snd.snd_2.on_ := 0;
    ...
    if (gb_snd.snd_2.env_length <> 0) then
    begin
      gb_snd.snd_2.env_count := gb_snd.snd_2.env_count + 1;
      if (gb_snd.snd_2.env_count >= gb_snd.snd_2.env_length) then
      begin
        gb_snd.snd_2.env_count := 0;
        gb_snd.snd_2.env_value := gb_snd.snd_2.env_value + gb_snd.snd_2.env_direction;
        if (gb_snd.snd_2.env_value < 0) then
          gb_snd.snd_2.env_value := 0;
        if (gb_snd.snd_2.env_value > 15) then
          gb_snd.snd_2.env_value := 15;
      end;
    end;
    if (gb_snd.snd_control.mode2_left <> 0) then
      LEFT := LEFT + sample;
    if (gb_snd.snd_control.mode2_right <> 0) then
      RIGHT := RIGHT + sample;
  end;
  ...
  begin
    ...
    if ((gb_snd.snd_3.offset mod 2) = 0) then
      sample := sample shr 4;
    sample := (sample and $F) - 8;
    if (gb_snd.snd_3.level <> 0) then
      sample := sshr(sample, (gb_snd.snd_3.level - 1))
    else
      sample := 0;
    gb_snd.snd_3.pos := gb_snd.snd_3.pos + 1;
    if (gb_snd.snd_3.pos >= (dword(gb_snd.snd_3.period shr 21) + gb_snd.snd_3.duty)) then
    begin
      gb_snd.snd_3.pos := 0;
      if (gb_snd.snd_3.dutycount = (dword(gb_snd.snd_3.period shr 16) mod 32)) then
      begin
      ...
      if (gb_snd.snd_3.offset > 31) then
      begin
        gb_snd.snd_3.offset := 0;
        gb_snd.snd_3.duty := 1;
        gb_snd.snd_3.dutycount := 0;
    ...
    if ((gb_snd.snd_3.length <> 0) and (gb_snd.snd_3.mode <> 0)) then
    begin
      gb_snd.snd_3.count := gb_snd.snd_3.count + 1;
      if (gb_snd.snd_3.count >= gb_snd.snd_3.length) then
      begin
        gb_snd.snd_3.on_ := 0;
        gb_snd.snd_regs[NR52] := gb_snd.snd_regs[NR52] and $FB;
      end;
    end;
    if (gb_snd.snd_control.mode3_left <> 0) then
      LEFT := LEFT + sample;
    if (gb_snd.snd_control.mode3_right <> 0) then
      RIGHT := RIGHT + sample;
  end;
  ...
  begin
    ...
    if (gb_snd.snd_4.pos = (gb_snd.snd_4.period shr 17)) then
    begin
      ...
      if gb_snd.snd_4.ply_step <> 0 then
        mode4_mask := (((gb_snd.snd_4.ply_value and $2) div 2) xor (gb_snd.snd_4.ply_value and
          $1)) shl 6
      else
        mode4_mask := (((gb_snd.snd_4.ply_value and $2) div 2) xor (gb_snd.snd_4.ply_value and
          $1)) shl 14;
      ...
      if gb_snd.snd_4.ply_step <> 0 then
        gb_snd.snd_4.ply_value := gb_snd.snd_4.ply_value and $7F
      else
        gb_snd.snd_4.ply_value := gb_snd.snd_4.ply_value and $7FFF;
      gb_snd.snd_4.signal := shortint(gb_snd.snd_4.ply_value);
    end
    else if (gb_snd.snd_4.pos > (gb_snd.snd_4.period shr 16)) then
    begin
      gb_snd.snd_4.pos := 0;
      if gb_snd.snd_4.ply_step <> 0 then
        mode4_mask := (((gb_snd.snd_4.ply_value and $2) div 2) xor (gb_snd.snd_4.ply_value and
    ...
    if ((gb_snd.snd_4.length <> 0) and (gb_snd.snd_4.mode <> 0)) then
    begin
      gb_snd.snd_4.count := gb_snd.snd_4.count + 1;
      if (gb_snd.snd_4.count >= gb_snd.snd_4.length) then
      begin
        gb_snd.snd_4.on_ := 0;
    ...
    if (gb_snd.snd_4.env_length <> 0) then
    begin
      gb_snd.snd_4.env_count := gb_snd.snd_4.env_count + 1;
      if (gb_snd.snd_4.env_count >= gb_snd.snd_4.env_length) then
      begin
        gb_snd.snd_4.env_count := 0;
        gb_snd.snd_4.env_value := gb_snd.snd_4.env_value + gb_snd.snd_4.env_direction;
        if (gb_snd.snd_4.env_value < 0) then
          gb_snd.snd_4.env_value := 0;
        if (gb_snd.snd_4.env_value > 15) then
          gb_snd.snd_4.env_value := 15;
      end;
    end;

    if (gb_snd.snd_control.mode4_left <> 0) then
      LEFT := LEFT + sample;
    if (gb_snd.snd_control.mode4_right <> 0) then
      RIGHT := RIGHT + sample;
  end;
  ...
  RIGHT := RIGHT shl 6;
  // Update the buffers */
  tsample[gb_snd.tsample, sound_status.sound_position] := LEFT;
  tsample[gb_snd.tsample, sound_status.sound_position + 1] := RIGHT;
  gb_snd.snd_regs[NR52] := (gb_snd.snd_regs[NR52] and $F0) or gb_snd.snd_1.on_ or
    (gb_snd.snd_2.on_ shl 1) or (gb_snd.snd_3.on_ shl 2) or (gb_snd.snd_4.on_ shl 3);
end;

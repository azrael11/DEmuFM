...
begin
  for f := 0 to QSOUND_CHANNELS do
    fillchar(qsound_state.channel[f]^, sizeof(QSOUND_CHANNEL_def), 0);
  qsound_state.data := 0;
...
begin
  ...
  case reg of
    ...
    1:
      chip.channel[ch].address := value; // start
    2:
      begin // pitch
        chip.channel[ch].pitch := value * 16;
        if (value = 0) then
          chip.channel[ch].key := 0; // Key off
      end;
    3:
      chip.channel[ch].reg3 := value; // unknown
    4:
      chip.channel[ch].loop := value; // loop offset
    5:
      chip.channel[ch].end_addr := value; // end
    6:
      begin // master volume
        if (value = 0) then
        begin
        ...
        else if (chip.channel[ch].key = 0) then
        begin
          // Key on
...
begin
  ...
  case offset of
    0:
      chip.data := (chip.data and $FF) or (data shl 8);
    1:
      chip.data := (chip.data and $FF00) or data;
    2:
      qsound_set_command(data, chip.data);
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (pC.address >= pC.end_addr) then
        begin
          if (pC.loop <> 0) then
          begin
            pC.key := 0;
      ...
      if chip.out_[0] < -32767 then
        chip.out_[0] := -32767
      else if chip.out_[0] > 32767 then
        chip.out_[0] := 32767;
      chip.out_[1] := chip.out_[1] + ((pC.lastdt * rvol) div 32);
      if chip.out_[1] < -32767 then
        chip.out_[1] := -32767
      else if chip.out_[1] > 32767 then
        chip.out_[1] := 32767;
...
procedure qsound_sound_update;
begin
  tsample[qsound_state.tsample, sound_status.sound_position] := qsound_state.out_[0];
  tsample[qsound_state.tsample, sound_status.sound_position + 1] := qsound_state.out_[1];
end;

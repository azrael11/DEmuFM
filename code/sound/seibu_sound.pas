...
begin
  case direccion of
    ...
    $2000 .. $27FF:
      oki_getbyte := mem_snd[direccion];
    $4008:
      oki_getbyte := ym3812_0.status;
    $4010:
      oki_getbyte := seibu_snd_0.sound_latch[0];
    $4011:
      oki_getbyte := seibu_snd_0.sound_latch[1];
    $4012:
      oki_getbyte := byte(seibu_snd_0.sub2main_pending);
    $4013:
      oki_getbyte := seibu_snd_0.input;
    $6000:
...
begin
  case direccion of
    0 .. $1FFF:
      if seibu_snd_0.z80.opcode then
        adpcm_getbyte := seibu_snd_0.decrypt[direccion]
      else
        adpcm_getbyte := mem_snd[direccion];
    $2000 .. $27FF, $8000 .. $FFFF:
      adpcm_getbyte := mem_snd[direccion];
    $4008:
      adpcm_getbyte := ym2151_0.status;
    $4010:
      adpcm_getbyte := seibu_snd_0.sound_latch[0];
    $4011:
      adpcm_getbyte := seibu_snd_0.sound_latch[1];
    $4012:
      adpcm_getbyte := byte(seibu_snd_0.sub2main_pending);
    $4013:
      adpcm_getbyte := seibu_snd_0.input;
  end;
end;
...
begin
  case direccion of
    ...
    $4006:
      seibu_snd_0.adpcm_0.adr_w(1, valor);
    $4008:
      ym2151_0.reg(valor);
    $4009:
    ...
    $4019:
      seibu_snd_0.sub2main[1] := valor;
    $401A:
      seibu_snd_0.adpcm_0.ctl_w(valor);
    $401B:
      ;
    $6005:
      seibu_snd_0.adpcm_1.adr_w(0, valor);
    $6006:
      seibu_snd_0.adpcm_1.adr_w(1, valor);
    $601A:
      seibu_snd_0.adpcm_1.ctl_w(valor);
  end;
end;
...
begin
  if irqstate = 1 then
    seibu_snd_0.update_irq_lines(RST10_ASSERT)
  else
...
begin
  case direccion of
    ...
    1:
      sound_latch[1] := valor;
    4:
      self.update_irq_lines(RST18_ASSERT);
    2, 6:
      begin
...
procedure seibu_snd_type.decript_sound;
  ...
  begin
    ...
    if (BIT(a, 11) and BIT(a, 4) and BIT(a, 1)) then
      src := src xor $40;
    if (not(BIT(a, 13)) and BIT(a, 12)) then
      src := src xor $20;
    ...
    if (BIT(a, 8) and BIT(a, 4)) then
      src := BITSWAP8(src, 7, 6, 5, 4, 2, 3, 1, 0);
    if (BIT(a, 12) and BIT(a, 9)) then
      src := BITSWAP8(src, 7, 6, 4, 5, 3, 2, 1, 0);
    if (BIT(a, 11) and not(BIT(a, 6))) then
      src := BITSWAP8(src, 6, 7, 5, 4, 3, 2, 1, 0);
    decrypt_opcode := src;
  end;

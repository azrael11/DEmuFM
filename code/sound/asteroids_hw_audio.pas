...
begin
  ...
  begin
    ...
    if (((polynome and $4000) = 0) and ((polynome and $0040) = 0)) then
      ...
      polynome := polynome shl 1;
    sample_counter_exp := sample_counter_exp + 1;
    if (sample_counter_exp = 16) then
    begin
      sample_counter_exp := 0;
      if (explosion_latch and EXPITCH0) <> 0 then
        sample_counter_exp := sample_counter_exp or (2 + 8)
      else
        sample_counter_exp := sample_counter_exp or 4;
      if (explosion_latch and EXPITCH1) <> 0 then
        sample_counter_exp := sample_counter_exp or (1 + 8);
    end;
    // ripple count output is high? */
    if (sample_counter_exp = 15) then
      out_exp := polynome and 1;
  end;
  if (out_exp <> 0) then
  begin
...
begin
  if (sound_latch[THRUSTEN] <> 0) then
  begin
    // SHPSND filter */
    counter_thr := counter_thr - 110;
    while (counter_thr <= 0) do
    begin
...
begin
  if (thump_latch and $10) <> 0 then
  begin
    counter_thu := counter_thu - thump_frequency;
    while (counter_thu <= 0) do
    begin
...
begin
  ...
  if (sound_latch[SAUCEREN] <> 0) then
  begin
    ...
    begin
      if (sound_latch[SAUCERSEL] <> 0) then
        ...
        vco_counter_sau := vco_counter_sau - NE555_T2(6000, 10E-6);
      if (vco_counter_sau <= 0) then
      begin
        steps := round(-vco_counter_sau / FREQ_BASE_AUDIO) + 1;
        vco_counter_sau := vco_counter_sau + steps * FREQ_BASE_AUDIO;
        vco_sau := vco_sau - steps;
        if (vco_sau <= VMAX * 1 / 3) then
        begin
...
begin
  if (sound_latch[LIFEEN] <> 0) then
  begin
    counter_lif := counter_lif - 3000;
    while (counter_lif <= 0) do
    begin
...
begin
  ...
  if not(hay_samples) then
    sum := sum + trunc(explosion / 7)
  else
    samples_update;
  if sum > 32767 then
    sum := 32767
    ...
    sum := -32767;
  tsample[tsample_as, sound_status.sound_position] := sum;
end;
...
begin
  ...
  else if (data <> explosion_latch) then
...
begin
  r0 := 1 / 47000;
  r1 := 1 / 1E12;
  if (data = thump_latch) then
    exit;
  thump_latch := data;
  if (thump_latch and 1) <> 0 then
    r1 := r1 + 1.0 / 220000
  else
    r0 := r0 + 1.0 / 220000;
  if (thump_latch and 2) <> 0 then
    r1 := r1 + 1.0 / 100000
  else
    r0 := r0 + 1.0 / 100000;
  if (thump_latch and 4) <> 0 then
    r1 := r1 + 1.0 / 47000
  else
    r0 := r0 + 1.0 / 47000;
  if (thump_latch and 8) <> 0 then
    r1 := r1 + 1.0 / 22000
  else
    r0 := r0 + 1.0 / 22000;
...
begin
  data := data and $80;
  if (data = sound_latch[offset]) then
    exit;

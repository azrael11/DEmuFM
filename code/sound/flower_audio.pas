...
begin
  ...
  for f := 0 to (MAX_VOICES * 128) - 1 do
  begin
...
begin
  res := 0;
  for voice := 0 to (MAX_VOICES - 1) do
  begin
    if not(flower_0.channel_list[voice].enable) then
      continue;
    volume := flower_0.channel_list[voice].volume;
    frequency := flower_0.channel_list[voice].frequency;
    if flower_0.channel_list[voice].repeat_ then
    begin
      raw_sample := flower_0.sample_rom[((flower_0.channel_list[voice].start_address shr 7) and
        $7E00) or ((flower_0.channel_list[voice].position shr 7) and $1FF)];
      // guess: cut off after a number of repetitions
      if ((flower_0.channel_list[voice].position shr 7) and $20000) <> 0 then
        flower_0.channel_list[voice].enable := false;
    end
    else
    begin
      raw_sample := flower_0.sample_rom
        [((flower_0.channel_list[voice].start_address + flower_0.channel_list[voice].position)
        shr 7) and $7FFF];
      if (raw_sample = $FF) then
        flower_0.channel_list[voice].enable := false;
    end;
...
begin
  ...
  timers.init(sound_status.cpu_num, sound_status.cpu_clock / (clock / 2), internal_update_flower,
    nil, true);
end;
  ...
  case (direccion and $47) of
    0 .. 3:
      begin // frequency_w
        ...
        channel_list[ch].frequency := channel_list[ch].frequency or
          (channel_list[ch].raw_frequency[3] shl 8);
        channel_list[ch].frequency := channel_list[ch].frequency or
          (channel_list[ch].raw_frequency[0] shl 4);
        channel_list[ch].frequency := channel_list[ch].frequency or
          (channel_list[ch].raw_frequency[1] shl 0);
      end;
    4:
      channel_list[ch].repeat_ := (valor and $10) <> 0;
    5:
      ; // unk_w
    7:
      channel_list[ch].volume := valor shr 4;
    $40 .. $45:
      begin
        channel_list[ch].start_nibbles[direccion and 7] := valor and $F;
        if ((direccion and 7) = 4) then
          channel_list[ch].effect := valor shr 4;
      end;
    $47:
      begin
        ...
        for f := 5 downto 0 do
          channel_list[ch].start_address := (channel_list[ch].start_address shl 4) or
            channel_list[ch].start_nibbles[f];
      end;
  end;
end;
...
begin
  res := 0;
  for f := 0 to (self.buffer_pos - 1) do
    res := res + self.buffer[f];
  res := sshr(res, self.buffer_pos - 1);
  if res > 32767 then
    res := 32767
  else if res < -32767 then
    res := -32767;

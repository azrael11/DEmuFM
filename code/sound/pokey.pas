...
begin
  ...
  out_ := 0;
  for i := 0 to 3 do
    out_ := out_ + (((self.output_ shr (4 * i)) and $0F));
...
begin
  ...
  case tipo of
    3:
      begin
        // serout_ready_cb */
        if (chip.IRQEN and IRQ_SEROR) <> 0 then
        begin
          chip.IRQST := chip.IRQST or IRQ_SEROR;
          if (@chip.irq_f <> nil) then
            chip.irq_f(IRQ_SEROR);
        end;
      end;
    4:
      begin
        // serout_complete */
        if (chip.IRQEN and IRQ_SEROC) <> 0 then
        begin
          chip.IRQST := chip.IRQST or IRQ_SEROC;
          if (@chip.irq_f <> nil) then
            chip.irq_f(IRQ_SEROC);
        end;
      end;
    5:
      begin
        // serin_ready */
        if (chip.IRQEN and IRQ_SERIN) <> 0 then
        begin
...
begin
  ...
  if ((self.counter = 0) and (self.borrow_cnt = 0)) then
  begin
    self.borrow_cnt := 3;
    if ((IRQEN and self.INTMask) <> 0) then
    begin
...
procedure poly_init_4_5(poly: pword; size, xorbit, invert: integer);
var
  mask, i, in_: integer;
  lfsr: dword;
  ptemp: pword;
...
procedure poly_init_9_17(poly: pword; size: integer);
var
...
begin
  ...
  case (offset and $F) of
    POT0_C, POT1_C, POT2_C, POT3_C, POT4_C, POT5_C, POT6_C, POT7_C:
      begin
        pot := offset and 7;
        if ((self.ALLPOT and (1 shl pot)) <> 0) then
        begin
    ...
    ALLPOT_C:
      begin
        ...
        if ((self.SKCTL and SK_RESET) = 0) then
        begin
          data := 0;
        end
        else
        begin
          if (@self.allpot_r_cb <> nil) then
          begin
    ...
    RANDOM_C:
      begin
        if ((self.AUDCTL and POLY9_) <> 0) then
        begin
    ...
    SERIN_C:
      begin
        if (@self.serin_r_cb <> nil) then
          self.SERIN := self.serin_r_cb(offset);
        data := self.SERIN;
      end;
    IRQST_C:
      begin
...
begin
  ...
  begin
    ...
    case pot of
      0:
        if (addr(self.pot0_r_cb) <> nil) then
        begin
          r := self.pot0_r_cb(pot);
          update_pot;
        end;
      1:
        if (addr(self.pot1_r_cb) <> nil) then
        begin
          r := self.pot1_r_cb(pot);
          update_pot;
        end;
      2:
        if (addr(self.pot2_r_cb) <> nil) then
        begin
          r := self.pot2_r_cb(pot);
          update_pot;
        end;
      3:
        if (addr(self.pot3_r_cb) <> nil) then
        begin
          r := self.pot3_r_cb(pot);
          update_pot;
        end;
      4:
        if (addr(self.pot4_r_cb) <> nil) then
        begin
          r := self.pot4_r_cb(pot);
          update_pot;
        end;
      5:
        if (addr(self.pot5_r_cb) <> nil) then
        begin
          r := self.pot5_r_cb(pot);
          update_pot;
        end;
      6:
        if (addr(self.pot6_r_cb) <> nil) then
        begin
          r := self.pot6_r_cb(pot);
          update_pot;
        end;
      7:
        if (addr(self.pot7_r_cb) <> nil) then
        begin
...
begin
  ...
  case (offset and $F) of
    AUDF1_C:
      self.channel[CHAN1].AUDF := data;
    AUDC1_C:
      self.channel[CHAN1].AUDC := data;
    AUDF2_C:
      self.channel[CHAN2].AUDF := data;
    AUDC2_C:
      self.channel[CHAN2].AUDC := data;
    AUDF3_C:
      self.channel[CHAN3].AUDF := data;
    AUDC3_C:
      self.channel[CHAN3].AUDC := data;
    AUDF4_C:
      self.channel[CHAN4].AUDF := data;
    AUDC4_C:
      self.channel[CHAN4].AUDC := data;
    AUDCTL_C:
      self.AUDCTL := data;
    STIMER_C:
      begin
        ...
        begin
          ...
          if (i < 2) then
            self.channel[i].filter_sample := 1
          else
            self.channel[i].filter_sample := 0;
        end;
      end;
    SKREST_C:
      self.SKSTAT := self.SKSTAT and not(SK_FRAME or SK_OVERRUN or SK_KBERR); // reset SKSTAT */
    POTGO_C:
      self.potgo;
    SEROUT_C:
      begin
    ...
    IRQEN_C:
      begin
        ...
        if ((self.IRQST and not(data)) <> 0) then
        begin
...
begin
  if (((self.channel[ch].AUDC and NOTPOLY5) <> 0) or ((self.poly5[self.p5] and 1) <> 0)) then
  begin
    if (self.channel[ch].AUDC and PURE) <> 0 then
      self.channel[ch].output_ := self.channel[ch].output_ xor 1
    else if (self.channel[ch].AUDC and POLY4_) <> 0 then
      self.channel[ch].output_ := self.poly4[self.p4] and 1
    else if (self.AUDCTL and POLY9_) <> 0 then
      self.channel[ch].output_ := self.poly9[self.p9] and 1
    else
      self.channel[ch].output_ := self.poly17[self.p17] and 1;
  end;
end;
...
begin
  ...
  for pot := 0 to 7 do
  begin
    if ((self.POTx[pot] < self.pot_counter) or (self.pot_counter = 228)) then
    begin
...
begin
  ...
  clock_triggered[1] := 0;
  clock_triggered[2] := 0;
  if ((self.AUDCTL and CLK_15KHZ) <> 0) then
    base_clock := CLK_114
  else
    base_clock := CLK_28;
  ...
  begin
    ...
    begin
      ...
      if (self.clock_cnt[clk] >= clock_divisors[clk]) then
      begin
    ...
    if (self.AUDCTL and CH1_HICLK) <> 0 then
      clk := CLK_1
    else
      clk := base_clock;
    if (clock_triggered[clk] <> 0) then
      self.channel[CHAN1].inc_chan(self.number, self.IRQEN);
    if ((self.AUDCTL and CH3_HICLK) <> 0) then
      clk := CLK_1
    else
      clk := base_clock;
    if (clock_triggered[clk] <> 0) then
      self.channel[CHAN3].inc_chan(self.number, self.IRQEN);
    if (clock_triggered[base_clock] <> 0) then
    begin
      if ((self.AUDCTL and CH12_JOINED) = 0) then
        self.channel[CHAN2].inc_chan(self.number, self.IRQEN);
      if ((self.AUDCTL and CH34_JOINED) = 0) then
  ...
  if (self.channel[CHAN2].check_borrow) then
  begin
    isJoined := (self.AUDCTL and CH12_JOINED) <> 0;
    if isJoined then
      self.channel[CHAN1].reset_channel;
    self.channel[CHAN2].reset_channel;
    self.process_channel(CHAN2);
    // check if some of the requested timer interrupts are enabled */
    if (((self.IRQST and IRQ_TIMR2) <> 0) and (@self.irq_f <> nil)) then
      self.irq_f(IRQ_TIMR2);
  end;
  if (self.channel[CHAN1].check_borrow) then
  begin
    isJoined := (self.AUDCTL and CH12_JOINED) <> 0;
    if isJoined then
      self.channel[CHAN2].inc_chan(self.number, self.IRQEN)
    ...
    // check if some of the requested timer interrupts are enabled */
    if (((self.IRQST and IRQ_TIMR1) <> 0) and (@self.irq_f <> nil)) then
      self.irq_f(IRQ_TIMR1);
  end;
  // do CHAN4 before CHAN3 because CHAN3 may set borrow! */
  if (self.channel[CHAN4].check_borrow) then
  begin
    isJoined := (self.AUDCTL and CH34_JOINED) <> 0;
    if isJoined then
      self.channel[CHAN3].reset_channel;
    self.channel[CHAN4].reset_channel;
    self.process_channel(CHAN4);
    // is this a filtering channel (3/4) and is the filter active? */
    if ((self.AUDCTL and CH2_FILTER) <> 0) then
      self.channel[CHAN2].sample
    else
      self.channel[CHAN2].filter_sample := 1;
    if (((self.IRQST and IRQ_TIMR4) <> 0) and (@self.irq_f <> nil)) then
      self.irq_f(IRQ_TIMR4);
  end;

  if (self.channel[CHAN3].check_borrow) then
  begin
    isJoined := (self.AUDCTL and CH34_JOINED) <> 0;
    if isJoined then
      self.channel[CHAN4].inc_chan(self.number, self.IRQEN)
    ...
    // is this a filtering channel (3/4) and is the filter active? */
    if (self.AUDCTL and CH1_FILTER) <> 0 then
      self.channel[CHAN1].sample
    else
...
begin
  tsample[self.tsample_num, sound_status.sound_position] := self.output_;
  if sound_status.stereo then
    tsample[self.tsample_num, sound_status.sound_position + 1] := self.output_;
end;

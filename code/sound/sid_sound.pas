...
var
  ...
  sidModeNormalTable: array [0 .. 15] of procedure(
    pVoice: psidOperator
  ) =
  (
    sidMode00,
  ...
  sidModeRingTable: array [0 .. 15] of procedure(
    pVoice: psidOperator
  ) =
  (
    sidMode00,
  ...
  enveModeTable: array [0 .. 31] of function(
    pVoice: psidOperator
  ): word =
  (
    // 0 */
...
var
  ...
  tmpint: integer;
begin
  if pVoice.filtEnabled then
  begin
    if (sid_0.filter.Type_ <> 0) then
    begin
      if (sid_0.filter.Type_ = $20) then
      begin
      ...
      else if (sid_0.filter.Type_ = $40) then
      begin
      ...
      begin
        ...
        if (sid_0.filter.Type_ = $10) then
          pVoice.filtIO := trunc(pVoice.filtLow)
        else if (sid_0.filter.Type_ = $30) then
          pVoice.filtIO := trunc(pVoice.filtLow)
        else if (sid_0.filter.Type_ = $50) then
          pVoice.filtIO := trunc(sample - sshr(tmpint, 1))
        else if (sid_0.filter.Type_ = $60) then
          pVoice.filtIO := tmpint
        else if (sid_0.filter.Type_ = $70) then
          pVoice.filtIO := trunc(sample - sshr(tmpint, 1));
      end;
    end
...
begin
  if (pVoice.cycleLenCount <= 0) then
  begin
    pVoice.wave_calc_cycle_len;
    if (pVoice.SIDctrl and $40) <> 0 then
    begin
      pVoice.pulseIndex := pVoice.newPulseIndex;
      if (pVoice.pulseIndex > 2048) then
        pVoice.waveStep := 0;
    end;
  end;
...
begin
  ...
  begin
    j := 0;
    while ((j < releaseTabLen) and (releaseTab[j] > i)) do
      j := j + 1;
    if (j < releaseTabLen) then
      releasePos[i] := j
    else
...
//    MessageDlg('ERROR: Chip de sonido inicializado sin CPU de sonido!', mtInformation, [mbOk], 0);
  ...
  for v := 0 to (max_voices - 1) do
    self.optr[v] := sidOperator.create;
  for v := 0 to (max_voices - 1) do
  begin
    mod_voi := rev[v];
...
begin
  ...
  for v := 0 to (max_voices - 1) do
  begin
    if (self.optr[v].sync and sync[v]) then
    begin
...
begin
  ...
  self.syncEm();
  tsample[self.tsample_num, sound_status.sound_position] := smallint(res);
  if sound_status.stereo then
    tsample[self.tsample_num, sound_status.sound_position + 1] := smallint(res);
end;
...
begin
  ...
  case dir of
    $1D .. $1F:
      res := $FF;
    $1B:
      res := self.optr[2].output;
    $1C:
      res := self.optr[2].enveVol;
  else
    res := self.reg[dir];
  end;
  read := res;
  ...
  case dir of
    ...
    $15 .. $18:
      begin
        ...
        if (self.filter.Type_ <> self.filter.CurType) then
        begin
        ...
        begin
          ...
          if (self.filter.Type_ = $20) then
            self.filter.Dy := self.bandPassParam[self.filter.Value]
          else
            self.filter.Dy := self.filterTable[self.filter.Value];
          self.filter.ResDy := self.filterResTable[self.reg[$17] shr 4] - self.filter.Dy;
          if (self.filter.ResDy < 1.0) then
            self.filter.ResDy := 1.0;
        end;
        for v := 0 to (max_voices - 1) do
        begin
          self.optr[v].set_;
    ...
    begin
      self.reg[dir] := valor;
      if (dir < 7) then
        self.optr[0].reg[dir] := valor
      else if (dir < 14) then
        self.optr[1].reg[dir - 7] := valor
      else if (dir < 21) then
        self.optr[2].reg[dir - 14] := valor;
      for v := 0 to (max_voices - 1) do
      begin
        self.optr[v].set_;
        // relies on sidEmuSet also for other channels!
...
begin
  ...
  if (self.cycleAddLenPnt > 65535) then
    self.cycleLenCount := self.cycleLenCount + 1;
  ...
  if (self.wavePre[diff].len <> cycleLenCount) then
  begin
...
begin
  ...
  if (newWave and 1) = 0 then
  begin
    if (oldWave and 1) <> 0 then
      enveTemp := ENVE_STARTRELEASE
  end
  else if ((oldWave and 1) = 0) then // gateOffCtrl
    enveTemp := ENVE_STARTATTACK;

  if ((oldWave xor newWave) and $F0) <> 0 then
    self.cycleLenCount := 0;

  ADtemp := self.reg[5];
  SRtemp := self.reg[6];
  if (self.SIDAD <> ADtemp) then
    enveTemp := enveTemp or ENVE_ALTER
  else if (self.SIDSR <> SRtemp) then
    enveTemp := enveTemp or ENVE_ALTER;

  self.SIDAD := ADtemp;
  self.SIDSR := SRtemp;
  tmpSusVol := masterVolumeLevels[SRtemp shr 4];
  if (self.ADSRctrl <> ENVE_SUSTAIN) then
    self.enveSusVol := tmpSusVol
  else if (self.enveSusVol > self.enveVol) then
    self.enveSusVol := 0
  else
    self.enveSusVol := tmpSusVol;
...
begin
  ...
  pVoice.waveProc(pVoice);
  if (pVoice.waveStep < pVoice.waveStepOld) then
  begin
...
begin
  ...
  self.sync := false;
  if ((self.SIDfreq < 16) or ((self.SIDctrl and 8) <> 0)) then
  begin
    self.outProc := @waveCalcMute;
    if (self.SIDfreq = 0) then
    begin
      self.cycleLen := 0;
      self.cycleLenPnt := 0;
      self.cycleAddLenPnt := 0;
    ...
    if (self.SIDctrl and 8) <> 0 then
    begin
      if self.noiseIsLocked then
      begin
        self.noiseIsLocked := false;
        self.noiseReg := noiseSeed;
      end;
  ...
  begin
    if (self.curSIDfreq <> self.SIDfreq) then
    begin
    ...
    if (((self.SIDctrl and $14) = $14) and (self.modulator.SIDfreq <> 0)) then
      self.waveProc := sidModeRingTable[self.SIDctrl shr 4]
    else
      self.waveProc := sidModeNormalTable[self.SIDctrl shr 4];
  end;
end;
...
begin
  ...
  if (pVoice.enveStep >= attackTabLen) then
  begin
...
begin
  pVoice.enveStep := trunc(pVoice.fenveStep) and $FFFF;
  if (pVoice.enveStep >= releaseTabLen) then
  begin
  ...
  begin
    ...
    if (pVoice.enveVol <= pVoice.enveSusVol) then
    begin
      pVoice.enveVol := pVoice.enveSusVol;
      enveEmuDecay := enveEmuAlterSustain(pVoice); // start sustain */
    end
...
begin
  pVoice.enveStep := trunc(pVoice.fenveStep) and $FFFF;
  if ((pVoice.enveStep >= attackTabLen) or (pVoice.enveShortAttackCount = 0)) then
  begin
...
begin
  ...
  if (pVoice.waveStepPnt > 65535) then
    pVoice.waveStep := pVoice.waveStep + 1;
...
begin
  ...
  if tword > 4095 then
    ...
    pVoice.output := waveform50[tword];
  waveAdvance(pVoice);
end;
...
begin
  ...
  if tword > 4095 then
    ...
    pVoice.output := waveform60[tword];
  waveAdvance(pVoice);
end;
...
begin
  ...
  if tword > 4095 then
    ...
    pVoice.output := waveform70[tword];
  waveAdvance(pVoice);
end;
...
procedure sidMode14(pVoice: psidOperator);
begin
  if (pVoice.modulator.waveStep < 2048) then
    pVoice.output := triangleTable[pVoice.waveStep and $FFF]
  else
    pVoice.output := $FF xor triangleTable[pVoice.waveStep and $FFF];
  waveAdvance(pVoice);
end;
...
begin
  if (pVoice.modulator.waveStep < 2048) then
    pVoice.output := waveform30[pVoice.waveStep and $FFF]
  else
    pVoice.output := $FF xor waveform30[pVoice.waveStep and $FFF];
  waveAdvance(pVoice);
end;
...
begin
  ...
  if tword > 4095 then
    ...
    tword := waveform50[tword];
  if (pVoice.modulator.waveStep < 2048) then
    pVoice.output := tword
  else
    pVoice.output := $FF xor tword;
  waveAdvance(pVoice);
end;

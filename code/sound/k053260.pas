...
begin
  ...
  case direccion of
    $00, $01:
      ret := self.portdata[direccion]; // main-to-sub ports
    $29:
      for f := 0 to 3 do
        ret := ret or (byte(self.voice[f].voice_playing) shl f); // voice status
    $2E:
      if (self.mode and 1) <> 0 then
        ret := self.voice[0].read_rom; // read ROM
  end;
  read := ret;
end;
...
begin
  direccion := direccion and $3F;
  // per voice registers
  if ((direccion >= $08) and (direccion <= $27)) then
  begin
  ...
  case direccion of
    ...
    $02, $03:
      self.portdata[direccion] := valor; // sub-to-main ports
    ...
    $28:
      begin // key on/off
        ...
        for f := 0 to 3 do
        begin
          if ((rising_edge and (1 shl f)) <> 0) then
            self.voice[f].key_on
          else if ((valor and (1 shl f)) = 0) then
            self.voice[f].key_off;
        end;
        self.keyon := valor;
      end;
    // 0x29 is a read register
    $2A:
      begin // loop and pcm/adpcm select
        for f := 0 to 3 do
        begin
...
begin
  out1 := 0;
  out2 := 0;
  if self.posicion <> 0 then
  begin
    for f := 0 to (self.posicion - 1) do
    begin
    ...
    if out1 < -32767 then
      out1 := -32767
    else if out1 > 32767 then
      out1 := 32767;
    if out2 < -32767 then
      out2 := -32767
    else if out2 > 32767 then
      out2 := 32767;
  end;
  if sound_status.stereo then
  begin
    tsample[self.tsample_num, sound_status.sound_position] := round(out1 * self.amp);
    tsample[self.tsample_num, sound_status.sound_position + 1] := round(out2 * self.amp);
  end
  else
  begin
    // Channel 1
    tsample[self.tsample_num, sound_status.sound_position] := round(out1 * self.amp);
    // Channel 2
    tsample[self.tsample_num2, sound_status.sound_position] := round(out2 * self.amp);
  end;
  self.posicion := 0;
  for f := 0 to 4 do
  begin
...
var
  f: byte;
begin
  if (self.mode and 2) <> 0 then
  begin
    for f := 0 to 3 do
    begin
      if (self.voice[f].voice_playing) then
        self.voice[f].play;
...
begin
  case offset and $7 of
    0:
      self.pitch := (self.pitch and $0F00) or data; // pitch, lower 8 bits
    1:
      self.pitch := (self.pitch and $00FF) or ((data shl 8) and $0F00); // pitch, upper 4 bits
    2:
      self.length := (self.length and $FF00) or data; // length, lower 8 bits
    3:
      self.length := (self.length and $00FF) or (data shl 8); // length, upper 8 bits
    4:
      self.start := (self.start and $1FFF00) or data; // start, lower 8 bits
    5:
      self.start := (self.start and $1F00FF) or (data shl 8); // start, middle 8 bits
    6:
      self.start := (self.start and $00FFFF) or ((data shl 16) and $1F0000); // start, upper 5 bits
    7:
      begin // volume, 7 bits
...
begin
  ...
  self.position := byte(self.kadpcm);
  // for kadpcm low bit is nybble offset, so must start at 1 due to preincrement
...
begin
  ...
  begin
    ...
    if self.kadpcm then
    begin
      if (self.position and 1) <> 0 then
        romdata := romdata shr 4; // decode low nybble, then high nybble

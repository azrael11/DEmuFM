...
const
  ...
  STATE_NIBBLE_MSN = 11;
  STATE_NIBBLE_LSN = 12;
  upd7759_step: array [0 .. 15, 0 .. 15] of integer = ((0, 0, 1, 2, 3, 5, 7, 10, 0, 0, -1, -2, -3, -5, -7, -10), (0, 1, 2, 3, 4, 6, 8, 13, 0, -1, -2, -3, -4, -6, -8, -13),
    (0, 1, 2, 4, 5, 7, 10, 15, 0, -1, -2, -4, -5, -7, -10, -15), (0, 1, 3, 4, 6, 9, 13, 19, 0, -1, -3, -4, -6, -9, -13, -19), (0, 2, 3, 5, 8, 11, 15, 23, 0, -2, -3, -5, -8, -11, -15, -23),
    (0, 2, 4, 7, 10, 14, 19, 29, 0, -2, -4, -7, -10, -14, -19, -29), (0, 3, 5, 8, 12, 16, 22, 33, 0, -3, -5, -8, -12, -16, -22, -33),
    (1, 4, 7, 10, 15, 20, 29, 43, -1, -4, -7, -10, -15, -20, -29, -43), (1, 4, 8, 13, 18, 25, 35, 53, -1, -4, -8, -13, -18, -25, -35, -53),
    (1, 6, 10, 16, 22, 31, 43, 64, -1, -6, -10, -16, -22, -31, -43, -64), (2, 7, 12, 19, 27, 37, 51, 76, -2, -7, -12, -19, -27, -37, -51, -76),
    (2, 9, 16, 24, 34, 46, 64, 96, -2, -9, -16, -24, -34, -46, -64, -96), (3, 11, 19, 29, 41, 57, 79, 117, -3, -11, -19, -29, -41, -57, -79, -117),
    (4, 13, 24, 36, 50, 69, 96, 143, -4, -13, -24, -36, -50, -69, -96, -143), (4, 16, 29, 44, 62, 85, 118, 175, -4, -16, -29, -44, -62, -85, -118, -175),
...
begin
  ...
  if (self.adpcm_state < 0) then
    self.adpcm_state := 0
  else if (self.adpcm_state > 15) then
    self.adpcm_state := 15;
end;
...
begin
  case self.state of
    ...
    STATE_LAST_SAMPLE:
      begin
        if self.rom <> nil then
          self.last_sample := self.rom^
        else
          self.last_sample := self.fifo_in;
        self.drq := 1;
        // 28 cycles later, we will latch this value and request another byte */
        self.clocks_left := 28; // 28 - breaks cotton */
        if (self.req_sample > self.last_sample) then
          self.state := STATE_IDLE
        else
          self.state := STATE_DUMMY1;
      end;
    ...
    STATE_ADDR_MSB:
      begin
        ...
        begin
          ...
          self.offset := self.fifo_in shl 9;
        self.drq := 1;
        // 44 cycles later, we will latch this value and request another byte */
        self.clocks_left := 44;
    ...
    STATE_ADDR_LSB:
      begin
        ...
        begin
          ...
          self.offset := self.offset or (self.fifo_in shl 1);
        self.drq := 1;
    ...
    STATE_BLOCK_HEADER:
      begin
        ...
        begin
          ...
          self.block_header := self.fifo_in;
        self.offset := self.offset + 1;
        self.drq := 1;
        ...
        case (self.block_header and $C0) of
          $00:
            begin // silence */
              ...
              if ((self.block_header = 0) and ((self.first_valid_header and 1) <> 0)) then
                self.state := STATE_IDLE
              else
                self.state := STATE_BLOCK_HEADER;
              self.sample := 0;
        ...
        if (self.block_header <> 0) then
          self.first_valid_header := 1;
      end;
    ...
    STATE_NIBBLE_COUNT:
      begin
        ...
        begin
          ...
          self.nibbles_left := self.fifo_in + 1;
        self.offset := self.offset + 1;
        self.drq := 1;
        // 36?? cycles later, we will latch this value and request another byte */
        self.clocks_left := 36; // just a guess */
    ...
    STATE_NIBBLE_MSN:
      begin
        ...
        if self.nibbles_left = 0 then
          self.state := STATE_BLOCK_HEADER
        else
          self.state := STATE_NIBBLE_LSN;
      end;
...
begin
  ...
  if ((oldreset <> 0) and ((not(self.reset_pin) and 1) <> 0)) then
    self.reset;
end;
...
begin
  ...
  if ((self.state = STATE_IDLE) and ((not(oldstart) and 1) <> 0) and ((self.start and 1) <> 0) and ((self.reset_pin and 1) <> 0)) then
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      while (pos >= FRAC_ONE) do
      begin
        clocks_this_time := pos shr FRAC_BITS;
        if (clocks_this_time > clocks_left) then
        ...
        if (clocks_left = 0) then
        begin
          // advance one state; if we hit idle, bail */
          old_drq := self.drq;
          self.advance_state;
          if old_drq <> self.drq then
          begin
            if @self.drq_call <> nil then
              self.drq_call(self.drq);
          end;
          if (self.state = STATE_IDLE) then
            break;
  ...
  if out_ > 32767 then
    out_ := 32767
  else if out_ < -32768 then
    out_ := -32768;
  tsample[self.tsample_num, sound_status.sound_position] := trunc(out_ * self.amp);
  if sound_status.stereo then
    tsample[self.tsample_num, sound_status.sound_position + 1] := trunc(out_ * self.amp);
end;

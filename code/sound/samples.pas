...
begin
  ...
  inc(ptemp, 12);
  inc(longitud, 12);
  if header.magic1 <> 'RIFF' then
  begin
    freemem(header);
    exit;
  end;
  if header.magic2 <> 'WAVE' then
  begin
    freemem(header);
    exit;
  end;
  ...
  begin
    ...
    begin
      ...
      // Tipo de compresion
      if fmt_info.audio_format <> 1 then
      begin // Solo soporto PCM!
        freemem(chunk);
        freemem(fmt_info);
        exit;
      end;
      // Numero de canales
      if ((fmt_info.num_channles <> 1) and (fmt_info.num_channles <> 2)) then
      begin
        freemem(chunk);
        freemem(fmt_info);
        exit;
    ...
    begin
      ...
      if (fmt_info.bits_per_sample = 16) then
        chunk.size := chunk.size shr 1;
      if (fmt_info.num_channles = 2) then
        chunk.size := chunk.size shr 1;
      long := round(chunk.size * lsamples_m) + 1;
      getmem(data, long * 2);
      data2 := data;
      for f := 0 to (chunk.size - 1) do
      begin
        if fmt_info.bits_per_sample = 8 then
        begin
        ...
        for h := 0 to (trunc(lsamples_loop) - 1) do
        begin
          data2^ := temp;
          inc(data2);
        end;
    ...
    if (fmt and datos) then
    begin
      salir := true;
      convert_wav := true;
    end;
    if longitud > source_long then
      salir := true;
  end;
  freemem(chunk);
  freemem(fmt_info);
...
//      MessageDlg('Error loading sample file: ' + '"' + nombre_samples[f].nombre + '"', mtError, [mbOk], 0);
  ...
  for f := 0 to MAX_CHANNELS do
    data_samples.tsample_reserved[f] := -1;
  if (sample_size - 1) > MAX_CHANNELS then
...
begin
  if data_samples = nil then
    exit;
  for f := 0 to data_samples.num_samples - 1 do
  begin
    data_samples.audio[f].playing := false;
    data_samples.audio[f].pos := 0;
  end;
...
begin
  for f := 0 to MAX_CHANNELS do
  begin
    if (not(data_samples.tsample_use[f]) and (data_samples.tsample_reserved[f] <> -1)) then
    begin
...
begin
  ...
  if not(data_samples.audio[num].playing) then
  begin
  ...
  begin // Si ya ha empezado, pero puede volver a comenzar lo hace
    if data_samples.audio[num].restart then
      data_samples.audio[num].pos := 0;
  end;
end;
...
begin
  if data_samples = nil then
    exit;
  if data_samples.audio[num].playing then
  begin
...
begin
  ...
  for f := 0 to (data_samples.num_samples - 1) do
  begin
    if data_samples.audio[f].playing then
    begin
      data_samples.audio[f].playing := false;
...
begin
  ...
  for f := 0 to data_samples.num_samples - 1 do
  begin
    if data_samples.audio[f] <> nil then
    begin
      if data_samples.audio[f].data <> nil then

...
const
  ...
  VMIN = 0;
  VMAX = 32768;
  MSM5232_ROM: array [0 .. 175] of word = (506, 7,

    478, 7, 451, 7, 426, 7, 402, 7, 379, 7, 358, 7, 338, 7, 319, 7, 301, 7, 284, 7, 268, 7, 253, 7,

    478, 6, 451, 6, 426, 6, 402, 6, 379, 6, 358, 6, 338, 6, 319, 6, 301, 6, 284, 6, 268, 6, 253, 6,

    478, 5, 451, 5, 426, 5, 402, 5, 379, 5, 358, 5, 338, 5, 319, 5, 301, 5, 284, 5, 268, 5, 253, 5,

    478, 4, 451, 4, 426, 4, 402, 4, 379, 4, 358, 4, 338, 4, 319, 4, 301, 4, 284, 4, 268, 4, 253, 4,

    478, 3, 451, 3, 426, 3, 402, 3, 379, 3, 358, 3, 338, 3, 319, 3, 301, 3, 284, 3, 268, 3, 253, 3,

    478, 2, 451, 2, 426, 2, 402, 2, 379, 2, 358, 2, 338, 2, 319, 2, 301, 2, 284, 2, 268, 2, 253, 2,

    478, 1, 451, 1, 426, 1, 402, 1, 379, 1, 358, 1, 338, 1, 319, 1, 301, 1, 284, 1, 268, 1, 253, 1,
...
begin
  case pos of
    0 .. 7:
      begin // pitch
        ...
        begin
          ...
          begin
            if (self.voice[ch].pitch <> (valor and $7F)) then
            begin
    ...
    8:
      for f := 0 to 3 do // group1 attack
        self.voice[f].ar_rate := self.ar_tbl[valor and $7] * self.external_capacitance[f];
    9:
      for f := 0 to 3 do // group2 attack
        self.voice[f + 4].ar_rate := self.ar_tbl[valor and $7] * self.external_capacitance[f + 4];
    $A:
      for f := 0 to 3 do // group1 decay
        self.voice[f].dr_rate := self.dr_tbl[valor and $F] * self.external_capacitance[f];
    $B:
      for f := 0 to 3 do // group2 decay
        self.voice[f + 4].dr_rate := self.dr_tbl[valor and $F] * self.external_capacitance[f + 4];
    $C:
      begin // group1 control
        self.control1 := valor;

        for f := 0 to 3 do
        begin
          if (((valor and $10) <> 0) and (self.voice[f].eg_sect = 1)) then
            self.voice[f].eg_sect := 0;
          self.voice[f].eg_arm := valor and $10;
        end;

        if (valor and 1) <> 0 then
          self.en_out16[0] := $FFFFFFFF
        else
          self.en_out16[0] := 0;
        if (valor and 2) <> 0 then
          self.en_out8[0] := $FFFFFFFF
        else
          self.en_out8[0] := 0;
        if (valor and 4) <> 0 then
          self.en_out4[0] := $FFFFFFFF
        else
          self.en_out4[0] := 0;
        if (valor and 8) <> 0 then
          self.en_out2[0] := $FFFFFFFF
        else
          self.en_out2[0] := 0;
      end;
    $D:
      begin // group2 control
        self.control2 := valor;
        self.gate_update;

        for f := 0 to 3 do
        begin
          if (((valor and $10) <> 0) and (self.voice[f + 4].eg_sect = 1)) then
            self.voice[f + 4].eg_sect := 0;
          self.voice[f + 4].eg_arm := valor and $10;
        end;

        if (valor and 1) <> 0 then
          self.en_out16[1] := $FFFFFFFF
        else
          self.en_out16[1] := 0;
        if (valor and 2) <> 0 then
          self.en_out8[1] := $FFFFFFFF
        else
          self.en_out8[1] := 0;
        if (valor and 4) <> 0 then
          self.en_out4[1] := $FFFFFFFF
        else
          self.en_out4[1] := 0;
        if (valor and 8) <> 0 then
          self.en_out2[1] := $FFFFFFFF
        else
          self.en_out2[1] := 0;
      end;
  end;
...
begin
  final_out := 0;
  if self.final_count <> 0 then
  begin
    for f := 0 to (self.final_count - 1) do
      final_out := final_out + self.final_buffer[f];
    final_out := final_out div self.final_count;
  end;
  tsample[self.tsample_num, sound_status.sound_position] := trunc(final_out * self.amp);
  self.final_count := 0;
end;
...
var
  new_state: integer;
begin
  if (self.control2 and $20) <> 0 then
    new_state := self.voice[7].gf
  else
    new_state := 0;
  if (self.gate <> new_state) then
  begin
    self.gate := new_state;
    if @self.gate_handler_cb <> nil then
      self.gate_handler_cb(new_state);
  end;
end;
...
begin
  ...
  begin
    ...
    if (self.voice[f + groupidx * 4].mode = 0) then
    begin // generate square tone
      left := 1 shl STEP_SH;
      repeat
        nextevent := left;

        if (self.voice[f + groupidx * 4].tg_cnt and self.voice[f + groupidx * 4].tg_out16) <> 0 then
          out16 := out16 + self.voice[f + groupidx * 4].tg_count;
        if (self.voice[f + groupidx * 4].tg_cnt and self.voice[f + groupidx * 4].tg_out8) <> 0 then
          out8 := out8 + self.voice[f + groupidx * 4].tg_count;
        if (self.voice[f + groupidx * 4].tg_cnt and self.voice[f + groupidx * 4].tg_out4) <> 0 then
          out4 := out4 + self.voice[f + groupidx * 4].tg_count;
        if (self.voice[f + groupidx * 4].tg_cnt and self.voice[f + groupidx * 4].tg_out2) <> 0 then
          out2 := out2 + self.voice[f + groupidx * 4].tg_count;

        self.voice[f + groupidx * 4].tg_count := self.voice[f + groupidx * 4].tg_count - nextevent;

        while (self.voice[f + groupidx * 4].tg_count <= 0) do
        begin
          ...
          if (self.voice[f + groupidx * 4].tg_cnt and self.voice[f + groupidx * 4].tg_out16) <> 0 then
            out16 := out16 + self.voice[f + groupidx * 4].tg_count_period;
          if (self.voice[f + groupidx * 4].tg_cnt and self.voice[f + groupidx * 4].tg_out8) <> 0 then
            out8 := out8 + self.voice[f + groupidx * 4].tg_count_period;
          if (self.voice[f + groupidx * 4].tg_cnt and self.voice[f + groupidx * 4].tg_out4) <> 0 then
            out4 := out4 + self.voice[f + groupidx * 4].tg_count_period;
          if (self.voice[f + groupidx * 4].tg_cnt and self.voice[f + groupidx * 4].tg_out2) <> 0 then
            out2 := out2 + self.voice[f + groupidx * 4].tg_count_period;

          if (self.voice[f + groupidx * 4].tg_count > 0) then
            break;
    ...
    begin // generate noise
      if (self.noise_clocks and 8) <> 0 then
        out16 := out16 + (1 shl STEP_SH);
      if (self.noise_clocks and 4) <> 0 then
        out8 := out8 + (1 shl STEP_SH);
      if (self.noise_clocks and 2) <> 0 then
        out4 := out4 + (1 shl STEP_SH);
      if (self.noise_clocks and 1) <> 0 then
        out2 := out2 + (1 shl STEP_SH);
    end;
...
begin
  ...
  begin
    ...
    if (self.noise_rng and 1) <> 0 then
      self.noise_rng := self.noise_rng xor $24000;
    self.noise_rng := self.noise_rng shr 1;
    if ((self.noise_rng and (1 shl 16)) <> tmp) then
      self.noise_clocks := self.noise_clocks + 1; // level change detect
    cnt := cnt - 1;
  end;
  if (self.noise_rng and (1 shl 16)) <> 0 then
    outputs[10] := 1
  else
    outputs[10] := 0;
  final_out := 0;
  ...
  if divt <> 0 then
    final_out := final_out div divt;
  if final_out > 32768 then
    final_out := 32768
  else if final_out < -32767 then
    final_out := -32767;

...
const
  ...
  eg_rate_select_init: array [0 .. (32 + 64 + 32) - 1] of byte = (
...
begin
  ...
  begin
    ...
    if ((SLOT.ar + SLOT.ksr) < (32 + 62)) then
    begin
...
begin
  ...
  if (CH.SLOT[SLOT1].key = 0) then
  begin
    FM_KEYON(CH, SLOT1);
    FM_KEYOFF(CH, SLOT1);
  end;
  if (CH.SLOT[SLOT2].key = 0) then
  begin
    FM_KEYON(CH, SLOT2);
    FM_KEYOFF(CH, SLOT2);
  end;
  if (CH.SLOT[SLOT3].key = 0) then
  begin
    FM_KEYON(CH, SLOT3);
    FM_KEYOFF(CH, SLOT3);
  end;
  if (CH.SLOT[SLOT4].key = 0) then
  begin
    FM_KEYON(CH, SLOT4);
    FM_KEYOFF(CH, SLOT4);
  end;
...
begin
  ...
  if (ST.irq = 0) { and ((ST.status and ST.irqmask)<>0) } then
  begin
    ST.irq := 1;
    // callback user interrupt handler (IRQ is OFF to ON)
    if (addr(ST.IRQ_Handler) <> nil) then
      ST.IRQ_Handler(ASSERT_LINE);
  end;
end;
...
begin
  ...
  // reset Timer b flag
  if ((v and $20) <> 0) then
    FM_STATUS_RESET(ST, $02);
  // reset Timer a flag
  if ((v and $10) <> 0) then
    FM_STATUS_RESET(ST, $01);
  // load b
  if ((v and $02) <> 0) then
  begin
    if ST.TBC = 0 then
...
begin
  ...
  for f := 0 to 127 do
    eg_rate_select[f] := eg_rate_select_init[f] * RATE_STEPS;
  for f := 0 to 14 do
    sl_table[f] := round(f * (4.0 / ENV_STEP));
  ...
  for f := 0 to (chan - 1) do
  begin
    getmem(OPN.P_CH[f], sizeof(fm_chan));
    OPN.P_CH[f].pms := 0;
    for g := 0 to 3 do
      getmem(OPN.P_CH[f].SLOT[g], sizeof(fm_slot));
  end;
  for f := chan to 7 do
    OPN.P_CH[f] := nil;
  opn_init := OPN;
...
begin
  case r of
    ...
    // #endif
    $24:
      OPN.ST.TA := (OPN.ST.TA and $03) or (v shl 2); // timer A High 8
    $25:
      OPN.ST.TA := (OPN.ST.TA and $3FC) or (v and 3); // timer A Low 2
    $26:
      OPN.ST.TB := v; // timer B
    $27:
      set_timers(OPN.ST, v); // mode , timer control
    $28:
      begin // key on / off
        c := v and $03;
        if (c = 3) then
          exit;
        if (((v and $04) <> 0) and ((OPN.type_ and TYPE_6CH) <> 0)) then
          c := c + 3;
        CH := OPN.P_CH[c];
        // csm mode
        if ((v and $10) <> 0) then
...
begin
  old_KSR := SLOT.KSR_m;
  if (v and $1F) <> 0 then
    SLOT.ar := 32 + ((v and $1F) shl 1)
  else
    SLOT.ar := 0;
  SLOT.KSR_m := 3 - (v shr 6);
  if (SLOT.KSR_m <> old_KSR) then
    CH.SLOT[SLOT1].Incr := -1;
  // refresh Attack rate
  if ((SLOT.ar + SLOT.ksr) < (32 + 62)) then
  begin
...
begin
  // frequency base
  if OPN.ST.rate <> 0 then
    OPN.ST.freqbase := (OPN.ST.clock / OPN.ST.rate) / pres
  else
    OPN.ST.freqbase := 0;
  ...
  if (SSGpres <> 0) then
    if @OPN.ST.SSG_Clock_change <> nil then
      OPN.ST.SSG_Clock_change(trunc(OPN.ST.clock * 2 / SSGpres));
  // make time tables
...
begin
  case addr of
    0:
      OPN.ST.prescaler_sel := 2; // when reset
    1:
      ; // when postload
    $2D:
      OPN.ST.prescaler_sel := OPN.ST.prescaler_sel or $02; // divider sel : select 1/1 for 1/3line
    $2E:
      OPN.ST.prescaler_sel := OPN.ST.prescaler_sel or $01;
    // divider sel , select 1/3line for output
    $2F:
      OPN.ST.prescaler_sel := 0; // divider sel , clear both selector to 1/2,1/2
  end;
...
begin
  ...
  case (r and $F0) of
    $30:
      set_det_mul(OPN.ST, CH, SLOT, v); // DET , MUL
    $40:
      set_tl(SLOT, v); // TL
    $50:
      set_ar_ksr(CH, SLOT, v); // KS, AR
    $60:
      begin // DR
    ...
    $A0:
      begin
        case (OPN_SLOT(r)) of
          ...
          1:
            OPN.ST.fn_h := v and $3F; // 0xa4-0xa6 : FNUM2,BLK
          2:
            begin // 0xa8-0xaa : 3CH FNUM1
              if (r < $100) then
              begin
          ...
          3:
            if (r < $100) then
              OPN.SL3.fn_h := v and $3F; // 0xac-0xae : 3CH FNUM2,BLK
        end;
      end;
    $B0:
      begin
        case OPN_SLOT(r) of
          ...
          1:
            begin // 0xb4-0xb6 : L , R , AMS , PMS (YM2612/YM2608)
...
begin
  ...
  repeat
    ...
    case SLOT.state of
      EG_ATT:
        begin // attack phase
          if ((OPN.eg_cnt and ((1 shl SLOT.eg_sh_ar) - 1)) = 0) then
          begin
            tmp := (not(SLOT.volume) * eg_inc[SLOT.eg_sel_ar + ((OPN.eg_cnt shr SLOT.eg_sh_ar) and
              7)]) / 16;
            ...
            if (SLOT.volume <= MIN_ATT_INDEX) then
            begin
      ...
      EG_DEC:
        begin // decay phase
          if (SLOT.ssg and $08) <> 0 then
          begin // SSG EG type envelope selected
            if ((OPN.eg_cnt and ((1 shl SLOT.eg_sh_d1r) - 1)) = 0) then
            begin
              SLOT.volume := SLOT.volume +
                (4 * eg_inc[SLOT.eg_sel_d1r + ((OPN.eg_cnt shr SLOT.eg_sh_d1r) and 7)]);
              if (SLOT.volume >= SLOT.sl) then
                SLOT.state := EG_SUS;
            end;
          end
          else
          begin
            ...
            begin
              SLOT.volume := SLOT.volume +
                (eg_inc[SLOT.eg_sel_d1r + ((OPN.eg_cnt shr SLOT.eg_sh_d1r) and 7)]);
              if (SLOT.volume >= SLOT.sl) then
                SLOT.state := EG_SUS;
            end;
          end;
      ...
      EG_SUS:
        begin // sustain phase
          if (SLOT.ssg and $08) <> 0 then
          begin // SSG EG type envelope selected
            if ((OPN.eg_cnt and ((1 shl SLOT.eg_sh_d2r) - 1)) = 0) then
            begin
              SLOT.volume := SLOT.volume +
                (4 * eg_inc[SLOT.eg_sel_d2r + ((OPN.eg_cnt shr SLOT.eg_sh_d2r) and 7)]);
              if (SLOT.volume >= ENV_QUIET) then
              begin
                if (SLOT.ssg and $01) <> 0 then
                begin // bit 0 = hold
                  if (SLOT.ssgn and 1) <> 0 then
                  begin // have we swapped once ???
          ...
          begin
            ...
            begin
              SLOT.volume := SLOT.volume +
                (eg_inc[SLOT.eg_sel_d2r + ((OPN.eg_cnt shr SLOT.eg_sh_d2r) and 7)]);
              if (SLOT.volume >= MAX_ATT_INDEX) then
              begin
      ...
      EG_REL:
        begin // release phase
          if ((OPN.eg_cnt and ((1 shl SLOT.eg_sh_rr) - 1)) = 0) then
          begin
            SLOT.volume := SLOT.volume +
              (eg_inc[SLOT.eg_sel_rr + ((OPN.eg_cnt shr SLOT.eg_sh_rr) and 7)]);
            if (SLOT.volume >= MAX_ATT_INDEX) then
            begin
              SLOT.volume := MAX_ATT_INDEX;
              SLOT.state := EG_OFF;
    ...
    if ((SLOT.ssg and $08) <> 0) and ((SLOT.ssgn and 2) <> 0) and (SLOT.state > EG_REL) then
...
begin
  ...
  begin
    ...
    if (N and 1) <> 0 then
      ...
      N := N shr 1; // 11 bits here (rounded)

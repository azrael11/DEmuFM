...
begin
  if @self.external_call <> nil then
    self.external_call;
  // reset check at last hieddge of VCLK
  if self.reset_data then
  begin
    self.signal := 0;
    self.step := 0;
    self.data_val := -1;
  ...
  begin // Clock signal
    ...
    if (self.signal > 2047) then
      self.signal := 2047
    else if (self.signal < -2048) then
      self.signal := -2048;
    self.step := self.step + index_shift[self.data and 7];
    if (self.step > 48) then
      self.step := 48
    ...
    0:
      begin
        ...
        if msm5205_1 <> nil then
          if msm5205_1.prescaler = 0 then
            msm5205_1.stream_update;
      end;
    1:
      msm5205_1.stream_update;
  end;
...
begin
  tsample[self.tsample_num, sound_status.sound_position] := trunc((self.signal shl 4) * self.amp);
  if sound_status.stereo then
    tsample[self.tsample_num, sound_status.sound_position + 1] := round((self.signal shl 4) * self.amp);
end;
...
begin
  ...
  begin
    ...
    if ((msm5205_0.pos >= msm5205_0.end_) or (msm5205_0.pos > msm5205_0.rom_size)) then
...
procedure advance_1;
begin
  if msm5205_1.idle then
    exit;
  if (msm5205_1.data_val <> -1) then
  begin
    msm5205_1.data_w(msm5205_1.data_val and $F);
    msm5205_1.data_val := -1;
    msm5205_1.pos := msm5205_1.pos + 1;
    if ((msm5205_1.pos >= msm5205_1.end_) or (msm5205_1.pos = msm5205_1.rom_size)) then
...
begin
  ...
  if (select and 4) <> 0 then
    bitwidth := 4
  else
    bitwidth := 3;
  if (self.prescaler <> prescaler) then
  begin
    ...
    begin
      ...
      timers.enabled(self.timer_, false);
  end;
  if (self.bitwidth <> bitwidth) then
    self.bitwidth := bitwidth;
end;

procedure MSM5205_chip.data_w(data: byte);
begin
  if (self.bitwidth = 4) then
    self.data := data and $F
  else
    self.data := (data and $7) shl 1; // unknown
end;

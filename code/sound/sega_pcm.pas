...
begin
  tsample[self.tsample_num, sound_status.sound_position] := trunc(self.outl * self.amp);
  if sound_status.stereo then
    tsample[self.tsample_num, sound_status.sound_position + 1] := trunc(self.outr * self.amp);
end;
  ...
  begin
    ...
    begin
      ...
      if (control and 1) <> 0 then
        ...
        sega_pcm_0.low[f] := addr and $FF;
    end;
  end; // del chnnel
  if sega_pcm_0.outl < -32767 then
    sega_pcm_0.outl := -32767
  else if sega_pcm_0.outl > 32767 then
    sega_pcm_0.outl := 32767;
  if sega_pcm_0.outr < -32767 then
    sega_pcm_0.outr := -32767
  else if sega_pcm_0.outr > 32767 then
    sega_pcm_0.outr := 32767;
end;

...
const
  ...
  volume_table: array [0 .. 15] of integer = ($20, // 0 dB
    $16, // -3.2 dB
    $10, // -6.0 dB
    $0B, // -9.2 dB
    $08, // -12.0 dB
...
begin
  temp := data;
  copymemory(@self.command, temp, 4);
  inc(temp, 4);
  copymemory(@self.bank_offs, temp, 4);
  inc(temp, 4);
  copymemory(@self.out_, temp, 4);
  inc(temp, 4);
  copymemory(@self.bank_installed, temp, sizeof(boolean));
  inc(temp, sizeof(boolean));
  copymemory(@self.ntimer, temp, 1);
  inc(temp, 1);
  copymemory(@self.amp, temp, sizeof(single));
  inc(temp, sizeof(single));
  for f := 0 to (OKIM6295_VOICES - 1) do
    copymemory(@self.voice[f], temp, sizeof(ADPCMVoice));
  inc(temp, sizeof(ADPCMVoice));
end;
...
begin
  temp := data;
  copymemory(temp, @self.command, 4);
  inc(temp, 4);
  size := 4;
  copymemory(temp, @self.bank_offs, 4);
  inc(temp, 4);
  size := size + 4;
  copymemory(temp, @self.out_, 4);
  inc(temp, 4);
  size := size + 4;
  copymemory(temp, @self.bank_installed, sizeof(boolean));
  inc(temp, sizeof(boolean));
  size := size + sizeof(boolean);
  copymemory(temp, @self.ntimer, 1);
  inc(temp, 1);
  size := size + 1;
  copymemory(temp, @self.amp, sizeof(single));
  inc(temp, sizeof(single));
  size := size + sizeof(single);
  for f := 0 to (OKIM6295_VOICES - 1) do
    copymemory(temp, @self.voice[f], sizeof(ADPCMVoice));
  inc(temp, sizeof(ADPCMVoice));
  size := size + sizeof(ADPCMVoice);
  save_snapshot := size;
end;
...
begin
  ...
  else if (self.voice[num_voice].adpcm.signal < -2048) then
    self.voice[num_voice].adpcm.signal := -2048;
  // adjust the step size and clamp */
  self.voice[num_voice].adpcm.step := self.voice[num_voice].adpcm.step + index_shift[nibble and 7];
  if (self.voice[num_voice].adpcm.step > 48) then
    self.voice[num_voice].adpcm.step := 48
  else if (self.voice[num_voice].adpcm.step < 0) then
    self.voice[num_voice].adpcm.step := 0;
  // return the signal */
...
begin
  ...
  if (sample and 1) = 0 then
    ...
    nibble := nibble and $F; // (((sample and 1) shl 2) xor 4);
  // next! */
  sample := sample + 1;
  if (sample >= count) then
    self.voice[num_voice].playing := false;
...
var
  divisor: byte;
begin
  if pin7 = OKIM6295_PIN7_HIGH then
    divisor := 132
  else
    divisor := 165;
...
begin
  ...
  begin
    ...
    for i := 0 to (OKIM6295_VOICES - 1) do
    begin
      ...
      begin
        ...
        if (start < stop) then
        begin
          if not(self.voice[i].playing) then
          begin // fixes Got-cha and Steel Force */
  ...
  begin // if this is the start of a command, remember the sample number for next time */
    if (valor and $80) <> 0 then
    begin
      self.command := valor and $7F;
    end
    else
    begin // otherwise, see if this is a silence command */
      temp := valor shr 3;

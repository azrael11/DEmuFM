...
begin
  ...
  begin
    ...
    if (reg.status and 1) <> 0 then
    begin // Key On
      div_ := (reg.status and $80) shr 7;
      if (reg.status and $2) = 0 then
      begin // PCM Sampling
      ...
      begin // Wave form
        ...
        if (((reg.status and 4) <> 0) and (delta >= $80)) then
        begin
          reg.status := reg.status and $FE; // Key off
          break;
        end;
...
begin
  ...
  begin
    tsample[self.tsample_, sound_status.sound_position] := self.lsignal;
    tsample[self.tsample_, sound_status.sound_position + 1] := self.rsignal;
  end
  else
    tsample[self.tsample_, sound_status.sound_position] := self.lsignal + self.rsignal;
end;

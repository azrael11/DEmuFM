...
const
  ...
  R52 = 20000;
begin
  if (sound_latch_a and $40) <> 0 then
  begin
    if (c24_state.level > VMIN) then
    begin
      temp := (c24_state.level - VMIN) / (R52 * C24);
      c24_state.counter := c24_state.counter - trunc(temp);
      if (c24_state.counter <= 0) then
      begin
...
const
  ...
  R54 = 47000;
begin
  if (sound_latch_a and $80) <> 0 then
  begin
    if (c25_state.level < VMAX) then
    begin
      temp := (VMAX - c25_state.level) / ((R50 + R53) * C25);
      c25_state.counter := c25_state.counter - trunc(temp);
      if (c25_state.counter <= 0) then
      begin
...
var
  ...
  sum: integer;
begin
  for numero_voz := 0 to 1 do
  begin
    if phoenix_sound[numero_voz].activa then
    begin

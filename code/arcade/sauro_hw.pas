...
const
  sauro_rom: array [0 .. 1] of tipo_roms = ((n: 'sauro-2.bin'; l: $8000; p: 0; crc: $19F8DE25),
    (n: 'sauro-1.bin'; l: $8000; p: $8000; crc: $0F8B876F));
  sauro_snd_rom: tipo_roms = (n: 'sauro-3.bin'; l: $8000; p: 0; crc: $0D501E1B);
  sauro_pal: array [0 .. 2] of tipo_roms = ((n: '82s137-3.bin'; l: $400; p: 0; crc: $D52C4CD0),
    (n: '82s137-2.bin'; l: $400; p: $400; crc: $C3E96D5D), (n: '82s137-1.bin'; l: $400; p: $800;
    crc: $BDFCF00C));
  sauro_char_bg: array [0 .. 1] of tipo_roms = ((n: 'sauro-6.bin'; l: $8000; p: 0; crc: $4B77CB0F),
    (n: 'sauro-7.bin'; l: $8000; p: $8000; crc: $187DA060));
  sauro_char_fg: array [0 .. 1] of tipo_roms = ((n: 'sauro-4.bin'; l: $8000; p: $0; crc: $9B617CDA),
    (n: 'sauro-5.bin'; l: $8000; p: $8000; crc: $A6E2640D));
  sauro_sprites: array [0 .. 3] of tipo_roms = ((n: 'sauro-8.bin'; l: $8000; p: $0; crc: $E08B5D5E),
  ...
  // DIP
  sauro_dip_a: array [0 .. 6] of def_dip = ((mask: $2; name: 'Demo Sounds'; number: 2;
    ...
    dip: ((dip_val: $30; dip_name: 'Very Easy'), (dip_val: $20; dip_name: 'Easy'), (dip_val: $10;
    dip_name: 'Hard'), (dip_val: $30; dip_name: 'Very Hard'), (), (), (), (), (), (), (), (), (),
    ...
    (), (), (), (), (), (), (), ())), ());
  sauro_dip_b: array [0 .. 3] of def_dip = ((mask: $3; name: 'Coin A'; number: 4;
    ...
    dip_name: '1C 4C'), (dip_val: $0; dip_name: '1C 5C'), (), (), (), (), (), (), (), (), (), (),
    (), ())), (mask: $30; name: 'Lives'; number: 4;
    dip: ((dip_val: $30; dip_name: '2'), (dip_val: $20; dip_name: '3'), (dip_val: $10;
...
begin
  ...
  begin
    ...
    nchar := memory[$E803 + (f * 4) + 1] + ((attr and $03) shl 8);
    x := memory[$E803 + (f * 4) + 2];
    y := 236 - y;
    color := ((attr shr 4) and $0F) or pal_bank;
...
begin
  case direccion of
    ...
    $F000 .. $F7FF:
      ...
      begin // Video + color ram 1 BG
        memory[direccion] := valor;
        gfx[0].buffer[direccion and $3FF] := true;
      end;
    $F800 .. $FFFF:
      if memory[direccion] <> valor then
      begin // Video + color ram 2 FG
        memory[direccion] := valor;
...
begin
  case (port and $FF) of
    $00:
      sauro_inbyte := marcade.dswa; // dsw1
    $20:
      sauro_inbyte := marcade.dswb; // dsw2
    $40:
...
const
  ...
  pc_y: array [0 .. 7] of dword = (0 * 4 * 8, 1 * 4 * 8, 2 * 4 * 8, 3 * 4 * 8, 4 * 4 * 8, 5 * 4 * 8,
    6 * 4 * 8, 7 * 4 * 8);
  ps_x: array [0 .. 15] of dword = (1 * 4, 0 * 4, 3 * 4, 2 * 4, 5 * 4, 4 * 4, 7 * 4, 6 * 4, 9 * 4,
    8 * 4, 11 * 4, 10 * 4, 13 * 4, 12 * 4, 15 * 4, 14 * 4);
  ps_y: array [0 .. 15] of dword = ($18000 * 8 + 0 * 4 * 16, $10000 * 8 + 0 * 4 * 16,
    $8000 * 8 + 0 * 4 * 16, 0 + 0 * 4 * 16, $18000 * 8 + 1 * 4 * 16, $10000 * 8 + 1 * 4 * 16,
    $8000 * 8 + 1 * 4 * 16, 0 + 1 * 4 * 16, $18000 * 8 + 2 * 4 * 16, $10000 * 8 + 2 * 4 * 16,
    $8000 * 8 + 2 * 4 * 16, 0 + 2 * 4 * 16, $18000 * 8 + 3 * 4 * 16, $10000 * 8 + 3 * 4 * 16,
    $8000 * 8 + 3 * 4 * 16, 0 + 3 * 4 * 16);
begin
  ...
  marcade.dswa := $66;
  marcade.dswb := $2F;
  marcade.dswa_val := @sauro_dip_a;
  marcade.dswb_val := @sauro_dip_b;
  // final
  reset_sauro;
  start_sauro := true;
end;

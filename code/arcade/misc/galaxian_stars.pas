...
begin
  case tipo of
    GALAXIANS:
      timers.init(num_cpu, clock / 30, stars_advance, nil, true);
    SCRAMBLE:
      timers.init(num_cpu, clock * (0.693 * (100000 + 2 * 10000) * 0.00001), stars_advance, nil, true);
  end;
end;
...
var
  ...
  x, y, color: word;
begin
  if not(galaxian_stars_0.enable) then
    exit;
  ...
  begin
    ...
    if ((y and 1) xor ((x shr 3) and 1)) <> 0 then
    begin
  ...
  begin
    ...
    begin
      case (galaxian_stars_0.blinking and 3) of
        0:
          if (galaxian_stars_0.stars[f].color and 1) = 0 then
            continue;
        1:
          if (galaxian_stars_0.stars[f].color and 4) = 0 then
            continue;
        2:
          if (galaxian_stars_0.stars[f].y and 2) = 0 then
            continue;
      end;
      color := paleta[galaxian_stars_0.stars[f].color];
      putpixel(y + ADD_SPRITE, x, 1, @color, screen);
...
begin
  ...
  begin
    ...
    begin
      ...
      if ((((not(generator) shr 16) and 1) <> 0) and ((generator and $FF) = $FF)) then
      begin

...
begin
  if addr(cchip_0.in_ca) <> nil then
    ca_cb := cchip_0.in_ca
  else
    ca_cb := $FF;
end;
...
begin
  if addr(cchip_0.in_cb) <> nil then
    cb_cb := cchip_0.in_cb
  else
    cb_cb := $FF;
end;
...
begin
  if addr(cchip_0.in_cc) <> nil then
    cc_cb := cchip_0.in_cc
  else
    cc_cb := $FF;
end;
...
begin
  if addr(cchip_0.in_cd) <> nil then
    cd_cb := cchip_0.in_cd
  else
    cd_cb := $FF;
end;
...
begin
  if addr(cchip_0.in_cf) <> nil then
    cf_cb := cchip_0.in_cf
  else
    cf_cb := $FF;
end;
...
begin
  case direccion of
    0 .. $FFF:
      cchip_getbyte := cchip_0.cchip_rom[direccion];
    $1000 .. $13FF:
      cchip_getbyte := cchip_0.mem_r(direccion and $3FF);
    $1400 .. $17FF:
      cchip_getbyte := cchip_0.asic_r(direccion and $3FF);
    $2000 .. $3FFF:
      cchip_getbyte := cchip_0.eeprom[direccion and $1FFF];
    $FF00 .. $FFFF:
      cchip_getbyte := cchip_0.upd7810.ram[direccion and $FF];
  end;
end;
...
begin
  case direccion of
    0 .. $FFF, $2000 .. $3FFF:
      ; // ROM + EEPROM
    $1000 .. $13FF:
      cchip_0.mem_w(direccion and $3FF, valor);
    $1400 .. $17FF:
      cchip_0.asic_w(direccion and $3FF, valor);
    $FF00 .. $FFFF:
      cchip_0.upd7810.ram[direccion and $FF] := valor;
  end;
end;
...
begin
  ...
  carga_rom_zip(dir + 'cchip.zip', tcchip_rom.n, @self.cchip_rom, tcchip_rom.l,
    tcchip_rom.crc, false);
...
begin
  if (direccion < $200) then
    asic_r := self.asic_ram[direccion and 3] // 400-5ff is asic 'ram'
  else
    asic_r := 0;
  // 600-7ff is write-only(?) asic banking reg, may read as open bus or never assert /DTACK on read?
end;
...
begin
  if (direccion = $200) then
    ...
    self.asic_ram[direccion and 3] := valor;
end;

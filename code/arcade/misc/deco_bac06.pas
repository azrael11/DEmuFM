...
type
  ...
  bac06_chip = class
    constructor create(pri1, pri2, pri3: boolean; color_add1, color_add2, color_add3, num_mask1,
      num_mask2, num_mask3: word; mult1, mult2, mult3: byte; sprite_color: word);
    destructor free;
  public
...
constructor bac06_chip.create(pri1, pri2, pri3: boolean; color_add1, color_add2, color_add3,
  num_mask1, num_mask2, num_mask3: word; mult1, mult2, mult3: byte; sprite_color: word);
var
  sc_pri1, sc_pri2, sc_pri3: byte;
begin
  if mult1 <> 0 then
    screen_init(1, 1024 * mult1, 1024, true);
  if mult2 <> 0 then
    screen_init(2, 1024 * mult2, 1024, true);
  if mult3 <> 0 then
    screen_init(3, 1024 * mult3, 1024, true);
  sc_pri1 := 4;
  sc_pri2 := 5;
  sc_pri3 := 6;
  if pri1 then
    screen_init(4, 1024, 1024, true)
  else
    sc_pri1 := 1;
  if pri2 then
...
begin
  ...
  begin
    ...
    color := x shr 12;
    if ((color and pri_mask) <> pri_val) then
      continue;
    if (((x and $800) <> 0) and ((main_vars.frames_sec and 1) <> 0)) then
      continue;
    fx := (y and $2000) <> 0;
    fy := (y and $4000) <> 0;
    ...
    begin
      ...
      begin
        ...
        update_gfx_sprite(x, (y + mult * multi) and $1FF, 7, num_gfx);
      end;
      multi := multi - 1;
    end;
  end;
...
begin
  ...
  begin
    ...
    atrib := self.data[pos];
    color := (atrib shr 12);
    if (self.buffer[pos] or self.buffer_color[color]) then
    begin
      nchar := atrib and self.num_mask;
      if trans then
        put_gfx_trans(x * 8, y * 8, nchar, (color shl 4) + self.color_add, self.screen, gfx_num)
      else
        put_gfx(x * 8, y * 8, nchar, (color shl 4) + self.color_add, self.screen, gfx_num);
...
begin
  ...
  begin
    ...
    begin
      nchar := atrib and self.num_mask;
      if trans then
        put_gfx_trans(x * 16, y * 16, nchar, (color shl 4) + self.color_add, self.screen, gfx_num)
      else
        put_gfx(x * 16, y * 16, nchar, (color shl 4) + color_add, self.screen, gfx_num);
      if pri then
      begin
        if (color and $8) = 8 then
          put_gfx_dec0(x * 16, y * 16, nchar, (color shl 4) + self.color_add,
            self.screen_pri, gfx_num)
        else
          put_gfx_block_trans(x * 16, y * 16, self.screen_pri, 16, 16);
      end;
      self.buffer[pos] := false;
    end;
  end;
...
begin
  if (self.control_0[0] and 1) = 0 then
    self.update_16x16(gfx_num, trans, pri)
  else
    self.update_8x8(gfx_num, trans);
end;
...
begin
  ...
  begin
    if (self.control and $4) <> 0 then
      copymemory(@self.pos_scroll_x, @self.rowscroll, $100 * 2)
    else
      fillchar(self.pos_scroll_x, $100 * 2, 0);
    if (self.control and $8) <> 0 then
      copymemory(@self.pos_scroll_y, @self.colscroll, $10 * 2)
    else
      fillchar(self.pos_scroll_y, $10 * 2, 0);
    scroll_xy_part(self.screen, 7, self.long_bloque_x, self.long_bloque_y, @self.pos_scroll_x[0],
      @self.pos_scroll_y[0], self.scroll_x, self.scroll_y);
  end;
end;
...
begin
  if self.control = 0 then
    scroll_x_y(self.screen_pri, 7, self.scroll_x, self.scroll_y)
  else
    scroll_xy_part(self.screen_pri, 7, self.long_bloque_x, self.long_bloque_y,
      @self.pos_scroll_x[0], @self.pos_scroll_y[0], self.scroll_x, self.scroll_y);
end;
...
var
  tempw: word;
begin
  if self.control_0[pos] = valor then
    exit;
  self.control_0[pos] := valor;
  case pos of
    0:
      self.control := valor and $C;
    3:
      begin
        fillchar(self.buffer, $1000, 1);
        case (self.control_0[3] and $3) of
...
begin
  if self.control_1[pos] = valor then
    exit;
  ...
  case pos of
    ...
    2:
      if dec1 then
        self.long_bloque_x := 16 shr (valor and $3)
      else
        self.long_bloque_x := 1 shl (valor and $F);
    3:
      self.long_bloque_y := 16 shl (valor and $F);
  end;
end;

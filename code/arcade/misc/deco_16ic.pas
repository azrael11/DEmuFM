...
type
  ...
  chip_16ic = class
    constructor create(pant1, pant2: byte; color_base1, color_base2, tmask1, tmask2: word;
      gfx_plane8, gfx_plane16, col_bank1, col_bank2: byte;
      call_bank1, call_bank2: tipo_deco16ic_bank);
    destructor free;
  public
    pf1, pf2: type_pf;
...
constructor chip_16ic.create(pant1, pant2: byte; color_base1, color_base2, tmask1, tmask2: word;
  gfx_plane8, gfx_plane16, col_bank1, col_bank2: byte; call_bank1, call_bank2: tipo_deco16ic_bank);
begin
...
begin
  ...
  begin
    // ¿es de 8x8?
    if (self.control[6] and $80) <> (valor and $80) then
      fillchar(self.pf1.buffer[0], $800, 1);
    if (self.control[6] and $8000) <> (valor and $8000) then
...
begin
  ...
  begin
    x := f mod 64;
    y := f div 64;
    if self.pf1.is_8x8 then
      pos := f
    else
      pos := (x and $1F) + ((y and $1F) shl 5) + ((x and $20) shl 5) + ((y and $20) shl 6);
    atrib := self.pf1.data[pos];
    flipx := false;
    flipy := false;
    if self.pf1.is_8x8 then
      color_mask := self.color_mask[1]
    else
      color_mask := self.color_mask[2];
    if (atrib and $8000) <> 0 then
    begin
      if (self.control[6] and 1) <> 0 then
      begin
        flipx := true;
        color_mask := color_mask shr 1;
      end;
      if (self.control[6] and 2) <> 0 then
      begin
        flipy := true;
        color_mask := color_mask shr 1;
      end;
    ...
    if ((self.pf1.buffer[pos]) or (self.pf1.buffer_color[color])) then
    begin
      nchar := (atrib and $FFF) or self.pf1.bank;
      if self.pf1.is_8x8 then
      begin // 8x8
        if trans then
          put_gfx_trans_flip(x * 8, y * 8, nchar, ((color + self.pf1.color_bank) shl 4) +
            self.color_base[1], self.pf1.pant, self.gfx_plane[1], flipx, flipy)
        else
          put_gfx_flip(x * 8, y * 8, nchar, ((color + self.pf1.color_bank) shl 4) + self.color_base
            [1], self.pf1.pant, self.gfx_plane[1], flipx, flipy);
      end
      else
      begin // 16x16
        if trans then
          put_gfx_trans_flip(x * 16, y * 16, nchar, ((color + self.pf1.color_bank) shl 4) +
            self.color_base[2], self.pf1.pant, self.gfx_plane[2], flipx, flipy)
        else
          put_gfx_flip(x * 16, y * 16, nchar, ((color + self.pf1.color_bank) shl 4) +
            self.color_base[2], self.pf1.pant, self.gfx_plane[2], flipx, flipy);
      end;
  ...
  case (self.control[6] and $60) of
    ...
    $20:
      begin // col_scroll
        ...
        atrib := 1024 div cols;
        scroll__y_part2(self.pf1.pant, screen, atrib, @self.pf1.rowscroll[$200], self.control[1],
          self.control[2]);
      end;
    $40:
      begin // row_scroll
        ...
        begin
          ...
          atrib := 512 div rows;
        scroll__x_part2(self.pf1.pant, screen, atrib, @self.pf1.rowscroll[0], self.control[1],
          self.control[2]);
      end;
    $60:
...
begin
  ...
  begin
    ...
    if (atrib and $8000) <> 0 then
    begin
      if (self.control[6] and $100) <> 0 then
      begin
        flipx := true;
        color_mask := color_mask shr 1;
      end;
      if (self.control[6] and $200) <> 0 then
      begin
        flipy := true;
        color_mask := color_mask shr 1;
      end;
    ...
    color := (atrib shr 12) and color_mask;
    if ((self.pf2.buffer[pos]) or (self.pf2.buffer_color[color])) then
    begin
      nchar := (atrib and $FFF) or self.pf2.bank;
      if self.pf2.is_8x8 then
      begin // 8x8
        if trans then
          put_gfx_trans_flip(x * 8, y * 8, nchar, ((color + self.pf2.color_bank) shl 4) +
            self.color_base[1], self.pf2.pant, self.gfx_plane[1], flipx, flipy)
        else
          put_gfx_flip(x * 8, y * 8, nchar, ((color + self.pf2.color_bank) shl 4) + self.color_base
            [1], self.pf2.pant, self.gfx_plane[1], flipx, flipy);
      end
      else
      begin // 16x16
        if trans then
          put_gfx_trans_flip(x * 16, y * 16, nchar, ((color + self.pf2.color_bank) shl 4) +
            self.color_base[2], self.pf2.pant, self.gfx_plane[2], flipx, flipy)
        else
          put_gfx_flip(x * 16, y * 16, nchar, ((color + self.pf2.color_bank) shl 4) +
            self.color_base[2], self.pf2.pant, self.gfx_plane[2], flipx, flipy);
      end;
  ...
  case ((self.control[6] shr 8) and $60) of
    $00:
      scroll_x_y(self.pf2.pant, screen, self.control[3], self.control[4]);
    $20:
      begin // col_scroll
        cols := 8 shl ((self.control[5] shr 8) and $7);
        atrib := 1024 div cols;
        scroll__y_part2(self.pf2.pant, screen, atrib, @self.pf2.rowscroll[$200], self.control[3],
          self.control[4]);
      end;
    $40:
      begin // row_scroll
...
var
  bank: word;
begin
  if @self.pf1.call_bank <> nil then
  begin
    bank := self.pf1.call_bank(self.control[7] and $FF);
    if bank <> self.pf1.bank then
    begin
      self.pf1.bank := bank;
      fillchar(self.pf1.buffer[0], $800, 1);
    end;
  end;
  if (self.control[5] and $80) <> 0 then
    self.update_plane_1(screen, trans);
...
var
  bank: word;
begin
  if @self.pf2.call_bank <> nil then
  begin
    bank := self.pf2.call_bank((self.control[7] shr 8) and $FF);
    if bank <> self.pf2.bank then
    begin
      self.pf2.bank := bank;
      fillchar(self.pf2.buffer[0], $800, 1);
    end;
  end;
  if (self.control[5] and $8000) <> 0 then
    self.update_plane_2(screen, trans);

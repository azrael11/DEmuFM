...
begin
  ...
  if (dstaddr < $C000) then
    curpix := memory[dstaddr]
  else
    curpix := self.getbyte(dstaddr); // current pixel values at dest
  solid := self.ram[1];
  keepmask := $FF;
  // what part of original dst byte should be kept, based on NO_EVEN and NO_ODD flags
  // even pixel (D7-D4)
  if (((self.ram[0] and CONTROLBYTE_FOREGROUND_ONLY) <> 0) and ((srcdata and $F0) = 0)) then
  begin // FG only and src even pixel=0
    if (self.ram[0] and CONTROLBYTE_NO_EVEN) <> 0 then
      keepmask := keepmask and $0F;
  end
  else
  begin
    if ((self.ram[0] and CONTROLBYTE_NO_EVEN)) = 0 then
  ...
  // odd pixel (D3-D0)
  if (((self.ram[0] and CONTROLBYTE_FOREGROUND_ONLY) <> 0) and ((srcdata and $0F) = 0)) then
  begin // FG only and src odd pixel=0
    if (self.ram[0] and CONTROLBYTE_NO_ODD) <> 0 then
      keepmask := keepmask and $F0;
  end
  else
  begin
    if ((self.ram[0] and CONTROLBYTE_NO_ODD)) = 0 then
  ...
  if (self.ram[0] and CONTROLBYTE_SOLID) <> 0 then
    curpix := curpix or (solid and not(keepmask))
  else
    curpix := curpix or (srcdata and not(keepmask));
  ...
  if (not(self.window_enable) or (dstaddr < self.clip_address) or (dstaddr >= $C000)) then
    self.putbyte(dstaddr, curpix);
end;
...
begin
  ...
  if (self.ram[0] and CONTROLBYTE_SRC_STRIDE_256) <> 0 then
  begin
    sxadv := $100;
    syadv := 1;
  end
  ...
  if (self.ram[0] and CONTROLBYTE_DST_STRIDE_256) <> 0 then
  begin
    dxadv := $100;
    dyadv := 1;
  end
  ...
  begin
    ...
    for x := 0 to (w - 1) do
    begin
      if ((self.ram[0] and CONTROLBYTE_SHIFT) = 0) then
      begin // no shift
        self.blit_pixel(dest, self.remap[self.getbyte(source)]);
      end
      else
      begin // shift one pixel right
    ...
    if (self.ram[0] and CONTROLBYTE_DST_STRIDE_256) <> 0 then
      dstart := (dstart and $FF00) or ((dstart + dyadv) and $FF)
    else
      dstart := dstart + dyadv;
    if (self.ram[0] and CONTROLBYTE_SRC_STRIDE_256) <> 0 then
      sstart := (sstart and $FF00) or ((sstart + syadv) and $FF)
    else
      sstart := sstart + syadv;
  end;
...
begin
  ...
  if (valor and CONTROLBYTE_SLOW) <> 0 then

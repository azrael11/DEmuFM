...
begin
  ...
  if ((self.op and 8) <> 0) then
    addr := addr or ((self.op and 7) shl 4);
  dvg_state_addr := addr;
end;

function avgdvg_chip.dvg_handler_0: byte; // dvg_dmapush
begin
  if ((self.op and 1) = 0) then
  begin
...
begin
  if (self.flip_x) then
    x := x + (self.xcenter - x) shl 1;
  if (self.flip_y) then
    y := y + (self.ycenter - y) shl 1;
end;
...
begin
  ...
  // Count up or down
  if (self.dvx and $400) <> 0 then
    dx := -1
  else
    dx := 1;
  if (self.dvy and $400) <> 0 then
    dy := -1
  else
    dy := 1;
  ...
  begin
    ...
    for bit := 0 to 11 do
    begin
      if ((c and ((1 shl (bit + 1)) - 1)) = ((1 shl bit) - 1)) then
      begin
    ...
    begin
      ...
      if (((self.ypos and $400) = 0) and (((self.xpos xor (self.xpos + dx)) and $400) <> 0)) then
      begin
    ...
    if county then
    begin
      if (((self.xpos and $400) = 0) and (((self.ypos xor (self.ypos + dy)) and $400) <> 0)) then
      begin
        if ((self.xpos and $400) = 0) then
        begin
          if ((self.ypos + dy) and $400) <> 0 then
            dvg_draw_to(self.xpos, self.ypos, self.intensity)
          else
            dvg_draw_to(self.xpos, (self.ypos + dy) and $FFF, 0);
        end;
      end;
...
begin
  self.dvy := self.dvy and $F00;
  if (self.op = $F) then
    self.dvg_handler_7 // dvg_latch3
  else
    self.dvy := (self.dvy and $F00) or self.data;
  self.pc := self.pc + 1;
  dvg_handler_4 := 0;
end;
...
begin
  self.dvx := self.dvx and $F00;
  if (self.op <> $F) then
    self.dvx := (self.dvx and $F00) or self.data;
  if (((self.op and 2) <> 0) and ((self.op and 8) <> 0)) then
...
begin
  self.data := memory[self.membase + (self.pc shl 1) + (self.state_latch and 1)];
end;
  ...
  begin
    ...
    begin
      ...
      case (self.state_latch and 7) of
        0:
          cycles := cycles + self.dvg_handler_0;
        1:
          cycles := cycles + self.dvg_handler_1;
        2:
          cycles := cycles + self.dvg_handler_2;
        3:
          cycles := cycles + self.dvg_handler_3;
        4:
          cycles := cycles + self.dvg_handler_4;
        5:
          cycles := cycles + self.dvg_handler_5;
        6:
          cycles := cycles + self.dvg_handler_6;
        7:
          cycles := cycles + self.dvg_handler_7;
      end;
    end;
    if ((self.halt <> 0) and ((self.state_latch and $10) = 0)) then
    begin
...
begin
  self.dvy := 0;
  self.op := 0;
  if ((self.sync_halt <> 0) and (self.nvect > 10)) then
  begin
...
begin
  ...
  cy1 := $500;
  f := 0;
  while (self.vectbuf[f].status = VGCLIP) do
    f := f + 1;
  xs := self.vectbuf[f].x;
  ys := self.vectbuf[f].y;
  for f := 0 to self.nvect do
  begin
    if (self.vectbuf[f].status = VGVECTOR) then
    begin
      xe := self.vectbuf[f].x;
      ye := self.vectbuf[f].y;
      x0 := xs;
      ...
      xs := xe;
      ys := ye;
      if (((x0 < cx0) and (x1 < cx0)) or ((x0 > cx1) and (x1 > cx1))) then
        continue;
      if (x0 < cx0) then
      begin
      ...
      if (((y0 < cy0) and (y1 < cy0)) or ((y0 > cy1) and (y1 > cy1))) then
        continue;
      if (y0 < cy0) then
      begin

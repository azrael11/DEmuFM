...
type
  ...
  tatari_mo = class
    constructor create(slip_ram: pword; sprite_ram: pword; config: atari_motion_objects_config;
      screen: byte; xmax, ymax: word);
    destructor free;
...
constructor tatari_mo.create(slip_ram: pword; sprite_ram: pword;
  config: atari_motion_objects_config; screen: byte; xmax, ymax: word);
var
...
begin
  ...
  if self.config.slipheight <> 0 then
    self.slipshift := compute_log(self.config.slipheight)
  else
    self.slipshift := 0;
  self.slipramsize := self.bitmapheight shr self.slipshift;
  self.sliprammask := self.slipramsize - 1;
  if (self.config.maxperline = 0) then
    self.config.maxperline := MAX_PER_BANK
  else
    self.config.maxperline := self.config.maxperline + 1;
  // PQ?!?!?!?!?! Si no pongo esto Atari SYS1 no muestra algunos sprites...
...
begin
  ...
  if (not self.lower.set_(input.data_lower)) then
  begin
    set_ := false;
    exit;
  end;
  if (not self.upper.set_(input.data_upper)) then
  begin
    set_ := false;
    exit;
  end;
...
begin
  ...
  for f := 0 to (self.config.maxperline - 1) do
  begin
    // copy the current entry into the list
    modata := current;
    if (not(self.config.split)) then
    begin
      srcdata := bankbase;
      inc(srcdata, link * 4);
      inc(current);
    ...
    visited[link] := true;
    if self.config.linked then
      link := self.linkmask.extract(modata)
    else
      link := (link + 1) and self.linkmask.mask;
...
var
  ...
  link: word;
begin
  if (self.slipshift = 0) then
    stopband := 0
  else
    stopband := ((512 div self.config.slipheight) shr 1) - 1;
  ...
  begin
    ...
    // safety check
    if (@self.activelist[0] = self.activelast) then
      continue;
    // set the start and end points
    if self.config.reverse then
    begin
...
begin
  ...
  if ((priority <> prio) and (prio <> -1)) then
    exit;
  ...
  if (self.absolutemask.extract(entry) = 0) then
  begin
    xpos := xpos - xscroll;
    ypos := ypos - yscroll;
  end;
  ...
  if (self.next_xpos <> 123456) then
    xpos := self.next_xpos;
  self.next_xpos := 123456;
  // check for the hold bit
  if (self.neighbormask.extract(entry) <> 0) then
  begin
    if (not self.config.nextneighbor) then
      xpos := self.last_xpos + self.tilewidth
    else
      self.next_xpos := xpos + self.tilewidth;
  end;
  self.last_xpos := xpos;
  ...
  // is this one special?
  if ((self.specialmask.mask = 0) or (self.specialmask.extract(entry) <> self.config.specialvalue))
  then
  begin
    // adjust for h flip
    xadv := self.tilewidth;
    ...
    if (not self.config.swapxy) then
    begin
      ...
      begin
        ...
        begin
          if (((sx < self.xmax) or (sx > 503)) and ((sy < self.ymax) or (sy > 503))) then
          begin
            put_gfx_sprite(code, color, hflip, vflip, numero_gfx);
            update_gfx_sprite(sx, sy, self.screen, numero_gfx);
          end;
          sx := sx + xadv;
          code := code + 1;
        end;

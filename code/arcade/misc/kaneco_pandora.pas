...
begin
  if self.clear_screen then
    fill_full_screen(screen, self.bg_color);
  x := 0;
  y := 0;
  for f := 0 to $1FF do
  begin
    ...
    if nchar = 0 then
      continue;
    put_gfx_sprite(nchar and self.mask_nchar, (color and $F0) + self.color_offset,
      (atrib and $80) <> 0, (atrib and $40) <> 0, ngfx);
    update_gfx_sprite(x, y, screen, ngfx);
  end;
end;

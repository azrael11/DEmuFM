...
begin
  ...
  gen_adpcm[num].timer := timers.init(sound_status.cpu_num, sound_status.cpu_clock / clock, nil,
    gen_update_adpcm, false, num);
  if num = 0 then
...
begin
  ...
  if (gen_adpcm[index].nibble = 4) then
  begin
    gen_adpcm[index].current := gen_adpcm[index].current + 1;
    if (gen_adpcm[index].current >= gen_adpcm[index].end_) then
    begin
...
begin
  ...
  if (gen_adpcm[num].nibble = 4) then
  begin
    gen_adpcm[num].current := gen_adpcm[num].current + 1;
    if (gen_adpcm[num].current >= gen_adpcm[num].end_) then
    begin
...
procedure gen_adpcm_update(num: byte);
begin
  tsample[gen_adpcm[num].tsample, sound_status.sound_position] := gen_adpcm[num].signal shl 4;
  if sound_status.stereo then
    tsample[gen_adpcm[num].tsample, sound_status.sound_position + 1] :=
      gen_adpcm[num].signal shl 4;
end;
